...
func parseUnstable(opts *BuildOptions, option string) {
	...
	case "script-overrides":
		requireUnstableValue(key, value, hasValue)
		opts.scriptOverrides = value
	case "manifest-overrides":
		requireUnstableValue(key, value, hasValue)
		opts.manifestOverride = value
	case "lib-search":
		requireUnstableValue(key, value, hasValue)
...
mrustc options are passed as -Zname=value: vendor-dir, script-overrides,
manifest-overrides, lib-search; flags: emit-mmir, dry-run and pause.
`, command)
}
