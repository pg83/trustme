    ...
    , m_hygiene(t.m_hygiene)
{
    ...
    TU_ARMA(None, e) {
        ...
        TU_ARMA(Ident, e) {
            m_data = Data::make_Ident(e);
        }
        TU_ARMA(String, e) {
            m_data = Data::make_String(e);
        }
        TU_ARMA(Integer, e) {
            m_data = Data::make_Integer(e);
        }
        TU_ARMA(Float, e) {
            m_data = Data::make_Float(e);
        }
        TU_ARMA(Fragment, e) {
...
::std::string Token::to_str() const {
    ...
    switch (m_type) {
        ...
        // Value tokens
        case TOK_IDENT:
            return m_data.as_Ident().name.c_str();
        case TOK_LIFETIME:
            return FMT("'" << m_data.as_Ident().name.c_str());
        case TOK_INTEGER: {
        ...
        case TOK_STRING:
            return FMT("\"" << EscapedString(m_data.as_String()) << "\"" << m_hygiene);
        case TOK_CSTRING:
            return FMT("c\"" << EscapedString(m_data.as_String()) << "\"" << m_hygiene);
        case TOK_BYTESTRING:
