...
namespace {
    ...
    MsvcDetection detect_msvc() {
        auto rv = MsvcDetection{"C:\\Program Files (x86)\\Microsoft Visual Studio 14.0\\VC\\vcvarsall.bat"};
    ...
    class CodeGenerator_C: public CodeGenerator {
        enum class Mode {
            //FullStd,
            ...
            , m_of(m_outfile_path_c)
        {
            ...
            switch (m_compiler) {
                case Compiler::Gcc:
                    ...
                    break;
                case Compiler::Msvc:
                    m_of << "static inline int32_t __builtin_popcountll(uint64_t v) {\n"
...
// If the previous is also a marker, don't push
                        ...
                        case CodegenOutput::DynamicLibrary:
                            ...
                            for (auto l_d : libraries_and_dirs) {
                                switch (l_d.first) {
                                    ...
                                    case LinkList::Ty::Directory:
                                        args.push_back(FMT("/LIBPATH:" << l_d.second));
                                        break;
                                    case LinkList::Ty::Implicit:
                                        args.push_back(std::string(l_d.second) + ".lib");
                                        break;
                                    case LinkList::Ty::Explicit:
                                        args.push_back(l_d.second);
                                        break;
        ...
        void emit_struct_inner(const ::HIR::TypeRef& ty, const TypeRepr* repr, unsigned packing_max_align) {
            ...
            for (const auto& ent : repr->fields) {
                ...
                {
                    size_t al_c = al;
                    if (Target_CapsMemberAlignment() && sz > 0 && ent.offset != 0 && al_c > 4 && !Target_TypeHasUserAlignment(sp, m_resolve, ty)) {
                        al_c = 4;
                    }
                ...
                TU_ARMA(Array, te) {
                    ...
                    // HACK: MSVC doesn't allow arrays larger than 2^31-1 elements
                        ...
                        do {
                            ...
                            m_of << " ";
                            emit_ctype(te.inner);
                            m_of << "DATA_" << i << "[" << std::min(len, chunk_len) << "];";
                        } while (len > chunk_len);
        ...
        void emit_function_code(const ::HIR::Path& p, const ::HIR::Function& item, const Trans_Params& params, bool is_extern_def, const ::MIR::FunctionPointer& code) override {
            ...
            if (m_compiler == Compiler::Msvc && p.m_data.is_Generic()) {
                ...
                if (gp.m_path.crate_name().compare(0, 5, "core-") == 0 && gp.m_path.components()[0] == "core_arch") {
                    ...
                    if (false || fcn_name.compare(0, 6, "_mm256") == 0 || fcn_name.compare(0, 6, "_mm512") == 0 || fcn_name.compare(0, 4, "_mm_") == 0) {
                        // HACK: These aren't supported on msvc2019
                        if (true || fcn_name == "_mm256_zextpd128_pd256") {
                            m_of << "\tabort();";
                            m_of << "}\n";
                            return;
                        }
                        ...
                        struct H {
                            static const char* get_intr_type(const HIR::TypeRef& ty) {
                                if (ty.data().is_Path()) {
                                    ...
                                    if (false || fcn_name == "__m128" || fcn_name == "__m128d" || fcn_name == "__m128i" || fcn_name == "__m256" || fcn_name == "__m256d" || fcn_name == "__m256i") {
        ...
        void emit_borrow(const ::MIR::TypeResolve& mir_res, HIR::BorrowType bt, const MIR::LValue& val) {
            ...
            if (!special && m_options.disallow_empty_structs && val_ref.is_Index() && this->type_is_bad_zst(ty)) {
                ...
                if (const auto* array = parent_ty.data().opt_Array()) {
                    element_ty = &array->inner;
                } else if (const auto* slice = parent_ty.data().opt_Slice()) {
            ...
            if (!special && m_options.disallow_empty_structs && val.is_Field() && this->type_is_bad_zst(ty)) {
                ...
                if (field_inner.is_Downcast()) {
                    ...
                    if (element_ty) {
                        ...
                        auto* repr = Target_GetTypeRepr(sp, m_resolve, parent_ty);
                        assert(repr);
        ...
        void emit_rvalue_cast(const ::MIR::TypeResolve& mir_res, const ::MIR::LValue& dst, const ::MIR::RValue::Data_Cast& ve) {
            ...
            if (ve.type.data().is_Pointer() && !is_dst(ve.type.data().as_Pointer().inner)) {
                ...
                if ((ty.data().is_Borrow() && is_dst(ty.data().as_Borrow().inner)) || (ty.data().is_Pointer() && is_dst(ty.data().as_Pointer().inner))) {
                    emit_lvalue(ve.val);
                    m_of << ".PTR";
                    special = true;
                }
        ...
        void emit_term_switch(const ::MIR::TypeResolve& mir_res, const ::MIR::LValue& val, size_t n_arms, unsigned indent_level, ::std::function<void(size_t)> cb, size_t odd_arm = -1) {
            ...
            TU_ARMA(NonZero, e) {
                    ...
                    MIR_ASSERT(mir_res, slot_ty.data().is_Pointer() || slot_ty.data().is_Function() || slot_ty.data().is_Borrow() || slot_ty.data().is_Primitive(), "Invalid niche type: " << slot_ty << " in " << ty);
        ...
        void emit_asm_msvc(const ::MIR::TypeResolve& mir_res, const ::MIR::Statement::Data_Asm& e, unsigned indent_level) {
            ...
            if (e.tpl == "") {
                ...
                return;
            } else if (matches_template("cpuid\n", /*input=*/{"{eax}", "{ecx}"}, /*output=*/{"={eax}", "={ebx}", "={ecx}", "={edx}"}) || matches_template("cpuid", /*input=*/{"{eax}", "{ecx}"}, /*output=*/{"={eax}", "={ebx}", "={ecx}", "={edx}"})) {
                m_of << indent << "{";
                m_of << " int cpuid_out[4];";
                m_of << " __cpuidex(cpuid_out, ";
                emit_lvalue(e.inputs[0].second);
                m_of << ", ";
                emit_lvalue(e.inputs[1].second);
                m_of << ");";
                ...
                m_of << indent << "__writeeflags(";
                emit_lvalue(e.inputs[0].second);
                m_of << ");\n";
                return;
            } else if (matches_template("xgetbv", /*input=*/{"{ecx}"}, /*output=*/{"={eax}", "={edx}"})) {
                m_of << indent << "{";
                m_of << " unsigned __int64 v = _xgetbv(";
                emit_lvalue(e.inputs[0].second);
                m_of << ");";
                m_of << " ";
                emit_lvalue(e.outputs[0].second);
                m_of << " = (uint32_t)(v & 0xFFFFFFFF);";
                m_of << " ";
                emit_lvalue(e.outputs[1].second);
                m_of << " = (uint32_t)(v >> 32);";
                m_of << " }\n";
        ...
        void emit_asm2_msvc(const ::MIR::TypeResolve& mir_res, const ::MIR::Statement& stmt, unsigned indent_level) {
            ...
            // - CPUID
                ...
                m_of << " = cpuid_out[0];"; // EAX
                m_of << " ";
                emit_lvalue(m.output(0));
                m_of << " = cpuid_out[1];"; // EBX
                m_of << " ";
                emit_lvalue(m.output(2));
                m_of << " = cpuid_out[2];"; // ECX
                m_of << " ";
                emit_lvalue(m.output(3));
                m_of << " = cpuid_out[3];"; // EDX
                m_of << " }\n";
    ...
    private:
        ...
        void emit_function_header(const ::HIR::Path& p, const ::HIR::Function& item, const Trans_Params& params) {
            ...
            auto cb = FMT_CB(
                ...
                if (item.m_abi == "system" && m_compiler == Compiler::Msvc) { ss << " __stdcall"; } ss << " " << Trans_Mangle(p) << "(";
        ...
        void emit_intrinsic_call(const RcString& name, const ::HIR::PathParams& params, const ::MIR::Terminator::Data_Call& e) {
            ...
            auto get_atomic_suffix_msvc = [&](Ordering o) -> const char* {
                switch (o) {
                    ...
                    case Ordering::Relaxed:
                        return "NoFence";
                    case Ordering::AcqRel:
                        return ""; // this is either Acquire or Release
                }
                throw "";
            };
                ...
                switch (m_compiler) {
                    ...
                    case Compiler::Msvc:
                        if (params.m_types.at(0) == ::HIR::CoreType::U128 || params.m_types.at(0) == ::HIR::CoreType::I128) {
                            ...
                            m_of << ";\n\t";
                            emit_lvalue(e.ret_val);
                            m_of << "._1 = InterlockedCompareExchange128(";
                            m_of << "(volatile uint64_t*)";
                            emit_param(e.args.at(0));
                            m_of << ", ";
                    ...
                    case Compiler::Msvc:
                        switch (op) {
                            ...
                            case AtomicOp::Sub:
                                ...
                                return;
                            case AtomicOp::And:
                                emit_msvc_atomic_op("InterlockedAnd", ordering);
                                break;
                            case AtomicOp::Or:
                                emit_msvc_atomic_op("InterlockedOr", ordering);
                                break;
                            case AtomicOp::Xor:
                                emit_msvc_atomic_op("InterlockedXor", ordering);
                                break;
                        }
                        emit_param(e.args.at(0));
                        m_of << ", ";
            ...
            // --- Type identity ---
                ...
                auto is_ptr = [](const ::HIR::TypeRef& ty) {
                    return ty.data().is_Borrow() || ty.data().is_Pointer();
                };
            ...
            // HACK: Uses GCC intrinsics
...

                {
                    switch (m_compiler) {
                        ...
                        case Compiler::Msvc:
                            emit_lvalue(e.ret_val);
                            m_of << "._1 = __builtin_add_overflow_" << params.m_types.at(0).data().as_Primitive();
                            m_of << "(";
                            emit_param(e.args.at(0));
                            m_of << ", ";
                            emit_param(e.args.at(1));
                ...
                if (m_options.emulated_i128 && params.m_types.at(0) == ::HIR::CoreType::U128) {
                    ...
                    switch (m_compiler) {
                        ...
                        case Compiler::Msvc:
                            emit_lvalue(e.ret_val);
                            m_of << "._1 = __builtin_sub_overflow_" << params.m_types.at(0).data().as_Primitive();
                            m_of << "(";
                            emit_param(e.args.at(0));
                            m_of << ", ";
                            emit_param(e.args.at(1));
                ...
                if (m_options.emulated_i128 && params.m_types.at(0) == ::HIR::CoreType::U128) {
                    ...
                    switch (m_compiler) {
                        ...
                        case Compiler::Msvc:
                            emit_lvalue(e.ret_val);
                            m_of << "._1 = __builtin_mul_overflow_" << params.m_types.at(0).data().as_Primitive();
                            m_of << "(";
                            emit_param(e.args.at(0));
                            m_of << ", ";
                            emit_param(e.args.at(1));
                ...
                if (name == "atomic_xadd" || name.compare(0, 7 + 4 + 1, "atomic_xadd_") == 0) {
                    ...
                    switch (m_compiler) {
                        ...
                        case Compiler::Msvc:
                            ...
                            emit_msvc_atomic_op("InterlockedCompareExchange", ordering);
                            emit_param(e.args.at(0));
                            m_of << ", 0, 0)";
                            break;
                    }
            ...
            // -- Platform Intrinsics (and SIMD) --
                ...
                struct SimdInfo {
                    ...
                    static SimdInfo for_ty(const CodeGenerator_C& self, const HIR::TypeRef& ty) {
                        ...
                        const auto& ty_val = ity.data().is_Primitive() ? ity : ty_repr->fields[0].ty.data().as_Array().inner;
                        DEBUG("ty_val = " << ty_val);
                        size_t size_val = 0;
            ...
            TU_ARMA(Return, _e) {
                ...
                TU_ARMA(Field, field_index) {
                    ...
                    if (ty.data().is_Slice()) {
                        if (inner.is_Deref()) {
                            m_of << "((";
                            emit_ctype(ty.data().as_Slice().inner);
                            m_of << "*)";
                            emit_lvalue(inner.inner_ref());
                            m_of << ".PTR)";
                        } else {
