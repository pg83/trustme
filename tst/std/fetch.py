...
node — shared by every project test.
    ...
    fetch.py <out.tar> <rust-src.patch>
...
def main() -> int:
    if len(sys.argv) != 3:
        raise SystemExit("usage: fetch.py OUT_TAR RUST_SRC_PATCH")
    out = os.path.abspath(sys.argv[1])
    patch = os.path.abspath(sys.argv[2])
    with lib.workdir() as work:
        ...
        if local:
            ...
            with open(patch) as patch_file:
                subprocess.run(
                    ["patch", "-p0"], cwd=src, stdin=patch_file, check=True
                )
