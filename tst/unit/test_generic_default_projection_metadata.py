#!/usr/bin/env python3
"""Check that a projected generic default survives rlib metadata."""
...
def main() -> int:
    if len(sys.argv) != 6:
        raise SystemExit(
            "usage: test_generic_default_projection_metadata.py "
    ...
    with tempfile.TemporaryDirectory(prefix="trustme-generic-default-metadata-") as work:
        libstd = lib.untar(libstd_tar, os.path.join(work, "libstd"))
        search = os.path.join(libstd, "release")
        producer_rlib = os.path.join(
            work, "libgeneric_default_projection_metadata_producer.rlib"
        )
        ...
        run(
            lib.wrap_gdb([
                ...
                producer,
                "--crate-name",
                "generic_default_projection_metadata_producer",
                "--crate-type",
                "rlib",
                "--edition",
                "2021",
        ...
        run(
            lib.wrap_gdb([
                ...
                search,
                "--extern",
                "generic_default_projection_metadata_producer=" + producer_rlib,
                "-Cemit-cpp-only",
                "-o",
                output,
            ]),
