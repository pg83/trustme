#!/usr/bin/env python3
"""Compile and run one shard of solved Rustlings exercises."""
...
def fail_output(result: subprocess.CompletedProcess, case: str, stage: str) -> int:
    ...
    print(f"FAIL Rustlings {case}: {stage} exit {result.returncode}", file=sys.stderr)
...
def main() -> int:
    ...
    with lib.workdir() as work:
        ...
        for index, (case, mode) in enumerate(selected):
            print(f"[Rustlings {mode}] {case}", file=sys.stderr, flush=True)
            ...
            try:
                ...
                print(f"FAIL Rustlings {case}: timed out after 60 seconds", file=sys.stderr)
                return 1
