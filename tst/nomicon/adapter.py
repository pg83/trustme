#!/usr/bin/env python3
"""Compile and execute one shard of Rustonomicon code fences."""
...
def fail_output(result: subprocess.CompletedProcess, case: str, stage: str) -> int:
    ...
    print(f"FAIL Rustonomicon {case}: {stage} exit {result.returncode}", file=sys.stderr)
...
def main() -> int:
    ...
    with lib.workdir() as work:
        ...
        for index, (case, origin, edition, mode) in enumerate(selected):
            print(f"[Rustonomicon {mode}] {origin}", file=sys.stderr, flush=True)
            ...
            if mode not in ("pass", "panic"):
                raise SystemExit(f"unknown Rustonomicon mode {mode!r} for {case}")
            try:
                ...
                print(f"FAIL Rustonomicon {case}: timed out after 60 seconds", file=sys.stderr)
                return 1
