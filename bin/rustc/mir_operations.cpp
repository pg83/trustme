                    ...
                    TU_ARMA(Use, rse) {
                            ...
                            TU_ARMA(EnumVariant, rse) {
                                ...
                                if (rse.vals.size() > 0) {
                                    ...
                                    const auto& var_ty = resolve.monomorphExpandOpt(sp, tmp, variant.type, MonomorphStatePtr(state.crate.types, nullptr, &rse.path.mParams, nullptr));
                                    const auto& str = *var_ty->as_Path().binding.as_Struct();
                                    const auto& sPath = var_ty->as_Path().path.mData.as_Generic();
                            ...
                            TU_ARMA(MakeDst, rse) {
                                ...
                                if (dstTy->is_Borrow()) {
                                    if (rse.ptrVal.is_Borrow()) {
                                        ...
                                        HIR::TypeRef tmp2;
                                        const auto& src_ty = state.getParamType(tmp2, rse.ptrVal);
                                        borrowState.lifetimeAssign(dstTy->as_Borrow().lifetime, src_ty->as_Borrow().lifetime);
                                    }
                                }
                            }
                            TU_ARMA(UniOp, rse) {
...
void MIRValidateValState(::MIR::TypeResolve& state, const ::MIR::Function& fcn) {
    ...
    while (nextBlockToVisit < to_visit_blocks.size()) {
        auto block = to_visit_blocks[nextBlockToVisit++];
            ...
            switch (stmt.tag()) {
                ...
                case ::MIR::Statement::TAG_Asm:
                    ...
                    for (const auto& v : stmt.as_Asm().outputs) {
                        val_state.markValidity(state, v.second, true);
                    }
                    break;
                    TU_ARM(stmt, Asm2, e) {
                    ...
                    TU_ARMA(Const, v) {
                                ...
                                TU_ARMA(Reg, v) {
                                    if (v.input) {
                                        val_state.moveVal(state, *v.input);
                                    }
                                    if (v.output) {
                                        val_state.markValidity(state, *v.output, true);
                                    }
                                }
                    }
                        }
                ...
                case ::MIR::Statement::TAG_Assign:
                    ...
                    TU_MATCH(
                        ...
                        (stmt.as_Assign().src),
                        (se),
                        (Use, val_state.moveVal(state, se);),
                        (
                            Constant,
                            //(void)state.get_const_type(se);
                        ),
                        (SizedArray, val_state.moveVal(state, se.val);),
                        ...
                        (BinOp, val_state.moveVal(state, se.val_l); val_state.moveVal(state, se.val_r);),
                        (UniOp, val_state.moveVal(state, se.val);),
                        (DstMeta, val_state.ensureValid(state, se.val);),
                        (DstPtr, val_state.ensureValid(state, se.val);),
                        (MakeDst,
                         //val_state.move_val(state, se.ptr_val);
                         if (const auto* e = se.ptrVal.opt_LValue()) val_state.ensureValid(state, *e);
                         val_state.moveVal(state, se.metaVal);),
                        (Tuple, for (const auto& v : se.vals) val_state.moveVal(state, v);),
                        (Array, for (const auto& v : se.vals) val_state.moveVal(state, v);),
                        (UnionVariant, val_state.moveVal(state, se.val);),
                        (EnumVariant, for (const auto& v : se.vals) val_state.moveVal(state, v);),
                        (Struct, for (const auto& v : se.vals) val_state.moveVal(state, v);)
                    )
                    // Mark destination as valid
                    val_state.markValidity(state, stmt.as_Assign().dst, true);
                    break;
        ...
        TU_ARMA(Incomplete, e) {
            ...
            TU_ARMA(Switch, e) {
                if (e.valid_flag == ~0u) {
                    ...
                    addToVisit(e.invalidTarget, val_state);
                }
            ...
            TU_ARMA(Drop, e) {
                ...
                val_state.markValidity(state, e.slot, false);
            ...
            TU_ARMA(Call, e) {
                ...
                val_state.markValidity(state, e.retVal, true);
                addToVisit(e.retBlock, val_state);
            }
        }
    }
}
                    ...
                    TU_ARMA(Use, e) {
                        ...
                        default:
                            MIR_BUG(state, "Invalid cast: " << dstTy << " from " << src_ty);
                                    ...
                                    TU_ARMA(Path, sE) {
                                        MIR_ASSERT(state, sE.binding.is_Enum(), "Invalid cast: " << dstTy << " from " << src_ty);
                                        MIR_ASSERT(state, sE.binding.as_Enum()->isValue(), "Invalid cast: " << dstTy << " from " << src_ty);
                                ...
                                MIR_BUG(state, "Invalid cast: " << dstTy << " from " << src_ty);
                                            ...
                                            TU_ARMA(Primitive, dE) {
                                                ...
                                                if (dE == HIR::CoreType::Char)
                                                    MIR_ASSERT(state, sE == HIR::CoreType::U8, "Invalid cast: " << dstTy << " from " << src_ty);
                                            }
                            }
                                    }
                                    ...
                                    TU_ARMA(Borrow, sE) {
                                        ...
                                        MIR_ASSERT(state, dstTy->as_Pointer().type <= sE.type, "Casting borrow to invalid type: " << dstTy << " from " << src_ty);
                                        MIR_ASSERT(state,
                                                ...
                                                if (sE.inner != dE.inner) {
                                                    ...
                                                    if (dMeta != MetadataType::None && dMeta != MetadataType::Zero) {
                                                        if (dMeta != MetadataType::Unknown && sMeta != MetadataType::Unknown) {
                                                            MIR_ASSERT(state, dMeta == sMeta, "Casting has mismatched metadata: " << dstTy << " from " << src_ty << " (" << dMeta << " from " << sMeta << ")");
                                                        }
                                                    }
                                                }
                                            }
                                            TU_ARMA(Primitive, dE) {
                                                switch (dE) {
                                                    ...
                                                    case ::HIR::CoreType::F64:
                                                        ...
                                                        break;
                                                    default:
                                                        MIR_ASSERT(state, sMeta == MetadataType::None || sMeta == MetadataType::Zero, "Casting fat pointer to integer: " << dstTy << " from " << src_ty);
                                                        break;
                                                }
                                            }
                            }
...
// > Borrows vs pointers are fun
            ...
            TU_ARMA(Incomplete, e) {
                ...
                TU_ARMA(Switch, e) {
                    ...
                    MIR_ASSERT(state, (e.valid_flag == ~0u) == (e.invalidTarget == ~0u), "Conditional switch flag/target mismatch");
                    if (e.valid_flag != ~0u) {
                ...
                TU_ARMA(Call, e) {
                    if (e.fcn.is_Value()) {
                        ...
                        outParams.set_consteval_state(state.crate, HIR::ItemPath(p));
                        ...
                        // Check return
                        const auto& slot_ty = state.getLvalueType(tmp1, e.retVal);
...
namespace {
        ...
        bool isEquivalentTo(const ValueStates& x) const {
            struct H {
                static bool equal(const ValueStates& vss_a, const State& a, const ValueStates& vss_b, const State& b) {
                    ...
                    const auto& states_a = vss_a.innerStates.at(a.index - 1);
                    const auto& states_b = vss_b.innerStates.at(b.index - 1);
    ...
    public:
        ...
        const State& getLvalueState(const ::MIR::TypeResolve& mir_res, const ::MIR::LValue& lv) const {
            ...
            TU_MATCHA((lv.root), (e), (Return, state_p = &returnValue;), (Argument, state_p = &args.at(e);), (Local, state_p = &locals.at(e);), (Static, static State state_of_static(true); return state_of_static;))

            for (const auto& w : lv.wrappers) {
                if (w.is_Index()) {
                    const auto& vs_i = getLvalueState(mir_res, ::MIR::LValue::newLocal(w.as_Index()));
                    MIR_ASSERT(mir_res, vs_i.isValid(), "Indexing with an invalidated value");
                }
            }
            for (const auto& w : lv.wrappers) {
                if (!state_p->isComposite()) {
        ...
        void set_lvalue_state(const ::MIR::TypeResolve& mir_res, const ::MIR::LValue& lv, State newVs) {
            TRACE_FUNCTION_F(lv << " = " << StateFmt(*this, newVs) << " (from " << StateFmt(*this, getLvalueState(mir_res, lv)) << ")");
            State* state_p = nullptr;
            TU_MATCHA((lv.root), (e), (Return, state_p = &returnValue;), (Argument, state_p = &args.at(e);), (Local, state_p = &locals.at(e);), (Static, return;))
            ...
            for (const auto& w : lv.wrappers) {
                ...
                TU_ARMA(Field, e) {
                    ...
                    TU_ARMA(Index, e) {
                        ...
                        MIR_ASSERT(mir_res, !vs_i.isComposite(), "");

                        MIR_ASSERT(mir_res, curVs.isValid(), "Indexing an invalid value");
                        MIR_ASSERT(mir_res, vs_i.isValid(), "Indexing with an invalid index");
    ...
    struct StateSet {
        ...
        bool addState(const ValueStates& state_set) {
            for (const auto& s : this->knownStateSets) {
                if (s.isEquivalentTo(state_set)) {
                    return false;
                }
            }
            this->knownStateSets.push_back(state_set.clone());
...
void MIRValidateFullValState(::MIR::TypeResolve& mir_res, const ::MIR::Function& fcn) {
    ...
    while (!todo_queue.empty()) {
        ...
        TU_MATCHA(
            ...
            (Switch, if (te.valid_flag != ~0u && !state.dropFlags.at(te.valid_flag)) { todo_queue.push_back(::std::make_pair(te.invalidTarget, mv$(state))); } else {
            ...
            (Drop, if (te.flagIdx == ~0u || state.dropFlags.at(te.flagIdx)) {
                if (te.kind == ::MIR::eDropKind::SHALLOW) {
                    ...
                    MIR_ASSERT(mir_res, sub_states[0].isValid(), "Shallow drop on deallocated Box - " << te.slot << " (state=" << StateFmt(state, vs) << ")");
            ...
            (Call, if (const auto* e = te.fcn.opt_Value()) { state.ensureLvalueValid(mir_res, *e); } for (auto& arg : te.args) {
             ...
             todo_queue.push_back(::std::make_pair(te.retBlock, mv$(state)));)
        )
    }
}
    ...
    default:
        ...
        TU_ARMA(Path, te) {
            ...
            if (te.binding.is_Struct()) {
                ...
                auto innerRval = MIRCleanupLiteralToRValue(state, mutator, lit, repr->fields[var_idx].ty, params, mv$(path));
                auto innerLval = mutator.inTemporary(monomorphEraseLifetimes.monomorphType(state.sp, repr->fields[var_idx].ty), mv$(innerRval));
                return ::MIR::RValue::make_UnionVariant({te.path.mData.as_Generic().clone(), var_idx, mv$(innerLval)});
            } else {
        ...
        TU_ARMA(Pointer, te) {
            if (lit.getReloc()) {
                ...
                auto lval = mutator.inTemporary(mv$(ty_borrow), mv$(rval));
                ...
                switch (metaTy) {
                    case MetadataType::None:
                        ...
                        if (src_ty != te.inner) {
                            ...
                            auto src_ty_ref = mutator.inTemporary(src_ref_ty, mv$(ptrVal));
                            auto src_ty_ptr = mutator.inTemporary(src_ptr_ty, ::MIR::RValue::make_Cast({mv$(src_ty_ref), src_ptr_ty}));
                            auto innerLval = mutator.inTemporary(innerPtrTy, ::MIR::RValue::make_Cast({mv$(src_ty_ptr), innerPtrTy}));
                    ...
                    case MetadataType::Slice: {
                        ...
                        return ::MIR::RValue::make_MakeDst({::MIR::Param(mv$(ptrVal)), mv$(size_val)});
                        break;
                    }
                    case MetadataType::TraitObject: {
                        ...
                        return ::MIR::RValue::make_MakeDst({::MIR::Param(mv$(ptrVal)), mv$(vtable_val)});
                        break;
                    }
                    case MetadataType::Unknown:
...
::MIR::LValue MIRCleanupVirtualize(const Span& sp, const ::MIR::TypeResolve& state, MirMutator& mutator, ::MIR::LValue& receiverLvp, const ::HIR::Path::Data::Data_UfcsKnown& pe) {
    ...
    auto vtable_lv = mutator.newTemporary(mv$(vtable_ty));
        ...
        static ::MIR::LValue getUnitPtr(const ::MIR::TypeResolve& state, MirMutator& mutator, ::HIR::TypeRef ty, ::MIR::LValue lv, ::MIR::LValue& outInnerPtr) {
            if (ty->is_Path()) {
                ...
                auto monomorph = [&](const auto& t) {
                    return MonomorphStatePtr(state.crate.types, nullptr, &ty_path.mParams, nullptr).monomorphType(state.sp, t);
                };
                ::std::vector<::MIR::Param> vals;
                TU_MATCH_HDRA( (str.mData), {)
                TU_ARMA(Unit, se) {
    ...
    auto vtable_rval = ::MIR::RValue::make_DstMeta({mv$(innerDynPtr)});
    mutator.pushStatement(::MIR::Statement::make_Assign({vtable_lv.clone(), mv$(vtable_rval)}));

    // Update the terminator with the new information.
    return fcnLval;
}

bool MIRCleanupUnsizeGetMetadata(const ::MIR::TypeResolve& state, MirMutator& mutator, const ::HIR::TypeData* dstTy, const ::HIR::TypeData* src_ty, const ::MIR::LValue& ptrValue, ::MIR::Param& outMetaVal, ::HIR::TypeRef& outMetaTy, bool& outSrcIsDst) {
    TU_MATCH_HDRA( (*dstTy), { )
    default:
        ...
        TU_ARMA(Unit, se) {
                ...
                TU_ARMA(Tuple, se) {
                    ...
                    auto ty_d = monomorphCbD.monomorphType(state.sp, ty_tpl, false);
                    auto ty_s = monomorphCbS.monomorphType(state.sp, ty_tpl, false);

                    return MIRCleanupUnsizeGetMetadata(state, mutator, ty_d, ty_s, ptrValue, outMetaVal, outMetaTy, outSrcIsDst);
                }
                TU_ARMA(Named, se) {
        ...
        TU_ARMA(Slice, de) {
            ...
            if (src_ty->is_Array()) {
                const auto& inArray = src_ty->as_Array();
        ...
        TU_ARMA(TraitObject, de) {
            ...
            outMetaTy = state.crate.types.pointer(::HIR::BorrowType::Shared, vtable_ty);
            ...
            if (const auto* se = src_ty->opt_TraitObject()) {
                ...
                if (se->mTrait.traitPtr != de.mTrait.traitPtr) {
                    ...
                    auto inMetaTy = state.crate.types.pointer(::HIR::BorrowType::Shared, vtable_ty);
                ...
                outMetaVal = createVtable(src_ty, de.mTrait);
            }
            return true;
        }
    }
...
::MIR::RValue MIRCleanupUnsize(const ::MIR::TypeResolve& state, MirMutator& mutator, const ::HIR::TypeData* dstTy, const ::HIR::TypeData* src_ty_inner, ::MIR::LValue ptrValue) {
    ...
    if (MIRCleanupUnsizeGetMetadata(state, mutator, dstTyInner, src_ty_inner, ptrValue, metaValue, metaType, source_is_dst)) {
        ...
        if (source_is_dst) {
            ...
            auto thin_ptr_lval = mutator.inTemporary(mv$(ty_unit_ptr), ::MIR::RValue::make_DstPtr({mv$(ptrValue)}));

            return ::MIR::RValue::make_MakeDst({mv$(thin_ptr_lval), mv$(metaValue)});
        } else {
    ...
    if (dstTy->is_Path()) {
        ...
        TU_ARMA(Unit, se) {
            ...
            TU_ARMA(Tuple, se) {
                ...
                for (unsigned int i = 0; i < se.size(); i++) {
                    if (i == str.structMarkings.coerceUnsizedIndex) {
                        auto ty_d = monomorphCbD.monomorphType(state.sp, se[i].ent, false);
                        auto ty_s = monomorphCbS.monomorphType(state.sp, se[i].ent, false);

                        auto newRval = MIRCleanupCoerceUnsized(state, mutator, ty_d, ty_s, ::MIR::LValue::newField(value.clone(), i));
                        auto newLval = mutator.inTemporary(mv$(ty_d), mv$(newRval));

                        ents.push_back(mv$(newLval));
                    } else if (state.mResolve.isTypePhantomData(se[i].ent)) {
                        auto ty_d = monomorphCbD.monomorphType(state.sp, se[i].ent, false);

                        auto newRval = ::MIR::RValue::make_Struct({ty_d->as_Path().path.mData.as_Generic().clone(), {}});
                        auto newLval = mutator.inTemporary(mv$(ty_d), mv$(newRval));
            ...
            TU_ARMA(Named, se) {
                ...
                for (unsigned int i = 0; i < se.size(); i++) {
                    if (i == str.structMarkings.coerceUnsizedIndex) {
                        auto ty_d = monomorphCbD.monomorphType(state.sp, se[i].ty, false);
                        auto ty_s = monomorphCbS.monomorphType(state.sp, se[i].ty, false);

                        auto newRval = MIRCleanupCoerceUnsized(state, mutator, ty_d, ty_s, ::MIR::LValue::newField(value.clone(), i));
                        auto newLval = mutator.newTemporary(mv$(ty_d));
    ...
    for(size_t i = 0; i < lval.wrappers.size(); i ++)
    {
        ...
        if (state.mResolve.isTypeOwnedBox(ty)) {
            ...
            while (typ->is_Path()) {
                ...
                tmp = MonomorphStatePtr(state.crate.types, nullptr, &te.path.mData.as_Generic().mParams, nullptr).monomorphType(state.sp, ty_tpl);
                typ = tmp;
    ...
    if( p.is_Constant() && p.as_Constant().is_Const() )
    {
        ...
        if (litPtr) {
            ...
            if (auto* lv = newRval.opt_Use()) {
                ...
                auto tmp_lv = mutator.inTemporary(mv$(cTy), mv$(newRval));
            ...
            if( stmt.is_Assign() )
            {
                ...
                if (auto* e = se.src.opt_MakeDst()) {
                    if (TU_TEST2(e->metaVal, Constant, , ItemAddr, .get() == nullptr)) {
                        ...
                        const auto& src_ty = state.getParamType(tmp, e->ptrVal);
                        ...
                        se.src = MIRCleanupCoerceUnsized(state, mutator, dstTy, src_ty, mv$(e->ptrVal.as_LValue()));
                    }
                }
                ...
                if (auto* e = se.src.opt_MakeDst()) {
                    if (TU_TEST2(e->metaVal, Constant, , ItemAddr, .get() == nullptr)) {
                        ...
                        ::HIR::TypeRef tmp;
                        const auto& src_ty = state.getParamType(tmp, e->ptrVal);
                        MIR_ASSERT(state, monomorphiseTypeNeeded(src_ty), "MakeDst Unsize with known source - " << src_ty);
                    }
                }
            }
...
namespace {
    ...
    bool visit_mir_lvalues_mut(::MIR::RValue& rval, ::std::function<bool(::MIR::LValue&, ValUsage)> cb) {
        ...
        TU_ARMA(Use, se) {
            ...
            TU_ARMA(MakeDst, se) {
                rv |= visit_mir_lvalue_mut(se.ptrVal, ValUsage::Move, cb);
                rv |= visit_mir_lvalue_mut(se.metaVal, ValUsage::Read, cb); // Note, metadata has to be Copy
            }
            TU_ARMA(Tuple, se) {
                for (auto& v : se.vals) {
    ...
    bool visit_mir_lvalues_mut(::MIR::Terminator& term, ::std::function<bool(::MIR::LValue&, ValUsage)> cb) {
        ...
        TU_ARMA(Incomplete, e) {
            ...
            TU_ARMA(Call, e) {
                ...
                rv |= visit_mir_lvalue_raw_mut(e.retVal, ValUsage::Write, cb);
            }
        }
        return rv;
    }
        ...
        if (auto* te = block.terminator.opt_Switch()) {
            ...
            for (auto& t : te->targets) {
                ...
                if (fcn.blocks[t].statements.empty() && fcn.blocks[t].terminator.is_Switch()) {
                    ...
                    if (nTe.valid_flag == ~0u && nTe.val == te->val) {
...
bool MIROptimiseInlining(::MIR::TypeResolve& state, ::MIR::Function& fcn, bool minimal, const TransList* list /*=nullptr*/) {
    ...
    struct Cloner: public ::MIR::Cloner {
        ...
        ::MIR::BasicBlock cloneBb(const ::MIR::BasicBlock& src, unsigned src_idx, unsigned newIdx) const {
            ::MIR::BasicBlock rv;
            rv.isCleanup = src.isCleanup;
            rv.statements.reserve(src.statements.size());
            for (const auto& stmt : src.statements) {
                DEBUG("BB" << src_idx << "->BB" << newIdx << "/" << rv.statements.size() << ": " << stmt);
                rv.statements.push_back(this->cloneStmt(stmt));
                DEBUG("-> " << rv.statements.back());
            }
            DEBUG("BB" << src_idx << "->BB" << newIdx << "/" << rv.statements.size() << ": " << src.terminator);
    ...
    for (unsigned int i = 0; i < fcn.blocks.size(); i++) {
        ...
        if (auto* te = fcn.blocks[i].terminator.opt_Call()) {
            ...
            // Allocate a temporary for the return value
            {
                ...
                ::HIR::TypeRef tmp_ty;
                fcn.locals.push_back(state.getLvalueType(tmp_ty, te->retVal));
        ...
        for (auto ref = start; ref.bbIdx != end.bbIdx || ref.stmt_idx < end.stmt_idx;) {
            ...
            if (ref.stmt_idx < bb.statements.size()) {
                ...
                // positions.
                    ...
                    // Possibly loop into the next block
                    if (!visted_bbs.insert(te->retBlock).second) {
                        DEBUG("> Loop abort");
                        return IterPathRes::Abort;
                    }
                    ref.stmt_idx = 0;
...
bool MIROptimiseDeTemporarySingleSetAndUse(::MIR::TypeResolve& state, ::MIR::Function& fcn) {
    ...
    for (size_t var_idx = 0; var_idx < fcn.locals.size(); var_idx++) {
        ...
        DEBUG("_" << var_idx << ": " << slot.nWrite << "," << slot.nRead << "," << slot.nBorrow);
        if (slot.nWrite == 1 && slot.nRead == 1 && slot.nBorrow == 0) {
            ...
            if (slot.set_loc.stmt_idx < set_bb.statements.size()) {
                ...
                set_loc_next.bbIdx = set_bb.terminator.as_Call().retBlock;
            ...
            if (slot.set_loc.stmt_idx < set_bb.statements.size() && TU_TEST1(set_bb.statements[slot.set_loc.stmt_idx], Assign, .src.is_Use())) {
                ...
                bool src_copy = src.wrappers.empty() && state.lvalueIsCopy(src);
                ...
                if (!invalidated && std::any_of(src.wrappers.begin(), src.wrappers.end(), [](const MIR::LValue::Wrapper& w) {
                    ...
                    invalidated = IterPathRes::Complete != iterPath(fcn, set_loc_next, use_loc_inc, [&](auto loc, const auto& stmt) -> bool {
                ...
                if (!invalidated) {
                    ...
                    if (slot.use_loc.stmt_idx < use_bb.statements.size()) {
                        ...
                        bool found = visit_mir_lvalues_mut(use_stmt, replaceCb);
                        if (!found) {
                        ...
                        bool found = visit_mir_lvalues_mut(use_term, replaceCb);
                        if (!found) {
...
bool MIROptimiseDeTemporaryBorrows(::MIR::TypeResolve& state, ::MIR::Function& fcn) {
    ...
    for (size_t var_idx = 0; var_idx < fcn.locals.size(); var_idx++) {
        ...
        if (slot.nDerefRead > 1 && src_lv.wrappers.size() >= 2) {
        ...
        if (slot.nDerefRead + slot.nOtherRead > 1 && src_borrow.type != ::HIR::BorrowType::Shared) {
            DEBUG(this_var << " - Multi-use non-shared borrow, too complex to do");
            continue;
        }
        DEBUG(this_var << " - Borrow of " << src_lv << " at " << slot.set_loc << ", used " << slot.nDerefRead << " times (dropped {" << slot.dropLocs << "})");
        bool src_copy = state.lvalueIsCopy(src_lv);
        ...
        for (bool stop = false; !stop;) {
            ...
            for (; curLoc.stmt_idx < curBb.statements.size(); curLoc.stmt_idx++) {
                ...
                // Replace usage
                visit_mir_lvalues_mut(stmt, replaceCb);
            ...
            // Replace usage
            visit_mir_lvalues_mut(curBb.terminator, replaceCb);
            if (numReplaced == slot.nDerefRead) {
                stop = true;
                break;
            }
        ...
        if (src_lv.root.is_Local() && !src_lv.wrappers.empty() && src_lv.wrappers.front().is_Deref()) {
            usage_info[src_lv.root.as_Local()].nDerefRead += numReplaced;
            if (numReplaced == slot.nDerefRead) {
                usage_info[src_lv.root.as_Local()].nDerefRead -= 1;
            }
        }
    ...
    for (unsigned int bbIdx = 0; bbIdx < fcn.blocks.size(); bbIdx++) {
                ...
                // Source invalidated?
                    ...
                    switch (vu) {
                        ...
                        case ValUsage::Move:   // Moved? Now invalid
                            visit_mir_lvalues(src_rvalue, [&](const ::MIR::LValue& sLv, auto sVu) {
                                //DEBUG("   " << s_lv << " ?= " << lv);
                                if (sLv.root == lv.root) {
        ...
        auto cbApplyReplacements = [&](auto& top_lv, auto top_usage) {
            ...
            if (top_lv.root.is_Local()) {
                ...
                auto it = localAssignments.find(top_lv.root.as_Local());
                if (it != localAssignments.end()) {
                    const auto& newVal = bb.statements[it->second].as_Assign().src.as_Use();
                    // - Copy? All is good.
                    if (state.lvalueIsCopy(ilv)) {
                        top_lv = newVal.cloneWrapped(top_lv.wrappers.begin(), top_lv.wrappers.end());
        ...
        for (unsigned int stmt_idx = 0; stmt_idx < bb.statements.size(); stmt_idx++) {
            ...
            if (stmt.is_Assign() && stmt.as_Assign().dst.is_Local() && stmt.as_Assign().src.is_Use()) {
                ...
                if (visit_mir_lvalues_inner(src_lv, ValUsage::Read, [&](const auto& lv, auto) {
                    ...
                    localAssignments.insert(::std::make_pair(stmt.as_Assign().dst.as_Local(), stmt_idx));
...
bool MIROptimiseUnifyTemporaries(::MIR::TypeResolve& state, ::MIR::Function& fcn) {
    ...
    for (unsigned int localIdx = 0; localIdx < fcn.locals.size(); localIdx++) {
        ...
        if (!slot_lifetimes[localIdx].isUsed()) {
            continue;
        }
        ...
        for (unsigned int i = localIdx + 1; i < fcn.locals.size(); i++) {
            ...
            if (!slot_lifetimes[i].isUsed()) {
                continue;
            }
            // Variables are of the same type, check if they overlap
            if (slot_lifetimes[localIdx].overlaps(slot_lifetimes[i])) {
                continue;
            }
            // They don't overlap, unify
            slot_lifetimes[localIdx].unify(slot_lifetimes[i]);
...
bool MIROptimisePropagateKnownValues(::MIR::TypeResolve& state, ::MIR::Function& fcn) {
    ...
    auto getField = [&](const ::MIR::LValue& slot_lvalue, unsigned field, size_t start_bb_idx, size_t start_stmt_idx) -> const ::MIR::LValue* {
        ...
        bool slot_copy = state.lvalueIsCopy(slot_lvalue);
            ...
            while (stmt_idx--) {
                ...
                if (const auto* se = stmt.opt_Assign()) {
                    if (se->dst == slot_lvalue) {
                        ...
                        bool src_copy = state.lvalueIsCopy(src_lval);
        ...
        for (size_t i = 0; i < block.statements.size(); i++) {
            ...
            visit_mir_lvalues_mut(block.statements[i], [&](::MIR::LValue& lv, auto vu) {
                if (vu == ValUsage::Read && lv.wrappers.size() > 1 && lv.wrappers.front().is_Field() && lv.root.is_Local()) {
                    ...
                    ::HIR::TypeRef tmp;
                    if (!state.mResolve.type_is_copy(state.sp, state.getLvalueType(tmp, innerLv))) {
                        DEBUG(state << "- not Copy, can't optimise");
                        return false;
                    }
                    const auto* source_lvalue = getField(innerLv, fieldIndex, bbIdx, i);
                    if (source_lvalue) {
                        if (outerLv != *source_lvalue) {
...
bool MIROptimiseConstPropagate(::MIR::TypeResolve& state, ::MIR::Function& fcn) {
    ...
    for (auto& bb : fcn.blocks) {
        ...
        // NOTE: Quick special-case for bswap<u8/i8> (a no-op)
            ...
            if (!visit_ty_with(ty, [](const ::HIR::TypeData* ty) -> bool {
                ...
                bool needsDrop = state.mResolve.type_needs_drop_glue(state.sp, ty);
    ...
    for (auto& bb : fcn.blocks) {
        ...
        for (auto& stmt : bb.statements) {
                ...
                TU_ARMA(Use, se) {
                    ...
                    TU_ARMA(Cast, se) {
                        ...
                        if (!nv.is_ItemAddr()) {
                                ...
                                switch (*te) {
                                    ...
                                    case ::HIR::CoreType::Usize:
                                        if (const auto* vp = nv.opt_Uint()) {
                                            newValue = ::MIR::Constant::make_Uint({H::truncate_u(*te, vp->v), *te});
                                        } else if (const auto* vp = nv.opt_Int()) {
                                            newValue = ::MIR::Constant::make_Uint({H::truncate_u(*te, vp->v.getInner()), *te});
                                            ...
                                            if (FloatValue() <= vp->v && vp->v < FloatValue(18446744073709551616.0)) {
                                                newValue = ::MIR::Constant::make_Uint({H::truncate_u(*te, U128(static_cast<uint64_t>(vp->v))), *te});
                                            } else {
                                    ...
                                    case ::HIR::CoreType::Isize:
                                        if (const auto* vp = nv.opt_Uint()) {
                                            newValue = ::MIR::Constant::make_Int({H::truncate_s(*te, vp->v), *te});
                                        } else if (const auto* vp = nv.opt_Int()) {
                                            newValue = ::MIR::Constant::make_Int({H::truncate_s(*te, vp->v), *te});
                                        } else if (const auto* vp = nv.opt_Bool()) {
                            ...
                            auto variant_idx = knownValuesVar.at(se.val);
                            ...
                            switch (ct) {
                                ...
                                case ::HIR::CoreType::U128:
                                case ::HIR::CoreType::Usize:
                                    newValue = ::MIR::Constant::make_Uint({H::truncate_u(ct, value.getInner()), ct});
                                    break;
                                case ::HIR::CoreType::I8:
                                case ::HIR::CoreType::I16:
                                case ::HIR::CoreType::I32:
                                ...
                                case ::HIR::CoreType::I128:
                                case ::HIR::CoreType::Isize:
                                    newValue = ::MIR::Constant::make_Int({H::truncate_s(ct, value), ct});
                                    break;
                                case ::HIR::CoreType::F16:
                                case ::HIR::CoreType::F32:
                                case ::HIR::CoreType::F64:
                        ...
                        if (se.val_l.is_Constant() && se.val_r.is_Constant()) {
                            ...
                            if (val_l.is_Const() || val_r.is_Const()) {
                                ...
                                switch (se.op) {
                                    ...
                                    case ::MIR::eBinOp::EQ:
                                        if (!val_l.is_Float()) {
                                            newValue = ::MIR::Constant::make_Bool({val_l == val_r});
                                        }
                                        break;
                                    case ::MIR::eBinOp::NE:
                                        if (!val_l.is_Float()) {
                                ...
                                default:
                                    break;
                                            ...
                                            TU_ARMA(Int, le, re) {
                                                MIR_ASSERT(state, le.t == re.t, "Mismatched types for eBinOp::ADD - " << val_l << " + " << val_r);
                                                newValue = ::MIR::Constant::make_Int({H::truncate_s(le.t, le.v + re.v), le.t});
                                            }
                                            TU_ARMA(Uint, le, re) {
                                                MIR_ASSERT(state, le.t == re.t, "Mismatched types for eBinOp::ADD - " << val_l << " + " << val_r);
                                                newValue = ::MIR::Constant::make_Uint({H::truncate_u(le.t, le.v + re.v), le.t});
                                            }
                                }
                                break;
                            case ::MIR::eBinOp::SUB:
                            ...
                            case ::MIR::eBinOp::BIT_SHL: {
                                ...
                                default:
                                    break;
                                                TU_ARMA(Int, le) {
                                                    newValue = ::MIR::Constant::make_Int({H::truncate_s(le.t, le.v << shift_len), le.t});
                                                }
                                                TU_ARMA(Uint, le) {
                                                    newValue = ::MIR::Constant::make_Uint({H::truncate_u(le.t, le.v << shift_len), le.t});
                                                }
                                }
                                } break;
                            case ::MIR::eBinOp::BIT_SHR:{
                            ...
                            switch (se.op) {
                                ...
                                case ::MIR::eBinOp::SUB:
                                    if (se.val_r.is_Constant() && se.val_r.as_Constant().is_Uint() && se.val_r.as_Constant().as_Uint().v == 0) {
                                        newValue = mv$(se.val_l);
                                    }
                                    break;
                                // `foo % 1 == 0`
                                case ::MIR::eBinOp::MOD:
                                    if (se.val_r.is_Constant() && se.val_r.as_Constant().is_Uint() && se.val_r.as_Constant().as_Uint().v == 1) {
                                        newValue = ::MIR::Constant::make_Uint({U128(0), se.val_r.as_Constant().as_Uint().t});
                                    }
                                    break;
                                // `foo / 1 == foo`
                                case ::MIR::eBinOp::DIV:
            ...
            visit_mir_lvalues(stmt, [&knownValues, &knownValuesVar](const ::MIR::LValue& lv, ValUsage vu) -> bool {
...
bool MIROptimiseSplitAggregates(::MIR::TypeResolve& state, ::MIR::Function& fcn) {
    ...
    for (auto& p : potentials) {
        ...
        for (size_t i = 0; i < vals.size(); i++) {
            ...
            block.statements[stmt_idx + i] = MIR::Statement::make_Assign({MIR::LValue::newLocal(new_local), paramToRvalue(mv$(vals[i]))});
        ...
        if (offset > 0) {
            for (auto& otherP : potentials) {
                if (otherP.second.src_bb_idx == bbIdx && otherP.second.src_stmt_idx > stmt_idx) {
                    otherP.second.src_stmt_idx += offset;
                }
            }
        }
    }
    ...
    // > Replace usage with the inner of the original `Use`
    {
        ...
        for (const auto& block : fcn.blocks) {
            ...
            for (unsigned int stmt_idx = 0; stmt_idx < block.statements.size(); stmt_idx++) {
                ...
                if (e.dst.is_Local()) {
                    const auto& vu = val_uses.localUses[e.dst.as_Local()];
                ...
                for (unsigned int si2 = stmt_idx + 1; si2 < block.statements.size(); si2++) {
                    ...
                    // Usage found.
                    if (visit_mir_lvalues(stmt2, isLvalueUsage)) {
        ...
        for (auto& block : fcn.blocks) {
            for (auto it = block.statements.begin(); it != block.statements.end(); ++it) {
                ...
                if (to_replace_lval.is_Local()) {
                    const auto& vu = val_uses.localUses[to_replace_lval.as_Local()];
                    if (!(vu.read == 1 && vu.write == 1 && vu.borrow == 0)) {
                        continue;
                    }
                } else {
                ...
                for (auto it2 = it + 1; it2 != block.statements.end(); ++it2) {
                    ...
                    if (newDstLval.is_Local()) {
                        const auto& vu = val_uses.localUses[newDstLval.as_Local()];
                        if (!(vu.read == 1 && vu.write == 1 && vu.borrow == 0)) {
                            break;
                        }
                    ...
                    if (!was_invalidated) {
                        DEBUG(state << "Replace assignment of " << to_replace_lval << " with " << newDstLval);
    ...
    // > Replace function call result value with target of assignment
    {
        ...
        for (auto& block : fcn.blocks) {
            ...
            if (block.terminator.is_Call()) {
                ...
                const auto& vu = val_uses.localUses[e.retVal.as_Local()];
                if (!(vu.read == 1 && vu.write == 1 && vu.borrow == 0)) {
                    continue;
                }
        ...
        for (auto& block : fcn.blocks) {
            for (auto it = block.statements.begin(); it != block.statements.end(); ++it) {
                ...
                if (const auto& se = it->opt_Assign()) {
                    ...
                    if (se->dst.is_Local()) {
                        const auto& vu = val_uses.localUses[se->dst.as_Local()];
...
bool MIROptimiseDeadDropFlags(::MIR::TypeResolve& state, ::MIR::Function& fcn) {
    ...
    ::std::vector<bool> used_drop_flags(fcn.dropFlags.size());
    {
        ...
        visit_blocks(state, fcn, [&readDropFlags, &used_drop_flags](auto, const ::MIR::BasicBlock& block) {
            ...
            if (const auto* e = block.terminator.opt_Switch()) {
                if (e->valid_flag != ~0u) {
                    readDropFlags[e->valid_flag] = true;
                    used_drop_flags[e->valid_flag] = true;
                }
            }
        });
        DEBUG("Un-read drop flags:" << FMT_CB(ss, for (size_t i = 0; i < readDropFlags.size(); i++) if (!readDropFlags[i] && used_drop_flags[i]) ss << " " << i;));
        visit_blocks_mut(state, fcn, [&readDropFlags, &removedStatement](auto _id, auto& block) {
            for (auto it = block.statements.begin(); it != block.statements.end();) {
    ...
    // Find any drop flags that are never assigned with a value other than their default, then remove those dead assignments.
    {
        ...
        visit_blocks_mut(state, fcn, [&editedDropFlags, &removedStatement, &fcn](auto _id, auto& block) {
            for (auto it = block.statements.begin(); it != block.statements.end();) {
            ...
            if (it->is_Assign() && it->as_Assign().dst.is_Local() && it->as_Assign().src.is_Borrow() && it->as_Assign().src.as_Borrow().val.is_Deref()) {
                ...
                for (auto it2 = it + 1; it2 != bb.statements.end(); ++it2) {
                    ...
                    if (it2->is_Assign() && it2->as_Assign().src.is_Cast() && it2->as_Assign().src.as_Cast().val == dstLv) {
                        ...
                        HIR::TypeRef tmp;
                        const auto& origTy = state.getLvalueType(tmp, src_lv);
                        if (origTy == dstTy) {
...
bool MIROptimiseGotoAssign(::MIR::TypeResolve& state, ::MIR::Function& fcn) {
    ...
    for (auto& dstBb : fcn.blocks) {
        ...
        for (auto bbIdx : sources) {
            ...
            if (TU_TEST1(src_bb.terminator, Call, .retVal == src)) {
                DEBUG("- Source block: BB" << bbIdx << " - term " << src_bb.terminator);
                src_bb.terminator.as_Call().retVal = dst.clone();
...
bool MIROptimiseGarbageCollect(::MIR::TypeResolve& state, ::MIR::Function& fcn) {
    ...
    for (unsigned int i = 0, j = 0; i < nLocals; i++) {
        ...
        localRewriteTable.push_back(used_locals[i] ? j++ : ~0u);
    }
    ...
    for (unsigned int i = 0; i < visited.size(); i++) {
        if (visited[i]) {
            ...
            for (auto& stmt : it->statements) {
                ...
                visit_mir_lvalues_mut(stmt, lvalueCb);
            ...
            visit_mir_lvalues_mut(it->terminator, lvalueCb);
            if (auto* drop = it->terminator.opt_Drop()) {
                if (drop->flagIdx != ~0u) {
...
void MIRSortBlocks(const StaticTraitResolve& resolve, const ::HIR::ItemPath& path, ::MIR::Function& fcn) {
    ...
    while (!todo.empty()) {
        ...
        TU_MATCHA((bb.terminator), (te),
            ...
            (Switch, for (auto dst : te.targets) todo.push_back(Todo{dst, ++branches, info.level + 1}); if (te.valid_flag != ~0u) todo.push_back(Todo{te.invalidTarget, ++branches, info.level + 1});),
    ...
    for (auto idx : idxes) {
        ...
        visit_terminator_target_mut(newBlockList.back().terminator, [&](auto& te) {
            te = fixBbIdx(te);
        });
    }
