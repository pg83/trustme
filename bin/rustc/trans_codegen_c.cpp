...
namespace {
    ...
    public:
        ...
        void finalise(const TransOptions& opt, CodegenOutput out_ty, const ::std::string& hirFile) override {
            const bool createShims = (out_ty == CodegenOutput::Executable);
            ...
            if (out_ty == CodegenOutput::Executable && !crate.noMain) {
                ...
                if (cStartPath == ::HIR::SimplePath()) {
                    ...
                    if (crate.isNoCore && start_path == ::HIR::SimplePath()) {
                        ...
                        auto start_gpath = ::HIR::GenericPath(mResolve.crate.getLangItemPath(Span(), "start"));
                        start_gpath.mParams.types.push_back(mainFcn.returnType);
                        of << "\treturn " << TransMangle(start_gpath) << "(" << TransMangle(::HIR::GenericPath(mainPath)) << ", argc, (uint8_t**)argv";
                ...
                // Allocator/panic shims
                {
                    ...
                    for (size_t i = 0; i < NUM_ALLOCATOR_METHODS; i++) {
                        struct H {
                            ...
                            static void emitProto(::std::ostream& os, const AllocatorMethod& method, const char* name_prefix, const ::std::vector<const char*>& args) {
                        ...
                        if (!hasGlobalAllocator) {
                            ...
                            const auto method_path = TransAllocatorMethodPath(crate, globalAllocator->mType, method);
                    ...
                    {
                        auto oom_method = crate.getLangItemPathOpt("mrustc-alloc_error_handler");
                        ...
                        if (oom_method != HIR::SimplePath()) {
                            ...
                            of << "void oom_impl(struct s_" << TransMangle(layoutPath) << "_A l) {"
                                 << " extern void " << TransMangle(oom_method) << "(struct s_" << TransMangle(layoutPath) << "_A l);"
...
// If the previous is also a marker, don't push
            ...
            switch (out_ty) {
                ...
                case CodegenOutput::DynamicLibrary:
                    ...
                    for (const auto& path : opt.librarySearchDirs) {
                        librariesAndDirs.push_dir(path.c_str());
                    }
                    for (const auto& path : opt.libraries) {
                        librariesAndDirs.push_lib(path.c_str());
                    }
                    librariesAndDirs.push_border();
                    ...
                    for (const auto& lib : crate.extLibs) {
                        ...
                        librariesAndDirs.push_lib(lib.name.c_str());
                    }
                    ...
                    for (const auto& crate_name : crate.extCratesOrdered) {
                        ...
                        for (const auto& lib : extCrate.mData->extLibs) {
                            ...
                            if (path != "") {
                                librariesAndDirs.push_explicit(std::move(path));
                            } else {
                                librariesAndDirs.push_lib(lib.name.c_str());
                            }
                        }
        ...
        void emitBoxDrop(unsigned indent_level, const ::HIR::TypeData* innerType, const ::HIR::TypeData* boxType, const ::MIR::LValue& slot, bool run_destructor) {
            ...
            if (run_destructor) {
                ...
                emitDestructorCall(::MIR::LValue::newDeref(mv$(innerPtr)), innerType, /*unsized_valid=*/true, indent_level);
            }
            ...
            for (const auto& fieldRepr : repr->fields) {
                if (mResolve.type_needs_drop_glue(sp, fieldRepr.ty)) {
                    emitDestructorCall(field, fieldRepr.ty, /*unsized_valid=*/false, indent_level);
                }
                field.incField();
            }
        }
        ...
        void emitStructInner(const ::HIR::TypeData* ty, const TypeRepr* repr, unsigned packing_max_align) {
            ...
            if (packing_max_align == 0 && !hasManualAlign && repr->align == 1 && repr->size > 1) {
                packing_max_align = 1;
            }
            ...
            for (unsigned fld : fields) {
                ...
                if (s == SIZE_MAX) {
                    ...
                    a = packing_max_align > 0 ? std::min<size_t>(packing_max_align, fieldAlign) : fieldAlign;
                    DEBUG("a = " << a);
                    while (curOfs % a != 0) {
                        curOfs++;
                    }
            ...
            if (sized_fields == 0 && !hasUnsized && options.disallowEmptyStructs) {
                of << "\tchar _d;\n";
            }
            of << "}";
            if (hasManualAlign) {
        ...
        void emitType(const ::HIR::TypeData* ty) override {
            ...
            default:
                ...
                TU_ARMA(Array, te) {
                    ...
                    const bool isZeroSized = rust_size == 0;
        ...
        void emitConstructorEnum(const Span& sp, const ::HIR::GenericPath& path, const ::HIR::Enum& item, size_t var_idx) override {
            ...
            emitStatement(*mirRes, ::MIR::Statement::make_Assign({::MIR::LValue::newReturn(), ::MIR::RValue::make_EnumVariant({p.clone(), static_cast<unsigned>(var_idx), mv$(vals)})}));
            of << "\treturn rv;\n";
            of << "}\n";
            mirRes = nullptr;
        }
        ...
        void emitFunctionCode(const ::HIR::Path& p, const ::HIR::Function& item, const TransParams& params, bool isExternDef, const ::MIR::FunctionPointer& code) override {
            ...
            // Variables
            of << "\t";
            emitCtype(ret_type, FMT_CB(ss, ss << "rv";));
            of << ";\n";
            ...
            if (!cleanupBlocks.empty()) {
                emitCleanupRunner(mir_res, cleanupBlocks);
            }
            ...
            for (unsigned i = 0; i < code->blocks.size(); i++) {
                ...
                for (const auto& stmt : block.statements) {
                    ...
                    emitStatement(mir_res, stmt, 1);
                }
                mir_res.set_cur_stmt_term(i);
                emitBlockTerminator(mir_res, block.terminator, i, false, 1);
                of << "}\n";
            }
            of << "}\n";
            of.flush();
        ...
        void emitOperationWithUnwind(const ::MIR::UnwindAction& action, unsigned indent_level, ::std::function<void(unsigned)> emitOperation) {
            auto indent = RepeatLitStr{"\t", static_cast<int>(indent_level)};
            TU_MATCH_HDRA((action), {)
                TU_ARMA(Continue, _) {
                    emitOperation(indent_level);
                }
                TU_ARMA(Cleanup, target) {
                    of << indent << "try {\n";
                    emitOperation(indent_level + 1);
                    of << indent << "} catch (...) {\n";
        ...
        void emitBlockTerminator(::MIR::TypeResolve& mir_res, const ::MIR::Terminator& term, unsigned blockIndex, bool cleanup, unsigned indent_level) {
            auto indent = RepeatLitStr{"\t", static_cast<int>(indent_level)};
            ...
            TU_MATCH_HDRA((term), {)
                ...
                TU_ARMA(Switch, e) {
                    if (e.valid_flag != ~0u) {
                        of << indent << "if(!df" << e.valid_flag << ") goto " << (cleanup ? "cleanup_bb" : "bb") << e.invalidTarget << ";\n";
                    }
                    emitTermSwitch(mir_res, e.val, e.targets.size(), indent_level, [&](size_t idx) {
                        of << "goto " << (cleanup ? "cleanup_bb" : "bb") << e.targets[idx] << ";";
                    });
                }
                TU_ARMA(SwitchValue, e) {
                    emitTermSwitchvalue(mir_res, e.val, e.values, indent_level, [&](size_t idx) {
                        const auto target = idx == SIZE_MAX ? e.defTarget : e.targets[idx];
                        of << "goto " << (cleanup ? "cleanup_bb" : "bb") << target << ";";
                    });
                }
                TU_ARMA(Drop, e) {
                    emitOperationWithUnwind(e.unwind, indent_level, [&](unsigned operation_indent) {
                        emitDropOperation(mir_res, e, operation_indent);
                    });
                    emitTarget(e.target);
                }
                TU_ARMA(Call, e) {
                    emitOperationWithUnwind(e.unwind, indent_level, [&](unsigned operation_indent) {
                        emitTermCall(mir_res, e, operation_indent);
                    });
                    emitTarget(e.ret_block);
                }
            }
            of << indent << "// ^ " << term << "\n";
            (void)blockIndex;
        ...
        void emitCleanupRunner(::MIR::TypeResolve& mir_res, const ::std::set<unsigned>& cleanupBlocks) {
            ...
            for (auto blockIndex : cleanupBlocks) {
                ...
                for (const auto& stmt : block.statements) {
                    ...
                    emitStatement(mir_res, stmt, 2);
                }
                mir_res.set_cur_stmt_term(blockIndex);
                emitBlockTerminator(mir_res, block.terminator, blockIndex, true, 2);
                of << "\t}\n";
            }
            of << "\t};\n";
        }
        ...
        bool lvalueIsBadZst(const ::MIR::LValue& lv) const {
            if (options.disallowEmptyStructs) {
                HIR::TypeRef tmp;
                return type_is_bad_zst(mirRes->getLvalueType(tmp, lv));
            } else {
                return false;
            }
        }
        ...
        bool lvalueRootIsBadZst(const ::MIR::LValue& lv) const {
            if (options.disallowEmptyStructs) {
                HIR::TypeRef tmp;
                return type_is_bad_zst(mirRes->getLvalueType(tmp, lv, lv.wrappers.size()));
            } else {
                return false;
            }
        }
        ...
        ::MIR::LValue lvalueZstIndexBacking(const ::MIR::LValue& lv) const {
            ...
            while (::MIR::LValue::CRef(rv).is_Index()) {
                ...
                HIR::TypeRef tmp;
                if (!this->type_is_bad_zst(mirRes->getLvalueType(tmp, inner))) {
                    break;
                }
        ...
        void emitBorrow(const ::MIR::TypeResolve& mir_res, HIR::BorrowType bt, const MIR::LValue& val) {
            ::HIR::TypeRef tmp;
            const auto& ty = mir_res.getLvalueType(tmp, val);

            if (this->type_is_bad_zst(ty) && !this->lvalueRootIsBadZst(val)) {
                auto backing = this->lvalueZstIndexBacking(val);
                if (backing.wrappers.size() != val.wrappers.size()) {
                    emitBorrow(mir_res, bt, backing);
                    return;
                }
            }
            ...
            // - Needs to get metadata from bottom-level pointer.
                ...
                if (meta_ty != MetadataType::None) {
                    ...
                    if (meta_ty == MetadataType::TraitObject) {
                        ::HIR::TypeRef baseTmp;
                        const auto& baseTy = mir_res.getLvalueType(baseTmp, baseVal.clone());
            ...
            if (!special && options.disallowEmptyStructs && val_ref.is_Index() && this->type_is_bad_zst(ty)) {
                auto inner = val_ref.innerRef();
                ::HIR::TypeRef tmp;
                const auto& parent_ty = mir_res.getLvalueType(tmp, inner);
                ...
                MIR_ASSERT(mir_res, elementTy, "Index of non-array type in ZST borrow path: " << parent_ty);
                size_t elementSize = 0;
                MIR_ASSERT(mir_res, TargetGetSizeOf(sp, mResolve, elementTy, elementSize), "Unknown array element size for " << parent_ty);
                MIR_ASSERT(mir_res, elementSize == 0, "Non-ZST element in ZST borrow path: " << elementTy);
            ...
            if (!special && options.disallowEmptyStructs && val.is_Field() && this->type_is_bad_zst(ty)) {
                // Work backwards to the first non-ZST field
                auto val_fp = ::MIR::LValue::CRef(val);
                assert(val_fp.is_Field());
                while (val_fp.innerRef().is_Field()) {
                    ::HIR::TypeRef tmp;
                    const auto& ty = mir_res.getLvalueType(tmp, val_fp.innerRef());
                    if (!this->type_is_bad_zst(ty)) {
                        break;
                    }
                    val_fp.try_unwrap();
                ...
                auto fieldInner = val_fp.innerRef();
                if (fieldInner.is_Downcast()) {
                    of << "(void*)& ";
                    emitLvalue(fieldInner.innerRef());
                } else if (val_fp.as_Field() == 0) {
                    ::HIR::TypeRef tmp;
                    const auto& parent_ty = mir_res.getLvalueType(tmp, fieldInner);
                    if (parent_ty->is_Slice()) {
                        MIR_ASSERT(mir_res, fieldInner.is_Deref(), "Raw slice lvalue in ZST borrow path");
                        of << "(void*)";
                        emitLvalue(fieldInner.innerRef());
                        of << ".PTR";
                    } else {
                    ...
                    if (elementTy) {
                        ...
                        of << " + " << elementSize * val_fp.as_Field() << ") /*ZST*/";
                    } else {
                        ...
                        auto tmp_lv = ::MIR::LValue::newField(fieldInner.clone(), val_fp.as_Field() + 1);
                        bool found = false;
        ...
        void emitCompositeAssign(const ::MIR::TypeResolve& mir_res, ::std::function<void()> emitSlot, const ::std::vector<::MIR::Param>& vals, unsigned indent_level, bool prepend_newline = true) {
            auto indent = RepeatLitStr{"\t", static_cast<int>(indent_level)};
            bool hasEmitted = prepend_newline;
            for (unsigned int j = 0; j < vals.size(); j++) {
                if (options.disallowEmptyStructs) {
                    ::HIR::TypeRef tmp;
                    const auto& ty = mir_res.getParamType(tmp, vals[j]);
        ...
        void emitDropOperation(const ::MIR::TypeResolve& mir_res, const ::MIR::Terminator::Data_Drop& e, unsigned indent_level) {
            auto indent = RepeatLitStr{"\t", static_cast<int>(indent_level)};
            ::HIR::TypeRef tmp;
            const auto& ty = mir_res.getLvalueType(tmp, e.slot);
            if (e.flagIdx != ~0u) {
            ...
            switch (e.kind) {
                case ::MIR::eDropKind::SHALLOW:
                    if (const auto* ity = mResolve.isTypeOwnedBox(ty)) {
                        emitBoxDrop(indent_level + (e.flagIdx != ~0u ? 1 : 0), ity, ty, e.slot, false);
                    } else {
                        MIR_BUG(mir_res, "Shallow drop on non-Box - " << ty);
                    }
                    break;
                case ::MIR::eDropKind::DEEP:
                    emitDestructorCall(e.slot, ty, true, indent_level + (e.flagIdx != ~0u ? 1 : 0));
                    break;
            }
            if (e.flagIdx != ~0u) {
                of << indent << "}\n";
        ...
        void emitStatement(const ::MIR::TypeResolve& mir_res, const ::MIR::Statement& stmt, unsigned indent_level = 1) {
            DEBUG(stmt);
            auto indent = RepeatLitStr{"\t", static_cast<int>(indent_level)};
            switch (stmt.tag()) {
                ...
                case ::MIR::Statement::TAG_Asm:
                    this->emitAsmGcc(mir_res, stmt.as_Asm(), indent_level);

                    of << indent << "// ^ " << stmt << "\n";
                    break;
                case ::MIR::Statement::TAG_Asm2:
                    this->emitAsm2Gcc(mir_res, stmt, indent_level);
                ...
                case ::MIR::Statement::TAG_Assign: {
                    ...
                    ::HIR::TypeRef tmp;
                    const auto& ty = mir_res.getLvalueType(tmp, e.dst);
                ...
                TU_ARMA(Use, ve) {
                            ::HIR::TypeRef tmp;
                            const auto& ty = mir_res.getLvalueType(tmp, ve);
                        ...
                        TU_ARMA(Borrow, ve) {
                            ...
                            emitCtype(ty);
                            of << ">(";
                            if (this->type_is_bad_zst(mirRes->getLvalueType(tmp, ve.val, ve.val.wrappers.size()))) {
                                of << "(void*)&rv";
                            } else {
                                emitBorrow(mir_res, ve.type, ve.val);
                            }
                            of << ")";
                        }
                        TU_ARMA(Cast, ve) {
                            emitRvalueCast(mir_res, e.dst, ve);
                        }
                        TU_ARMA(BinOp, ve) {
                            emitLvalue(e.dst);
                            of << " = ";
                            ::HIR::TypeRef tmp, tmp_r;
                            const auto& ty = mir_res.getParamType(tmp, ve.val_l);
                            const auto& ty_r = mir_res.getParamType(tmp_r, ve.val_r);
                            if (ty->is_Borrow()) {
                                of << "(slice_cmp(";
                                emitParam(ve.val_l);
                                of << ", ";
                        ...
                        TU_ARMA(MakeDst, ve) {
                            ...
                            switch (meta) {
                                case MetadataType::Slice:
                                    of << "make_sliceptr";
                                    of << "(";
                                    emitParam(ve.ptr_val, false);
                                    of << ", ";
                                    emitParam(ve.meta_val);
                                    of << ")";
                                    break;
                                case MetadataType::TraitObject:
                                    of << "make_traitobjptr";
                                    of << "(";
                                    emitParam(ve.ptr_val);
                                    of << ", ";
                                    emitTraitMetadataParam(mir_res, ve.meta_val);
                                    of << ")";
                                    break;
                                case MetadataType::Zero:
                                case MetadataType::Unknown:
                        ...
                        TU_ARMA(UnionVariant, ve) {
                            MIR_ASSERT(mir_res, crate.getTypeitemByPath(sp, ve.path.mPath).is_Union(), "");
                            if (!this->type_is_bad_zst(mirRes->getParamType(tmp, ve.val))) {
                                emitLvalue(e.dst);
                                of << ".var_" << ve.index << " = ";
                                emitParam(ve.val);
                            }
                                ...
                                TU_ARMA(Linear, re) {
                                    ...
                                    if (!re.isNiche(ve.index)) {
                                        ...
                                        if (re.field.sub_fields.empty() || type_is_bad_zst(repr->fields[ve.index].ty)) {
                                            emitLvalue(e.dst);
                                            const auto& slot_ty = emitEnumPath(repr, re.field);
                                            of << " = ";
                                    ...
                                    if (enmP->isValue()) {
                                        ...
                                        emitCompositeAssign(mir_res, [&]() {
                                            emitLvalue(e.dst);
                                            of << ".DATA.var_" << ve.index;
                                        }, ve.vals, indent_level, emitNewline);
                                    }
                                }
                                TU_ARMA(Values, re) {
                                    if (re.field.index == 0) {
        ...
        void emitRvalueCast(const ::MIR::TypeResolve& mir_res, const ::MIR::LValue& dst, const ::MIR::RValue::Data_Cast& ve) {
            ...
            ::HIR::TypeRef tmp;
            const auto& ty = mir_res.getLvalueType(tmp, ve.val);
            ...
            // Standard cast
            ::HIR::TypeRef dstTmp;
            const auto& dstTy = mir_res.getLvalueType(dstTmp, dst);
            emitLvalue(dst);
            of << " = ";
            of << "(";
            emitCtype(dstTy);
        ...
        void emitTermSwitch(const ::MIR::TypeResolve& mir_res, const ::MIR::LValue& val, size_t n_arms, unsigned indent_level, ::std::function<void(size_t)> cb, size_t odd_arm = -1) {
            auto indent = RepeatLitStr{"\t", static_cast<int>(indent_level)};
            ...
            TU_ARMA(NonZero, e) {
                    ...
                    of << indent << "if( ";
                    emitLvalue(val);
                    const auto& slot_ty = emitEnumPath(repr, e.field);
                ...
                TU_ARMA(Values, e) {
                    ...
                    const bool is128 = tag_ty == ::HIR::CoreType::I128 || tag_ty == ::HIR::CoreType::U128;
                    const bool emulated128 = type_is_emulated_i128(tag_ty);
                    auto emitTag = [&]() {
                        emitLvalue(val);
                        emitEnumPath(repr, e.field);
                    };
        ...
        void emitTermSwitchvalue(const ::MIR::TypeResolve& mir_res, const ::MIR::LValue& val, const ::MIR::SwitchValues& values, unsigned indent_level, ::std::function<void(size_t)> cb) {
            auto indent = RepeatLitStr{"\t", static_cast<int>(indent_level)};
            ...
            if (const auto* ve = values.opt_String()) {
                ...
                const bool emulatedU128 = options.emulated_i128 && ty == ::HIR::CoreType::U128;
                if (emulatedU128) {
                    of << indent << "if(";
                    emitLvalue(val);
                    of << ".hi != 0) { ";
        ...
        void emitTermCall(const ::MIR::TypeResolve& mir_res, const ::MIR::Terminator::Data_Call& e, unsigned indent_level) {
            auto indent = RepeatLitStr{"\t", static_cast<int>(indent_level)};
            of << indent;
            ...
            {
                ::HIR::TypeRef tmp;
                if (mirRes->getLvalueType(tmp, e.ret_val) == crate.types.unit()) {
                    omit_assign = true;
                }

                if (this->type_is_bad_zst(mirRes->getLvalueType(tmp, e.ret_val))) {
                    omit_assign = true;
                }
            }
            ...
            TU_ARMA(Value, e2) {
                    {
                        ::HIR::TypeRef tmp;
                        const auto& ty = mir_res.getLvalueType(tmp, e2);
                ...
                TU_ARMA(Path, e2) {
                    ...
                    TU_ARMA(Generic, pe) {
                            ...
                            TU_ARMA(UfcsInherent, pe) {
                                ...
                                omit_assign |= mResolve.crate.findTypeImpls(pe.type, HIR::ResolvePlaceholdersNop(), [&](const auto& impl) {
        ...
        void emitAsmGcc(const ::MIR::TypeResolve& mir_res, const ::MIR::Statement::Data_Asm& e, unsigned indent_level) {
            auto indent = RepeatLitStr{"\t", static_cast<int>(indent_level)};
        ...
        void emitAsm2Gcc(const ::MIR::TypeResolve& mir_res, const ::MIR::Statement& stmt, unsigned indent_level) {
            auto indent = RepeatLitStr{"\t", static_cast<int>(indent_level)};
    ...
    private:
        ...
        void emitIntrinsicCall(const RcString& name, const ::HIR::PathParams& params, const ::MIR::Terminator::Data_Call& e) {
            ...
            auto getAtomicOrdering = [&](const RcString& name, size_t prefix_len) -> Ordering {
            ...
            auto getPrimSize = [&mir_res](const ::HIR::TypeData* ty) -> unsigned {
                if (ty->is_Pointer()) {
            ...
            auto emitAtomicCxchg = [&](const auto& e, Ordering o_succ, Ordering o_fail, bool isWeak) {
                switch (o_fail) {
                    case Ordering::Release:
                ...
                of << ", " << getAtomicTyGcc(o_succ) << ", " << getAtomicTyGcc(o_fail) << ")";
            ...
            if (name == "size_of") {
                ...
                size_t val = mir_res.intrinsicOffsetOf(params.types.at(0), e.args);
                emitLvalue(e.ret_val);
                of << " = " << val;
                ...
                // slice metadata (`[T]` and `str`)
                    ...
                    if (const auto* te = innerTy->opt_Slice()) {
                        MIR_ASSERT(mir_res, TargetGetSizeAndAlignOf(sp, mResolve, te->inner, itemSize, itemAlign), "Can't get size of " << te->inner);
                    } else {
                    ...
                    if (!ty->is_Slice() && !ty->is_Primitive()) {
                        ...
                        if (wrapper_align > itemAlign) {
                            itemAlign = wrapper_align;
                            alignNeeded = true;
                            of << "ALIGN_TO(";
                        }
                        const auto* repr = TargetGetTypeRepr(sp, mResolve, ty);
                ...
                // Trait object metadata.
                    ...
                    MIR_BUG(mir_res, "Unknown inner unsized type " << innerTy << " for " << ty);
                }
                // TODO: Align up
            ...
            // --- Type assertions ---
                ...
                emitTermCall(mir_res, pseudo_term, 1);
            }
            // --- Type identity ---
                ...
                auto name = mir_res.intrinsicTypeName(params.types.at(0));
                emitLvalue(e.ret_val);
                ...
                if (this->type_is_bad_zst(ty_dst)) {
                    ...
                    of << ")); ";
                    of << "}";
                } else if (isPtr(ty_dst) && isPtr(ty_src)) {
                    auto src_meta = metadata_type(ty_src->is_Pointer() ? ty_src->as_Pointer().inner : ty_src->as_Borrow().inner);
                    auto dstMeta = metadata_type(ty_dst->is_Pointer() ? ty_dst->as_Pointer().inner : ty_dst->as_Borrow().inner);
                    if (src_meta == MetadataType::None || src_meta == MetadataType::Zero) {
                        MIR_ASSERT(*mirRes, dstMeta == MetadataType::None || dstMeta == MetadataType::Zero, "Transmuting to fat pointer from thin: " << ty_src << " -> " << ty_dst);
                        emitLvalue(e.ret_val);
                        of << " = (";
                        emitCtype(ty_dst);
                        of << ")";
                        ...
                        MIR_BUG(*mirRes, "Transmuting from fat pointer to thin: (" << src_meta << "->" << dstMeta << ") " << ty_src << " -> " << ty_dst);
                    } else if (src_meta != dstMeta) {
                        emitLvalue(e.ret_val);
                        of << ".PTR = ";
                        emitParam(e.args.at(0));
                        of << ".PTR; ";
                ...
                if (this->type_is_emulated_i128(dstTy)) {
                    of << "abort()";
            ...
            // - exact_div is UB to call on a non-multiple
                ...
                if (type_is_emulated_i128(params.types.at(0))) {
                    ...
                    ::HIR::TypeRef tmp;
                    const auto& shift_ty = mir_res.getParamType(tmp, e.args.at(1));
            ...
            // -- Platform Intrinsics (and SIMD) --
                ...
                auto simd_cmp = [&](const char* op) {
                    auto src_info = SimdInfo::forTy(*this, params.types.at(0));
                    auto dstInfo = SimdInfo::forTy(*this, params.types.at(1));
                    MIR_ASSERT(mir_res, src_info.count == dstInfo.count, "Element counts must match for " << name);
                ...
                // Truncate into a bitmask - Converts a collection of [0,!0] into bits
                    ...
                    TargetGetSizeOf(sp, mResolve, mapTy, size_map);
                ...
                // Select between two values
                    ...
                    auto val_info = SimdInfo::forTy(*this, params.types.at(1));
                    MIR_ASSERT(mir_res, maskInfo.count == val_info.count, "Element counts must match for " << name);
                    of << "for(int i = 0; i < " << val_info.count << "; i++) ";
                    of << "((";
                    val_info.emitValTy(*this);
                    of << "*)&";
                    emitLvalue(e.ret_val);
                    of << ")[i] ";
                    of << "= ((";
        ...
        void emitDestructorLoop(
            ...
            of << "; i++) {\n";
            of << indent << "\ttry {\n";
            emitDestructorCall(element, elementTy, false, indent_level + 2);
            of << "\n" << indent << "\t} catch (...) {\n";
            of << indent << "\t\tfor(i++; i < ";
            emitCount();
            of << "; i++) {\n";
            of << indent << "\t\t\ttry {\n";
            emitDestructorCall(element, elementTy, false, indent_level + 4);
            of << "\n" << indent << "\t\t\t} catch (...) { abort(); }\n";
            of << indent << "\t\t}\n";
            of << indent << "\t\tthrow;\n";
            of << indent << "\t}\n";
        ...
        void emitTupleDestructor(
            ...
            for (size_t i = 0; i < tuple.size(); i++) {
                if (mResolve.type_needs_drop_glue(sp, tuple[i])) {
                    ...
                    fieldUnsized.push_back(unsized_valid && i == tuple.size() - 1);
                }
                field.incField();
            }
            if (fields.empty()) {
            ...
            for (size_t i = 0; i < fields.size(); i++) {
                emitDestructorCall(fields[i], fieldTypes[i], fieldUnsized[i], indent_level + 2);
            ...
            for (size_t i = 1; i < fields.size(); i++) {
                ...
                of << indent << "\t\t\ttry {\n";
                emitDestructorCall(fields[i], fieldTypes[i], fieldUnsized[i], indent_level + 4);
                of << indent << "\t\t\t} catch (...) { abort(); }\n";
                of << indent << "\t\t}\n";
            }
            of << indent << "\t\tthrow;\n";
        ...
        void emitDestructorCall(const ::MIR::LValue& slot, const ::HIR::TypeData* ty, bool unsized_valid, unsigned indent_level) {
            ...
            TU_ARMA(Diverge, te) {
                ...
                TU_ARMA(Borrow, te) {
                    if (te.type == ::HIR::BorrowType::Owned) {
                        ...
                        emitDestructorCall(::MIR::LValue::newDeref(slot.clone()), te.inner, true, indent_level);
                    }
                }
                TU_ARMA(Path, te) {
                    // Call drop glue
                    ...
                    switch (metadata_type(ty)) {
                        ...
                        case MetadataType::None:
                        case MetadataType::Zero:
                            if (this->type_is_bad_zst(ty) && this->lvalueRootIsBadZst(slot)) {
                                ...
                                ::HIR::TypeRef tmp;
                                if (this->type_is_bad_zst(mirRes->getLvalueType(tmp, v)) && (v.is_Field() || v.is_Downcast())) {
                                    v = v.innerRef();
                                }
                                of << indent << TransMangle(p) << "((";
                                emitCtype(ty);
                ...
                TU_ARMA(Array, te) {
                    ...
                    if (te.size.as_Known() > 0) {
                        emitDestructorLoop(slot, te.inner, [&] { of << te.size.as_Known(); }, indent_level);
                    }
                }
                TU_ARMA(Tuple, te) {
                    emitTupleDestructor(slot, te, unsized_valid, indent_level);
                }
            ...
            TU_ARMA(Int, c) {
                    switch (c.t) {
                        ...
                        case ::HIR::CoreType::I128:
                            ...
                            } else if (c.v.isI64() && c.v.truncate_i64() != INT64_MIN) {
                                of << "(int128_t)";
                                of << c.v;
                                of << "ll";
                            } else {
                ...
                TU_ARMA(ItemAddr, c) {
                    ...
                    if (c->mData.is_UfcsInherent() && c->mData.as_UfcsInherent().item == "#type_id") {
                        ...
                        auto v = mResolve.getValue(sp, *c, ms_tmp, /*signature_only=*/true);
        ...
        void emitParam(const ::MIR::Param& p, bool type_bytes = true) {
            TU_MATCH_HDRA( (p), {)
            TU_ARMA(LValue, e) {
                    emitLvalue(e);
                }
        ...
        void emitTraitMetadataParam(const ::MIR::TypeResolve& mir_res, const ::MIR::Param& param) {
            ::HIR::TypeRef tmp;
            const auto& ty = mir_res.getParamType(tmp, param);
            emitParam(param);
        ...
        void emitTraitObjectDstTailAlign(const ::HIR::TypeData* outer_ty, const ::HIR::TypeData* tail_ty, const ::MIR::Param& value) {
            const auto maxAlign = getPackingMaxAlign(outer_ty);
            if (maxAlign != 0) {
                of << "mrustc_min(";
            }
            emitTraitObjectDstAlign(tail_ty, value);
            if (maxAlign != 0) {
                of << ", " << maxAlign << ")";
            }
        }
        ...
        void emitCtypePtr(const ::HIR::TypeData* innerTy, ::FmtLambda inner) {
            ...
            //else
            {
                switch (this->metadata_type(innerTy)) {
                    case MetadataType::Unknown:
