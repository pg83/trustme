...
namespace {
    ...
    struct NewvalState: public HIREvaluator::Newval {
        ...
        /* The crate's serial for lifted statics (the wire board's `id`), so that the
           evaluations of a generic constant's instantiations name theirs apart. */
        u32& serial;

        NewvalState(HIRModule& mod, const HIRItemPath& modPath, std::string prefix, u32& serial);
    ...
    struct TypeInfo {
        ...
        static TypeInfo forType(const HIRType* ty);
    ...
    struct Expander: public HIRVisitor {
        ...
        void evalulateConstGeneric(const Span& sp, const HIRType* ty, HIRConstGeneric& v);
        ...
        static void visitEnumVariant(const WireBoard& wb, HIRCrate& crate, const HIRItemPath& p, HIRModule& mod, const HIRItemPath& modPath, const char* name, HIREnum& item, size_t idx);

        static void visitEnumInner(const WireBoard& wb, HIRCrate& crate, const HIRItemPath& p, HIRModule& mod, const HIRItemPath& modPath, const char* name, HIREnum& item);
    };
    ...
    bool typeCanMonomorph(const HIRType* type, const MonomorphState& ms) {
        if (!type) {
            return true;
        }
        auto copy = type;
    ...
    bool pathParamsAreConcrete(const HIRPathParams& params) {
        for (const auto& type : params.types) {
            if (monomorphiseTypeNeeded(type) || visitTyWith(type, [](const HIRType* inner) {
    ...
    bool typeIsConcrete(const HIRType* type) {
        return !type || (!monomorphiseTypeNeeded(type) && !visitTyWith(type, [](const HIRType* inner) {
            return inner->is_Infer();
        }));
    }

    bool unevaluatedEnvIsConcrete(const HIRConstGenericUnevaluated& ue) {
        return typeIsConcrete(ue.selfType) && pathParamsAreConcrete(ue.paramsImpl) && pathParamsAreConcrete(ue.paramsItem);
    }
    ...
    const HIRExprState::Captures& exprCaptures(HIRTypeInterner& types, HIRExprPtr& expr) {
...
struct HIREvaluator::MIREvalAllocationPtr final: public MIREvalPtr<MIREvalAllocation> {
    static MIREvalAllocationPtr allocate(ObjPool* pool, const StaticTraitResolve& resolve, const MIRTypeResolve& state, const HIRType* ty);
...
struct HIREvaluator::MIREvalAllocation final: public IValue {
    ...
    MIREvalAllocation(u8* data, size_t len, const HIRType* ty);
...
struct HIREvaluator::MIREvalCallStackEntry {
    ...
    const std::vector<std::pair<HIRPattern, const HIRType*>> argDefs;
    ...
    MIREvalCallStackEntry(ObjPool* valuePool, unsigned frameIndex, const Span& rootSpan, const StaticTraitResolve& resolve, const MIREvalPathCallback& printPath, const HIRType* expTy, std::vector<std::pair<HIRPattern, const HIRType*>> argDefs, const MIRFunction& fcn, MonomorphState ms, std::vector<MIREvalAllocationPtr> args, const HIRGenericParams* itemParamsDef, const HIRGenericParams* implParamsDef, SourceLocation callerLocation, bool tracksCaller);

    const HIRType* monomorphExpand(const HIRType* ty) const;

    unsigned readEnumVariant(const HIRType* ty, MIREvalValueRef value) const;
    ...
    bool valueNeedsNonConstDrop(const HIRType* ty, MIREvalValueRef value) const;

    MIREvalStaticRefPtr getStaticrefMono(const HIRPath& p, const HIRType** outTy = nullptr);

    MIREvalStaticRefPtr getStaticref(HIRPath p, const HIRType** outTy = nullptr);

    MIREvalValueRef getLval(const MIRLValue& lv, MIREvalValueRef* meta = nullptr, u64* rawAddress = nullptr);

    const EncodedLiteral& getConst(const HIRPath& inP, const HIRType** outTy);
    ...
    const EncodedLiteral& getConst(const HIRConstGeneric& v, EncodedLiteral& tmp);

    FloatValue readParamFloat(unsigned bits, const MIRParam& p);

    U128 readParamFloatBits(unsigned bits, const MIRParam& p);

    U128 readParamUint(unsigned bits, const MIRParam& p);

    S128 readParamSint(unsigned bits, const MIRParam& p);

    std::pair<u64, MIREvalRelocPtr> readParamPtr(const MIRParam& p);

    size_t sizeOfOrBug(const HIRType* ty) const;
};
...
static EntPtr getEntFullpath(const Span& sp, const ::StaticTraitResolve& resolve, const HIRPath& path, EntNS ns, MonomorphState& outMs, const HIRGenericParams** outImplParamsDef = nullptr) {
...
static const RcString* getRustcIntrinsicName(const Span& sp, const StaticTraitResolve& resolve, const HIRPath& path) {
...
static void resolveStaticPointer(MIREvalCallStackEntry& localState, std::pair<u64, MIREvalRelocPtr>& value) {
...
static bool samePointerProvenance(const MIREvalRelocPtr& left, const MIREvalRelocPtr& right) {
    if (left == right) {
        return true;
    }
...
static u8 pointerGuaranteedCmp(const std::pair<u64, MIREvalRelocPtr>& left, const std::pair<u64, MIREvalRelocPtr>& right) {
...
static std::pair<MIREvalValueRef, MIREvalValueRef> getTupleTBool(const MIREvalCallStackEntry& localState, MIREvalValueRef& src, const HIRType* t) {
    Vector<const HIRType*> tupleTypes;
...
static bool doArithChecked(MIREvalCallStackEntry& localState, const HIRType* ty, MIREvalValueRef& dst, const MIRParam& valL, MIRBinOp op, const MIRParam& valR, bool saturate = false) {
    ...
    if (op == MIRBinOp::BIT_SHL || op == MIRBinOp::BIT_SHR) {
        const HIRType* tmpR;
        const auto& tyR = localState.state.getParamType(valR);
    ...
    {
        const HIRType* tmpR;
        MIR_ASSERT(state, ty == localState.state.getParamType(valR), StringView("BinOp with mismatched types"));
    }
...
static void writeCtfeEnumVariant(const StaticTraitResolve& resolve, MIREvalCallStackEntry& localState, MIREvalValueRef dst, const HIRType* ty, size_t index, size_t valueCount, WriteField&& writeField) {
    const auto& state = localState.state;
...
MIREvalAllocationPtr MIREvalAllocationPtr::allocate(ObjPool* pool, const StaticTraitResolve& resolve, const MIRTypeResolve& state, const HIRType* ty) {
    size_t len;
...
MIREvalAllocationPtr MIREvalAllocationPtr::allocateScratch(ObjPool* pool, size_t size) {
    ...
    MIREvalAllocationPtr rv;
    rv.ptr = pool->make<MIREvalAllocation>(data, size, nullptr);
    return rv;
}
...
MIREvalAllocationPtr MIREvalAllocationPtr::allocateRo(ObjPool* pool, const void* dataIn, size_t len) {
    auto* data = static_cast<u8*>(pool->allocate(len + ((len + 7) / 8)));
    MIREvalAllocationPtr rv;
    rv.ptr = pool->make<MIREvalAllocation>(data, len, nullptr);
...
void HIREvaluator::runStatement(MIREvalCallStackEntry& localState, const MIRStatement& stmt) {
    ...
    switch (sa.src.tag()) {
        ...
        case MIRRValue::TAG_Cast: {
            ...
            const HIRType* tmp;
            const auto& srcTy = state.getLvalueType(e.val);
            ...
            switch ((*castType).tag()) {
                ...
                case HIRType::TAG_Pointer: {
                    ...
                    if (const auto* e = srcTy->opt_NamedFunction()) {
                        ...
                        if (se && dst.getLen() > inval.getLen() && localState.resolve.metadataType(state.sp, se->inner) == MetadataType::None) {
        ...
        case MIRRValue::TAG_BinOp: {
            ...
            const HIRType* tmp;
            const auto& tyL = state.getParamType(e.valL);
        ...
        case MIRRValue::TAG_UniOp: {
            ...
            const HIRType* tmp;
            const auto& tyL = state.getLvalueType(e.val);
        ...
        case MIRRValue::TAG_Tuple: {
            ...
            const HIRType* tmp;
            const auto& ty = state.getLvalueType(sa.dst);
        ...
        case MIRRValue::TAG_EnumVariant: {
            ...
            const HIRType* tmp;
            const auto& ty = state.getLvalueType(sa.dst);
            writeCtfeEnumVariant(resolve, localState, dst, ty, e.index, e.vals.size(), [&](MIREvalValueRef fieldDst, size_t i) {
...
unsigned HIREvaluator::runTerminator(MIREvalCallStackEntry& localState, const MIRTerminator& terminator) {
    ...
    switch (terminator.tag()) {
        ...
        case MIRTerminator::TAG_Switch: {
            ...
            const HIRType* tmp;
            const auto& ty = state.getLvalueType(e.val);
        ...
        case MIRTerminator::TAG_Drop: {
            ...
            const HIRType* tmp;
            const auto& ty = state.getLvalueType(e.slot);
        ...
        case MIRTerminator::TAG_Call: {
            ...
            auto callPath = [&](HIRPath* fcnp, bool indirect) -> unsigned {
                ...
                for (const auto& a : e.args) {
                    const HIRType* tmp;
                    const auto& ty = state.getParamType(a);
            ...
            if (const auto* te = e.fcn.opt_Intrinsic()) {
                ...
                if (te->name == "size_of") {
                    ...
                    const HIRType* tmp;
                    auto ti = TypeInfo::forType(state.getParamType(e.args.at(0)));
                    ...
                    if (ti.ty == TypeInfo::Signed) {
                        ...
                        MIR_BUG(state, StringView("`three_way_compare` with unsupported type ") << state.getParamType(e.args.at(0)));
                    }
                ...
                if (rustcIntrinsic && (*rustcIntrinsic == "const_allocate" || *rustcIntrinsic == "const_deallocate" || *rustcIntrinsic == "const_make_global")) {
                ...
                const HIRType* tmp;
                const auto& ty = state.getLvalueType(*te);
        ...
        case MIRTerminator::TAG_TailCall: {
            ...
            auto callPath = [&](HIRPath* fcnp, bool indirect) -> unsigned {
                ...
                for (const auto& arg : e.args) {
                    const HIRType* tmp;
                    const auto& ty = state.getParamType(arg);
                    callArgs.push_back(MIREvalAllocationPtr::allocate(localState.valuePool, resolve, state, ty));
            ...
            if (const auto* value = e.fcn.opt_Value()) {
                const HIRType* tmp;
                const auto& ty = state.getLvalueType(*value);
...
bool HIREvaluator::callFunction(MIREvalCallStackEntry& localState, const MIRLValue& rvSlot, HIRPath* fcnPath, std::vector<MIREvalAllocationPtr> callArgs, const SourceLocation& callsite, bool indirect) {
    ...
    if (const auto* fcnP = rv.opt_Function()) {
        HIRFunction& fcn = resolve.hirCrateMut().findFunctionMut(resolve.board(), state.sp, path, **fcnP);
        auto& ep = fcn.code;
        if (ep && ep.state->stage < HIRExprState::Stage::ConstEval) {
            ...
            ConvertHIRConstantEvaluateFcnSig(resolve.board(), resolve.hirCrate(), implParamsDef, path, fcn);
            ep.state->stage = prev;
        }
        ...
        const auto* mir = this->resolve.hirCrateMut().getOrGenMir(this->resolve.board(), HIRItemPath(*fcnPath), fcn);
        ...
        for (const auto& a : fcn.args) {
            ...
            argTy = this->resolve.revealOpaqueTypes(this->rootSpan, argTy);
        ...
        retTy = this->resolve.revealOpaqueTypes(this->rootSpan, retTy);
        ...
        const HIRType* tmp;
        const auto& ty = state.getLvalueType(rvSlot);
        auto* repr = TargetGetTypeRepr(state.sp, resolve, ty);
        if (!repr) {
            BUG(state.sp, StringView("Layout not computable during const evaluation - ") << StringView("repr of ") << ty);
        }
        ...
        const HIRType* tmp;
        const auto& ty = state.getLvalueType(rvSlot);
        writeCtfeEnumVariant(resolve, localState, dst, ty, enumEnt.idx, callArgs.size(), [&](MIREvalValueRef fieldDst, size_t i) {
...
void HIREvaluator::callConstDestructor(MIREvalCallStackEntry& localState, const HIRType* ty, const MIRLValue& slot) {
    const auto& state = localState.state;
...
void HIREvaluator::runConstDrop(MIREvalCallStackEntry& localState, const HIRType* ty, const MIRLValue& slot) {
    const auto& state = localState.state;
...
EncodedLiteral HIREvaluator::allocationToEncoded(const HIRType* ty, const MIREvalAllocation& a) {
...
EncodedLiteral HIREvaluator::evaluateConstant(const HIRItemPath& ip, HIRExprPtr& expr, const HIRType* exp) {
    return evaluateConstant(ip, expr, exp, MonomorphState(resolve.hirCrate().types));
}

EncodedLiteral HIREvaluator::evaluateConstant(const HIRItemPath& ip, HIRExprPtr& expr, const HIRType* exp, MonomorphState ms) {
    TRACE_FUNCTION_F(ip);
    DEBUG(StringView("ms = ") << ms);
    const auto mirResult = this->resolve.hirCrateMut().getOrGenMir(this->resolve.board(), ip, expr, exp);
    const auto* mir = mirResult.mir;
    exp = mirResult.type;

    exp = resolve.revealOpaqueTypes(expr.span(), exp);
...
void ConvertHIRConstantEvaluateExpr(const WireBoard& wb, HIRCrate& crate, const HIRItemPath& ip, HIRExprPtr& exprPtr) {
    TRACE_FUNCTION_F(ip);
    Expander exp{wb};
...
void ConvertHIRConstantEvaluateEnum(const WireBoard& wb, HIRCrate& crate, const HIRItemPath& ip, HIREnum& enm) {
    ...
    auto& mod = crate.getModByPathMut(Span(), modPath);

    Expander::visitEnumInner(wb, crate, ip, mod, modPath, itemName.c_str(), enm);
}
...
void ConvertHIRConstantEvaluateEnumVariant(const WireBoard& wb, HIRCrate& crate, const HIRItemPath& ip, HIREnum& enm, size_t idx) {
    ...
    Expander::visitEnumVariant(wb, crate, ip, mod, modPath, itemName.c_str(), enm, idx);
}
...
void ConvertHIRConstantEvaluateConstant(const StaticTraitResolve& callerResolve, const HIRGenericParams* implParams, const HIRItemPath& ip, HIRConstant& e) {
    ...
    auto& crate = callerResolve.hirCrateMut();
...
auto CtfeContext::vtableName() const -> const RcString& {
    return vtableName_;
}

auto CtfeContext::discriminantActive(const HIREnum& enm, size_t idx) const -> bool {
...
auto CtfeContext::pushDiscriminant(const HIREnum& enm, size_t idx) -> void {
    activeDiscriminants_.pushBack({&enm, idx});
}

auto CtfeContext::popDiscriminant() -> void {
...
[[nodiscard]] auto MonomorphAvailability::visitType(const HIRType* type) -> const HIRType* {
    if (!available) {
        return type;
    }
    if (type->is_Infer()) {
...
auto MonomorphAvailability::visitConstgeneric(HIRConstGeneric& value) -> void {
    if (!available) {
        return;
    }
    if (value.is_Infer()) {
...
auto ExprCaptureScan::addType(const HIRGenericRef& g) -> void {
    if (g.isSelf()) {
        out.usesSelf = true;
    } else {
...
[[nodiscard]] auto ExprCaptureScan::visitType(const HIRType* ty) -> const HIRType* {
    if (ty) {
        if (ty->flags & (HIRType::HAS_UNEVALUATED_CONST | HIRType::HAS_DEFERRED_CONST)) {
            out.unknown = true;
        }
        if (ty->flags & HIRType::HAS_TYPE_PARAM) {
...
auto ExprCaptureScan::visitPathParams(HIRPathParams& pp) -> void {
...
auto ExprCaptureScan::visit(HIRExprNodeConstParam& node) -> void {
    addSlot(out.valueMask, node.binding);
    HIRExprVisitorDef::visit(node);
}

auto ExprCaptureScan::visit(HIRExprNodeCallMethod& node) -> void {
    out.unknown = true;
    HIRExprVisitorDef::visit(node);
}
...
auto NewvalStateNop::newStatic(const HIRType* type, EncodedLiteral value, size_t alignment) -> HIRPath {
    TODO(this->sp, StringView("new_static while evaluating a const generic"));
}

NewvalState::NewvalState(HIRModule& mod, const HIRItemPath& modPath, std::string prefix, u32& serial)
    : mod(mod)
    , modPath(modPath)
    , namePrefix(prefix)
    , nextItemIdx(0)
...
/* Lifted statics are named across every evaluation of the crate: a generic constant
   (`impl<const R: u8, ..> Color for CustomColor<R, ..> { const ANSI_FG: &str =
   bytes_to_str(&Self::ANSI_FG_U8); }`) is evaluated once per instantiation, each with
   a state of its own, and a name made of the item and a per-state index alone gave
   every instantiation's `&Self::ANSI_FG_U8` the same static, the last one's for all. */
auto NewvalState::newStatic(const HIRType* type, EncodedLiteral value, size_t alignment) -> HIRPath {
    ASSERT_BUG(Span(), type != nullptr, StringView(""));
    auto name = RcString::newInterned(FMT(namePrefix << nextItemIdx << StringView("_") << ++serial));
    nextItemIdx++;
...
template <typename T>
MIREvalPtr<T>::operator bool() const {
    return ptr != 0;
}

template <typename T>
auto MIREvalPtr<T>::operator->() -> T* {
    return ptr;
}

template <typename T>
auto MIREvalPtr<T>::operator->() const -> const T* {
    return ptr;
}
...
auto IValue::writeBytes(size_t ofs, const void* data, size_t len) -> void {
...
MIREvalRelocPtr::operator bool() const {
    return ptr != 0;
}

auto MIREvalRelocPtr::operator==(const MIREvalRelocPtr& x) const -> bool {
    return ptr == x.ptr;
}

auto MIREvalRelocPtr::asValue() -> IValue& {
    return *asValuePtr();
}

auto MIREvalRelocPtr::asValue() const -> const IValue& {
    return *asValuePtr();
}

auto MIREvalRelocPtr::asAllocation() const -> MIREvalAllocation* {
    return (ptr != 0 && (ptr & 3) == TAG_Allocation) ? reinterpret_cast<MIREvalAllocation*>(ptr - TAG_Allocation) : nullptr;
}

auto MIREvalRelocPtr::asConstant() const -> MIREvalConstant* {
    return (ptr != 0 && (ptr & 3) == TAG_Constant) ? reinterpret_cast<MIREvalConstant*>(ptr - TAG_Constant) : nullptr;
}

auto MIREvalRelocPtr::asStaticref() const -> MIREvalStaticRef* {
...
auto MIREvalConstant::size() const -> size_t {
    return length;
}

auto MIREvalConstant::getBytes(size_t ofs, size_t len, bool /*check_mask*/) const -> const u8* {
...
auto MIREvalConstant::readMask(u8* dst, size_t dstOfs, size_t /*ofs*/, size_t len) const -> void {
    dst += dstOfs / 8;
    dstOfs %= 8;
    if (dstOfs != 0) {
        while (len--) {
...
auto MIREvalConstant::isWritable() const -> bool {
    return false;
}

auto MIREvalConstant::extWriteBytes(size_t ofs, size_t len) -> u8* {
    abort();
}

auto MIREvalConstant::writeMaskFrom(size_t ofs, const IValue& src, size_t srcOfs, size_t len) -> void {
    abort();
}

auto MIREvalConstant::getReloc(size_t ofs) const -> MIREvalRelocPtr {
    return MIREvalRelocPtr();
}

auto MIREvalConstant::setReloc(size_t ofs, MIREvalRelocPtr ptr) -> void {
    abort();
}

MIREvalAllocation::MIREvalAllocation(u8* data, size_t len, const HIRType* ty)
    : length(len)
    , isReadonly(false)
    , type_(ty)
    , isConstHeap(false)
...
auto MIREvalAllocation::size() const -> size_t {
    return length;
}

auto MIREvalAllocation::getBytes(size_t ofs, size_t len, bool checkMask) const -> const u8* {
    if (!(ofs <= length) || !(len <= length) || !(ofs + len <= length)) {
        return nullptr;
    }
...
auto MIREvalAllocation::isWritable() const -> bool {
    return !isReadonly;
}
...
auto MIREvalAllocation::getReloc(size_t ofs) const -> MIREvalRelocPtr {
...
auto MIREvalAllocation::setReloc(size_t ofs, MIREvalRelocPtr ptr) -> void {
...
auto MIREvalAllocation::getType() const -> const HIRType* {
    return type_;
}

auto MIREvalAllocation::isConstHeapAllocation() const -> bool {
    return isConstHeap;
}

auto MIREvalAllocation::isAlive() const -> bool {
    return isLive;
}

auto MIREvalAllocation::wasMadeGlobal() const -> bool {
    return isGlobal;
}

auto MIREvalAllocation::getHeapAlignment() const -> size_t {
...
auto MIREvalAllocation::getRelocations() const -> const std::vector<Reloc>& {
    return relocations;
}

auto MIREvalAllocation::getMask() -> u8* {
    return data + length;
}

auto MIREvalAllocation::getMask() const -> const u8* {
    return data + length;
}

MIREvalStaticRef::MIREvalStaticRef(ObjPool* pool, HIRPath p, const EncodedLiteral* lit, size_t len, bool valuePending)
    : pool(pool)
    , path_(std::move(p))
    , encoded(lit)
    , length(len)
...
auto MIREvalStaticRef::size() const -> size_t {
    return length;
}

auto MIREvalStaticRef::hasValue() const -> bool {
    return encoded;
}
...
auto MIREvalStaticRef::readMask(u8* dst, size_t dstOfs, size_t ofs, size_t len) const -> void {
    dst += dstOfs / 8;
    dstOfs %= 8;
    if (dstOfs != 0) {
        while (len--) {
...
auto MIREvalStaticRef::isWritable() const -> bool {
    return false;
}

auto MIREvalStaticRef::extWriteBytes(size_t ofs, size_t len) -> u8* {
    abort();
}

auto MIREvalStaticRef::writeMaskFrom(size_t ofs, const IValue& src, size_t srcOfs, size_t len) -> void {
    abort();
}

auto MIREvalStaticRef::getReloc(size_t ofs) const -> MIREvalRelocPtr {
    if (encoded) {
...
auto MIREvalStaticRef::setReloc(size_t ofs, MIREvalRelocPtr ptr) -> void {
    abort();
}

auto MIREvalStaticRef::path() const -> const HIRPath& {
    return path_;
}
...
auto MIREvalValueRef::isValid() const -> bool {
    return storage;
}

auto MIREvalValueRef::getStorage() const -> MIREvalRelocPtr {
    return storage;
}

auto MIREvalValueRef::getOfs() const -> size_t {
    return ofs;
}

auto MIREvalValueRef::getLen() const -> size_t {
    return len;
}

auto MIREvalValueRef::copyFrom(const MIRTypeResolve& state, const MIREvalValueRef& other) -> void {
    ensureLive(state);
...
auto MIREvalValueRef::copyFromOverlapping(const MIRTypeResolve& state, const MIREvalValueRef& other, ObjPool* pool) -> void {
    ensureLive(state);
    other.ensureLive(state);
...
auto MIREvalValueRef::writeByte(const MIRTypeResolve& state, u8 v) -> void {
    writeBytes(state, &v, 1);
}

auto MIREvalValueRef::writeFloat(const MIRTypeResolve& state, unsigned bits, FloatValue v) -> void {
    switch (bits) {
        case 16: {
            F16 vF(v);
...
auto MIREvalValueRef::setReloc(MIREvalRelocPtr reloc) -> void {
    storage.asValue().setReloc(ofs, std::move(reloc));
}
...
auto MIREvalValueRef::readFloat(const MIRTypeResolve& state, unsigned bits) const -> FloatValue {
    switch (bits) {
        case 16: {
            F16 vF16;
...
auto MIREvalValueRef::readUsize(const MIRTypeResolve& state) const -> u64 {
    return readUint(state, TargetGetPointerBits()).truncateU64();
}

auto MIREvalValueRef::readPtr(const MIRTypeResolve& state) const -> std::pair<u64, MIREvalRelocPtr> {
...
auto TypeInfo::forPrimitive(HIRCoreType te) -> TypeInfo {
    switch (te) {
        case HIRCoreType::I8:
...
auto TypeInfo::forType(const HIRType* ty) -> TypeInfo {
...
auto TypeInfo::mask(S128 v) const -> U128 {
    if (v < 0) {
...
auto TypeInfo::mask(double v) const -> double {
    return v;
}
...
MIREvalCallStackEntry::MIREvalCallStackEntry(ObjPool* valuePool, unsigned frameIndex, const Span& rootSpan, const StaticTraitResolve& resolve, const MIREvalPathCallback& printPath, const HIRType* expTy, std::vector<std::pair<HIRPattern, const HIRType*>> argDefs, const MIRFunction& fcn, MonomorphState ms, std::vector<MIREvalAllocationPtr> args, const HIRGenericParams* itemParamsDef, const HIRGenericParams* implParamsDef, SourceLocation callerLocation, bool tracksCaller)
    : valuePool(valuePool)
    ...
    , dropFlags(fcn.dropFlags)
{
    ...
    for (size_t i = 0; i < state.fcn.locals.length(); i++) {
        ...
        localType = state.resolve.revealOpaqueTypes(state.sp, localType);
...
auto MIREvalCallStackEntry::monomorphExpand(const HIRType* ty) const -> const HIRType* {
    ...
    rv = this->resolve.revealOpaqueTypes(this->state.sp, rv);
    return rv;
}

auto MIREvalCallStackEntry::readEnumVariant(const HIRType* ty, MIREvalValueRef value) const -> unsigned {
...
auto MIREvalCallStackEntry::allocationReachableFrom(const MIREvalAllocation* allocation, const MIREvalAllocation* target, std::set<const MIREvalAllocation*>& visited) -> bool {
...
auto MIREvalCallStackEntry::valueReachableFromReturn(MIREvalValueRef value) const -> bool {
...
auto MIREvalCallStackEntry::valueNeedsNonConstDrop(const HIRType* ty, MIREvalValueRef value) const -> bool {
...
auto MIREvalCallStackEntry::getStaticrefMono(const HIRPath& p, const HIRType** outTy) -> MIREvalStaticRefPtr {
    return getStaticref(ms.monomorphPath(state.sp, p), outTy);
}

auto MIREvalCallStackEntry::getStaticref(HIRPath p, const HIRType** outTy) -> MIREvalStaticRefPtr {
...
auto MIREvalCallStackEntry::getLval(const MIRLValue& lv, MIREvalValueRef* meta, u64* rawAddress) -> MIREvalValueRef {
    const HIRType* tmpTy = nullptr;
    const HIRType* typ = nullptr;
    ...
    switch (lv.root.tag()) {
        ...
        case MIRLValue::Storage::TAG_Static: {
            ...
            if (!lv.wrappers.empty()) {
                MIR_ASSERT(state, tmpTy != nullptr, StringView("Type not set?"));
            }
            typ = tmpTy;
            break;
        }
...
auto MIREvalCallStackEntry::getConst(const HIRPath& inP, const HIRType** outTy) -> const EncodedLiteral& {
    ...
    auto& item = rootResolve.hirCrateMut().findConstantMut(rootResolve.board(), state.sp, p, *ent.as_Constant());
    const auto& c = item;
...
auto MIREvalCallStackEntry::writeConst(MIREvalValueRef dst, const MIRConstant& c) -> void {
    switch (c.tag()) {
...
auto MIREvalCallStackEntry::writeBorrow(MIREvalValueRef dst, HIRBorrowType bt, const MIRLValue& lv) -> void {
...
auto MIREvalCallStackEntry::writeParam(MIREvalValueRef dst, const MIRParam& p) -> void {
    switch (p.tag()) {
        case MIRParam::TAG_LValue: {
            auto& e = p.as_LValue();
...
auto MIREvalCallStackEntry::getConst(const HIRConstGeneric& v, EncodedLiteral& tmp) -> const EncodedLiteral& {
    switch (v.tag()) {
...
auto MIREvalCallStackEntry::readParamFloat(unsigned bits, const MIRParam& p) -> FloatValue {
    switch (p.tag()) {
        case MIRParam::TAG_LValue: {
            auto& e = p.as_LValue();
            return getLval(e).readFloat(state, bits);
        }
        case MIRParam::TAG_Borrow: {
...
auto MIREvalCallStackEntry::readParamFloatBits(unsigned bits, const MIRParam& p) -> U128 {
    switch (p.tag()) {
        case MIRParam::TAG_LValue: {
            auto& e = p.as_LValue();
            return getLval(e).readUint(state, bits);
        }
        case MIRParam::TAG_Borrow: {
            MIR_BUG(state, StringView("Expected a float, got a MIR::Param::Borrow"));
            break;
...
auto MIREvalCallStackEntry::readParamUint(unsigned bits, const MIRParam& p) -> U128 {
    switch (p.tag()) {
        case MIRParam::TAG_LValue: {
            auto& e = p.as_LValue();
            return getLval(e).readUint(state, bits);
...
auto MIREvalCallStackEntry::readParamSint(unsigned bits, const MIRParam& p) -> S128 {
    switch (p.tag()) {
        case MIRParam::TAG_LValue: {
            auto& e = p.as_LValue();
            return getLval(e).readSint(state, bits);
        }
        case MIRParam::TAG_Borrow: {
            MIR_BUG(state, StringView("Expected an integer, got a MIR::Param::Borrow"));
            break;
...
auto MIREvalCallStackEntry::readParamPtr(const MIRParam& p) -> std::pair<u64, MIREvalRelocPtr> {
    switch (p.tag()) {
        case MIRParam::TAG_LValue: {
            auto& e = p.as_LValue();
            return getLval(e).readPtr(state);
        }
        case MIRParam::TAG_Borrow: {
            auto& e = p.as_Borrow();
            MIREvalValueRef meta;
            u64 rawAddress = 0;
            auto value = getLval(e.val, &meta, &rawAddress);
            if (rawAddress) {
...
auto MIREvalCallStackEntry::sizeOfOrBug(const HIRType* ty) const -> size_t {
    size_t rv;
...
template <typename F>
auto GenericParamsCb<F>::get(const Span& sp) -> const HIRGenericParams& {
    return f(sp);
}

Expander::Expander(const WireBoard& wb)
...
auto Expander::getParamsForDef(const HIRGenericParams& tpl, bool isFunctionLevel) const -> HIRPathParams {
    return tpl.makeNopParams(crate.types, isFunctionLevel ? 1 : 0);
}

auto Expander::visitModule(HIRItemPath p, HIRModule& mod) -> void {
...
auto Expander::visitTraitImpl(const HIRSimplePath& traitPath, HIRTraitImpl& impl) -> void {
    ...
    modPath = &mp;
    mod = &crate.getModByPathMut(sp, impl.srcModule);
...
auto Expander::visitInherentType(HIRItemPath p, HIRTypeAlias& item) -> void {
...
auto Expander::visitTrait(HIRItemPath ip, HIRTrait& trait) -> void {
...
auto Expander::evalulateConstGeneric(const Span& sp, const HIRType* ty, HIRConstGeneric& v) -> void {
    if (v.is_Unevaluated()) {
...
auto Expander::visitAtyParams(const HIRGenericPath& sourceTrait, const RcString& name, HIRPathParams& params) -> void {
    auto saved = getParams;
    auto callback = makeCallable<GenericParamsCb>([&](const Span& sp) -> const HIRGenericParams& {
...
auto Expander::visitTraitPath(HIRTraitPath& p) -> void {
...
[[nodiscard]] auto Expander::visitType(const HIRType* ty) -> const HIRType* {
    ...
    if (ty->is_Pattern()) {
        ...
        for (auto& range : e.pattern.alternatives) {
            if (range.hasStart) {
                evaluateEndpoint(range.start);
            }
            if (range.hasEnd) {
                evaluateEndpoint(range.end);
            }
        }
        ty = crate.types.intern(mv$(data));
    }
...
auto Expander::enumTagIsSigned(HIRCoreType ty) -> bool {
    switch (ty) {
        case HIRCoreType::I8:
        case HIRCoreType::I16:
        case HIRCoreType::I32:
...
auto Expander::visitEnumVariant(const WireBoard& wb, HIRCrate& crate, const HIRItemPath& p, HIRModule& mod, const HIRItemPath& modPath, const char* name, HIREnum& item, size_t idx) -> void {
    if (item.discriminantsEvaluated) {
        return;
    }
    RcString varName;
    HIRExprPtr* expr = nullptr;
    U128* slot = nullptr;
    bool* known = nullptr;
...
auto Expander::visitEnumInner(const WireBoard& wb, HIRCrate& crate, const HIRItemPath& p, HIRModule& mod, const HIRItemPath& modPath, const char* name, HIREnum& item) -> void {
    if (item.discriminantsEvaluated) {
        return;
    }
    const auto ty = HIREnum::getReprType(item.tagRepr);
    switch (item.data.tag()) {
        case HIREnumClass::TAG_Value: {
            ...
            checkEnumDiscriminants(Span(), ty, e.variants, [](const auto& var) {
                return var.val;
            });
            break;
        }
        case HIREnumClass::TAG_Data: {
            ...
            checkEnumDiscriminants(Span(), ty, e, [](const auto& var) {
                return var.discriminantValue;
            });
            break;
        }
    }
...
EnumValueExpander::EnumValueExpander(const WireBoard& wb)
    : HIRVisitor(nullptr, wb.crate->types)
    , wb(wb)
    , crate(*wb.crate)
    , typeck(wb)
...
auto EnumValueExpander::visitEnum(HIRItemPath p, HIREnum& item) -> void {
