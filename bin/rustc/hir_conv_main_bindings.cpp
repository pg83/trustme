...
namespace {
    ...
    struct BindVisitor: public HIRVisitor {
        ...
        [[nodiscard]] const HIRTypeData* visitTypeInner(const HIRTypeData* ty, bool doBind = true);
    ...
    struct ReceiverValidator {
        ...
        const HIRTypeData* replaceImplTypeWithSelf(const HIRTypeData* ty);
    ...
    struct Expander: public HIRVisitor {
        ...
        struct ActiveTypeAlias {
            ...
            const HIRTypeData* recursiveReference;
        ...
        void collectTraitAliasBounds(const Span& sp, const HIRTraitPath& aliasPath, const HIRTypeData* selfType, GenericBounds& out, const ActiveTraitAlias* active = nullptr);
            ...
            switch (t) {
                ...
                case Target::Struct:
                    if (ti.is_Struct()) {
                        ...
                        ERROR(sp, E0000, StringView("Expected a struct at ") << path << StringView(", got a ") << ti.tagStr());
                    }
                    break;
                case Target::Enum:
                    if (ti.is_Enum()) {
                        ...
                        ERROR(sp, E0000, StringView("Expected a enum at ") << path << StringView(", got a ") << ti.tagStr());
                    }
                    break;
            }
            UNREACHABLE();
    ...
    const HIRTypeData* ConvertHIRExpandAliasesGetExpansion(const HIRCrate& crate, const HIRPath& path, bool isExpr) {
        Span sp;
        switch (path.data.tag()) {
            ...
            case HIRPath::Data::TAG_UfcsInherent: {
                DEBUG(StringView("TODO: Locate impl blocks for types - path=") << path);
                break;
            }
            case HIRPath::Data::TAG_UfcsKnown: {
                DEBUG(StringView("TODO: Locate impl blocks for traits on types - path=") << path);
                break;
            }
                ...
                struct H {
                    static bool containsTrait(const Span& sp, const HIRCrate& crate, const HIRGenericPath& path, const HIRGenericPath& desPath) {
                        ...
                        if (const auto* t = ti.opt_Trait()) {
                            ...
                            BUG(sp, StringView("Not a trait path ") << path << StringView(": ") << ti.tagStr());
                        }
                        return false;
                    }
                    ...
                    static HIRTraitPath& findEntry(const Span& sp, const HIRCrate& crate, const HIRGenericPath& desPath, std::vector<HIRTraitPath>& rv) {
                        ...
                        BUG(sp, StringView("Unable to find a trait in expansion list for ") << desPath);
                    }
                };
    ...
    void sortImplGroup(HIRCrate::ImplGroup<std::unique_ptr<T>>& ig, F fmt) {
        auto newEnd = std::remove_if(ig.generic.begin(), ig.generic.end(), [&ig, &fmt](std::unique_ptr<T>& tyImpl) {
            ...
            if (path) {
                DEBUG(*path << StringView(" += ") << FMT_CB(os, fmt(os, *tyImpl)));
    ...
    void pushIndexInherentMethods(HIRInherentCache& icache, const HIRSimplePath& langBox, const HIRCrate& src) {
        TRACE_FUNCTION_F(StringView("src = ") << src.crateName);
...
const HIRTypeData* ConvertHIRExpandTypeAlias(const Span& sp, const HIRCrate& crate, const HIRGenericPath& path, bool isExpr) {
    ...
    if (const auto* ep = ti.opt_TypeAlias()) {
        const auto& ta = *ep;
        DEBUG(path << StringView(" -> type ") << ta.params.fmtArgs() << StringView(" = ") << ta.type);
        auto pp = ConvertHIRCompleteAliasParams(crate.types, sp, ta.params, path, isExpr);
        auto ms = MonomorphStatePtr(crate.types, nullptr, &pp, nullptr);
        const HIRTypeData* rv = ms.monomorphType(sp, ta.type);
        DEBUG(path << StringView(" -> ") << path.path << pp << StringView(" -> ") << rv);
        return rv;
    }
    return crate.types.infer();
}
...
void ConvertHIRResolveUFCSExpr(const WireBoard& wb, const HIRCrate& crate, const HIRItemPath& ip, HIRExprPtr& exprPtr) {
    TRACE_FUNCTION_F(ip);
    UfcsVisitor exp{wb, true};
    ASSERT_BUG(Span(), exprPtr.state, StringView("No ExprState for ") << ip);
    if (exprPtr.state->currentTraitImpl) {
        auto traitPath = HIRItemPath(exprPtr.state->currentTraitImpl->type, exprPtr.state->currentTraitPath, exprPtr.state->currentTraitImpl->traitArgs);
...
void ConvertHIRResolveUFCSSortImpls(WireBoard& wb, HIRCrate& crate) {
    sortImplGroup<HIRTypeImpl>(crate.typeImpls, [](ZeroCopyOutput& os, const HIRTypeImpl& i) {
        os << StringView("impl") << i.params.fmtArgs() << StringView(" ") << i.type;
    });
    DEBUG(StringView("Type impl counts: ") << crate.typeImpls.named.size() << StringView(" path groups, ") << crate.typeImpls.nonNamed.size() << StringView(" primitive, ") << crate.typeImpls.generic.size() << StringView(" ungrouped"));
    for (auto& implGroup : crate.traitImpls) {
        sortImplGroup<HIRTraitImpl>(implGroup.second, [&](ZeroCopyOutput& os, const HIRTraitImpl& i) {
            os << StringView("impl") << i.params.fmtArgs() << StringView(" ") << implGroup.first << i.traitArgs << StringView(" for ") << i.type;
        });
    }
    for (auto& implGroup : crate.markerImpls) {
        sortImplGroup<HIRMarkerImpl>(implGroup.second, [&](ZeroCopyOutput& os, const HIRMarkerImpl& i) {
            os << StringView("impl") << i.params.fmtArgs() << StringView(" ") << implGroup.first << i.traitArgs << StringView(" for ") << i.type << StringView(" {}");
        });
    }
...
auto BindVisitor::interner() const -> HIRTypeInterner& {
    return crate.types;
}

auto BindVisitor::visitModule(HIRItemPath p, HIRModule& mod) -> void {
...
auto BindVisitor::visitTraitPath(HIRTraitPath& p) -> void {
    Span sp;
...
auto BindVisitor::visitLiteral(const Span& sp, EncodedLiteral& lit) -> void {
...
auto BindVisitor::visitPatternValue(const Span& sp, HIRPattern& pat, HIRPattern::Value& val) -> void {
        ...
        if (auto* pe = ve->path.data.opt_Generic()) {
            ...
            if (path.components().size() == 1) {
                ...
                if (const auto& enm = ti.opt_Enum()) {
                    if (!isSingleValue) {
                        ERROR(sp, E0000, StringView("Enum variant in range pattern - ") << pat);
                    }

                    auto idx = enm->findVariant(pc);
                    if (idx == SIZE_MAX) {
                        BUG(sp, StringView("'") << pc << StringView("' isn't a variant in path ") << path);
                    }
                    ...
                    BUG(sp, StringView("Node ") << path.components().size() - 2 << StringView(" of path ") << ve->path << StringView(" wasn't a module"));
                }
            }
            ...
            if (mod) {
                ...
                if (it == mod->valueItems.end()) {
                    BUG(sp, StringView("Couldn't find final component of ") << path);
                }
                switch (it->second->ent.tag()) {
                    default:
                        ERROR(sp, E0000, StringView("Value pattern ") << pat << StringView(" pointing to unexpected item type - ") << it->second->ent.tagStr());
                    ...
                    case HIRValueItem::TAG_StructConstant: {
                        ...
                        if (!isSingleValue) {
                            ERROR(sp, E0000, StringView("Struct in range pattern - ") << pat);
                        }
                        auto path = mv$(*pe);
...
auto BindVisitor::visitPattern(HIRPattern& pat) -> void {
    Span _sp = Span();
    const Span& sp = _sp;
...
auto BindVisitor::visitParams(HIRGenericParams& params) -> void {
    Span sp;
...
auto BindVisitor::visitAssociatedtype(HIRItemPath p, HIRAssociatedType& item) -> void {
    Span sp;
    HIRVisitor::visitAssociatedtype(p, item);
    const HIRTypeData* ty = crate.types.path(p.getFullPath(), {});
...
auto BindVisitor::traitRequiresSizedSelf(const HIRTrait& trait) const -> bool {
...
[[nodiscard]] auto BindVisitor::visitType(const HIRTypeData* ty) -> const HIRTypeData* {
    return visitTypeInner(ty);
}

[[nodiscard]] auto BindVisitor::visitTypeInner(const HIRTypeData* ty, bool doBind) -> const HIRTypeData* {
    Span sp;
    ...
    if (auto* e = data.opt_Path()) {
        switch (e->path.data.tag()) {
            case HIRPathData::TAG_Generic: {
                ...
                switch (item.tag()) {
                    case HIRTypeItem::TAG_TypeAlias: {
                        BUG(sp, StringView("TypeAlias encountered after `Resolve Type Aliases` - ") << ty);
                        break;
                    }
                    case HIRTypeItem::TAG_ExternType: {
                        auto& e3 = item.as_ExternType();
                        e->binding = HIRTypePathBinding::make_ExternType(&e3);
                        DEBUG(StringView("- ") << ty);
                        break;
                    }
                    ...
                    default: {
                        ERROR(sp, E0000, StringView("Unexpected item type returned for ") << pe.path << StringView(" - ") << item.tagStr());
                        break;
                    }
                }
                break;
            ...
            case HIRPathData::TAG_UfcsKnown: {
                ...
                if (pe.type->is_Path() && pe.type->as_Path().binding.is_Opaque()) {
                    ...
                    //TODO(sp, StringView("Resolve known UfcsKnown - ") << ty);
                }
                break;
            }
        }
    } else if (auto* te = data.opt_ErasedType()) {
        const HIRTypeData* tyEself = crate.types.generic("ErasedSelf", GENERICErasedSelf);
        ...
        if (auto* ee = te->inner.opt_Fcn()) {
            DEBUG(StringView("Set origin of ErasedType - ") << ty);
            if (ee->origin != HIRSimplePath()) {
            } else if (fcnPath) {
                BUG_ASSERT(fcnPtr);

                DEBUG(*fcnPath << StringView(" ") << fcnErasedCount);
            ...
            // - Add a un-namable generic parameter (TODO: Prevent this from being explicitly set when called)
                ...
                auto name = RcString::newInterned(FMT(StringView("erased$") << idx));
                DEBUG(StringView("-> ") << name);
                ...
                ERROR(sp, E0000, StringView("Use of an erased type outside of a function return - ") << ty);
            }
        }
        ...
        if (te->trait.path.path != HIRSimplePath()) {
            ...
            if (traitRequiresSizedSelf(trait)) {
                ERROR(sp, E0000, StringView("The trait ") << te->trait.path.path << StringView(" is not dyn compatible: it requires `Self: Sized`"));
            }
...
auto BindVisitor::checkImplParamsConstrained(const HIRGenericParams& params, const HIRTypeData* selfTy, const HIRPathParams* traitArgs) -> void {
    ...
    for (size_t i = 0; i < n; i++) {
        if ((used & (uint64_t(1) << i)) == 0) {
            ERROR(Span(), E0000, StringView("the type parameter `") << params.types[i].name << StringView("` is not constrained by the impl trait, self type, or predicates - impl for ") << selfTy);
        }
    }
}

auto BindVisitor::visitTypeImpl(HIRTypeImpl& impl) -> void {
    TRACE_FUNCTION_F(StringView("impl ") << impl.type << StringView(" - from ") << impl.srcModule);
...
auto BindVisitor::visitTraitImpl(const HIRSimplePath& traitPath, HIRTraitImpl& impl) -> void {
...
auto BindVisitor::visitTrait(HIRItemPath p, HIRTrait& item) -> void {
...
auto BindVisitor::visitStruct(HIRItemPath p, HIRStruct& item) -> void {
    auto _ = this->ms.setImplGenerics(item.params);
    HIRVisitor::visitStruct(p, item);
}

auto BindVisitor::visitUnion(HIRItemPath p, HIRUnion& item) -> void {
    auto _ = this->ms.setImplGenerics(item.params);
    HIRVisitor::visitUnion(p, item);
}

auto BindVisitor::visitTypeAlias(HIRItemPath p, HIRTypeAlias& item) -> void {
    auto _ = this->ms.setImplGenerics(item.params);
    HIRVisitor::visitTypeAlias(p, item);
}

auto BindVisitor::visitFunction(HIRItemPath p, HIRFunction& item) -> void {
    auto _ = this->ms.setItemGenerics(item.params);
    fcnPtr = &item;
...
auto BindVisitor::visitStatic(HIRItemPath p, HIRStatic& item) -> void {
    HIRVisitor::visitStatic(p, item);
    visitLiteral(Span(), item.valueRes);
}

auto BindVisitor::visitConstant(HIRItemPath p, HIRConstant& item) -> void {
    auto _ = this->ms.setItemGenerics(item.params);
    HIRVisitor::visitConstant(p, item);
    visitLiteral(Span(), item.valueRes);
}

auto BindVisitor::visitExpr(HIRExprPtr& expr) -> void {
    ...
    if (expr.get() != nullptr) {
        ...
        struct MirVisitor: public MIRVisitorMut {
            ...
            const HIRTypeData* visitType(const HIRTypeData* type) override {
                return upperVisitor.visitType(type);
            }
...
auto VisitorEnumSuperTraits::visitTrait(HIRItemPath ip, HIRTrait& tr) -> void {
    ...
    struct Enumerate {
        ...
        const Span& sp;
        const HIRTypeData* tySelf;
        std::vector<HIRTraitPath> supertraits;
        Vector<const HIRTraitPath*> tpStack;

        Enumerate(HIRTypeInterner& types, const Span& sp, const HIRTypeData* tySelf)
            : types(types)
            , sp(sp)
            , tySelf(tySelf)
        {
            ...
            for (auto& st : supertraits) {
                for (auto& tb : path.typeBounds) {
                    if (tb.second.sourceTrait == st.path) {
                        DEBUG(StringView("Add TypeBound: ") << tb.first << StringView(" = ") << tb.second.type);
                        st.typeBounds.insert(std::make_pair(tb.first, std::move(tb.second)));
                    }
                }
                for (auto& tb : path.traitBounds) {
                    if (tb.second.sourceTrait == st.path) {
                        DEBUG(StringView("Add TraitBound: ") << tb.first << StringView(": ") << tb.second.traits);
        ...
        void fillTypeAliases(HIRTraitPath& outPath) const {
            ...
            for (const auto& ty : tr.types) {
                if (outPath.typeBounds.count(ty.first) == 0) {
                    ...
                    DEBUG(ty.first << StringView(" = ") << found);
                    if (found) {
                ...
                if (outPath.traitBounds.count(ty.first) == 0) {
                    ...
                    DEBUG(ty.first << StringView(": ") << traits);
                    if (!traits.empty()) {
    ...
    DEBUG(StringView("supertraits = ") << e.supertraits);
    if (e.supertraits.size() > 0) {
        ...
        for (auto it = e.supertraits.begin() + 1; it != e.supertraits.end();) {
            if (prev->path == it->path) {
                DEBUG(StringView("MERGE:"));
                DEBUG(StringView("- ") << *prev);
                DEBUG(StringView("- ") << *it);
                for (auto& e : it->typeBounds) {
                    ...
                    if (have != prev->typeBounds.end() && have->second.type != e.second.type) {
                        ...
                        if (isProjection(have->second.type) && !isProjection(e.second.type)) {
                            ...
                            ASSERT_BUG(sp, false, StringView("TODO: Handle mismatched type bounds in merging supertrait ATY bounds: ") << e.first << StringView(" =\n ") << have->second << StringView("\n ") << e.second.type);
                        }
                    }
                ...
                for (auto& e : it->traitBounds) {
                    if (prev->traitBounds.count(e.first)) {
                        TODO(sp, StringView("Merge trait bounds (and make sure to check the source trait)"));
                    }
            ...
            DEBUG(StringView("supertraits dd = ") << e.supertraits);
        }
    }
...
auto VisitorPost::visitModule(HIRItemPath p, HIRModule& mod) -> void {
    ms.pushTraits(p, mod);
    HIRVisitor::visitModule(p, mod);
    ms.popTraits(mod);
}

[[nodiscard]] auto VisitorPost::visitType(const HIRTypeData* ty) -> const HIRTypeData* {
    return visitTypeInner(ty);
}

[[nodiscard]] auto VisitorPost::visitTypeInner(const HIRTypeData* ty, bool doBind) -> const HIRTypeData* {
    Span sp;
...
auto VisitorPost::visitTypeImpl(HIRTypeImpl& impl) -> void {
    auto _ = this->ms.setImplGenerics(impl.params);
...
auto VisitorPost::visitInherentType(HIRItemPath p, HIRTypeAlias& item) -> void {
    auto _ = this->ms.setItemGenerics(item.params);
    HIRVisitor::visitInherentType(p, item);
}

auto VisitorPost::visitTraitImpl(const HIRSimplePath& traitPath, HIRTraitImpl& impl) -> void {
    auto _ = this->ms.setImplGenerics(impl.params);
...
auto VisitorPost::visitMarkerImpl(const HIRSimplePath& traitPath, HIRMarkerImpl& impl) -> void {
    auto _ = this->ms.setImplGenerics(impl.params);
...
auto VisitorPost::visitTrait(HIRItemPath p, HIRTrait& item) -> void {
    auto _ = this->ms.setImplGenerics(item.params);
    HIRVisitor::visitTrait(p, item);
}

auto VisitorPost::visitEnum(HIRItemPath p, HIREnum& item) -> void {
    auto _ = this->ms.setImplGenerics(item.params);
    HIRVisitor::visitEnum(p, item);
}

auto VisitorPost::visitStruct(HIRItemPath p, HIRStruct& item) -> void {
    auto _ = this->ms.setImplGenerics(item.params);
    HIRVisitor::visitStruct(p, item);
}

auto VisitorPost::visitUnion(HIRItemPath p, HIRUnion& item) -> void {
    auto _ = this->ms.setImplGenerics(item.params);
    HIRVisitor::visitUnion(p, item);
}

auto VisitorPost::visitTypeAlias(HIRItemPath p, HIRTypeAlias& item) -> void {
    auto _ = this->ms.setImplGenerics(item.params);
    HIRVisitor::visitTypeAlias(p, item);
}

auto VisitorPost::visitFunction(HIRItemPath p, HIRFunction& item) -> void {
    auto _ = this->ms.setItemGenerics(item.params);
    HIRVisitor::visitFunction(p, item);
}

auto VisitorPost::visitStatic(HIRItemPath p, HIRStatic& item) -> void {
    HIRVisitor::visitStatic(p, item);
}

auto VisitorPost::visitConstant(HIRItemPath p, HIRConstant& item) -> void {
    auto _ = this->ms.setItemGenerics(item.params);
    HIRVisitor::visitConstant(p, item);
}

auto VisitorPost::visitExpr(HIRExprPtr& expr) -> void {
    struct ExprVisitor: public HIRExprVisitorDef {
...
auto Expander::typeAlias(const Span& sp, const HIRTypeData* type) const -> const HIRTypeAlias* {
...
auto Expander::activeTypeAlias(const HIRTypeAlias* alias) const -> const ActiveTypeAlias* {
...
auto Expander::collectTraitAliasBounds(const Span& sp, const HIRTraitPath& aliasPath, const HIRTypeData* selfType, GenericBounds& out, const ActiveTraitAlias* active) -> void {
...
auto Expander::interner() const -> HIRTypeInterner& {
    return crate.types;
}
...
[[nodiscard]] auto Expander::visitType(const HIRTypeData* ty) -> const HIRTypeData* {
    Span sp;
    ...
    if (ty->is_ErasedType() || ty->is_TraitObject()) {
        ...
        if (auto* e = data.opt_ErasedType()) {
            ...
            for (auto it = e->markers.begin(); it != e->markers.end();) {
                ...
                for (auto& expanded : n) {
                    ASSERT_BUG(sp, expanded.traitBounds.empty() && expanded.typeBounds.empty(), StringView("Trait alias with bounds used as a marker - ") << expanded);
            ...
            if (crate.getTypeitemByPath(sp, e->trait.path.path).is_TraitAlias()) {
                ...
                if (!n.empty()) {
                    ...
                    for (size_t i = 1; i < n.size(); i++) {
                        ASSERT_BUG(sp, n[i].traitBounds.empty() && n[i].typeBounds.empty(), StringView("Trait alias with bounds used as a marker - ") << n[i]);
    ...
    if (const auto* e = ty->opt_Path()) {
        const HIRTypeData* newType = ConvertHIRExpandAliasesGetExpansion(crate, e->path, inExpr);
        const HIRTypeData* recursiveReference = ty;
        ...
        ASSERT_BUG(sp, numExp < MAX_RECURSIVE_TYPE_EXPANSIONS, StringView("Recursion limit hit expanding ") << ty << StringView(" (currently on ") << newType << StringView(")"));
        if (!newType->is_Infer()) {
            DEBUG(StringView("Replacing ") << ty << StringView(" with ") << newType << StringView(" (") << numExp << StringView(" expansions)"));
            ty = mv$(newType);
        }
        if (alias) {
...
auto Expander::visitTraitPath(HIRTraitPath& tp) -> void {
    Span sp;
    ASSERT_BUG(sp, crate.getTypeitemByPath(sp, tp.path.path).is_Trait(), StringView("Bad trait path - ") << tp.path << StringView(" : ") << crate.getTypeitemByPath(sp, tp.path.path).tagStr());
...
auto Expander::expandAliasPath(const Span& sp, const HIRPath& path) -> HIRPath {
    const unsigned int MAX_RECURSIVE_TYPE_EXPANSIONS = 100;
    ...
    do {
        ...
        if (!ty->is_Path()) {
            ERROR(sp, E0000, StringView("Type alias referenced in generic path doesn't point to a path"));
        }
    ...
    ASSERT_BUG(sp, numExp < MAX_RECURSIVE_TYPE_EXPANSIONS, StringView("Recursion limit expanding ") << path << StringView(" (currently on ") << *cur << StringView(")"));
    return mv$(rv);
}
...
auto Expander::visitPatternPathBinding(const Span& sp, HIRPath& path) -> HIRPattern::PathBinding {
    ...
    if (path.data.is_UfcsUnknown()) {
        ...
        if (idx == ~0u) {
            TODO(sp, StringView("Variant ") << name << StringView(" not found in ") << gp);
        }
        path = std::move(gp2);
    ...
    if (path.data.is_UfcsKnown()) {
        ...
        if (!revealed->is_Path() || !revealed->as_Path().path.data.is_Generic()) {
            ERROR(sp, E0000, StringView("Expected a struct behind the associated type in a pattern, got ") << path);
        }
...
auto Expander::visitPattern(HIRPattern& pat) -> void {
    Span sp;
    ...
    switch (pat.data.tag()) {
        ...
        case HIRPatternData::TAG_PathValue: {
            ...
            if (newPath != HIRGenericPath()) {
                DEBUG(StringView("Replacing ") << e.path << StringView(" with ") << newPath);
                e.path = mv$(newPath);
            }
...
auto Expander::visitParams(HIRGenericParams& params) -> void {
    Span sp;
    for (size_t i = 0; i < params.bounds.size();) {
        ...
        for (size_t j = 0; j < expandedTraits.size(); j++) {
            params.bounds.insert(params.bounds.begin() + i + j, HIRGenericBound::make_TraitBound({originalType, mv$(expandedTraits[j]), HIRBoundConstness::Never, isTrivial}));
        }
    }
    HIRVisitor::visitParams(params);
}

auto Expander::visitExpr(HIRExprPtr& expr) -> void {
...
auto Expander::visitFunction(HIRItemPath p, HIRFunction& item) -> void {
    ...
    for (const auto& arg : item.args) {
        visitTyWith(arg.second, [&](const HIRTypeData* type) {
            ...
            auto name = RcString::newInterned(FMT(StringView("erased$") << erasedIndex));
...
auto Expander::visitTraitAlias(HIRItemPath p, HIRTraitAlias& item) -> void {
...
auto Expander::visitTrait(HIRItemPath p, HIRTrait& item) -> void {
    expandTraitList(Span(), item.parentTraits);
    HIRVisitor::visitTrait(p, item);
}

auto Expander::visitAssociatedtype(HIRItemPath p, HIRAssociatedType& item) -> void {
    expandTraitList(Span(), item.traitBounds);
    HIRVisitor::visitAssociatedtype(p, item);
}

auto Expander::visitTypeImpl(HIRTypeImpl& impl) -> void {
    implType = impl.type;
    HIRVisitor::visitTypeImpl(impl);
    implType = nullptr;
}

auto Expander::visitTraitImpl(const HIRSimplePath& traitPath, HIRTraitImpl& impl) -> void {
    Span sp;
    implType = impl.type;
    HIRVisitor::visitTraitImpl(traitPath, impl);
    implType = nullptr;
...
[[nodiscard]] auto ExpanderSelf::visitType(const HIRTypeData* ty) -> const HIRTypeData* {
    ty = HIRVisitor::visitType(ty);

    if (const auto* te = ty->opt_Generic()) {
        if (te->binding == GENERICSelf) {
            if (implType) {
                DEBUG(StringView("Replace Self with ") << implType);
                return implType;
            } else {
                DEBUG(StringView("Self outside of an `impl` block"));
            }
        }
    }
    return ty;
...
auto ExpanderSelf::visitExpr(HIRExprPtr& expr) -> void {
    struct Visitor: public HIRExprVisitorDef {
...
auto ExpanderSelf::visitEnum(HIRItemPath p, HIREnum& enm) -> void {
    const HIRTypeData* ty = crate.types.path(HIRGenericPath(p.getSimplePath(), enm.params.makeNopParams(crate.types, 0)), &enm);
    implType = ty;
...
auto ExpanderSelf::visitStruct(HIRItemPath p, HIRStruct& str) -> void {
    const HIRTypeData* ty = crate.types.path(HIRGenericPath(p.getSimplePath(), str.params.makeNopParams(crate.types, 0)), &str);
...
auto ExpanderSelf::visitUnion(HIRItemPath p, HIRUnion& unn) -> void {
    const HIRTypeData* ty = crate.types.path(HIRGenericPath(p.getSimplePath(), unn.params.makeNopParams(crate.types, 0)), &unn);
    implType = ty;
    HIRVisitor::visitUnion(p, unn);
    implType = nullptr;
}

auto ExpanderSelf::visitTypeImpl(HIRTypeImpl& impl) -> void {
    implType = impl.type;
    HIRVisitor::visitTypeImpl(impl);
    implType = nullptr;
}

auto ExpanderSelf::visitTraitImpl(const HIRSimplePath& traitPath, HIRTraitImpl& impl) -> void {
    Span sp;
    implType = impl.type;
    HIRVisitor::visitTraitImpl(traitPath, impl);
    implType = nullptr;
...
[[nodiscard]] auto AliasConstGenericParamBinder::visitType(const HIRTypeData* ty) -> const HIRTypeData* {
    return visitTypeDefaultViaHooks(ty);
}

auto AliasConstGenericParamBinder::visitTypeAlias(HIRItemPath p, HIRTypeAlias& item) -> void {
    Guard guard(*this, item.params);
    HIRVisitor::visitTypeAlias(p, item);
}

auto AliasConstGenericParamBinder::visitTraitAlias(HIRItemPath p, HIRTraitAlias& item) -> void {
    Guard guard(*this, item.params);
    HIRVisitor::visitTraitAlias(p, item);
}
...
AliasConstGenericParamBinder::Guard::~Guard() {
    binder.implParams = old;
}

auto ReceiverValidator::replaceImplTypeWithSelf(const HIRTypeData* ty) -> const HIRTypeData* {
    if (ty == crate.types.self()) {
        return ty;
    }
    if (ty == implType) {
        return crate.types.self();
    }
    if (ty->is_Generic()) {
        return ty;
    }
    ...
    if (const auto* borrow = ty->opt_Borrow()) {
        ...
        return crate.types.borrow(borrow->type, inner);
    }
    if (const auto* pointer = ty->opt_Pointer()) {
        ...
        return crate.types.pointer(pointer->type, inner);
    }
    return nullptr;
}

auto ReceiverValidator::needsProjectionValidation(const HIRFunction& item) const -> bool {
    if (item.receiver != HIRFunction::Receiver::Custom) {
        return false;
    }
    ASSERT_BUG(Span(), item.receiverType, StringView("Custom receiver without a receiver type"));
...
auto ReceiverValidator::validateFunction(HIRFunction& item) -> void {
    if (item.receiver == HIRFunction::Receiver::Custom) {
        ASSERT_BUG(Span(), item.receiverType, StringView("Custom receiver without a receiver type"));
        ASSERT_BUG(Span(), !item.args.empty(), StringView("Custom receiver without arguments"));
        ...
        if (needsProjectionValidation(item)) {
            ...
            receiverType = resolve.expandAssociatedTypes(Span(), receiverType);
        }
        const auto* replacedReceiverType = replaceImplTypeWithSelf(receiverType);
        if (!replacedReceiverType) {
            ERROR(Span(), E0000, StringView("Unknown receiver type - ") << *item.receiverType);
        }
        receiverType = replacedReceiverType;
...
auto ReceiverValidator::validateModule(HIRModule& module) -> void {
...
MarkingsVisitor::MarkingsVisitor(const WireBoard& wb)
    : HIRVisitor(nullptr, wb.crate->types)
    , crate(*wb.crate)
    , resolve_(wb)
        ...
        for (size_t i = 0; i < str.params.types.size(); i++) {
            ...
            if (!param.isSized) {
                if (visitTyWith(lastFieldTy, [&](const auto& t) {
                    ...
                    ASSERT_BUG(Span(), str.structMarkings.unsizedParam == ~0u, StringView("Multiple unsized params to ") << ip);
                    str.structMarkings.unsizedParam = i;
                }
            }
        }
        ASSERT_BUG(Span(), str.structMarkings.unsizedParam != ~0u, StringView("No unsized param for type ") << ip);
...
auto MarkingsVisitor::getFieldDstType(const HIRTypeData* ty, const HIRGenericParams& innerDef, const HIRGenericParams& paramsDef, const HIRPathParams* params) -> HIRStructMarkings::DstType {
    if (const auto* te = ty->opt_Generic()) {
...
auto MarkingsVisitor::getStructDstType(const HIRStruct& str, const HIRGenericParams& def, const HIRPathParams* params) -> HIRStructMarkings::DstType {
    switch (str.data.tag()) {
...
auto MarkingsVisitor::visitTraitImpl(const HIRSimplePath& traitPath, HIRTraitImpl& impl) -> void {
    Span sp;
    ...
    if (impl.type->is_Path()) {
        ...
        if (markingsPtr) {
            ...
            if (traitPath == langUnsize_) {
                ...
                DEBUG(StringView("Type ") << impl.type << StringView(" can Coerce"));
                if (impl.traitArgs.types.size() != 1) {
                    ERROR(sp, E0000, StringView("Unexpected number of arguments for CoerceUnsized"));
                }
                const auto& dstTy = impl.traitArgs.types[0];
                if (!te.binding.is_Struct()) {
                    ERROR(sp, E0000, StringView("Cannot implement CoerceUnsized on non-structs"));
                }
                if (!dstTy->is_Path()) {
                    ERROR(sp, E0000, StringView("Cannot implement CoerceUnsized from non-structs"));
                }
                ...
                if (dstTe.binding.as_Struct() != te.binding.as_Struct()) {
                    ERROR(sp, E0000, StringView("CoerceUnsized can only be implemented between variants of the same struct"));
                }
                    ...
                    case HIRStructData::TAG_Tuple: {
                        ...
                        for (unsigned int i = 0; i < se.size(); i++) {
                            if (monomorphiseTypeNeeded(se[i].ent)) {
                                ...
                                if (tyL != tyR) {
                                    if (field != ~0u) {
                                        ERROR(sp, E0000, StringView("CoerceUnsized impls can only differ by one field"));
                                    }
                                    field = i;
                                }
                            }
                ...
                if (field == ~0u) {
                    ERROR(sp, E0000, StringView("CoerceUnsized requires a field to differ between source and destination"));
                }
                structMarkings.coerceUnsizedIndex = field;
            } else if (traitPath == langDeref_) {
                DEBUG(StringView("Type ") << impl.type << StringView(" can Deref"));
                markings.hasADeref = true;
            } else if (traitPath == langCopy_) {
                DEBUG(StringView("Type ") << impl.type << StringView(" has a Copy impl"));
...
auto Visitor2::getUnsizeParamIdx(const Span& sp, const HIRTypeData* pointee) const -> size_t {
    if (const auto* te = pointee->opt_Generic()) {
        ...
        ASSERT_BUG(sp, te->binding.is_Struct(), StringView("Pointer to non-Unsize type - ") << pointee);
        const auto& ism = te->binding.as_Struct()->structMarkings;
        ASSERT_BUG(sp, ism.unsizedParam != ~0u, StringView("Pointer to non-Unsize type - ") << pointee);
        const auto& gp = te->path.data.as_Generic();
        return getUnsizeParamIdx(sp, gp.params.types.at(ism.unsizedParam));
    } else {
        BUG(sp, StringView("Pointer to non-Unsize type? - ") << pointee);
    }
}

auto Visitor2::getCoerceType(const Span& sp, HIRItemPath ip, const HIRStruct& str, size_t& outParamIdx) const -> HIRStructMarkings::Coerce {
    ...
    BUG_ASSERT(fieldTy);
    const HIRTypeData* normalizedFieldTy;
tryAgain:
    normalizedFieldTy = fieldTy;
    normalizedFieldTy = resolve_.expandAssociatedTypes(sp, normalizedFieldTy);
    fieldTy = normalizedFieldTy;

    DEBUG(StringView("field_ty = ") << fieldTy);
    if (const auto* te = fieldTy->opt_Path()) {
        ASSERT_BUG(sp, te->binding.is_Struct(), StringView("CoerceUnsized impl differs on Path that isn't a struct - ") << ip << StringView(" fld=") << fieldTy);
        const auto* istr = te->binding.as_Struct();
        const auto& gp = te->path.data.as_Generic();

        size_t innerIdx = 0;
        auto innerType = getCoerceType(sp, {fieldTy}, *istr, innerIdx);
        ASSERT_BUG(sp, innerType != HIRStructMarkings::Coerce::None, StringView("CoerceUnsized impl differs on a non-CoerceUnsized type - ") << ip << StringView(" fld=") << fieldTy);
        ...
        TODO(sp, StringView("Handle CoerceUnsized type ") << fieldTy);
    }
    BUG(sp, StringView("Reached end of get_coerce_type - ") << fieldTy);
}

auto Visitor2::visitStruct(HIRItemPath ip, HIRStruct& str) -> void {
    Span sp;
...
auto UfcsVisitor::pushModTraits(HIRSimplePath path, const HIRModule& mod) -> ModTraitsGuard {
    Span sp;
    DEBUG(StringView(""));
...
auto UfcsVisitor::visitModule(HIRItemPath p, HIRModule& mod) -> void {
    auto _ = this->pushModTraits(p.getSimplePath(), mod);
    HIRVisitor::visitModule(p, mod);
}

auto UfcsVisitor::visitParams(HIRGenericParams& params) -> void {
...
auto UfcsVisitor::visitUnion(HIRItemPath p, HIRUnion& item) -> void {
...
auto UfcsVisitor::visitEnum(HIRItemPath p, HIREnum& item) -> void {
    auto _ = resolve_.setImplGenerics(MetadataType::None, item.params);
    auto ty = crate.types.path(HIRGenericPath(p.getSimplePath()), &item);
    currentType_ = ty;
    DeclaredTypeGuard declaredTypes(*this);
...
auto UfcsVisitor::visitFunction(HIRItemPath p, HIRFunction& item) -> void {
...
auto UfcsVisitor::visitStatic(HIRItemPath p, HIRStatic& item) -> void {
    auto _ = resolve_.setItemGenerics(item.params);
    DeclaredTypeGuard declaredTypes(*this);
    HIRVisitor::visitStatic(p, item);
}

auto UfcsVisitor::visitTypeAlias(HIRItemPath p, HIRTypeAlias& item) -> void {
    auto _ = resolve_.setImplGenerics(MetadataType::Unknown, item.params);
    DeclaredTypeGuard declaredTypes(*this);
    HIRVisitor::visitTypeAlias(p, item);
}

auto UfcsVisitor::visitTrait(HIRItemPath p, HIRTrait& trait) -> void {
    inTraitDef_ = true;
    currentTrait = &trait;
    currentTraitPath_ = &p;
...
auto UfcsVisitor::visitTraitAlias(HIRItemPath p, HIRTraitAlias& item) -> void {
    auto _ = resolve_.setImplGenerics(MetadataType::Unknown, item.params);
    DeclaredTypeGuard declaredTypes(*this);
    HIRVisitor::visitTraitAlias(p, item);
}

auto UfcsVisitor::visitAssociatedtype(HIRItemPath p, HIRAssociatedType& item) -> void {
...
auto UfcsVisitor::visitTypeImpl(HIRTypeImpl& impl) -> void {
...
auto UfcsVisitor::visitInherentType(HIRItemPath p, HIRTypeAlias& item) -> void {
    auto _ = resolve_.setItemGenerics(item.params);
    DeclaredTypeGuard declaredTypes(*this);
    HIRVisitor::visitInherentType(p, item);
}

auto UfcsVisitor::visitMarkerImpl(const HIRSimplePath& traitPath, HIRMarkerImpl& impl) -> void {
    HIRItemPath p(impl.type, traitPath, impl.traitArgs);
    TRACE_FUNCTION_F(StringView("impl") << impl.params.fmtArgs() << StringView(" ") << traitPath << impl.traitArgs << StringView(" for ") << impl.type << StringView(" (mod=") << impl.srcModule << StringView(")"));
    auto _t = this->pushModTraits(impl.srcModule, this->crate.getModByPath(Span(), impl.srcModule));
...
auto UfcsVisitor::visitTraitImpl(const HIRSimplePath& traitPath, HIRTraitImpl& impl) -> void {
    ...
    resolve_.forEachTypeEquality([&](const HIRTypeData* ty) {
        return visitType(ty);
    });
...
auto UfcsVisitor::visitExpr(HIRExprPtr& expr) -> void {
    struct ExprVisitor: public HIRExprVisitorDef {
        ...
        void visit(HIRExprNodeCallPath& node) override {
            ...
            if (node.path.data.is_Generic()) {
                ...
                if (gp.path.components().size() > 1) {
                    ...
                    if (ent.is_Enum() && ent.as_Enum().findVariant(gp.path.components().back()) != SIZE_MAX) {
                        ...
                        DEBUG(static_cast<const void*>(&node) << StringView(": Replacing with TupleVariant ") << static_cast<const void*>(replacement.get()));
                        return;
                    }
                }
            }
            ...
            if (v.is_Constant() || v.is_Static()) {
                ...
                DEBUG(static_cast<const void*>(&node) << StringView(": Replacing with CallValue ") << static_cast<const void*>(replacement.get()));
                return;
            }
        }
                ...
                if (gp.path.components().size() > 1) {
                    ...
                    if (ent.is_Enum()) {
                        ...
                        if (enm.data.is_Value() || enm.data.as_Data().at(idx).type == upperVisitor.crate.types.unit()) {
                            ...
                            DEBUG(static_cast<const void*>(&node) << StringView(": Replacing with UnitVariant ") << static_cast<const void*>(replacement.get()));
                        } else {
        ...
        void visit(HIRExprNodeStructLiteral& node) override {
            ...
            if (node.type->is_Path() && node.type->as_Path().path.data.is_Generic()) {
                ...
                if (gp.path.components().size() > 1) {
                    const auto& ent = upperVisitor.crate.getTypeitemByPath(sp, gp.path, /*ign_crate*/ false, true);
                    if (ent.is_Enum()) {
                        DEBUG(static_cast<const void*>(&node) << StringView(": Tagging as an enum"));
                        node.isStruct = false;
        ...
        void visit(HIRExprNodeBlock& node) override {
            TRACE_FUNCTION_F(StringView("_Block"));
...
auto UfcsVisitor::locateTraitItemInBounds(HIRVisitor::PathContext pc, const HIRTypeData* tr, const HIRGenericParams& params, HIRPath::Data& pd) -> bool {
    Span sp;
    for (const auto& b : params.bounds) {
...
auto UfcsVisitor::locateItemInTrait(HIRVisitor::PathContext pc, const HIRTrait& trait, HIRPath::Data& pd) -> bool {
...
auto UfcsVisitor::locateInTraitAndSet(HIRVisitor::PathContext pc, const HIRGenericPath& traitPath, const HIRTrait& trait, HIRPath::Data& pd) -> bool {
    ...
    auto monomorphGpIfNeeded = [&](const HIRGenericPath& tpl) -> const HIRGenericPath& {
        if (monomorphiseGenericpathNeeded(tpl) /*&& tpl.m_params != trait_path.m_params*/) {
            DEBUG(StringView("[monomorph_gp_if_needed] Monomorph tpl=") << tpl);
    ...
    for (const auto& pt : trait.parentTraits) {
        ...
        DEBUG(StringView("- Check ") << parTraitPath);
    ...
    for (const auto& pt : trait.allParentTraits) {
        const auto& parTraitPath = monomorphGpIfNeeded(pt.path);
        DEBUG(StringView("- Check (all) ") << parTraitPath);
...
auto UfcsVisitor::setFromTraitImpl(const Span& sp, HIRVisitor::PathContext pc, const HIRGenericPath& traitPath, const HIRTrait& trait, HIRPath::Data& pd) -> bool {
...
auto UfcsVisitor::locateInTraitImplAndSet(const Span& sp, HIRVisitor::PathContext pc, const HIRGenericPath& traitPath, const HIRTrait& trait, HIRPath::Data& pd) -> bool {
    if (this->locateItemInTrait(pc, trait, pd)) {
        return setFromTraitImpl(sp, pc, traitPath, trait, pd);
        DEBUG(StringView("- Item ") << pd.as_UfcsUnknown().item << StringView(" not in trait ") << traitPath.path);
    }
...
auto UfcsVisitor::resolve_UfcsUnknown_trait(const HIRPath& p, HIRVisitor::PathContext pc, HIRPath::Data& pd) -> bool {
    ...
    DEBUG(StringView("m_traits.size() = ") << traits.size());
    for (const auto& traitInfo : ::reverse(traits)) {
        ...
        DEBUG(e.item << StringView(" in? ") << *traitInfo.first);
        switch (pc) {
            case HIRVisitor::PathContext::VALUE:
                if (trait.values.find(e.item) == trait.values.end()) {
                    continue;
...
[[nodiscard]] auto UfcsVisitor::visitType(const HIRTypeData* ty) -> const HIRTypeData* {
    // TODO: Add a span parameter.
    Span sp;

    const bool definesContainedOpaque = inExpr && definingOpaqueAliasCount_ > 0 && ty->is_Path() && ty->as_Path().path.data.is_UfcsKnown() && visitTyWith(ty, [&](const HIRTypeData* inner) {
    ...
    if (runEat && (!preserveDeclaredProjections_ || inExpr) && !definesContainedOpaque) {
        ...
        while (true) {
            const auto* expanded = resolve_.expandAssociatedTypesSingle(sp, ty);
            if (expanded == ty) {
                break;
            }
            ty = expanded;
            ...
            ty = rewriteTyWith(crate.types, ty, [&](const HIRTypeData*, HIRTypeData& data) -> const HIRTypeData* {
...
auto UfcsVisitor::visitConstgeneric(HIRConstGeneric& val) -> void {
...
auto UfcsVisitor::visitPath(HIRPath& p, HIRVisitor::PathContext pc) -> void {
    Span sp;
    ...
    if (auto* pe = p.data.opt_UfcsKnown()) {
        ...
        if (tp.params.types.size() < trait.params.types.size()) {
            //TODO(sp, StringView("Defaults in UfcsKnown - ") << p << " - " << tp.m_params << " vs " << trait.m_params.fmt_args());
        }
    }
    ...
    if (auto* pe = p.data.opt_UfcsUnknown()) {
        ...
        if (currentType_) {
            rewritePathTysWith(crate.types, p, [&](const HIRTypeData*, HIRTypeData& data) -> const HIRTypeData* {
        ...
        if (const auto* traitObject = e.type->opt_TraitObject()) {
            ...
            if (principal.traitPtr && locateInTraitAndSet(pc, principal.path, *principal.traitPtr, p.data)) {
                DEBUG(StringView("Found in trait object bounds, p = ") << p);
                BUG_ASSERT(!p.data.is_UfcsUnknown());
                return;
            }
        }
        ...
        DEBUG(StringView("e.type = ") << e.type);
...
auto UfcsVisitor::visitPattern(HIRPattern& pat) -> void {
    Span _sp = Span();
    const Span& sp = _sp;
...
auto UfcsVisitor::resolvePatternBinding(const Span& sp, HIRPath& path, HIRPattern::PathBinding& binding) -> void {
    ...
    ASSERT_BUG(sp, te.path.data.is_Generic(), StringView("Pattern associated type didn't resolve to a generic path - ") << ty);
    path = te.path.clone();
    ...
    if (te.binding.is_Struct()) {
        ...
        ERROR(sp, E0000, StringView("Pattern associated type didn't resolve to a struct or union - ") << ty);
    }
}

auto UfcsVisitor::visitPatternValue(const Span& sp, const HIRPattern& pat, HIRPattern::Value& val) -> void {
    TRACE_FUNCTION_F(StringView("pat=") << pat << StringView(", val=") << val);
    if (auto* vep = val.opt_Named()) {
        ...
        switch (ve.path.data.tag()) {
            ...
            case HIRPathData::TAG_UfcsUnknown: {
                BUG(sp, StringView("UfcsUnknown still in pattern value - ") << pat);
                break;
            }
            case HIRPathData::TAG_UfcsInherent: {
                ...
                if (!rv) {
                    ERROR(sp, E0000, StringView("Constant ") << ve.path << StringView(" couldn't be found"));
                }
                break;
            }
            case HIRPathData::TAG_UfcsKnown: {
...
UfcsVisitor::DeclaredTypeGuard::~DeclaredTypeGuard() {
...
UfcsVisitor::ModTraitsGuard::~ModTraitsGuard() {
    if (v) {
        DEBUG(StringView("Stack pop: ") << this->v->traits.size() << StringView(" -> ") << this->oldImports.size());
