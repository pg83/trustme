...
void HIRExprVisitorDef::visitNodePtr(HIRExprNodeP& nodePtr) {
    BUG_ASSERT(nodePtr);
    TRACE_FUNCTION_F(static_cast<const void*>(&*nodePtr) << StringView(" ") << nodePtr->typeName());
    nodePtr->visit(*this);
...
DEF_VISIT_H(HIRExprNodeConstBlock, node) {
    TRACE_FUNCTION_F(StringView("_ConstBlock"));
...
DEF_VISIT_H(HIRExprNodeUse, node) {
    TRACE_FUNCTION_F(StringView("_Use"));
    visitNodePtr(node.value);
}
...
DEF_VISIT_H(HIRExprNodeLoop, node) {
    TRACE_FUNCTION_F(StringView("_Loop"));
    visitNodePtr(node.code);
}

DEF_VISIT_H(HIRExprNodeLoopControl, node) {
    TRACE_FUNCTION_F(StringView("_LoopControl"));
    if (node.value) {
        visitNodePtr(node.value);
    }
}
...
DEF_VISIT_H(HIRExprNodeMatch, node) {
    TRACE_FUNCTION_F(StringView("_Match"));
    visitNodePtr(node.value);
...
DEF_VISIT(HIRExprNodeAssign, node, TRACE_FUNCTION_F(StringView("_Assign")); visitNodePtr(node.slot); visitNodePtr(node.value);)
DEF_VISIT(HIRExprNodeBinOp, node, TRACE_FUNCTION_F(StringView("_BinOp")); visitNodePtr(node.left); visitNodePtr(node.right);)
DEF_VISIT(HIRExprNodeUniOp, node, TRACE_FUNCTION_F(StringView("_UniOp")); visitNodePtr(node.value);)
DEF_VISIT(HIRExprNodeBorrow, node, TRACE_FUNCTION_F(StringView("_Borrow")); visitNodePtr(node.value);)
DEF_VISIT(HIRExprNodeRawBorrow, node, visitNodePtr(node.value);)

DEF_VISIT_H(HIRExprNodeCast, node) {
    TRACE_FUNCTION_F(StringView("_Cast ") << node.dstType);
    node.dstType = visitType(node.dstType);
    visitNodePtr(node.value);
}

DEF_VISIT_H(HIRExprNodeUnsize, node) {
    TRACE_FUNCTION_F(StringView("_Unsize ") << node.dstType);
    node.dstType = visitType(node.dstType);
    visitNodePtr(node.value);
}

DEF_VISIT_H(HIRExprNodeIndex, node) {
    TRACE_FUNCTION_F(StringView("_Index"));
    visitNodePtr(node.value);
...
DEF_VISIT_H(HIRExprNodeEmplace, node) {
    TRACE_FUNCTION_F(StringView("_Emplace"));
    if (node.place) {
...
DEF_VISIT_H(HIRExprNodeField, node) {
    TRACE_FUNCTION_F(StringView("_Field: ") << node.field);
    visitNodePtr(node.value);
}

DEF_VISIT(HIRExprNodeLiteral, node, TRACE_FUNCTION_F(StringView("_Literal"));)
DEF_VISIT(HIRExprNodeUnitVariant, node, TRACE_FUNCTION_F(StringView("_UnitVariant: ") << node.path); visitGenericPath(HIRVisitor::PathContext::VALUE, node.path);)
DEF_VISIT(HIRExprNodePathValue, node, TRACE_FUNCTION_F(StringView("_PathValue: ") << node.path); visitPath(HIRVisitor::PathContext::VALUE, node.path);)
DEF_VISIT(HIRExprNodeVariable, node, TRACE_FUNCTION_F(StringView("_Variable: #") << node.slot);)
DEF_VISIT(HIRExprNodeConstParam, node, TRACE_FUNCTION_F(StringView("_ConstParam"));)

DEF_VISIT_H(HIRExprNodeStructLiteral, node) {
    TRACE_FUNCTION_F(StringView("_StructLiteral: ") << node.realPath);
...
DEF_VISIT_H(HIRExprNodeClosure, node) {
    TRACE_FUNCTION_F(StringView("_Closure"));
    if (node.objPath != HIRGenericPath()) {
        ...
        node.returnType = visitType(node.returnType);
        visitNodePtr(node.code);
    }
}
...
DEF_VISIT_H(HIRExprNodeGenerator, node) {
    ...
    node.returnType = visitType(node.returnType);
    node.yieldTy = visitType(node.yieldTy);
    node.resumeTy = visitType(node.resumeTy);
...
DEF_VISIT_H(HIRExprNodeAsyncBlock, node) {
    TRACE_FUNCTION_F(StringView("_AsyncBlock"));
    node.returnType = visitType(node.returnType);
    if (node.code) {
        visitNodePtr(node.code);
    } else {
...
void stl::output<ZeroCopyOutput, HIRExprNodeClosure::AvuCache::Capture>(ZeroCopyOutput& os, const HIRExprNodeClosure::AvuCache::Capture& x) {
    ...
    os << StringView("[") << x.usage << StringView("]");
    return;
}
...
void stl::output<ZeroCopyOutput, HIRValueUsage>(ZeroCopyOutput& os, HIRValueUsage x) {
    switch (x) {
        ...
        case HIRValueUsage::Borrow:
            os << StringView("Borrow");
            break;
        case HIRValueUsage::Mutate:
            os << StringView("Mutate");
            break;
