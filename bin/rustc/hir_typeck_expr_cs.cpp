...
namespace {
    ...
    inline ::HIR::SimplePath getParentPath(const ::HIR::SimplePath& sp) {
        return sp.parent();
    }

    inline ::HIR::GenericPath getParentPath(const ::HIR::GenericPath& gp) {
        return ::HIR::GenericPath(getParentPath(gp.mPath), gp.mParams.clone());
    }

    bool type_contains_impl_placeholder(HIR::TypeInterner& types, const ::HIR::TypeData* t) {
...
#define NEWNODE(TY, SP, CLASS, ...) mk_exprnodep(context.crate.pool->make<HIR::ExprNode##CLASS>(SP, ##__VA_ARGS__), TY)
...
namespace {
    ...
    public:
        ...
        void visit(::HIR::ExprNodeBlock& node) override {
            const auto is_diverge = [&](const ::HIR::TypeData* rty) -> bool {
            ...
            const auto& last_ty = this->context.getType(node.nodes.back()->resType);
            ...
            if (const auto* e = last_ty->opt_Infer()) {
                switch (e->ty_class) {
                    ...
                    case ::HIR::InferClass::Float:
                        ...
                        break;
                    default:
                        this->context.possible_equate_type_unknown(node.span(), node.resType, Context::IvarUnknownType::From);
                        return;
                }
        ...
        void bad_cast(const Span& sp, const ::HIR::TypeData* src_ty, const ::HIR::TypeData* tgt_ty, const char* where) {
            ERROR(
                sp,
                E0000,
                "Invalid cast [" << where << "]:\n"
                                 << "from " << this->context.ivars.fmtType(src_ty) << "\n"
                                 << " to  " << this->context.ivars.fmtType(tgt_ty)
            );
        }

        void visit(::HIR::ExprNodeCast& node) override {
            const auto& sp = node.span();
            const auto& tgt_ty = this->context.getType(node.resType);
            const auto& src_ty = this->context.getType(node.mValue->resType);
            TRACE_FUNCTION_F(src_ty << " as " << tgt_ty);

            if (this->context.ivars.types_equal(src_ty, tgt_ty)) {
                this->completed = true;
                return;
            }
            ...
            TU_ARMA(Infer, e) {
                ...
                TU_ARMA(Primitive, e) {
                    ...
                    if (e == ::HIR::CoreType::Char) {
                        if (this->isFallback) {
                            this->context.equateTypes(sp, src_ty, context.crate.types.primitive(::HIR::CoreType::U8));
                            this->completed = true;
                        } else if (!this->context.getType(src_ty)->is_Infer()) {
                            this->completed = true;
                        } else {
                ...
                TU_ARMA(Path, e) {
                    this->context.equateTypesCoerce(sp, tgt_ty, node.mValue);
                    this->completed = true;
                    return;
                }
                TU_ARMA(Generic, e) {
                ...
                default:
                    ERROR(sp, E0000, "Invalid cast to pointer from " << src_ty);
                        ...
                        case ::HIR::TypeData::TAG_NamedFunction:
                            ...
                            if (ity == context.crate.types.unit() || ity == ::HIR::CoreType::U8 || ity == ::HIR::CoreType::I8) {
                                this->completed = true;
                            ...
                            TU_ARMA(Infer, s_e) {
                                switch (s_e.ty_class) {
                                    ...
                                    case ::HIR::InferClass::Integer:
                                        this->context.equateTypes(sp, src_ty, context.crate.types.primitive(::HIR::CoreType::Usize));
                                        this->completed = true;
                                        break;
                            ...
                            TU_ARMA(Borrow, s_e) {
                                ...
                                const auto& src_inner = this->context.getType(s_e.inner);
                                ...
                                if (const auto* dEI = ity->opt_Infer()) {
                                    this->context.possible_equate_ivar(sp, dEI->index, s_e.inner, Context::PossibleTypeSource::UnsizeFrom);
                                }

                                // If this looks like `&mut [?; N]` -> `*mut ?` then do a possible equate between the two types
                                if (src_inner->is_Array()) {
                                    if (const auto* s_e_i = this->context.getType(src_inner->as_Array().inner)->opt_Infer()) {
                                        this->context.possible_equate_ivar(sp, s_e_i->index, ity, Context::PossibleTypeSource::UnsizeTo);
                                        return;
                                    }
                                }
                                ...
                                // - NOTE: Crude, and likely to break if ether inner isn't known.
                                    ...
                                    auto ty = context.crate.types.pointer(e.type, src_inner);
                                    node.mValue = NEWNODE(ty, sp, Cast, mv$(node.mValue), ty);
                                    this->completed = true;
                                } else {
                                    bool found = !this->context.mResolve.mLangUnsize.components().empty()
                                                 && this->context.mResolve.findTraitImpls(sp, this->context.mResolve.mLangUnsize, ::HIR::PathParams(e.inner), s_e.inner, [](auto, auto) {
                                                        return true;
                                                    });
                                    if (found) {
                                        auto ty = context.crate.types.borrow(e.type, e.inner);
                                        node.mValue = NEWNODE(ty, sp, Unsize, mv$(node.mValue), ty);
                                        this->context.addTraitBound(sp, s_e.inner, this->context.mResolve.mLangUnsize, ::HIR::PathParams(e.inner));
                                    } else {
                                        this->context.equateTypes(sp, e.inner, s_e.inner);
                                    }
                                    this->completed = true;
                                }
                            }
                            TU_ARMA(Pointer, s_e) {
                                ...
                                if (dstInner->is_Infer()) {
                                    ...
                                    this->context.possible_equate_ivar_bounds(sp, dstInner->as_Infer().index, std::move(tys));
                                    return;
                                    ...
                                    this->context.equateTypes(sp, dstInner, src_inner);
                                } else {
                                }
                                this->completed = true;
                            }
                ...
                default:
                    bad_cast(sp, src_ty, tgt_ty, "fcn src");
                        TU_ARMA(Infer, s_e) {
                            ...
                            if (this->isFallback) {
                                this->context.equateTypes(sp, src_ty, tgt_ty);
                                this->completed = true;
                            }
                        }
                        TU_ARMA(NodeType, s_e) {
                            if (const auto* const* sn_pp = s_e.opt_Closure()) {
                                ...
                                if ((*sn_pp)->mArgs.size() != e.argTypes.size()) {
                                    bad_cast(sp, src_ty, tgt_ty, "fcn nargs");
                                }
                                this->context.equateTypes(sp, ms.monomorph_type(sp, e.mRettype), (*sn_pp)->returnType);
                                for (size_t i = 0; i < e.argTypes.size(); i++) {
                                    this->context.equateTypes(sp, ms.monomorph_type(sp, e.argTypes[i]), (*sn_pp)->mArgs[i].second);
                                }
                                this->completed = true;
                            } else {
                                bad_cast(sp, src_ty, tgt_ty, "fcn src");
                        ...
                        TU_ARMA(Function, s_e) {
                            ...
                            if (s_e.mAbi != e.mAbi || (s_e.is_unsafe && s_e.is_unsafe != e.is_unsafe) || s_e.argTypes.size() != e.argTypes.size()) {
                                bad_cast(sp, src_ty, tgt_ty, "fcn nargs");
                            }
                            this->context.equateTypes(sp, tgt_ty, src_ty);
                            this->completed = true;
                        }
                        TU_ARMA(NamedFunction, f) {
                            auto ft = context.mResolve.expandAssociatedTypes(sp, context.crate.types.function(f.decay(context.crate.types, sp)));
                            const auto& s_e = ft->as_Function();
                            // Check that the ABI and unsafety is correct
                            if (s_e.mAbi != e.mAbi || (s_e.is_unsafe && s_e.is_unsafe != e.is_unsafe) || s_e.argTypes.size() != e.argTypes.size()) {
                                bad_cast(sp, src_ty, tgt_ty, "fcn nargs");
                            }
                            this->context.equateTypes(sp, tgt_ty, ft);
                            this->completed = true;
                        }
                }
                }
        ...
        void visit(::HIR::ExprNodeIndex& node) override {
            ...
            const auto& val_ty = this->context.getType(node.mValue->resType);
            //const auto& idx_ty = this->context.get_type(node.m_index->m_res_type);
            const auto& idx_ty = this->context.getType(node.cache.index_ty);
            ...
            do {
                ...
                bool rv = this->context.mResolve.findTraitImpls(node.span(), langIndex, trait_pp, ty, [&](auto impl, auto cmp) {
                    DEBUG("[visit(_Index)] cmp=" << cmp << " - " << impl);
                    possible_res_type = impl.getType(context.crate.types, "Output", {});
                    count += 1;
                    if (cmp == ::HIR::Compare::Equal) {
                        return true;
                    }
                    possible_index_type = impl.getTraitTyParam(context.crate.types, 0);
                    return false;
                });
                if (rv) {
                    ...
                    if (possible_res_type == HIR::TypeRef()) {
                        possible_res_type = context.crate.types.path(::HIR::Path(ty, HIR::GenericPath(langIndex, mv$(trait_pp)), "Output"), HIR::TypePathBinding::make_Opaque({}));
                    }
                    // TODO: Node's result type could be an &-ptr?
                    this->context.equateTypes(node.span(), node.resType, possible_res_type);
                    break;
                } else if (count == 1) {
                    assert(possible_index_type != ::HIR::TypeRef());
                    this->context.equateTypesAssoc(node.span(), node.resType, langIndex, mv$(trait_pp), ty, "Output", {}, false);
                    break;
                } else if (count > 1) {
                ...
                derefCount += 1;
                currentTy = this->context.mResolve.autoderef(node.span(), ty, tmp_type);
                if (currentTy) {
            ...
            if (currentTy) {
                ...
                for (auto& ty_r : derefResTypes) {
                    auto ty = mv$(ty_r);

                    node.mValue = this->context.createAutoderef(mv$(node.mValue), mv$(ty));
                    context.ivars.getType(node.mValue->resType);
                }
        ...
        void visit(::HIR::ExprNodeDeref& node) override {
            ...
            const auto& op_trait = this->context.crate.getLangItemPathOpt("deref");
            auto use_builtin = [&](const ::HIR::TypeData* inner) {
            ...
            auto use_trait = [&]() {
                ...
                this->context.equateTypesAssoc(node.span(), node.resType, op_trait, {}, node.mValue->resType, "Target", {}, true, typeck::PrimitiveOperator::Deref);
            };

            TU_MATCH_HDRA( (*ty), {)
            default: {
                    if (const auto* inner = this->context.mResolve.type_is_owned_box(node.span(), ty)) {
                        use_builtin(inner);
                    } else {
                ...
                TU_ARMA(Borrow, e) {
                    if (op_trait.components().empty() || this->context.is_current_native_deref_receiver(op_trait, ty)) {
                        ...
                        bool hasVisibleImpl = this->context.mResolve.findTraitImpls(node.span(), op_trait, {}, ty, [&](ImplRef impl, HIR::Compare) {
        ...
        void visit_emplace_129(::HIR::ExprNodeEmplace& node) {
            const auto& sp = node.span();
            const auto& expTy = this->context.getType(node.resType);
            const auto& dataTy = this->context.getType(node.mValue->resType);
            const auto& placer_ty = this->context.getType(node.place->resType);
            const auto& langBoxed = this->context.mLangBox;
            TRACE_FUNCTION_F("exp_ty=" << expTy << ", data_ty=" << dataTy << ", placer_ty" << placer_ty);
            ASSERT_BUG(sp, node.mType == ::HIR::ExprNodeEmplace::Type::Boxer, "1.29 mode with non-box _Emplace node");
            ASSERT_BUG(sp, placer_ty == context.crate.types.unit(), "1.29 mode with box in syntax - placer type is " << placer_ty);
            ...
            auto p = ::HIR::GenericPath(langBoxed, {dataTy});
            p.mParams.types.push_back(MonomorphStatePtr(context.crate.types, nullptr, &p.mParams, nullptr).monomorph_type(sp, str.mParams.types.at(1).defaultValue));
            this->context.addIvars(p.mParams.types.back());
        ...
        void visit(::HIR::ExprNodeCallPath& node) override {
            ...
            if (!typecheck::visit_call_populate_cache(this->context, node.span(), node.mPath, node.cache)) {
            ...
            if (node.mArgs.size() != expArgc) {
                if (node.cache.fcn->variadic && node.mArgs.size() > expArgc) {
                } else {
                    ERROR(node.span(), E0000, "Incorrect number of arguments to " << node.mPath << " - exp " << expArgc << " got " << node.mArgs.size());
                }
            }
            for (unsigned int i = 0; i < node.cache.argTypes.size() - 1; i++) {
                this->context.equateTypesCoerce(node.span(), node.cache.argTypes[i], node.mArgs[i]);
            }
            this->context.equateTypes(node.span(), node.resType, node.cache.argTypes.back());
            this->context.require_sized(node.span(), node.resType);
            this->completed = true;
        }
        ...
        void visit(::HIR::ExprNodeCallValue& node) override {
            ...
            node.mValue->resType = this->context.mResolve.expandAssociatedTypes(
                node.span(), node.mValue->resType
            );
            const auto& ty_o = this->context.getType(node.mValue->resType);
            TRACE_FUNCTION_F("CallValue: ty=" << ty_o);

            //this->context.possible_equate_type_coerce_from_unknown(node.span(), node.m_res_type);
            this->context.possible_equate_type_unknown(node.span(), node.resType, Context::IvarUnknownType::Bound);
            // - Shadow (prevent ivar guessing) every parameter
            for (const auto& argTy : node.argIvars) {
                this->context.possible_equate_type_unknown(node.span(), argTy, Context::IvarUnknownType::To);
            }
            ...
            if (this->context.mResolve.hasAssociatedType(ty_o)
                && this->context.mResolve.type_contains_ivars(ty_o)) {
                return;
            }
            ...
            ::HIR::PathParams trait_pp;
            {
                ...
                for (const auto& argTy : node.argIvars) {
                    arg_types.push_back(this->context.getType(argTy));
                }
                trait_pp.types.push_back(context.crate.types.tuple(mv$(arg_types)));
            }
            ...
            do // while( keep_looping );
            {
                ...
                if (ty->is_NodeType() && ty->as_NodeType().is_Closure()) {
                    ...
                    node.argTypes.push_back(node_p->returnType);
                    node.traitUsed = ::HIR::ExprNodeCallValue::TraitUsed::Unknown;
                } else if (ty->is_Function() || ty->is_NamedFunction()) {
                    HIR::TypeRef tmp_ft;
                    const auto* e = ty->opt_Function();
                    if (!e) {
                        tmp_ft = context.crate.types.function(ty->as_NamedFunction().decay(context.crate.types, node.span()));
                        tmp_ft = this->context.mResolve.expandAssociatedTypes(node.span(), tmp_ft);
                    ...
                    for (const auto& arg : e->argTypes) {
                        node.argTypes.push_back(m.monomorph_type(node.span(), arg));
                    }
                    if (e->is_variadic) {
                    ...
                    node.argTypes.push_back(m.monomorph_type(node.span(), e->mRettype));
                ...
                // TODO: If autoderef is possible, do it and continue. Only look for impls once autoderef fails
                    ...
                    unsigned int count = 0;
                    this->context.mResolve.findTraitImpls(node.span(), langFnOnce, trait_pp, ty, [&](auto impl, auto cmp) -> bool {
                        ...
                        MonomorphHrlsOnly(context.crate.types, HIR::PathParams()).monomorph_type(node.span(), tup);
                        fcnArgsTup = mv$(tup);
                    ...
                    if (count == 1) {
                        ...
                        if (!ty->is_Generic()) {
                            bool found = this->context.mResolve.findTraitImplsCrate(node.span(), langFnOnce, trait_pp, ty, [&](auto impl, auto cmp) -> bool {
                                ...
                                fcnArgsTup = mv$(tup);
                                fcnRet = impl.getType(context.crate.types, "Output", {});
                                ASSERT_BUG(node.span(), fcnRet != ::HIR::TypeRef(), "Impl didn't have a type for Output - " << impl);
                                return true;
                            });
                            if (found) {
                        ...
                        if (const auto* next_ty_p = this->context.mResolve.autoderef(node.span(), ty, tmp_type)) {
                        ...
                        ERROR(node.span(), E0000, "Unable to find an implementation of Fn*" << trait_pp << " for " << this->context.ivars.fmtType(ty));
                    }
            ...
            if (derefCount > 0) {
                ty = ty_o;
                while (derefCount-- > 0) {
                    const auto* next_ty = this->context.mResolve.autoderef(node.span(), ty, tmp_type);
                    assert(next_ty);
                    ty = next_ty;
                    node.mValue = this->context.createAutoderef(mv$(node.mValue), ty);
                }
            }

            ASSERT_BUG(node.span(), node.argTypes.size() == node.mArgs.size() + 1, "Malformed cache in CallValue: " << node.argTypes.size() << " != 1+" << node.mArgs.size());
        ...
        void visit(::HIR::ExprNodeCallMethod& node) override {
            ...
            const auto& ty = this->context.getType(node.mValue->resType);
            TRACE_FUNCTION_F(
                "(CallMethod) {" << this->context.ivars.fmtType(ty) << "}." << node.method << node.mParams << "(" << FMT_CB(os, for (const auto& argNode : node.mArgs) os << this->context.ivars.fmtType(argNode->resType) << ", ";) << ")"
                                 << " -> " << this->context.ivars.fmtType(node.resType)
            );

            // Make sure that no mentioned types are inferred until this method is known
            this->context.possible_equate_type_unknown(node.span(), node.resType, Context::IvarUnknownType::From);
            for (const auto& argNode : node.mArgs) {
                this->context.possible_equate_type_unknown(node.span(), argNode->resType, Context::IvarUnknownType::To);
            }
            ...
            unsigned int derefCount = this->context.mResolve.autoderefFindMethod(node.span(), node.traits, node.traitParamIvars, node.traitParamTypeIvars, ty, node.method, possible_methods);
        try_again:
            if (derefCount != ~0u) {
                ...
                if (ty->is_Infer() && this->isFallback) {
                    auto new_end = std::remove_if(possible_methods.begin(), possible_methods.end(), [](const auto& e) {
                        return e.second.mData.is_UfcsInherent();
                    });
                ...
                if (possible_methods.size() > 1) {
                    ...
                    for (auto it_1 = possible_methods.begin(); it_1 != possible_methods.end(); ++it_1) {
                        ...
                        if (!it_1->second.mData.is_UfcsKnown()) {
                            continue;
                        }

                        auto& e1 = it_1->second.mData.as_UfcsKnown();
                        for (auto it_2 = it_1 + 1; it_2 != possible_methods.end(); ++it_2) {
                            if (!it_2->second.mData.is_UfcsKnown()) {
                                continue;
                            }
                            ...
                            const auto& e2 = it_2->second.mData.as_UfcsKnown();
                            ...
                            if (e1.trait.mPath != e2.trait.mPath) {
                                continue;
                            }
                            assert(!(e1.trait.mParams == e2.trait.mParams));
                            ...
                            //if( e1.trait .compare_with_placeholders(sp, e2.trait, context.m_ivars.callback_resolve_infer()) == ::HIR::Compare::Unequal )
                            {
                                ...
                                unsigned int n_params = e1.trait.mParams.types.size();
                                ASSERT_BUG(sp, node.traitParamTypeIvars <= ivars.size(), "Invalid method ivar split");
                                ASSERT_BUG(sp, n_params <= node.traitParamTypeIvars, "Not enough type ivars for duplicate trait method");
                                ::HIR::PathParams trait_params;
                                trait_params.types.reserve(n_params);
                                for (unsigned int i = 0; i < n_params; i++) {
                                    trait_params.types.push_back(context.crate.types.infer(ivars[i], ::HIR::InferClass::None));
                                    //ASSERT_BUG(sp, m_ivars.get_type( trait_params.m_types.back() ).m_data.as_Infer().index == ivars[i], "A method selection ivar was bound");
                                }
                                const unsigned int n_values = e1.trait.mParams.values.size();
                                ASSERT_BUG(sp, n_values <= ivars.size() - node.traitParamTypeIvars, "Not enough value ivars for duplicate trait method");
                                trait_params.values.reserve(n_values);
                                for (unsigned int i = 0; i < n_values; i++) {
                                    trait_params.values.push_back(::HIR::ConstGeneric::make_Infer({ivars[node.traitParamTypeIvars + i]}));
                                }
                                // If one of these was already using the placeholder ivars, then maintain the one with the palceholders
                                if (e1.trait.mParams != trait_params) {
                                    e1.trait.mParams = mv$(trait_params);
                                }
                            }
                    ...
                    if (possible_methods.size() > 1 && this->context.mResolve.current_trait_path()) {
                        const auto& tp = *this->context.mResolve.current_trait_path();
                        auto found = possible_methods.end();
                        bool hadInherent = false;
                        for (auto it = possible_methods.begin(); it != possible_methods.end(); ++it) {
                            hadInherent |= it->second.mData.is_UfcsInherent();
                            if (it->second.mData.is_UfcsKnown() && it->second.mData.as_UfcsKnown().trait.mPath == tp.mPath) {
                                found = it;
                            }
                        }
                        if (!hadInherent && found != possible_methods.end()) {
                ...
                if (possible_methods.size() != 1 && possible_methods.front().second.mData.is_UfcsKnown()) {
                    DEBUG("- Multiple options, deferring");
                    // TODO: If the type is fully known, then this is an error.
                    return;
                }
                auto& adBorrow = possible_methods.front().first;
                auto& fcn_path = possible_methods.front().second;
                DEBUG("- deref_count = " << derefCount << ", fcn_path = " << fcn_path);

                node.methodPath = mv$(fcn_path);
                ...
                if (!typecheck::visit_call_populate_cache(this->context, node.span(), node.methodPath, node.cache)) {
                    ...
                    if (this->isFallback && node.methodPath.mData.is_UfcsInherent()) {
                        unsigned n_remove = 1;
                        while (n_remove < possible_methods.size() && possible_methods[n_remove].second.mData.is_UfcsInherent()) {
                            n_remove += 1;
                        }
                ...
                if (node.mArgs.size() + 1 != node.cache.argTypes.size() - 1) {
                    ERROR(node.span(), E0000, "Incorrect number of arguments to " << node.methodPath << " - exp " << node.cache.argTypes.size() - 2 << " got " << node.mArgs.size());
                }
                ...
                for (unsigned int i = 0; i < node.mArgs.size(); i++) {
                    ...
                    this->context.equateTypesCoerce(sp, node.cache.argTypes[1 + i], node.mArgs[i]);
                }
                ...
                if (derefCount > 0) {
                    ...
                    // Get dereferencing!
                    auto& node_ptr = node.mValue;
                    ::HIR::TypeRef tmp_ty;
                    const ::HIR::TypeData* curTy = node_ptr->resType;
                    while (derefCount--) {
                        auto span = node_ptr->span();
                        auto source_ty = curTy;
                        ::std::optional<::HIR::TypeRef> impl_type;
                        auto result = this->context.mResolve.autoderefStep(
                        ...
                        if (impl_type) {
                            this->context.equateTypes(span, source_ty, *impl_type);
                            this->context.equateTypesAssoc(
                                span,
                                tmp_ty,
                                this->context.crate.getLangItemPath(span, "deref"),
                                {},
                                source_ty,
                                "Target",
                                {},
                ...
                if (adBorrow != TraitResolution::AutoderefBorrow::None) {
                    ...
                    auto ty = context.crate.types.borrow(bt, node.mValue->resType);
                    DEBUG("- Ref (cmd) " << &*node.mValue << " -> " << ty);
                    auto span = node.mValue->span();
                    node.mValue = NEWNODE(mv$(ty), span, Borrow, bt, mv$(node.mValue));
                } else {
                }

                // Equate the type for `self` (to ensure that Self's type params infer correctly)
                this->context.equateTypes(sp, node.cache.argTypes[0], node.mValue->resType);
        ...
        void visit(::HIR::ExprNodeField& node) override {
            ...
            TRACE_FUNCTION_F("(Field) name=" << fieldName << ", ty = " << this->context.ivars.fmtType(node.mValue->resType));
            ...
            do {
                ...
                if (this->context.mResolve.findField(node.span(), ty, fieldName, out_type)) {
                    this->context.equateTypes(node.span(), node.resType, out_type);
                    break;
                }
            ...
            if (!currentTy) {
                ERROR(node.span(), E0000, "Couldn't find the field " << fieldName << " in " << this->context.ivars.fmtType(node.mValue->resType));
            }

            assert(derefCount == derefResTypes.size());
            for (unsigned int i = 0; i < derefResTypes.size(); i++) {
                auto ty = mv$(derefResTypes[i]);
                DEBUG("- Deref " << &*node.mValue << " -> " << ty);
                if (node.mValue->resType->is_Array()) {
                    BUG(node.span(), "Field access from array/slice?");
                }
                node.mValue = NEWNODE(mv$(ty), node.span(), Deref, mv$(node.mValue));
                context.ivars.getType(node.mValue->resType);
            }
    ...
    public:
        ExprVisitorApply(const Context& context)
            : ::HIR::ExprVisitorDef(context.crate.types)
            , context(context)
            , ivars(context.ivars)
        {
            nop_impl = context.mResolve.impl_generics().make_nop_params(context.crate.types, 0);
            nop_item = context.mResolve.item_generics().make_nop_params(context.crate.types, 1);
        }
        ...
        void visit_node_ptr(::HIR::ExprPtr& node_ptr) {
            ...
            TRACE_FUNCTION_FR(&node << " " << &node << " " << node_ty << " : " << node.resType, node_ty);
            this->checkTypeResolvedTop(node.span(), node.resType);
            DEBUG(node_ty << " : = " << node.resType);

            node_ptr->visit(*this);

            for (auto& ty : node_ptr.mBindings) {
                this->checkTypeResolvedTop(node.span(), ty);
            }

            for (auto& ty : node_ptr.erasedTypes) {
                this->checkTypeResolvedTop(node.span(), ty);
            }
            ...
            for (auto& ent : context.erasedTypeAliases) {
                ...
                auto ty = cloneTyWith(context.crate.types, node.span(), t, [&](const HIR::TypeData* tpl, HIR::TypeRef& out_ty) -> bool {
                    for (size_t i = 0; i < ent.second.params.types.size(); i++) {
                        if (tpl == ent.second.params.types[i]) {
                            out_ty = context.crate.types.generic(ent.first->generics.types.at(i).mName, i);
                            return true;
                        }
                    }
                    return false;
                ...
                {
                    auto p = ent.first->generics.make_nop_params(context.crate.types, 0);
                    MonomorphStatePtr(context.crate.types, nullptr, &p, nullptr).monomorph_type(node.span(), ty);
                }
        ...
        void visit_node_ptr(::HIR::ExprNodeP& node_ptr) override {
            auto& node = *node_ptr;
            const char* node_ty = typeid(node).name();
            TRACE_FUNCTION_FR(&node_ptr << " " << &node << " " << node_ty << " : " << node.resType, &node << " " << node_ty);
            this->checkTypeResolvedTop(node.span(), node.resType);
            DEBUG(node_ty << " : = " << node.resType);
        ...
        void visit_pattern(const Span& sp, ::HIR::Pattern& pat) override {
            ...
            default:
                break;
                TU_ARMA(Value, e) {
                    TU_IFLET(::HIR::Pattern::Value, (e.val), Named, ve, this->checkTypeResolvedPath(sp, ve.path);)
                }
                TU_ARMA(Range, e) {
        ...
        void visit(::HIR::ExprNodeLet& node) override {
            this->checkTypeResolvedTop(node.span(), node.mType);
            ::HIR::ExprVisitorDef::visit(node);
        }
        ...
        void visit(::HIR::ExprNodeCallPath& node) override {
            this->visit_callcache(node.span(), node.cache);

            this->checkTypeResolvedPath(node.span(), node.mPath);
            ::HIR::ExprVisitorDef::visit(node);
        }
        ...
        void visit(::HIR::ExprNodeCallValue& node) override {
            ...
            {
                const auto& ty = context.getType(node.mValue->resType);
                if (ty->is_NodeType() && ty->as_NodeType().is_Closure()) {
                    node.traitUsed = ::HIR::ExprNodeCallValue::TraitUsed::Unknown;
                    ...
                    if (!this->context.mResolve.mLangFn.components().empty()
                        && this->context.mResolve.findTraitImpls(node.span(), this->context.mResolve.mLangFn, trait_pp, ty, [&](auto impl, auto cmp) {
                        //ASSERT_BUG(node.span(), cmp == ::HIR::Compare::Equal, "");
                        return true;
                    })) {
                        DEBUG("-- Using Fn");
                        node.traitUsed = ::HIR::ExprNodeCallValue::TraitUsed::Fn;
                    } else if (!this->context.mResolve.mLangFnMut.components().empty()
                        && this->context.mResolve.findTraitImpls(node.span(), this->context.mResolve.mLangFnMut, trait_pp, ty, [&](auto impl, auto cmp) {
        ...
        void visit(::HIR::ExprNodeUnitVariant& node) override {
            this->checkTypeResolvedGenericpath(node.span(), node.mPath);
        }
        ...
        void visit(::HIR::ExprNodeTupleVariant& node) override {
            this->checkTypeResolvedPp(node.span(), node.mPath.mParams, ::HIR::TypeRef());
            for (auto& ty : node.argTypes) {
                if (ty != ::HIR::TypeRef()) {
                    this->checkTypeResolvedTop(node.span(), ty);
                }
    ...
    private:
        void checkTypeResolvedTop(const Span& sp, ::HIR::TypeRef& ty) const {
            checkTypeResolved(sp, ty, ty);
            ty = this->context.mResolve.expandAssociatedTypes(sp, mv$(ty));
            MonomorphHrlsOnly(context.crate.types, HIR::PathParams()).monomorph_type(sp, ty);
            DEBUG(ty);
        }

        void checkTypeResolvedConstgeneric(const Span& sp, ::HIR::ConstGeneric& v, const ::HIR::TypeData* top_type) const {
            if (v.is_Infer()) {
        ...
        void checkTypeResolvedPp(const Span& sp, ::HIR::PathParams& pp, const ::HIR::TypeData* top_type) const {
            for (auto& ty : pp.types) {
                checkTypeResolved(sp, ty, top_type);
            }
            for (auto& val : pp.values) {
                checkTypeResolvedConstgeneric(sp, val, top_type);
            }
        }

        void checkTypeResolvedPath(const Span& sp, ::HIR::Path& path) const {
            auto tmp = context.crate.types.path(path.clone(), {});
            checkTypeResolvedPath(sp, path, tmp);
            TU_MATCH(::HIR::Path::Data, (path.mData), (pe), (Generic, for (auto& ty : pe.mParams.types) ty = this->context.mResolve.expandAssociatedTypes(sp, mv$(ty));), (UfcsInherent, pe.type = this->context.mResolve.expandAssociatedTypes(sp, mv$(pe.type)); for (auto& ty : pe.params.types) ty = this->context.mResolve.expandAssociatedTypes(sp, mv$(ty)); for (auto& ty : pe.impl_params.types) ty = this->context.mResolve.expandAssociatedTypes(sp, mv$(ty));), (UfcsKnown, pe.type = this->context.mResolve.expandAssociatedTypes(sp, mv$(pe.type)); for (auto& ty : pe.params.types) ty = this->context.mResolve.expandAssociatedTypes(sp, mv$(ty)); for (auto& ty : pe.trait.mParams.types) ty = this->context.mResolve.expandAssociatedTypes(sp, mv$(ty));), (UfcsUnknown, throw "";))
        }

        void checkTypeResolvedPath(const Span& sp, ::HIR::Path& path, const ::HIR::TypeData* top_type) const {
            TU_MATCH(::HIR::Path::Data, (path.mData), (pe), (Generic, checkTypeResolvedPp(sp, pe.mParams, top_type);), (UfcsInherent, checkTypeResolved(sp, pe.type, top_type); checkTypeResolvedPp(sp, pe.params, top_type); checkTypeResolvedPp(sp, pe.impl_params, top_type);), (UfcsKnown, checkTypeResolved(sp, pe.type, top_type); checkTypeResolvedPp(sp, pe.trait.mParams, top_type); checkTypeResolvedPp(sp, pe.params, top_type);), (UfcsUnknown, ERROR(sp, E0000, "UfcsUnknown " << path << " left in " << top_type);))
        }

        void checkTypeResolvedGenericpath(const Span& sp, ::HIR::GenericPath& path) const {
            auto tmp = context.crate.types.path(path.clone(), {});
            checkTypeResolvedPp(sp, path.mParams, tmp);
        }

        void checkTypeResolved(const Span& sp, ::HIR::TypeRef& ty, const ::HIR::TypeData* top_type) const {
            ...
            public:
                InnerVisitor(const ExprVisitorApply& parent, const Span& sp, const ::HIR::TypeData* top_type)
                    : HIR::Visitor(nullptr, parent.context.crate.types)
                    , parent(parent)
                    , sp(sp)
                    , top_type(top_type)
                {
                ...
                void visit_type(::HIR::TypeRef& ty) override {
                    if (ty->is_Infer()) {
                        auto new_ty = parent.ivars.getType(ty);
                        DEBUG(ty << " -> " << new_ty);
    ...
    public:
        ...
        void visit(::HIR::ExprNodeCast& node) override {
            os << "_Cast {" << context.ivars.fmtType(node.mValue->resType) << "}";
        }
        ...
        void visit(::HIR::ExprNodeIndex& node) override {
            os << "_Index {" << fmtResTy(*node.mValue) << "}[{" << fmtResTy(*node.index) << "}]";
        }

        void visit(::HIR::ExprNodeDeref& node) override {
            os << "_Deref {" << fmtResTy(*node.mValue) << "}";
        }

        void visit(::HIR::ExprNodeEmplace& node) override {
            os << "_Emplace(" << fmtResTy(*node.mValue) << " in " << fmtResTy(*node.place) << ")";
        }
        ...
        void visit(::HIR::ExprNodeCallValue& node) override {
            os << "_CallValue {" << fmtResTy(*node.mValue) << "}(";
        ...
        void visit(::HIR::ExprNodeCallMethod& node) override {
            os << "_CallMethod {" << fmtResTy(*node.mValue) << "}." << node.method << "(";
            for (const auto& arg : node.mArgs) {
                os << "{" << fmtResTy(*arg) << "}, ";
            }
            os << ")";
        ...
        void visit(::HIR::ExprNodeField& node) override {
            os << "_Field {" << fmtResTy(*node.mValue) << "}." << node.field;
        }
    ...
    private:
        HMTypeInferrence::FmtType fmtResTy(const ::HIR::ExprNode& n) {
...
void Context::dump() const {
    ...
    for (unsigned int i = 0; i < mBindings.size(); i++) {
        DEBUG(i << " " << mBindings[i].name << ": " << this->ivars.fmtType(mBindings[i].ty));
    }
    DEBUG("--- Ivars");
    ivars.dump();
    DEBUG("--- CS Context - " << link_coerce.size() << " Coercions, " << link_assoc.size() << " associated, " << to_visit.size() << " nodes, " << advRevisits.size() << " callbacks");
    for (const auto& vp : link_coerce) {
        const auto& v = *vp;
        //DEBUG(v);
        DEBUG("R" << v.rule_idx << " " << this->ivars.fmtType(v.left_ty) << " := " << v.right_node_ptr << " " << &**v.right_node_ptr << " (" << this->ivars.fmtType((*v.right_node_ptr)->resType) << ")");
    }
...
void Context::equateTypes(const Span& sp, const ::HIR::TypeData* li, const ::HIR::TypeData* ri) {
    const auto& li_res = this->ivars.getType(li);
    const auto& ri_res = this->ivars.getType(ri);
    if (li == ri || li_res == ri_res || li_res->equalsIgnoringRegions(ri_res)) {
        DEBUG(li << " == " << ri);
        return;
    }
    ...
    ASSERT_BUG(sp, !type_contains_impl_placeholder(crate.types, ri), "Type contained an impl placeholder parameter - " << ri);
    ASSERT_BUG(sp, !type_contains_impl_placeholder(crate.types, li), "Type contained an impl placeholder parameter - " << li);

    ::HIR::TypeRef l_tmp;
    ::HIR::TypeRef r_tmp;
    const auto& l_t = this->mResolve.expandAssociatedTypes(sp, this->ivars.getType(li), l_tmp);
    const auto& r_t = this->mResolve.expandAssociatedTypes(sp, this->ivars.getType(ri), r_tmp);

    // Strip HRLs, just in case
    MonomorphHrlsOnly(crate.types, HIR::PathParams()).monomorph_type(sp, l_t);
    MonomorphHrlsOnly(crate.types, HIR::PathParams()).monomorph_type(sp, r_t);
    ...
    equateTypesInner(sp, l_t, r_t);
}

void Context::equateTypesInner(const Span& sp, const ::HIR::TypeData* li, const ::HIR::TypeData* ri) {
    const auto& li_res = this->ivars.getType(li);
    const auto& ri_res = this->ivars.getType(ri);
    if (li == ri || li_res == ri_res || li_res->equalsIgnoringRegions(ri_res)) {
        return;
    ...
    const auto& l_t = this->mResolve.expandAssociatedTypes(sp, this->ivars.getType(li), l_tmp);
    const auto& r_t = this->mResolve.expandAssociatedTypes(sp, this->ivars.getType(ri), r_tmp);
    if (l_t == r_t || l_t->equalsIgnoringRegions(r_t)) {
        return;
    }

    auto bindInferToAlias = [&](const ::HIR::TypeData* infer, const ::HIR::TypeData* alias) {
        const auto* infer_data = infer->opt_Infer();
        if (!infer_data
            || infer_data->is_lit()
            || visit_ty_with(alias, [&](const ::HIR::TypeData* inner) {
                return inner == infer;
            })) {
            return false;
        }
        const auto ivar_idx = infer_data->index;
        if (ivar_idx < ivarsSized.size() && ivarsSized.at(ivar_idx)) {
            this->require_sized(sp, alias);
        }
        this->ivars.set_ivar_to(ivar_idx, alias);
        return true;
    };
    ...
    if (const auto* r_e = r_t->opt_Path()) {
        if (const auto* rpe = r_e->path.mData.opt_UfcsKnown()) {
            if (r_e->binding.is_Unbound()) {
                if (bindInferToAlias(l_t, r_t)) {
                    return;
                }
                this->equateTypesAssoc(sp, l_t, rpe->trait.mPath, rpe->trait.mParams.clone(), rpe->type, rpe->item.c_str(), rpe->params, false);
                return;
            }
        }
    }
    if (const auto* l_e = l_t->opt_Path()) {
        if (const auto* lpe = l_e->path.mData.opt_UfcsKnown()) {
            if (l_e->binding.is_Unbound()) {
                if (bindInferToAlias(r_t, l_t)) {
                    return;
                }
                this->equateTypesAssoc(sp, r_t, lpe->trait.mPath, lpe->trait.mParams.clone(), lpe->type, lpe->item.c_str(), lpe->params, false);
                return;
            }
        }
    }

    auto equateErasedAlias = [&](const ::HIR::TypeDataErasedType& erased, const auto& alias, const ::HIR::TypeData* hiddenType) {
        if (!alias.inner->is_public_to(mResolve.visPath)) {
            return false;
        }

        auto inserted = this->erasedTypeAliases.insert(std::make_pair(alias.inner.get(), Context::TaitEntry{alias.params, hiddenType}));
        if (!inserted.second) {
            equateTypesInner(sp, inserted.first->second.our_type, hiddenType);
            return true;
        }

        struct MonomorphErasedSelf: MonomorphiserNop {
            const ::HIR::TypeData* hiddenType;

            MonomorphErasedSelf(HIR::TypeInterner& types, const ::HIR::TypeData* hiddenType)
                : MonomorphiserNop(types)
                , hiddenType(hiddenType)
            {
            }

            ::HIR::TypeRef getType(const Span&, const ::HIR::GenericRef& type) const override {
        ...
        for (const auto& trait : erased.traits) {
            auto trait_mono = monomorph.monomorph_traitpath(sp, trait, false);
            if (trait_mono.typeBounds.empty()) {
                equateTypesAssoc(sp, crate.types.infer(), trait_mono.mPath.mPath, trait_mono.mPath.mParams.clone(), hiddenType, "", {}, false);
                continue;
            }
            for (const auto& aty : trait_mono.typeBounds) {
                equateTypesAssoc(sp, aty.second.type, aty.second.source_trait.mPath, aty.second.source_trait.mParams.clone(), hiddenType, aty.first.c_str(), aty.second.atyParams, false);
            }
        }
        return true;
    };
    ...
    if (const auto* et = r_t->opt_ErasedType()) {
        if (const auto* ee = et->inner.opt_Alias()) {
            if (!l_t->is_Infer() && equateErasedAlias(*et, *ee, l_t)) {
                return;
            }
        }
    }
    if (const auto* et = l_t->opt_ErasedType()) {
        if (const auto* ee = et->inner.opt_Alias()) {
            if (equateErasedAlias(*et, *ee, r_t)) {
                return;
            }
        }
    }

    auto set_ivar = [&](const HIR::TypeData* dst, const HIR::TypeData* src) {
        auto ivar_idx = dst->as_Infer().index;
        if (ivar_idx < ivarsSized.size() && ivarsSized.at(ivar_idx)) {
            this->require_sized(sp, src);
        }
        // Ensure no HRLs
        if (visit_ty_with(src, [&](const HIR::TypeData* ity) {
            return ity == dst;
        })) {
            ...
            // - `_0 = Ty<_1>`
            // - `<_0 as Foo>::Type = _1`
            auto new_src = cloneTyWith(crate.types, sp, src, [&](const HIR::TypeData* tpl, HIR::TypeRef& out_ty) -> bool {
                if (tpl->is_Path() && tpl->as_Path().binding.is_Unbound()) {
                    if (visit_ty_with(src, [&](const HIR::TypeData* ity) {
                        return ity == dst;
                    })) {
                        const auto& pe = tpl->as_Path().path.mData.as_UfcsKnown();
                        out_ty = this->ivars.new_ivar_tr();
                        this->equateTypesAssoc(sp, out_ty, pe.trait.mPath, pe.trait.mParams.clone(), pe.type, pe.item.c_str(), pe.params, false);
                        return true;
                    } else {
                    }
                }
            ...
            this->ivars.set_ivar_to(ivar_idx, std::move(new_src));
        } else {
            this->ivars.set_ivar_to(ivar_idx, src);
        }
    };
    ...
    if (const auto* r_e = r_t->opt_Infer()) {
        if (const auto* l_e = l_t->opt_Infer()) {
            ...
            if ((r_e->index < ivarsSized.size() && ivarsSized.at(r_e->index)) || (l_e->index < ivarsSized.size() && ivarsSized.at(l_e->index))) {
                this->require_sized(sp, l_t);
                this->require_sized(sp, r_t);
            }

            this->ivars.ivar_unify(l_e->index, r_e->index);
        } else {
        ...
        if (/*const auto* l_e =*/l_t->opt_Infer()) {
            ...
            auto equalityTypeparams = [&](const ::HIR::PathParams& l, const ::HIR::PathParams& r) {
            ...
            auto equalityPath = [&](const ::HIR::Path& l, const ::HIR::Path& r) -> bool {
                if (l.mData.tag() != r.mData.tag()) {
                    return false;
                }
                TU_MATCH_HDRA( (l.mData, r.mData), {)
                TU_ARMA(Generic, lpe, rpe) {
                        if (lpe.mPath != rpe.mPath) {
                            return false;
                        }
                        equalityTypeparams(lpe.mParams, rpe.mParams);
                    }
                    ...
                    TU_ARMA(UfcsKnown, lpe, rpe) {
                        if (lpe.trait.mPath != rpe.trait.mPath || lpe.item != rpe.item) {
                            return false;
                        }
                        equalityTypeparams(lpe.trait.mParams, rpe.trait.mParams);
                        equalityTypeparams(lpe.params, rpe.params);
                        this->equateTypesInner(sp, lpe.type, rpe.type);
                    }
...
// TODO: Should ! end up in an ivar?
            ...
            /*else if( l_t->is_Diverge() ) {
                ...
                if (const auto* r_e = ri->opt_Infer()) {
                    this->ivars.set_ivar_to(r_e->index, l_t);
                }
                return;
            } else {
            }
#else
            if (l_t.mData.is_Diverge() || r_t.mData.is_Diverge()) {
                return;
            }
#endif

            if (l_t->tag() != r_t->tag()) {
                ERROR(sp, E0000, "Type mismatch between " << this->ivars.fmtType(l_t) << " and " << this->ivars.fmtType(r_t));
            }
            ...
            TU_ARMA(Infer, l_e, r_e) {
                ...
                TU_ARMA(Path, l_e, r_e) {
                    if (!equalityPath(l_e.path, r_e.path)) {
                        ERROR(sp, E0000, "Type mismatch between " << l_t << " and " << r_t);
                    }
                }
                ...
                TU_ARMA(TraitObject, l_e, r_e) {
                    if (l_e.mTrait.mPath.mPath != r_e.mTrait.mPath.mPath) {
                        ERROR(sp, E0000, "Type mismatch between " << l_t << " and " << r_t);
                    }
                    equalityTypeparams(l_e.mTrait.mPath.mParams, r_e.mTrait.mPath.mParams);
                    for (auto it_l = l_e.mTrait.typeBounds.begin(), it_r = r_e.mTrait.typeBounds.begin(); it_l != l_e.mTrait.typeBounds.end(); it_l++, it_r++) {
                        if (it_l->first != it_r->first) {
                            ERROR(sp, E0000, "Type mismatch between " << l_t << " and " << r_t << " - associated bounds differ");
                        }
                        this->equateTypesInner(sp, it_l->second.type, it_r->second.type);
                    }
                    if (l_e.markers.size() != r_e.markers.size()) {
                    ...
                    for (unsigned int i = 0; i < l_e.markers.size(); i++) {
                        auto& l_p = l_e.markers[i];
                        auto& r_p = r_e.markers[i];
                        if (l_p.mPath != r_p.mPath) {
                            ERROR(sp, E0000, "Type mismatch between " << l_t << " and " << r_t);
                        }
                        equalityTypeparams(l_p.mParams, r_p.mParams);
                    }
                    // NOTE: Lifetime is ignored
                }
                TU_ARMA(ErasedType, l_e, r_e) {
                    if (l_e.inner.tag() != r_e.inner.tag()) {
                        ERROR(sp, E0000, "Type mismatch between " << l_t << " and " << r_t << " - different erased class");
                    }
                TU_MATCH_HDRA( (l_e.inner, r_e.inner), {)
                TU_ARMA(Fcn, lee, ree) {
                            ASSERT_BUG(sp, lee.origin != ::HIR::SimplePath(), "ErasedType " << l_t << " wasn't bound to its origin");
                            ASSERT_BUG(sp, ree.origin != ::HIR::SimplePath(), "ErasedType " << r_t << " wasn't bound to its origin");
                ...
                TU_ARMA(Array, l_e, r_e) {
                    this->equateTypesInner(sp, l_e.inner, r_e.inner);
                    if (l_e.size != r_e.size) {
                        if (l_e.size.is_Unevaluated() || r_e.size.is_Unevaluated()) {
                            ...
                            if (!l_e.size.is_Unevaluated()) {
                                ...
                                this->equateValues(sp, HIR::EncodedLiteralPtr(EncodedLiteral::make_usize(l_e.size.as_Known())), r_e.size.as_Unevaluated());
                            } else if (!r_e.size.is_Unevaluated()) {
                                assert(l_e.size.is_Unevaluated());
                                assert(r_e.size.is_Known());
                                this->equateValues(sp, l_e.size.as_Unevaluated(), HIR::EncodedLiteralPtr(EncodedLiteral::make_usize(r_e.size.as_Known())));
                            } else {
                                this->equateValues(sp, l_e.size.as_Unevaluated(), r_e.size.as_Unevaluated());
                            }
                        } else {
                ...
                TU_ARMA(Tuple, l_e, r_e) {
                    ...
                    for (unsigned int i = 0; i < l_e.size(); i++) {
                        this->equateTypesInner(sp, l_e[i], r_e[i]);
                    }
                }
                ...
                TU_ARMA(Function, l_e, r_e) {
                    if (l_e.is_unsafe != r_e.is_unsafe || l_e.mAbi != r_e.mAbi || l_e.argTypes.size() != r_e.argTypes.size()) {
                        ERROR(sp, E0000, "Type mismatch between " << l_t << " and " << r_t);
                    }
                    // TODO: HRLs
                    this->equateTypesInner(sp, l_e.mRettype, r_e.mRettype);
                    for (unsigned int i = 0; i < l_e.argTypes.size(); i++) {
                        this->equateTypesInner(sp, l_e.argTypes[i], r_e.argTypes[i]);
                    }
                }
...
namespace {
    struct ConstExprEquate {
        ...
        const ::HIR::ConstGeneric* getParam(const ::HIR::ConstGenericUnevaluated& value, unsigned int binding) const {
        ...
        bool equateLiteral(const ::HIR::ExprNodeLiteral& left, const ::HIR::ExprNodeLiteral& right) const {
        ...
        bool equateNode(const ::HIR::ConstGenericUnevaluated& left_value, const ::HIR::ExprNode& left, const ::HIR::ConstGenericUnevaluated& right_value, const ::HIR::ExprNode& right) const {
            if (const auto* l = cast<const ::HIR::ExprNodeConstParam>(&left)) {
                const auto* r = cast<const ::HIR::ExprNodeConstParam>(&right);
                if (!r) {
                    return false;
                }
                const auto* l_param = getParam(left_value, l->mBinding);
                const auto* r_param = getParam(right_value, r->mBinding);
                if (!l_param || !r_param) {
                    return l->mBinding == r->mBinding;
                }
                context.equateValues(sp, *l_param, *r_param);
                return true;
            }
            ...
            if (const auto* l = cast<const ::HIR::ExprNodeBinOp>(&left)) {
                ...
                return r && l->op == r->op && equateNode(left_value, *l->left, right_value, *r->left) && equateNode(left_value, *l->right, right_value, *r->right);
            }
            if (const auto* l = cast<const ::HIR::ExprNodeUniOp>(&left)) {
                const auto* r = cast<const ::HIR::ExprNodeUniOp>(&right);
                return r && l->op == r->op && equateNode(left_value, *l->mValue, right_value, *r->mValue);
            }
            if (const auto* l = cast<const ::HIR::ExprNodeCast>(&left)) {
                ...
                return equateNode(left_value, *l->mValue, right_value, *r->mValue);
            }
            if (const auto* l = cast<const ::HIR::ExprNodeConstBlock>(&left)) {
                const auto* r = cast<const ::HIR::ExprNodeConstBlock>(&right);
                return r && equateNode(left_value, *l->inner, right_value, *r->inner);
            }
            if (const auto* l = cast<const ::HIR::ExprNodeCallPath>(&left)) {
                ...
                for (unsigned int i = 0; i < l->mArgs.size(); i++) {
                    if (!equateNode(left_value, *l->mArgs[i], right_value, *r->mArgs[i])) {
                        return false;
                    }
                }
                return true;
            ...
            if (const auto* l = cast<const ::HIR::ExprNodeBlock>(&left)) {
                ...
                for (unsigned int i = 0; i < l->nodes.size(); i++) {
                    if (!equateNode(left_value, *l->nodes[i], right_value, *r->nodes[i])) {
                        return false;
                    }
                }
                return !l->valueNode || equateNode(left_value, *l->valueNode, right_value, *r->valueNode);
            }
            return false;
        }
...
void Context::equateValues(const Span& sp, const ::HIR::ConstGeneric& rl, const ::HIR::ConstGeneric& rr) {
    ...
    if (l != r) {
        ...
        if (l.is_Infer()) {
            if (r.is_Infer()) {
                // Unify ivars
                this->ivars.ivar_val_unify(l.as_Infer().index, r.as_Infer().index);
            } else {
                this->ivars.set_ivar_val_to(l.as_Infer().index, r.clone());
            }
        } else {
            if (r.is_Infer()) {
                this->ivars.set_ivar_val_to(r.as_Infer().index, l.clone());
...
void Context::addBindingInner(const Span& sp, const ::HIR::PatternBinding& pb, ::HIR::TypeRef type) {
    assert(pb.is_valid());
    switch (pb.mType) {
        case ::HIR::PatternBinding::Type::Move:
            this->addVar(sp, pb.slot, pb.mName, mv$(type));
            break;
        case ::HIR::PatternBinding::Type::Ref:
            this->addVar(sp, pb.slot, pb.mName, crate.types.borrow(::HIR::BorrowType::Shared, type));
            break;
        case ::HIR::PatternBinding::Type::MutRef:
            this->addVar(sp, pb.slot, pb.mName, crate.types.borrow(::HIR::BorrowType::Unique, type));
            break;
    }
}
...
namespace {
    ...
    void fixupPatternValuePaths(Context& context, const Span& sp, ::HIR::Pattern::Value& val) {
        if (auto* ve = val.opt_Named()) {
            if (ve->binding && ve->path.mData.is_UfcsKnown()) {
                auto& pe = ve->path.mData.as_UfcsKnown();
                context.ivars.addIvars(pe.type);
                context.ivars.addIvarsParams(pe.trait.mParams);
                context.ivars.addIvarsParams(pe.params);
                context.addTraitBound(sp, pe.type, pe.trait.mPath, pe.trait.mParams.clone());
            } else if (ve->binding && ve->path.mData.is_UfcsInherent()) {
                auto& pe = ve->path.mData.as_UfcsInherent();
                context.ivars.addIvars(pe.type);
                context.ivars.addIvarsParams(pe.params);
                context.ivars.addIvarsParams(pe.impl_params);
            }
        }
    }

    void fixupPatternValuePaths(Context& context, const Span& sp, ::HIR::Pattern& pat) {
        TU_MATCH_HDRA( (pat.mData), {)
        TU_ARMA(Any, e) {
            }
            TU_ARMA(Box, e) {
...
void Context::handlePattern(const Span& sp, ::HIR::Pattern& pat, const ::HIR::TypeData* type, bool is_irrefutable /*=false*/) {
    ...
    // still applies.
    {
        ...
        struct MatchErgonomicsRevisit: public Revisitor {
            ...
            ::HIR::TypeRef outerTy;
            ::HIR::Pattern& pattern;
            ::HIR::PatternBinding::Type outerMode;

            mutable ::std::vector<::HIR::TypeRef> tempIvars;
            mutable ::std::optional<::HIR::TypeRef> possibleType;
            mutable const ::HIR::Pattern* possibleTypePattern = nullptr;

            MatchErgonomicsRevisit(Span sp, bool is_irrefutable, ::HIR::TypeRef outer, ::HIR::Pattern& pat, ::HIR::PatternBinding::Type bindingMode = ::HIR::PatternBinding::Type::Move)
                : sp(mv$(sp))
                , isIrrefutable(is_irrefutable)
                , outerTy(mv$(outer))
                , pattern(pat)
            ...
            bool revisit(Context& context, bool is_fallback_mode) override {
                TRACE_FUNCTION_F("Match ergonomics - " << pattern << " : " << outerTy << (is_fallback_mode ? " (fallback)" : ""));
                outerTy = context.mResolve.expandAssociatedTypes(sp, mv$(outerTy));
                return this->revisit_inner_real(context, pattern, outerTy, outerMode, is_fallback_mode);
            }
            ...
            bool revisit_inner(Context& context, ::HIR::Pattern& pattern, const ::HIR::TypeData* type, ::HIR::PatternBinding::Type bindingMode) const {
                if (!revisit_inner_real(context, pattern, type, bindingMode, false)) {
            ...
            ::std::optional<::HIR::TypeRef> getPossibleTypeVal(Context& context, ::HIR::Pattern::Value& pv) const {
                TU_MATCH_HDR( (pv), {)
                TU_ARM(pv, Integer, ve) {
                        if (ve.type == ::HIR::CoreType::Str) {
                            return context.ivars.new_ivar_tr(::HIR::InferClass::Integer);
                        }
                        return context.crate.types.primitive(ve.type);
                    }
                    TU_ARM(pv, Float, ve) {
                        if (ve.type == ::HIR::CoreType::Str) {
                            return context.ivars.new_ivar_tr(::HIR::InferClass::Float);
                        }
                        return context.crate.types.primitive(ve.type);
                    }
                    TU_ARM(pv, String, ve) {
                        return context.crate.types.borrow(::HIR::BorrowType::Shared, context.crate.types.primitive(::HIR::CoreType::Str));
                    }
                    ...
                    TU_ARM(pv, Named, ve) {
                        ...
                        if (ve.binding) {
                            if (ve.path.mData.is_UfcsKnown()) {
                                // Trait-associated constant: its type can name trait params, so
                                // map them through the (pre-populated) params from the path.
                                const auto& pe = ve.path.mData.as_UfcsKnown();
                                auto ms = MonomorphStatePtr(context.crate.types, pe.type, &pe.trait.mParams, nullptr);
                                return ms.monomorph_type(sp, ve.binding->mType);
                            }
                            return ve.binding->mType;
                        } else if (ve.path.mData.is_Generic()) {
            ...
            ::std::optional<::HIR::TypeRef> getPossibleTypeInner(Context& context, ::HIR::Pattern& pattern) const {
                ...
                TU_ARM(pattern.mData, Any, pe) {
                        // No type information.
                    }
                    TU_ARM(pattern.mData, Value, pe) {
                        possible_type = getPossibleTypeVal(context, pe.val);
                    }
                    TU_ARM(pattern.mData, Range, pe) {
                        if (pe.start) {
                            possible_type = getPossibleTypeVal(context, *pe.start);
                        }
                        if (!possible_type) {
                            if (pe.end) {
                                possible_type = getPossibleTypeVal(context, *pe.end);
                            }
                        } else {
                            // TODO: Check that the type from .end matches .start
                        }
                    }
                    TU_ARM(pattern.mData, Box, pe) {
                    ...
                    TU_ARM(pattern.mData, Tuple, e) {
                        // Get type info `(T, U, ...)`
                        if (tempIvars.size() != e.sub_patterns.size()) {
                            for (size_t i = 0; i < e.sub_patterns.size(); i++) {
                                tempIvars.push_back(context.ivars.new_ivar_tr());
                            }
                        }
                        ...
                        possible_type = context.crate.types.tuple(::std::move(tuple));
                    }
                    TU_ARM(pattern.mData, SplitTuple, pe) {
                        // Can't get type information, tuple size is unkown
                    }
                    TU_ARM(pattern.mData, Slice, e) {
                        // Can be either a [T] or [T; n]. Can't provide a hint
                        // - Can provide the hint if not behind a borrow.
                        possible_type = context.crate.types.array(context.ivars.new_ivar_tr(), e.sub_patterns.size());
                    }
                    TU_ARM(pattern.mData, SplitSlice, pe) {
                        // Can be either a [T] or [T; n]. Can't provide a hint
                    }
                    TU_ARM(pattern.mData, PathValue, e) {
                    TU_MATCH_HDRA( (e.binding), {)
                    TU_ARMA(Unbound, _)
                        BUG(sp, "");
                            TU_ARMA(Struct, be) {
                                auto& p = e.path.mData.as_Generic();
                                assert(be);
                                context.addIvarsParams(p.mParams);
                                possible_type = context.crate.types.path(p.clone(), ::HIR::TypePathBinding(be));
                            }
                            TU_ARMA(Union, be) {
                                auto& p = e.path.mData.as_Generic();
                                assert(be);
                            ...
                            TU_ARMA(Enum, be) {
                                ...
                                assert(be.ptr);
                                context.addIvarsParams(p.mParams);
                                possible_type = context.crate.types.path(getParentPath(p), ::HIR::TypePathBinding(be.ptr));
                            }
                    }
                    }
                    TU_ARM(pattern.mData, PathTuple, e) {
                    TU_MATCH_HDRA( (e.binding), {)
                    TU_ARMA(Unbound, _)
                        BUG(sp, "");
                            TU_ARMA(Struct, be) {
                    ...
                    TU_ARM(pattern.mData, PathNamed, e) {
                    TU_MATCH_HDRA( (e.binding), {)
                    TU_ARMA(Unbound, _)
                        BUG(sp, "");
                            TU_ARMA(Struct, be) {
                    ...
                    TU_ARM(pattern.mData, Or, e) {
                        for (auto& subpat : e) {
                            possible_type = getPossibleTypeInner(context, subpat);
                            if (possible_type) {
                                break;
                            }
                        }
            ...
            const ::std::optional<::HIR::TypeRef>& getPossibleType(Context& context, ::HIR::Pattern& pattern) const {
            ...
            bool revisit_inner_real(Context& context, ::HIR::Pattern& pattern, const ::HIR::TypeData* type, ::HIR::PatternBinding::Type bindingMode, bool is_fallback) const {
                ...
                for (auto& pb : pattern.mBindings) {
                    ...
                    if (pb.mType == ::HIR::PatternBinding::Type::Move && !pb.isMutable) {
                        pb.mType = bindingMode;
                    }
                    ::HIR::TypeRef tmp;
                    const ::HIR::TypeData* bindingType = nullptr;
                    switch (pb.mType) {
                        case ::HIR::PatternBinding::Type::Move:
                            bindingType = type;
                            break;
                        case ::HIR::PatternBinding::Type::MutRef:
                            ...
                            bindingType = (tmp = context.crate.types.borrow(::HIR::BorrowType::Unique, type));
                            break;
                        case ::HIR::PatternBinding::Type::Ref:
                            // NOTE: Needs to deref and borrow to get just `&mut T` (where T isn't a &mut T)
                            bindingType = (tmp = context.crate.types.borrow(::HIR::BorrowType::Shared, type));
                            break;
                        default:
                ...
                if (pattern.mData.is_Any()) {
                    return true;
                }

                if (auto* pe = pattern.mData.opt_Ref()) {
                    // Require a &-ptr (hard requirement), then visit sub-pattern
                    auto inner_ty = context.ivars.new_ivar_tr();
                    auto new_ty = context.crate.types.borrow(pe->type, inner_ty);
                    context.equateTypes(sp, type, new_ty);

                    return this->revisit_inner(context, *pe->sub, inner_ty, ::HIR::PatternBinding::Type::Move);
                }
                if (auto* pe = pattern.mData.opt_Or()) {
                    bool rv = true;
                    for (auto& subpat : *pe) {
                        rv &= this->revisit_inner(context, subpat, type, bindingMode);
                    }
                    return rv;
                }
                ...
                DEBUG("- " << n_deref << " derefs of class " << bt << " to get " << ty);
                if (ty->is_Infer() || TU_TEST1(*ty, Path, .binding.is_Unbound())) {
                    ...
                    if (const auto* te = ty->opt_Infer()) {
                        if (te->index < context.possible_ivar_vals.size() && context.possible_ivar_vals[te->index].forceDisable) {
                    ...
                    const auto& possible_type = getPossibleType(context, pattern);
                    if (possible_type) {
                        DEBUG("n_deref = " << n_deref << ", possible_type = " << *possible_type);
                        const ::HIR::TypeData* possible_type_p = *possible_type;
                        ...
                        if (possible_type_p) {
                            ...
                            if (is_fallback) {
                                ...
                                context.equateTypes(sp, ty, possible_type);
                ...
                if (ty->is_Primitive() && ty->as_Primitive() == HIR::CoreType::Str) {
                ...
                pattern.implicitDerefCount = n_deref;
                ...
                switch (bt) {
                    ...
                    case ::HIR::BorrowType::Unique:
                        switch (bindingMode) {
                            case ::HIR::PatternBinding::Type::Move:
                            case ::HIR::PatternBinding::Type::MutRef:
                                bindingMode = ::HIR::PatternBinding::Type::MutRef;
                                break;
                            case ::HIR::PatternBinding::Type::Ref:
                                // No change
                                break;
                    ...
                    case ::HIR::BorrowType::Shared:
                        bindingMode = ::HIR::PatternBinding::Type::Ref;
                        break;
                }
                ...
                TU_ARM(pattern.mData, Ref, pe) {
                    ...
                    TU_ARM(pattern.mData, Box, pe) {
                        // Box<T>
                        if (TU_TEST2(*ty, Path, .path.mData, Generic, .mPath == context.mLangBox)) {
                            const auto& path = ty->as_Path().path.mData.as_Generic();
                            const auto& inner = path.mParams.types.at(0);
                            rv = this->revisit_inner(context, *pe.sub, inner, bindingMode);
                        } else {
                    ...
                    TU_ARM(pattern.mData, Tuple, e) {
                        ...
                        for (unsigned int i = 0; i < e.sub_patterns.size(); i++) {
                            rv &= this->revisit_inner(context, e.sub_patterns[i], te[i], bindingMode);
                        }
                    }
                    TU_ARM(pattern.mData, SplitTuple, pe) {
                        if (!ty->is_Tuple()) {
                        ...
                        for (size_t i = 0; i < pe.leading.size(); i++) {
                            rv &= this->revisit_inner(context, pe.leading[i], te[i], bindingMode);
                        }
                        for (size_t i = 0; i < pe.trailing.size(); i++) {
                            rv &= this->revisit_inner(context, pe.trailing[i], te[te.size() - pe.trailing.size() + i], bindingMode);
                        }
                    }
                    TU_ARM(pattern.mData, Slice, e) {
                        const ::HIR::TypeData* slice_inner;
                        if (const auto* te = ty->opt_Slice()) {
                            slice_inner = te->inner;
                        } else if (const auto* te = ty->opt_Array()) {
                            slice_inner = te->inner;
                            // Equate the array size
                            context.equateTypes(sp, ty, context.crate.types.array(slice_inner, e.sub_patterns.size()));
                        } else {
                            ERROR(sp, E0000, "Matching a non-array/slice with a slice pattern - " << ty);
                        }
                        rv = true;
                        for (auto& sub : e.sub_patterns) {
                            rv |= this->revisit_inner(context, sub, slice_inner, bindingMode);
                        }
                    }
                    TU_ARM(pattern.mData, SplitSlice, pe) {
                        const ::HIR::TypeData* slice_inner;
                        ...
                        if (pe.extraBind.is_valid()) {
                            ::HIR::TypeRef bindingTyInner = context.crate.types.slice(slice_inner);
                            // TODO: Do arrays get bound as arrays?
                            if (ty->is_Array()) {
                                size_t size_sub = pe.leading.size() + pe.trailing.size();
                                bindingTyInner = context.crate.types.array(slice_inner, ty->as_Array().size.as_Known() - size_sub);
                                //TODO(sp, "SplitSlice extra bind with array: " << pe.extra_bind << " on " << ty);
                            }
                            ::HIR::TypeRef bindingTy;
                            if (pe.extraBind.mType == ::HIR::PatternBinding::Type::Move) {
                                pe.extraBind.mType = bindingMode;
                            }
                            switch (pe.extraBind.mType) {
                                case ::HIR::PatternBinding::Type::Move:
                                    ...
                                    break;
                                case ::HIR::PatternBinding::Type::Ref:
                                    bindingTy = context.crate.types.borrow(::HIR::BorrowType::Shared, bindingTyInner);
                                    break;
                                case ::HIR::PatternBinding::Type::MutRef:
                                    bindingTy = context.crate.types.borrow(::HIR::BorrowType::Unique, bindingTyInner);
                                    break;
                            }
                    ...
                    TU_ARM(pattern.mData, PathValue, e) {
                        auto possible_type = getPossibleTypeInner(context, pattern);
                    ...
                    TU_ARMA(Unbound, be)    throw "";
                            TU_ARMA(Struct, be) {
                                const auto& str = *be;
                                ASSERT_BUG(sp, str.mData.is_Unit(), "PathValue used on non-unit struct variant");
                            }
                            TU_ARMA(Union, be) {
                    ...
                    TU_ARM(pattern.mData, PathTuple, e) {
                        ...
                        auto ms = MonomorphStatePtr(context.crate.types, nullptr, &e.path.mData.as_Generic().mParams, nullptr);
                        ::HIR::TypeRef tmp;
                        auto maybe_monomorph = [&](const ::HIR::TypeData* fieldType) -> const ::HIR::TypeData* {
                            return (monomorphise_type_needed(fieldType) ? (tmp = context.mResolve.expandAssociatedTypes(sp, ms.monomorph_type(sp, fieldType))) : fieldType);
                        };
                        ...
                        for (unsigned int i = 0; i < e.leading.size(); i++) {
                            ...
                            rv &= this->revisit_inner(context, sub_pat, var_ty, bindingMode);
                        }
                    ...
                    TU_ARM(pattern.mData, PathNamed, e) {
                        ...
                        if (e.sub_patterns.empty()) {
                            ...
                            rv = true;
                            for (auto& fieldPat : e.sub_patterns) {
                                unsigned int fIdx = ::std::find_if(sd.begin(), sd.end(), [&](const HIR::StructField& x) {
                                    return x.name == fieldPat.first;
                                }) - sd.begin();
                                if (fIdx == sd.size()) {
                                    ERROR(sp, E0000, "Struct " << e.path << " doesn't have a field " << fieldPat.first);
                                }
                                const ::HIR::TypeData* fieldType = maybe_monomorph(sd[fIdx].ty);
                                rv &= this->revisit_inner(context, fieldPat.second, fieldType, bindingMode);
                            }
                        }
                    }
                }
            ...
            static void disablePossibilitiesOnBindings(const Span& sp, Context& context, const ::HIR::Pattern& pat, bool is_top_level = false) {
                if (!is_top_level) {
                    for (const auto& pb : pat.mBindings) {
                        context.possible_equate_type_unknown(sp, context.getVar(sp, pb.slot), Context::IvarUnknownType::Bound);
                    }
                }
                TU_MATCH_HDRA( (pat.mData), {)
                TU_ARMA(Any, e) {
                    ...
                    TU_ARMA(SplitSlice, e) {
                        ...
                        if (e.extraBind.is_valid()) {
                            context.possible_equate_type_unknown(sp, context.getVar(sp, e.extraBind.slot), Context::IvarUnknownType::Bound);
                        }
            ...
            static void createBindings(const Span& sp, Context& context, ::HIR::Pattern& pat) {
                for (const auto& pb : pat.mBindings) {
                    context.addVar(sp, pb.slot, pb.mName, context.ivars.new_ivar_tr());
        ...
        this->addRevisitAdv(box$((MatchErgonomicsRevisit{sp, is_irrefutable, type, pat})));
        return;
    }
...
void Context::handlePatternDirectInner(const Span& sp, ::HIR::Pattern& pat, const ::HIR::TypeData* type) {
    TRACE_FUNCTION_F("pat = " << pat << ", type = " << type);
    ...
    struct H {
        static void handleValue(Context& context, const Span& sp, const ::HIR::TypeData* type, ::HIR::Pattern::Value& val) {
            TU_MATCH(
                ::HIR::Pattern::Value,
                (val),
                (v),
                (Integer, DEBUG("Integer " << v.type);
                 ...
                 if (v.type != ::HIR::CoreType::Str) { context.equateTypes(sp, type, context.crate.types.primitive(v.type)); }),
                (Float, DEBUG("Float " << v.type);
                 // TODO: Apply an ivar bound? (Require that this ivar be a float?)
                 if (v.type != ::HIR::CoreType::Str) { context.equateTypes(sp, type, context.crate.types.primitive(v.type)); }),
                (String, context.equateTypes(sp, type, context.crate.types.borrow(::HIR::BorrowType::Shared, context.crate.types.primitive(::HIR::CoreType::Str)));),
                (
                    ByteString,
                ...
                (Named,
                 ...
                 if (v.binding && v.path.mData.is_UfcsKnown()) {
                     const auto& pe = v.path.mData.as_UfcsKnown();
                     auto ms = MonomorphStatePtr(context.crate.types, pe.type, &pe.trait.mParams, nullptr);
                     context.equateTypes(sp, type, ms.monomorph_type(sp, v.binding->mType));
                 })
            )
        }

        static ::HIR::TypeRef getPathType(Context& context, const Span& sp, ::HIR::Path& path, const ::HIR::Pattern::PathBinding& binding) {
            TU_MATCH_HDRA( (binding), {)
            TU_ARMA(Unbound, _)
                BUG(sp, "");
                TU_ARMA(Struct, be) {
                    auto& p = path.mData.as_Generic();
                    assert(be);
                    context.addIvarsParams(p.mParams);
                    return context.crate.types.path(p.clone(), ::HIR::TypePathBinding(be));
                }
                TU_ARMA(Union, be) {
                    auto& p = path.mData.as_Generic();
                    assert(be);
                ...
                TU_ARMA(Enum, be) {
                    ...
                    assert(be.ptr);
                    context.addIvarsParams(p.mParams);
                    return context.crate.types.path(getParentPath(p), ::HIR::TypePathBinding(be.ptr));
                }
            }
            throw "";
        }
    ...
    TU_ARMA(Any, e) {
        ...
        TU_ARMA(Box, e) {
            ...
            if (const auto* te = ty->opt_Path()) {
                if (TU_TEST1(te->path.mData, Generic, .mPath == mLangBox)) {
                    // Box<T>
                    const auto& inner = te->path.mData.as_Generic().mParams.types.at(0);
                    this->handlePatternDirectInner(sp, *e.sub, inner);
                    break;
                }
            }

            auto inner = this->ivars.new_ivar_tr();
            this->handlePatternDirectInner(sp, *e.sub, inner);
            ::HIR::GenericPath path{mLangBox, ::HIR::PathParams(mv$(inner))};
            this->equateTypes(sp, type, crate.types.path(mv$(path), ::HIR::TypePathBinding(&crate.getStructByPath(sp, mLangBox))));
        }
        TU_ARMA(Ref, e) {
            const auto& ty = this->getType(type);
        ...
        TU_ARMA(Tuple, e) {
            ...
            if (const auto* tep = ty->opt_Tuple()) {
                ...
                for (unsigned int i = 0; i < e.sub_patterns.size(); i++) {
                    this->handlePatternDirectInner(sp, e.sub_patterns[i], te[i]);
                }
            } else {
                ::std::vector<::HIR::TypeRef> sub_types;
                for (unsigned int i = 0; i < e.sub_patterns.size(); i++) {
                    sub_types.push_back(this->ivars.new_ivar_tr());
                    this->handlePatternDirectInner(sp, e.sub_patterns[i], sub_types[i]);
                }
                this->equateTypes(sp, ty, crate.types.tuple(mv$(sub_types)));
            }
        }
        TU_ARMA(SplitTuple, e) {
            const auto& ty = this->getType(type);
            if (const auto* tep = ty->opt_Tuple()) {
                ...
                for (auto& subpat : e.leading) {
                    this->handlePatternDirectInner(sp, subpat, te[tup_idx++]);
                }
                ...
                for (auto& subpat : e.leading) {
                    leading_tys.push_back(this->ivars.new_ivar_tr());
                    this->handlePatternDirectInner(sp, subpat, leading_tys.back());
                }
                ::std::vector<::HIR::TypeRef> trailing_tys;
                for (auto& subpat : e.trailing) {
                    trailing_tys.push_back(this->ivars.new_ivar_tr());
                    this->handlePatternDirectInner(sp, subpat, trailing_tys.back());
                }

                struct SplitTuplePatRevisit: public Revisitor {
                    Span sp;
                    ::HIR::TypeRef outerTy;
                    ::std::vector<::HIR::TypeRef> leadingTys;
                    ::std::vector<::HIR::TypeRef> trailingTys;
                ...
                this->addRevisitAdv(box$((SplitTuplePatRevisit{sp, ty, mv$(leading_tys), mv$(trailing_tys), e.total_size})));
            }
        }
        TU_ARMA(Slice, e) {
            const auto& ty = this->getType(type);
        ...
        default:
            ERROR(sp, E0000, "Slice pattern on non-array/-slice - " << ty);
                ...
                TU_ARMA(Infer, te) {
                    ...
                    this->addRevisitAdv(box$((SlicePatRevisit{sp, mv$(inner), ty, static_cast<unsigned int>(e.sub_patterns.size())})));
                }
        }
        }
        TU_ARMA(SplitSlice, e) {
        ...
        default:
            ERROR(sp, E0000, "SplitSlice pattern on non-array/-slice - " << ty);
                ...
                TU_ARMA(Infer, te) {
                    inner = this->ivars.new_ivar_tr();
                    ::HIR::TypeRef var_ty;
                    if (e.extraBind.is_valid()) {
                        var_ty = this->ivars.new_ivar_tr();
                        this->addBindingInner(sp, e.extraBind, var_ty);
                    }
                        ...
                        ERROR(sp, E0000, "Slice pattern on non-array/-slice - " << ty);
                                ...
                                TU_ARMA(Slice, te) {
                                    ...
                                    if (this->var_ty != ::HIR::TypeRef()) {
                                        context.equateTypes(this->sp, this->var_ty, ty);
                                    }
                                }
                                TU_ARMA(Array, te) {
                                    ...
                                    if (this->var_ty != ::HIR::TypeRef()) {
                                        context.equateTypes(this->sp, this->var_ty, context.crate.types.array(this->inner, extra_len));
                                    }
                                }
                    }
                    return true;
                    ...
                    // Callback
                    this->addRevisitAdv(box$((SplitSlicePatRevisit{sp, inner, ty, mv$(var_ty), min_len})));
                }
        }
        ...
        TU_ARMA(Unbound, _)
            BUG(sp, "");
                TU_ARMA(Struct, str) {
                    assert(str->mData.is_Unit());
                }
                TU_ARMA(Union, unn) {
                    BUG(sp, "PathValue used for union");
                }
                TU_ARMA(Enum, be) {
                    if (const auto* ee = be.ptr->mData.opt_Data()) {
                        ASSERT_BUG(sp, be.var_idx < ee->size(), "");
                        const auto& var = (*ee)[be.var_idx];
        ...
        TU_ARMA(PathTuple, e) {
            ...
            auto ms = MonomorphStatePtr(crate.types, nullptr, &e.path.mData.as_Generic().mParams, nullptr);
            ::HIR::TypeRef tmp;
            ...
            for (size_t i = 0; i < e.leading.size(); i++) {
                /*const*/ auto& sub_pat = e.leading[i];
                this->handlePatternDirectInner(sp, sub_pat, maybe_monomorph(sd[i].ent));
            }
            for (size_t i = 0; i < e.trailing.size(); i++) {
                /*const*/ auto& sub_pat = e.trailing[i];
                this->handlePatternDirectInner(sp, sub_pat, maybe_monomorph(sd[sd.size() - e.trailing.size() + i].ent));
            }
        }
        TU_ARMA(PathNamed, e) {
            this->equateTypes(sp, type, H::getPathType(*this, sp, e.path, e.binding));
            ...
            for (auto& fieldPat : e.sub_patterns) {
                ...
                const ::HIR::TypeData* fieldType = sd[fIdx].ty;
                if (monomorphise_type_needed(fieldType)) {
                    auto fieldTypeMono = ms.monomorph_type(sp, fieldType);
...
void Context::record_coercion_hint(const ::HIR::TypeData* type, ::HIR::ExprNodeP& node_ptr) {
    auto* hintNode = node_ptr.get();
    // A block's tail expression is checked with the block's expectation.
    while (const auto* block = cast<const ::HIR::ExprNodeBlock>(hintNode)) {
        if (!block->valueNode) {
            break;
        }
...
void Context::equateTypesCoerce(const Span& sp, const ::HIR::TypeData* l, ::HIR::ExprNodeP& node_ptr) {
    ...
    this->ivars.mark_change();
}

void Context::possible_equate_type_unknown(const Span& sp, const ::HIR::TypeData* ty, Context::IvarUnknownType src) {
    TU_MATCH_HDRA( (*this->getType(ty)), {)
    default:
    ...
    TU_ARMA(Path, e) {
        ...
        default:
            // TODO: Ufcs?
        TU_ARMA(Generic, pe) {
                    for (const auto& sty : pe.mParams.types) {
        ...
        TU_ARMA(NodeType, e) {
            if (e.is_Closure()) {
                ...
                for (const auto& aty : node_p->mArgs) {
                    this->possible_equate_type_unknown(sp, aty.second, src);
                }
                this->possible_equate_type_unknown(sp, node_p->returnType, src);
            }
        }
        TU_ARMA(Infer, e) {
...
void Context::equateTypesAssoc(const Span& sp, const ::HIR::TypeData* l, const ::HIR::SimplePath& trait, ::HIR::PathParams pp, const ::HIR::TypeData* impl_ty, const char* name, const ::HIR::PathParams& atyPp, bool is_op, typeck::PrimitiveOperator operator_kind) {
    ...
    if (impl_ty->is_Path() && impl_ty->as_Path().path.mData.is_UfcsKnown()) {
        auto& i = impl_ty->as_Path().path.mData.as_UfcsKnown();
    ...
    this->link_assoc.push_back(
        Associated{
            this->next_rule_idx++,
            sp,
            MonomorphEraseHrls(crate.types).monomorph_type(sp, l, true),

            trait.clone(),
            MonomorphEraseHrls(crate.types).monomorph_path_params(sp, pp, true),
            MonomorphEraseHrls(crate.types).monomorph_type(sp, impl_ty, true),
            name,
            MonomorphEraseHrls(crate.types).monomorph_path_params(sp, atyPp, true),
            is_op,
            operator_kind
        }
    );
...
void Context::addRevisit(::HIR::ExprNode& node) {
...
void Context::require_sized(const Span& sp, const ::HIR::TypeData* ty_) {
    const auto& ty = ivars.getType(ty_);
    TRACE_FUNCTION_F(ty_ << " -> " << ty);
    if (mResolve.type_is_sized(sp, ty) == ::HIR::Compare::Unequal) {
    ...
    if (const auto* e = ty->opt_Infer()) {
        ...
        TU_MATCHA(
            ...
            (Opaque,
             ...
             params_def = nullptr;),
            (ExternType, static ::HIR::GenericParams emptyParams; params_def = &emptyParams;),
            (Enum, params_def = &pb->mParams;),
            (Union, params_def = &pb->mParams;),
            (Struct, params_def = &pb->mParams;

             if (pb->structMarkings.dst_type == ::HIR::StructMarkings::DstType::Possible) {
                 // Check sized-ness of the unsized param
                 this->require_sized(sp, e->path.mData.as_Generic().mParams.types.at(pb->structMarkings.unsized_param));
             })
        )

        if (params_def) {
            const auto& gpTys = e->path.mData.as_Generic().mParams.types;
            for (size_t i = 0; i < gpTys.size(); i++) {
                if (params_def->types.at(i).isSized) {
                    this->require_sized(sp, gpTys[i]);
                }
            }
        }
...
Context::IVarPossible* Context::getIvarPossibilities(const Span& sp, unsigned int ivar_index) {
    {
        const auto& real_ty = ivars.getType(ivar_index);
    ...
    return getPossibleIvarSink(ivar_index);
}
...
void Context::possible_equate_ivar(const Span& sp, unsigned int ivar_index, const ::HIR::TypeData* raw_t, PossibleTypeSource src) {
    const auto& t = this->ivars.getType(raw_t);
    DEBUG(ivar_index << " " << src << " " << raw_t << " " << t);
    auto* entp = getIvarPossibilities(sp, ivar_index);
    if (!entp) {
        return;
    }
    auto& ent = *entp;
...
void Context::possible_equate_ivar_bounds(const Span& sp, unsigned int ivar_index, std::vector<::HIR::TypeRef> types) {
    ...
    for (auto it = types.begin(); it != types.end();) {
        auto& e = *it;
        ASSERT_BUG(sp, !type_contains_impl_placeholder(crate.types, e), "Type contained an impl placeholder parameter - " << e);
        e = ivars.getType(e);
    ...
    if (ent.hasBounded) {
        ...
        DEBUG(ivar_index << " bounded as [" << ent.bounded << "], union from [" << types << "] has_self=" << ent.boundsIncludeSelf);
    } else {
        ent.hasBounded = true;
        ent.boundsIncludeSelf = hasSelf;
        ent.bounded = std::move(types);
        DEBUG(ivar_index << " bounded as [" << ent.bounded << "] has_self=" << hasSelf);
    }
}
...
void Context::possible_equate_ivar_unknown(const Span& sp, unsigned int ivar_index, IvarUnknownType src) {
    ...
    ASSERT_BUG(sp, ivars.getType(ivar_index)->is_Infer(), "possible_equate_ivar_unknown on known ivar");

    auto& ent = *getPossibleIvarSink(ivar_index);
    switch (src) {
...
void Context::addVar(const Span& sp, unsigned int index, const RcString& name, ::HIR::TypeRef type) {
    ...
    if (mBindings[index].name == "") {
        ...
        ASSERT_BUG(sp, mBindings[index].name == name, "");
        this->equateTypes(sp, mBindings[index].ty, type);
    }
}

const ::HIR::TypeData* Context::getVar(const Span& sp, unsigned int idx) const {
    if (idx < this->mBindings.size()) {
        ASSERT_BUG(sp, this->mBindings[idx].ty != HIR::TypeRef(), "Local #" << idx << " `" << this->mBindings[idx].name << "` with no populated type");
        return this->mBindings[idx].ty;
    } else {
        BUG(sp, "get_var - Binding index out of range - " << idx << " >=" << this->mBindings.size());
    }
}

::HIR::ExprNodeP Context::createAutoderef(::HIR::ExprNodeP val_node, ::HIR::TypeRef ty_dst) const {
    const auto& span = val_node->span();
    const auto& ty_src = val_node->resType;
    ...
    if (getType(ty_src)->is_Array()) {
        ASSERT_BUG(span, ty_dst->is_Slice(), "Array should only ever autoderef to Slice");

        const auto borrowType = ::HIR::BorrowType::Shared;
        auto ty_src_borrow = crate.types.borrow(borrowType, ty_src);
        auto ty_dst_borrow = crate.types.borrow(borrowType, ty_dst);
        auto ty_dst_borrow_copy = ty_dst_borrow;

        val_node = mk_exprnodep(crate.pool->make<HIR::ExprNodeBorrow>(span, borrowType, mv$(val_node)), mv$(ty_src_borrow));
        auto* unsize_node = crate.pool->make<HIR::ExprNodeUnsize>(span, mv$(val_node), mv$(ty_dst_borrow_copy));
        unsize_node->isArrayToSliceAdjustment = true;
        val_node = mk_exprnodep(unsize_node, mv$(ty_dst_borrow));
        val_node = mk_exprnodep(crate.pool->make<HIR::ExprNodeDeref>(span, mv$(val_node)), ty_dst);
        DEBUG("- Array-to-slice adjustment " << &*val_node << " -> " << val_node->resType);
    } else {
        val_node = mk_exprnodep(crate.pool->make<HIR::ExprNodeDeref>(span, mv$(val_node)), mv$(ty_dst));
        DEBUG("- Deref " << &*val_node << " -> " << val_node->resType);
    }
...
namespace {
    void addCoerceBorrow(Context& context, ::HIR::ExprNodeP& orig_node_ptr, const ::HIR::TypeData* desBorrowInner, ::std::function<void(::HIR::ExprNodeP& n)> cb) {
        auto borrowType = context.ivars.getType(orig_node_ptr->resType)->as_Borrow().type;
        ...
        while (auto* p = cast<::HIR::ExprNodeBlock>(&**node_ptr_ptr)) {
            DEBUG("- Moving into block");
            assert(p->valueNode);
            // Block result and the inner node's result must be the same type
            ASSERT_BUG(p->span(), context.ivars.types_equal(p->resType, p->valueNode->resType), "Block and result mismatch - " << context.ivars.fmtType(p->resType) << " != " << context.ivars.fmtType(p->valueNode->resType));
            // - Override the the result type to the desired result
            p->resType = context.crate.types.borrow(borrowType, desBorrowInner);
            node_ptr_ptr = &p->valueNode;
        }
#endif
        auto& node_ptr = *node_ptr_ptr;
        const auto& src_type = context.ivars.getType(node_ptr->resType);

        // - If the pointed node is a borrow operation, add the dereferences within its value
        if (auto* p = cast<::HIR::ExprNodeBorrow>(&*node_ptr)) {
            // Set the result of the borrow operation to the output type
            node_ptr->resType = context.crate.types.borrow(borrowType, desBorrowInner);

            node_ptr_ptr = &p->mValue;
        }
        // - Otherwise, create a new borrow operation behind which the dereferences happen
            ...
            auto inner_ty_ref = context.crate.types.borrow(borrowType, desBorrowInner);

            // 1. Dereference (resulting in the dereferenced input type)
            node_ptr = NEWNODE(src_inner_ty, span, Deref, mv$(node_ptr));
            DEBUG("- Deref " << &*node_ptr << " -> " << node_ptr->resType);
            // 2. Borrow (resulting in the referenced output type)
            auto* borrowNode = context.crate.pool->make<HIR::ExprNodeBorrow>(span, borrowType, mv$(node_ptr));
            node_ptr = mk_exprnodep(borrowNode, mv$(inner_ty_ref));
            DEBUG("- Borrow " << &*node_ptr << " -> " << node_ptr->resType);

            // - Set node pointer reference to point into the new borrow op
            node_ptr_ptr = &borrowNode->mValue;
        }

        cb(*node_ptr_ptr);

        context.ivars.mark_change();
    }
    ...
    CoerceResult checkUnsizeTys(const Context& context, const Span& sp, const ::HIR::TypeData* dstRaw, const ::HIR::TypeData* src_raw, Context* contextMut, ::HIR::ExprNodeP* node_ptr_ptr = nullptr) {
        const auto& dst = context.ivars.getType(dstRaw);
        const auto& src = context.ivars.getType(src_raw);
        TRACE_FUNCTION_F("dst=" << dst << ", src=" << src);

        // If the types are already equal, no operation is required
        if (context.ivars.types_equal(dst, src)) {
            DEBUG("Equal");
        ...
        if (dst->is_Infer() && src->is_Infer()) {
            ...
            if (contextMut) {
                contextMut->possible_equate_ivar(sp, src->as_Infer().index, dst, Context::PossibleTypeSource::UnsizeTo);
                contextMut->possible_equate_ivar(sp, dst->as_Infer().index, src, Context::PossibleTypeSource::UnsizeFrom);
            }
            DEBUG("Both ivars");
            ...
            if (contextMut) {
                contextMut->possible_equate_ivar(sp, dep->index, src, Context::PossibleTypeSource::UnsizeFrom);
                // Disable inner parts of the source? (E.g. if it's a closure)
                if (src->is_NodeType() && src->as_NodeType().is_Closure()) {
                    contextMut->possible_equate_type_unknown(sp, src, Context::IvarUnknownType::To);
                }
            }
            DEBUG("Dst ivar");
            return CoerceResult::Unknown;
            ...
            if (sep->is_lit()) {
                ...
                if (contextMut) {
                    contextMut->possible_equate_ivar(sp, sep->index, dst, Context::PossibleTypeSource::UnsizeTo);
                }
        ...
        if (node_ptr_ptr) {
            ...
            ::HIR::TypeRef tmp_ty;
            const ::HIR::TypeData* out_ty_p = src;
            unsigned int count = 0;
            ::std::vector<::HIR::TypeRef> types;
            while ((out_ty_p = context.mResolve.autoderef(sp, out_ty_p, tmp_ty))) {
                const auto& out_ty = context.ivars.getType(out_ty_p);
                DEBUG("From? " << out_ty);
                count += 1;
                ...
                if (const auto* sep = out_ty->opt_Infer()) {
                    if (!sep->is_lit()) {
                        ...
                        if (contextMut) {
                            ...
                            ::HIR::TypeRef tmp_ty2;
                            const ::HIR::TypeData* dTyP = dst;
                            //context_mut->possible_equate_ivar(sp, sep->index, dst, Context::PossibleTypeSource::UnsizeTo);
                            for (unsigned int i = 0; (dTyP = context.mResolve.autoderef(sp, dTyP, tmp_ty2)) && i < count - 1; i++) {
                            }
                            if (dTyP) {
                                // TODO: This should be a `DerefTo` (can't do other unsizings?)
                                contextMut->possible_equate_ivar(sp, sep->index, dTyP, Context::PossibleTypeSource::UnsizeTo);
                            } else {
                ...
                // Types aren't equal
                if (context.ivars.types_equal(dst, out_ty) == false) {
                    ...
                    if (dst->is_Slice()) {
                        if (contextMut) {
                            contextMut->equateTypes(sp, dst, out_ty);
                        }
                    } else if (dst->is_Borrow()) {
                        DEBUG("Borrow, continue");
                        continue;
                    } else {
                        if (dst->compareWithPlaceholders(sp, out_ty, context.ivars.callbackResolveInfer()) == ::HIR::Compare::Unequal) {
                ...
                if (contextMut && node_ptr_ptr) {
                    auto& node_ptr = *node_ptr_ptr;
                    addCoerceBorrow(*contextMut, node_ptr, types.back(), [&](auto& node_ptr) -> void {
                        ...
                        for (unsigned int i = 0; i < types.size(); i++) {
                            ...
                            ASSERT_BUG(span, !node_ptr->resType->is_Array(), "Array->Slice shouldn't be in deref coercions");
                            auto ty = mv$(types[i]);
                            node_ptr = ::HIR::ExprNodeP(context.crate.pool->make<::HIR::ExprNodeDeref>(mv$(span), mv$(node_ptr)));
                            DEBUG("- Deref " << &*node_ptr << " -> " << ty);
                            node_ptr->resType = mv$(ty);
                            context.ivars.getType(node_ptr->resType);
                        }
                    });
                }
        ...
        if (const auto* dep = dst->opt_TraitObject()) {
            if (const auto* sep = src->opt_TraitObject()) {
                ...
                if (dep->mTrait.mPath.mPath != sep->mTrait.mPath.mPath) {
// Trait mismatch!
                ...
                const auto& tys_d = dep->mTrait.mPath.mParams.types;
                const auto& tys_s = sep->mTrait.mPath.mParams.types;
                if (contextMut) {
                    for (size_t i = 0; i < tys_d.size(); i++) {
                        contextMut->equateTypes(sp, tys_d[i], tys_s.at(i));
                    }
                }
                ...
                const auto& trait = dep->mTrait.mPath;

                // Check for trait impl
                if (trait.mPath != ::HIR::SimplePath()) {
                    // Just call equate_types_assoc to add the required bounds.
                    if (contextMut) {
                        auto pp = dep->mTrait.hrtbs ? dep->mTrait.hrtbs->make_empty_params(true) : HIR::PathParams();
                        MonomorphHrlsOnly ms(context.crate.types, pp);
                        for (const auto& tyb : dep->mTrait.typeBounds) {
                            contextMut->equateTypesAssoc(sp, tyb.second.type, trait.mPath, ms.monomorph_path_params(sp, trait.mParams, true), src, tyb.first.c_str(), tyb.second.atyParams, false);
                        }
                        if (dep->mTrait.typeBounds.empty()) {
                            contextMut->addTraitBound(sp, src, trait.mPath, ms.monomorph_path_params(sp, trait.mParams, true));
                        }
                    } else {
                        // Check that the trait is implemented (so this only returns `Unsize` if the rule would be valid - use for check_ivar_poss)
                        if (!context.mResolve.findTraitImpls(sp, trait.mPath, trait.mParams, src, [&](auto _impl_ref, auto _cmp) {
                            return true;
                        })) {
                            return CoerceResult::Equality;
                        }
                ...
                if (contextMut) {
                    for (const auto& marker : dep->markers) {
                        contextMut->addTraitBound(sp, src, marker.mPath, marker.mParams.clone());
                    }
                } else {
        ...
        // Find an Unsize impl?
        struct H {
            static bool type_is_bounded(const ::HIR::TypeData* ty) {
                if (ty->is_Generic()) {
                    return true;
        ...
        if (H::type_is_bounded(src)) {
            ...
            bool found = context.mResolve.findTraitImpls(sp, context.mResolve.mLangUnsize, pp, src, [&bestImpl, &count, &context, &sp](auto impl, auto cmp) {
                ...
                if (!context.mResolve.impls_overlap(sp, impl, bestImpl)) {
                    // No overlap, count it as a new possibility
                    if (count == 0) {
                        bestImpl = mv$(impl);
                    }
                    count++;
                } else if (impl.more_specific_than(context.crate.types, bestImpl)) {
                    bestImpl = mv$(impl);
                } else {
                    // Less specific
                }
                        ...
                        TU_MATCHA(
                            ...
                            (Struct, if (sbe == dbe) {
                                ...
                                if (sm.dst_type == ::HIR::StructMarkings::DstType::Possible) {
                                    DEBUG("Possible DST");
                                    const auto& p_src = se.path.mData.as_Generic().mParams;
                                    const auto& p_dst = de.path.mData.as_Generic().mParams;
                                    const auto& isrc = p_src.types.at(sm.unsized_param);
                                    const auto& idst = p_dst.types.at(sm.unsized_param);
                                    ...
                                    switch (rv) {
                                        ...
                                        default:
                                            if (contextMut) {
                                                for (size_t i = 0; i < p_src.types.size(); i++) {
                                                    if (i != sm.unsized_param) {
                                                        contextMut->equateTypes(sp, p_dst.types.at(i), p_src.types[i]);
                                                    }
                                                }
                                                for (size_t i = 0; i < p_src.values.size(); i++) {
                                                    if (i != sm.unsized_param) {
                                                        contextMut->equateValues(sp, p_dst.values.at(i), p_src.values[i]);
                                                    }
                                                }
                                            }
                                            break;
    ...
    CoerceResult checkCoerceTys(const Context& context, const Span& sp, const ::HIR::TypeData* dst, const ::HIR::TypeData* src_r, Context* contextMut = nullptr, ::HIR::ExprNodeP* node_ptr_ptr = nullptr) {
        auto src = src_r;
        ...
        if (dst->is_Infer() && src->is_Infer()) {
            ...
            if (contextMut) {
                contextMut->possible_equate_ivar(sp, src->as_Infer().index, dst, Context::PossibleTypeSource::CoerceTo);
                contextMut->possible_equate_ivar(sp, dst->as_Infer().index, src, Context::PossibleTypeSource::CoerceFrom);
            }
            return CoerceResult::Unknown;
        }
        ...
        struct H {
            ...
            static ::HIR::TypeRef make_pruned(Context& context, const ::HIR::TypeData* ty) {
                const auto& binding = ty->as_Path().binding;
                const auto& sm = binding.as_Struct()->structMarkings;
                ::HIR::GenericPath gp = ty->as_Path().path.mData.as_Generic().clone();
                assert(sm.coerceParam != ~0u);
                gp.mParams.types.at(sm.coerceParam) = context.ivars.new_ivar_tr();
        ...
        if (H::type_is_bounded(src) || H::type_is_bounded(dst)) {
            ...
            if (!langCoerceUnsized.components().empty()) {
                ...
                ImplRef bestImpl;
                bool found = context.mResolve.findTraitImpls(sp, langCoerceUnsized, pp, src, [&](auto impl, auto cmp) -> bool {
                    ...
                    if (cmp == ::HIR::Compare::Fuzzy) {
                        fuzzyMatch = true;
                        if (impl.more_specific_than(context.crate.types, bestImpl)) {
                            bestImpl = mv$(impl);
                        } else {
        ...
        if (src->is_Infer() && TU_TEST2(*dst, Path, .binding, Struct, ->structMarkings.coerceUnsized != ::HIR::StructMarkings::Coerce::None)) {
            if (contextMut) {
                contextMut->possible_equate_ivar(sp, src->as_Infer().index, dst, Context::PossibleTypeSource::CoerceTo);
            }
        ...
        if (dst->is_Infer() && TU_TEST2(*src, Path, .binding, Struct, ->structMarkings.coerceUnsized != ::HIR::StructMarkings::Coerce::None)) {
            if (contextMut) {
                contextMut->possible_equate_ivar(sp, dst->as_Infer().index, src, Context::PossibleTypeSource::CoerceFrom);
            }
            // TODO: Avoid needless loop return
        ...
        if (TU_TEST1(*dst, Path, .binding.is_Struct()) && TU_TEST1(*src, Path, .binding.is_Struct())) {
            ...
            // Has to be equal?
            if (sm.coerceUnsized == ::HIR::StructMarkings::Coerce::None) {
                return CoerceResult::Equality;
            }

            // Equate all parameters that aren't the unsizing param
            const auto& pp_dst = dst->as_Path().path.mData.as_Generic().mParams;
            const auto& pp_src = src->as_Path().path.mData.as_Generic().mParams;
            DEBUG(pp_dst << " = " << pp_src);
            assert(pp_dst.types.size() == pp_src.types.size());
            for (size_t i = 0; i < pp_src.types.size(); i++) {
                if (i == sm.coerceParam) {
                    continue;
                }
                if (contextMut) {
                    contextMut->equateTypes(sp, pp_dst.types.at(i), pp_src.types.at(i));
                }
            }
            assert(pp_dst.values.size() == pp_src.values.size());
            for (size_t i = 0; i < pp_src.values.size(); i++) {
                TODO(sp, "Handle values in CoerceUnsized");
            }
            // Check coercion/unsizing of the target type
            const auto& idst = pp_dst.types.at(sm.coerceParam);
            const auto& isrc = pp_src.types.at(sm.coerceParam);
        ...
        if (dstIsUnboundPath || src_is_unbound_path) {
            if (contextMut) {
                if (const auto* dep = dst->opt_Infer(); dep && src_is_unbound_path) {
                    contextMut->possible_equate_ivar(sp, dep->index, src, Context::PossibleTypeSource::CoerceFrom);
                }
                if (const auto* sep = src->opt_Infer(); sep && dstIsUnboundPath) {
                    contextMut->possible_equate_ivar(sp, sep->index, dst, Context::PossibleTypeSource::CoerceTo);
                }
            }
            return CoerceResult::Unknown;
        }
        ...
        if (const auto* sep = src->opt_Infer()) {
            ...
            if (contextMut) {
                contextMut->possible_equate_type_unknown(sp, dst, Context::IvarUnknownType::From);
            }

            // If the other side is a pointer
            if (dst->is_Pointer() || dst->is_Borrow()) {
                if (contextMut) {
                    contextMut->possible_equate_ivar(sp, se.index, dst, Context::PossibleTypeSource::CoerceTo);
                }
                return CoerceResult::Unknown;
            }
            ...
            // - Unsure why, but if the below is unconditional then there's a type error with `Result<!,...>` and `Try`
                ...
                if (contextMut) {
                    contextMut->possible_equate_ivar(sp, se.index, dst, Context::PossibleTypeSource::UnsizeFrom);
                }
                return CoerceResult::Unknown;
            }
            ...
            if (const auto* dep = dst->opt_Infer()) {
                ...
                if (dep->type < se.type) {
                    if (node_ptr_ptr) {
                        ...
                        auto new_type = context.crate.types.pointer(dep->type, se.inner);
                        ...
                        while (auto* p = cast<::HIR::ExprNodeBlock>(npp->get())) {
                            DEBUG("- Propagate to the last node of a _Block");
                            ASSERT_BUG(p->span(), context.ivars.types_equal(p->resType, p->valueNode->resType), "Block and result mismatch - " << context.ivars.fmtType(p->resType) << " != " << context.ivars.fmtType(p->valueNode->resType));
                            if (!context.ivars.types_equal(p->resType, src)) {
                        ...
                        if (contextMut) {
                            ...
                            node_ptr = NEWNODE(new_type, span, Cast, mv$(node_ptr), new_type);
                            context.ivars.getType(node_ptr->resType);

                            contextMut->ivars.mark_change();
                        }

                        // Continue on with coercion (now that node_ptr is updated)
                        switch (checkUnsizeTys(context, sp, dep->inner, se.inner, contextMut, &node_ptr)) {
                            case CoerceResult::Fail:
                                return CoerceResult::Fail;
                            case CoerceResult::Unknown:
                                // Add new coercion at the new inner point
                                if (&node_ptr != node_ptr_ptr) {
                                    DEBUG("Unknown check_unsize_tys after autoderef - " << dst << " := " << node_ptr->resType);
                                    if (contextMut) {
                                        contextMut->equateTypesCoerce(sp, dst, node_ptr);
                                    }
                                    return CoerceResult::Custom;
                                } else {
                                    return CoerceResult::Unknown;
                            ...
                            case CoerceResult::Unsize:
                                if (contextMut) {
                                    ...
                                    auto span = node_ptr->span();
                                    node_ptr = NEWNODE(dst, span, Unsize, mv$(node_ptr), dst);
                                }
                                return CoerceResult::Custom;
                        }
                        throw "";
                ...
                // Call unsizing code
                return checkUnsizeTys(context, sp, dep->inner, se.inner, contextMut, node_ptr_ptr);
            } else {
            ...
            if (const auto* dep = dst->opt_Infer()) {
                ...
                // Add downcast
                switch (checkUnsizeTys(context, sp, dep->inner, se.inner, contextMut, node_ptr_ptr)) {
                    case CoerceResult::Fail:
                        return CoerceResult::Fail;
                    case CoerceResult::Unknown:
                        return CoerceResult::Unknown;
                    case CoerceResult::Custom:
                        return CoerceResult::Custom;
                    case CoerceResult::Equality:
                        if (node_ptr_ptr && contextMut) {
                            auto& node_ptr = *node_ptr_ptr;
                            {
                                DEBUG("- NEWNODE _Cast " << &*node_ptr << " -> " << dst);
                                auto span = node_ptr->span();
                                node_ptr = ::HIR::ExprNodeP(context.crate.pool->make<::HIR::ExprNodeCast>(mv$(span), mv$(node_ptr), dst));
                                node_ptr->resType = dst;
                            }
                        }
                    ...
                    case CoerceResult::Unsize:
                        if (node_ptr_ptr && contextMut) {
                            ...
                            DEBUG("- NEWNODE _Unsize " << &*node_ptr << " -> " << dstB);
                            {
                                auto span = node_ptr->span();
                                node_ptr = NEWNODE(dstB, span, Unsize, mv$(node_ptr), dstB);
                            }
                ...
                if (dep->type < se.type) {
                    if (node_ptr_ptr) {
                        ...
                        auto dstBt = dep->type;
                        auto new_type = context.crate.types.borrow(dstBt, inner_ty);
                        ...
                        while (auto* p = cast<::HIR::ExprNodeBlock>(npp->get())) {
                            ...
                            ASSERT_BUG(p->span(), context.ivars.types_equal(p->resType, p->valueNode->resType), "(borrow) Block and result mismatch - " << context.ivars.fmtType(p->resType) << " != " << context.ivars.fmtType(p->valueNode->resType));
                            ASSERT_BUG(p->span(), context.ivars.types_equal(p->resType, src), "(borrow) Block and result mismatch - " << context.ivars.fmtType(p->resType) << " != " << context.ivars.fmtType(src));
                            if (contextMut) {
                                p->resType = dst;
                            }
                            npp = &p->valueNode;
                        ...
                        if (contextMut) {
                            ...
                            node_ptr = NEWNODE(inner_ty, span, Deref, mv$(node_ptr));
                            context.ivars.getType(node_ptr->resType);
                            // &*<inner>
                            DEBUG("- Borrow -> " << new_type);
                            node_ptr = NEWNODE(mv$(new_type), span, Borrow, dstBt, mv$(node_ptr));
                            context.ivars.getType(node_ptr->resType);
            ...
            if (dst->is_ErasedType()) {
                ...
                ::std::vector<::HIR::TypeRef> closureArgs;
                closureArgs.reserve(node_p->mArgs.size());
                for (const auto& arg : node_p->mArgs) {
                    closureArgs.push_back(arg.second);
                }
                ::HIR::PathParams desiredParams{
                    context.crate.types.tuple(mv$(closureArgs))
                };

                ::HIR::PathParams expectedParams;
                ::HIR::TypeRef expectedOutput;
                const bool foundExpectation = context.mResolve.findTraitImpls(
                    sp,
                    context.mResolve.mLangFnOnce,
                    desiredParams,
                    dst,
                    [&](ImplRef impl, ::HIR::Compare) {
                        ...
                        if (args.size() != node_p->mArgs.size()) {
                            return false;
                        }
                ...
                if (foundExpectation && contextMut) {
                    ...
                    for (size_t i = 0; i < expectedArgs.size(); i++) {
                        contextMut->equateTypes(sp, node_p->mArgs[i].second, expectedArgs[i]);
                    }
                    contextMut->equateTypes(sp, node_p->returnType, expectedOutput);
                }
                return CoerceResult::Equality;
                ...
                if (node_ptr_ptr) {
                    ...
                    if (de.argTypes.size() != node_p->mArgs.size()) {
                    ...
                    if (contextMut) {
                        ...
                        for (size_t i = 0; i < de.argTypes.size(); i++) {
                            contextMut->equateTypes(sp, ms.monomorph_type(sp, de.argTypes[i]), node_p->mArgs[i].second);
                        }
                        contextMut->equateTypes(sp, ms.monomorph_type(sp, de.mRettype), node_p->returnType);
                        node_ptr = NEWNODE(dst, span, Cast, mv$(node_ptr), dst);
                    }
                }
                return CoerceResult::Custom;
            } else if (const auto* dep = dst->opt_Infer()) {
                if (contextMut) {
                    ...
                    for (const auto& at : node_p->mArgs) {
                        contextMut->possible_equate_type_unknown(sp, at.second, Context::IvarUnknownType::To);
                    }
                    contextMut->possible_equate_type_unknown(sp, node_p->returnType, Context::IvarUnknownType::Bound);
                    // Add as a possiblity
                    contextMut->possible_equate_ivar(sp, dep->index, src, Context::PossibleTypeSource::CoerceFrom);
                }
                return CoerceResult::Unknown;
            ...
            if (const auto* de = dst->opt_Function()) {
                auto ft = context.mResolve.expandAssociatedTypes(sp, context.crate.types.function(se->decay(context.crate.types, sp)));
                ...
                if (contextMut) {
                    ...
                    MonomorphHrlsOnly s_ms(context.crate.types, s_pp);
                    auto dPp = de->hrls.make_empty_params(true);
                    MonomorphHrlsOnly dMs(context.crate.types, dPp);
                    for (size_t i = 0; i < de->argTypes.size(); i++) {
                        contextMut->equateTypes(sp, dMs.monomorph_type(span, de->argTypes[i]), s_ms.monomorph_type(span, se->argTypes[i]));
                    }
                    contextMut->equateTypes(sp, dMs.monomorph_type(span, de->mRettype), s_ms.monomorph_type(span, se->mRettype));
                    node_ptr = NEWNODE(dst, span, Cast, mv$(node_ptr), dst);
                }
                return CoerceResult::Custom;
            }
    ...
    bool checkCoerce(Context& context, const Context::Coercion& v) {
        ...
        const auto& ty_dst = context.ivars.getType(v.left_ty);
        const auto& ty_src = context.ivars.getType(node_ptr->resType);
        TRACE_FUNCTION_FR(v << " - " << context.ivars.fmtType(ty_dst) << " := " << context.ivars.fmtType(ty_src), v << " - " << context.ivars.fmtType(v.left_ty) << " := " << context.ivars.fmtType(node_ptr->resType));
        ...
        const bool hasPendingDerefTarget = ::std::any_of(
            ...
            [&](const Context::Associated& rule) {
                return rule.operator_kind == typeck::PrimitiveOperator::Deref
                    && context.ivars.types_equal(rule.left_ty, ty_src);
            }
        );
        ...
        switch (checkCoerceTys(context, sp, ty_dst, ty_src, &context, &node_ptr)) {
            case CoerceResult::Fail:
                return false;
            case CoerceResult::Unknown:
                DEBUG("Unknown - keep");
            ...
            case CoerceResult::Equality:
                ...
                context.equateTypes(sp, ty_dst, ty_src);
                return true;
            case CoerceResult::Unsize:
                DEBUG("Add _Unsize " << &*node_ptr << " -> " << ty_dst);
                auto span = node_ptr->span();
                node_ptr = NEWNODE(ty_dst, span, Unsize, mv$(node_ptr), ty_dst);
                return true;
        }
        throw "";
    }

    void addImplBounds(Context& context, const Span& sp, const ImplRef& impl_ref) {
        const auto* ep = impl_ref.mData.opt_TraitImpl();
        if (!ep) {
            return;
        }
        ...
        const auto& e = *ep;
        assert(e.impl);
        for (const auto& bound : e.impl->mParams.bounds) {
        TU_MATCH_HDRA((bound), {)
        default:
            break;
                TU_ARMA(TraitBound, be) {
                    DEBUG("New bound (pre-mono) " << bound);
                    auto ms = impl_ref.getCbMonomorphTraitimpl(context.crate.types, sp, {});
                    static const HIR::GenericParams emptyParams;
                    bool outer_present = be.hrtbs && !be.hrtbs->is_empty();
                    auto _ = ms.push_hrb(outer_present ? *be.hrtbs : emptyParams);
                    auto bTyMono = ms.monomorph_type(sp, be.type);
                    auto bTpMono = ms.monomorph_traitpath(sp, be.trait, true);
                    DEBUG("- " << bTyMono << " : " << bTpMono);
                    ASSERT_BUG(sp, !outer_present || !static_cast<bool>(bTpMono.hrtbs), "Two layers of HRTBs not allowed (should have been disallowed in HIR lower)");
                    auto pp_hrl = outer_present ? be.hrtbs->make_empty_params(true) : (bTpMono.hrtbs ? bTpMono.hrtbs->make_empty_params(true) : HIR::PathParams());
                    if (outer_present) {
                    ...
                    DEBUG("pp_hrl = " << pp_hrl);
                    auto ms_hrl = MonomorphHrlsOnly(context.crate.types, pp_hrl);
                    if (bTpMono.typeBounds.size() > 0) {
                        for (const auto& atyBound : bTpMono.typeBounds) {
                            context.equateTypesAssoc(sp, atyBound.second.type, bTpMono.mPath.mPath, ms_hrl.monomorph_path_params(sp, bTpMono.mPath.mParams, true), ms_hrl.monomorph_type(sp, bTyMono, true), atyBound.first.c_str(), atyBound.second.atyParams, false);
                        }
                    } else {
                        context.addTraitBound(sp, ms_hrl.monomorph_type(sp, bTyMono, true), bTpMono.mPath.mPath, ms_hrl.monomorph_path_params(sp, bTpMono.mPath.mParams, true));
                    }
                }
        }
        }
    ...
    AssociatedCheckResult checkAssociated(Context& context, Context::Associated& v) {
        ...
        struct H {
            static bool type_is_num(const ::HIR::TypeData* t) {
                TU_MATCH_HDRA( (*t), {)
                default:
                    return false;
                    TU_ARMA(Primitive, e) {
        ...
        auto impl_has_builtin_operator_signature = [&](const ImplRef& impl) {
            auto impl_ty = impl.getImplType(context.crate.types);
            auto impl_params = impl.getTraitParams(context.crate.types);
            ...
            if (context.ivars.type_contains_ivars(impl_ty, /*only_unbound=*/true)
                || context.ivars.pathparams_contain_ivars(impl_params, /*only_unbound=*/true)) {
                return false;
            }
            impl_ty = context.mResolve.expandAssociatedTypes(sp, mv$(impl_ty));
            for (auto& ty : impl_params.types) {
                ty = context.mResolve.expandAssociatedTypes(sp, mv$(ty));
            }

            const bool hasBuiltinInputs = impl_params.types.size() == 0
                ? typeck::primitive_operator_has_builtin(v.operator_kind, impl_ty)
                : impl_params.types.size() == 1
                    && typeck::primitive_operator_has_builtin(
                        v.operator_kind, impl_ty, impl_params.types.front()
                    );
            if (!hasBuiltinInputs) {
                return false;
            }
            ...
            if (context.ivars.type_contains_ivars(output, /*only_unbound=*/true)) {
                return false;
            }
            output = context.mResolve.expandAssociatedTypes(sp, mv$(output));

            auto builtinOutput = impl_ty;
            ...
            return output->compareWithPlaceholders(
                sp, builtinOutput, context.ivars.callbackResolveInfer()
            ) == ::HIR::Compare::Equal;
        };
        ...
        const auto concreteCoercionSource = [&](const ::HIR::TypeData* type) {
            ...
            for (const auto& source : possible.types_coerce_from) {
                const auto* source_type = context.getType(source.ty);
        ...
        if (v.operator_kind != typeck::PrimitiveOperator::None) {
            ...
            if (probe_params.types.size() == 1) {
                if (const auto* source = concreteCoercionSource(probe_params.types.front())) {
                    probe_params.types.front() = source;
                }
            }
            context.mResolve.findTraitImpls(sp, v.trait, probe_params, v.impl_ty, [&](ImplRef impl, HIR::Compare) {
                ...
                if (context.is_current_operator_impl(impl)) {
                    ...
                    currentOperatorImplHasBuiltinSignature = impl_has_builtin_operator_signature(impl);
                    return false;
                }
        ...
        const bool canContextualisePrimitiveRhs =
            v.is_operator
            && !hasSemanticOperatorImpl
            && (!saw_current_operator_impl || currentOperatorImplHasBuiltinSignature)
            && v.params.types.size() == 1
            && !context.ivars.type_contains_ivars(v.impl_ty, /*only_unbound=*/true)
            && typeck::primitive_operator_lhs_determines_rhs(v.operator_kind, context.getType(v.impl_ty))
        ...
        const bool primitive_types_are_contextual =
            !context.ivars.type_contains_ivars(v.impl_ty, /*only_unbound=*/true)
            && !context.ivars.pathparams_contain_ivars(v.params, /*only_unbound=*/true)
            && (v.name == ""
                || !context.ivars.type_contains_ivars(v.left_ty, /*only_unbound=*/true));
        ...
        if (v.is_operator && !hasSemanticOperatorImpl && primitive_types_are_contextual) {
            if (v.params.types.size() == 0) {
                // Uni ops = If the value is a primitive, the output is the same type
                const auto& ty = context.getType(v.impl_ty);
                const auto& res = context.getType(v.left_ty);
                if (H::type_is_num(ty)) {
                ...
                const auto& left_ty = context.getType(left);
                const auto& right_ty = context.getType(right);
                ...
                if (primitive_or_literal_pair || language_primitive_candidate) {
                    ...
                    if (v.name != ""
                        && context.getType(left)->is_Infer()
                        && context.getType(right)->is_Infer()
                        && context.getType(res)->is_Infer()) {
        ...
        if (const auto* e = context.ivars.getType(v.impl_ty)->opt_Infer()) {
            // TODO: ?
            if (!e->is_lit() && v.params.types.empty()) {
        ...
        const auto currentOperatorUsesLanguagePrimitive = [&]() {
            ...
            const auto& impl_ty = context.getType(v.impl_ty);
            if (v.params.types.size() == 0) {
            ...
            if (v.params.types.size() == 1) {
                return typeck::primitive_operator_has_language_candidate(
                    v.operator_kind, impl_ty, context.getType(v.params.types.front())
                );
            }
            return false;
        };
        ...
        unsigned int count = 0;
        DEBUG("Searching for impl " << v.trait << v.params << " for " << context.ivars.fmtType(v.impl_ty));
        ...
        try {
            auto candidateCallback = [&](ImplRef impl, HIR::Compare cmp) {
                ...
                if (v.name != "") {
                    ...
                    auto out_ty_o = impl.getType(context.crate.types, v.name.c_str(), {});
                    if (out_ty_o == ::HIR::TypeRef()) {
                        out_ty_o = context.crate.types.path(::HIR::Path(v.impl_ty, ::HIR::GenericPath(v.trait, v.params.clone()), v.name, v.atyPp.clone()), {});
                    }
                    out_ty_o = context.mResolve.expandAssociatedTypes(sp, mv$(out_ty_o));
                    ...
                    auto cmp2 = v.left_ty->compareWithPlaceholders(sp, out_ty_o, context.ivars.callbackResolveInfer());
                    bool definingOpaqueOutput = false;
                    if (const auto* erased = v.left_ty->opt_ErasedType()) {
                        if (const auto* alias = erased->inner.opt_Alias()) {
                            definingOpaqueOutput = alias->inner->is_public_to(context.mResolve.visPath);
                        }
                    }
                    if (cmp2 == ::HIR::Compare::Unequal && !definingOpaqueOutput) {
                        DEBUG("[check_associated] - (fail) known result can't match (" << context.ivars.fmtType(v.left_ty) << " and " << context.ivars.fmtType(out_ty_o) << ")");
                        return false;
                    }
                ...
                if (cmp == ::HIR::Compare::Equal) {
                    ...
                    // - Equate the types
                    context.equateTypes(sp, v.impl_ty, impl.getImplType(context.crate.types));
                    ...
                    impl_ty = context.mResolve.expandAssociatedTypes(sp, std::move(impl_ty));
                    for (auto& t : impl_params.types) {
                        t = context.mResolve.expandAssociatedTypes(sp, mv$(t));
                    }
                    ...
                    // NOTE: `overlaps_with` (should be) reflective
                        ...
                        for (auto& possible_impl : possible_impls) {
                            const auto& bestImpl = possible_impl.impl_ref;
                            ...
                            if (context.mResolve.impls_overlap(sp, impl, bestImpl)) {
                                ...
                                else if (bestImpl.more_specific_than(context.crate.types, impl)) {
                            ...
                            if (possible_impl.impl_ty == impl_ty && possible_impl.params == impl_params) {
                                auto t1 = v.name == "" ? HIR::TypeRef() : possible_impl.impl_ref.getType(context.crate.types, v.name.c_str(), {});
                                auto t2 = v.name == "" ? HIR::TypeRef() : impl.getType(context.crate.types, v.name.c_str(), {});
                                if (v.name == "" || t1 == t2 || t2 == HIR::TypeRef()) {
                                    ...
                                    DEBUG("[check_associated] HACK: Same type and params, but ATY mismatch - " << possible_impl.impl_ref.getType(context.crate.types, v.name.c_str(), {}) << " != " << impl.getType(context.crate.types, v.name.c_str(), {}));
                                }
                            }
                        }
                        if (!was_used) {
            ...
            const bool found = gTraitSolverConfig.globally
                ? context.mResolve.findTraitImplsNext(
                    sp,
                    v.trait,
                    v.params,
                    v.impl_ty,
                    ...
                    v.name == "" ? nullptr : &v.atyPp
                )
                : context.mResolve.findTraitImpls(
                    sp, v.trait, v.params, v.impl_ty, candidateCallback
                );
            if (found) {
                // Fully-known impl
                ...
                if (v.name != "") {
                    ...
                    if ((*output_type)->is_Path() && (*output_type)->as_Path().path.mData.is_UfcsKnown()) {
                        const auto& te = (*output_type)->as_Path();
                        const auto& pe = te.path.mData.as_UfcsKnown();
                        // If the target type is unbound, and is this rule exactly, don't return success
                        if (te.binding.is_Unbound() && pe.type == v.impl_ty && pe.item == v.name && pe.trait.mPath == v.trait && pe.trait.mParams == v.params) {
                            DEBUG("Would re-create the same rule, setting opaque");
                            auto data = (*output_type)->cloneData();
                            data.as_Path().binding = ::HIR::TypePathBinding::make_Opaque({});
                            output_type = context.crate.types.intern(std::move(data));
                        }
                    }
                    context.equateTypes(sp, v.left_ty, *output_type);
                }
                ...
                if (v.name == "") {
                    DEBUG("No impl of " << v.trait << context.ivars.fmt(v.params) << " for " << context.ivars.fmtType(v.impl_ty));
                } else {
                    DEBUG("No impl of " << v.trait << context.ivars.fmt(v.params) << " for " << context.ivars.fmtType(v.impl_ty) << " with " << v.name << " = " << context.ivars.fmtType(v.left_ty));
                }
                ...
                if (!is_known) {
                    ...
                    const auto& src_ty = context.getType(v.impl_ty);
                    const auto& dstTy = context.getType(v.params.types[0]);

                    context.equateTypes(sp, dstTy, src_ty);
                    return AssociatedCheckResult::Complete;
                } else if (v.operator_kind != typeck::PrimitiveOperator::None
                           && (v.params.types.size() == 0
                               ? typeck::primitive_operator_has_builtin(v.operator_kind, context.getType(v.impl_ty))
                               : v.params.types.size() == 1
                                   && typeck::primitive_operator_has_builtin(v.operator_kind, context.getType(v.impl_ty), context.getType(v.params.types.at(0))))) {
                    // No trait implementation matched this expression.  The
                    // language-defined primitive candidate therefore wins.
                    return AssociatedCheckResult::Complete;
                } else {
                    if (v.name == "") {
                        ERROR(sp, E0000, "Failed to find an impl of " << v.trait << context.ivars.fmt(v.params) << " for " << context.ivars.fmtType(v.impl_ty));
                    } else {
                        ERROR(sp, E0000, "Failed to find an impl of " << v.trait << context.ivars.fmt(v.params) << " for " << context.ivars.fmtType(v.impl_ty) << " with " << v.name << " = " << context.ivars.fmtType(v.left_ty));
                    }
                }
            } else if (count == 1) {
                auto& possible_impl_ty = possible_impls.at(0).impl_ty;
                auto& possible_params = possible_impls.at(0).params;
                auto& bestImpl = possible_impls.at(0).impl_ref;
                DEBUG("Only one impl " << v.trait << context.ivars.fmt(possible_params) << " for " << context.ivars.fmtType(possible_impl_ty)
                ...
                if (bestImpl.hasMagicParams()) {
                    ...
                    struct Matcher: public HIR::MatchGenerics, public Monomorphiser {
                        Context& m_context;
                        mutable ::std::map<HIR::GenericRef, HIR::TypeRef> types;
                        mutable ::std::map<HIR::GenericRef, HIR::ConstGeneric> values;
                        ...
                        ::HIR::Compare cmpType(const Span& sp, const ::HIR::TypeData* ty_l, const ::HIR::TypeData* ty_r, HIR::t_cb_resolve_type resolve_cb) override {
                            const auto& l = (ty_l->is_Infer() ? resolve_cb.getType(sp, ty_l) : ty_l);
                            const auto& r = (ty_r->is_Infer() ? resolve_cb.getType(sp, ty_r) : ty_r);
                            ...
                            if (l->is_Infer() && !r->is_Infer()) {
                                ...
                                m_context.equateTypes(sp, l, new_ty);
                        ...
                        ::HIR::TypeRef getType(const Span& sp, const ::HIR::GenericRef& g) const override {
                            if (g.group() == ::HIR::GENERICPlaceholder) {
                                ...
                                if (it == types.end()) {
                                    it = types.insert(std::make_pair(g, m_context.ivars.new_ivar_tr())).first;
                                    DEBUG("New type ivar for placeholder " << g << " = " << it->second);
                                }
                                return it->second;
                            } else {
                                return m_context.crate.types.generic(g.name, g.binding);
                            }
                        }
                        ...
                        ::HIR::ConstGeneric getValue(const Span& sp, const ::HIR::GenericRef& g) const override {
                            if (g.group() == ::HIR::GENERICPlaceholder) {
                                ...
                                if (it == values.end()) {
                                    auto v = ::HIR::ConstGeneric::make_Infer(::HIR::ConstGeneric::Data_Infer{m_context.ivars.new_ivar_val()});
                    ...
                    m.cmpType(sp, v.impl_ty, possible_impl_ty, context.ivars.callbackResolveInfer());
                    for (size_t i = 0; i < possible_params.types.size(); i++) {
                        m.cmpType(sp, v.params.types[i], possible_params.types[i], context.ivars.callbackResolveInfer());
                    }
                    DEBUG("> Magic params present, wait");
                    return AssociatedCheckResult::Retry;
                }
                const auto& impl_ty = context.ivars.getType(v.impl_ty);
                ...
                context.equateTypes(sp, v.impl_ty, possible_impl_ty);
                for (unsigned int i = 0; i < possible_params.types.size(); i++) {
                    context.equateTypes(sp, v.params.types[i], possible_params.types[i]);
                }
                for (unsigned int i = 0; i < possible_params.values.size(); i++) {
                    context.equateValues(sp, v.params.values[i], possible_params.values[i]);
                }
                ...
                if (v.name != "") {
                    output_type = context.mResolve.expandAssociatedTypes(sp, mv$(*output_type));

                    // If the output type is just < v.impl_ty as v.trait >::v.name, return false
                    if (TU_TEST1(**output_type, Path, .path.mData.is_UfcsKnown())) {
                        auto& pe = (*output_type)->as_Path().path.mData.as_UfcsKnown();

                        if (pe.type == v.impl_ty && pe.trait.mPath == v.trait && pe.trait.mParams == v.params && pe.item == v.name) {
                            if (TU_TEST1(*v.left_ty, Path, .path.mData.is_UfcsKnown())) {
                                auto data = (*output_type)->cloneData();
                                data.as_Path().binding = HIR::TypePathBinding::make_Opaque({});
                                output_type = context.crate.types.intern(std::move(data));
                            }
                ...
                for (const auto& pi : possible_impls) {
                    ...
                    for (size_t i = 0; i < pi.params.types.size(); i++) {
                        ...
                        if (const auto* e = t->opt_Infer()) {
                            const auto& pi_t = pi.params.types[i];
                            HIR::TypeRef possible_ty;
                            if (!type_contains_impl_placeholder(context.crate.types, pi_t)) {
...
namespace {
    ...
    bool set_associated_stall(Context& context, Context::Associated& rule) {
        ...
        const auto type_can_stall = [](const ::HIR::TypeRef type) {
            return (type->flags & (::HIR::TypeData::HAS_UNEVALUATED_CONST | ::HIR::TypeData::HAS_DEFERRED_CONST)) == 0;
        };
        if (!type_can_stall(rule.impl_ty)
            || (rule.name != "" && !type_can_stall(rule.left_ty))
            || path_params_have_untracked_const(rule.params)
            || path_params_have_untracked_const(rule.atyPp)) {
            return false;
        }
        ...
        collector.addType(rule.impl_ty);
        if (rule.name != "") {
            collector.addType(rule.left_ty);
        }
        for (const auto type : rule.params.types) {
        ...
        if (collector.hasRawInfer || rule.stalled_on.empty()) {
            rule.stalled_on.clear();
            return false;
        }
        return true;
    ...
    struct IvarDependencyIndex {
        ...
        static void collectDirectIvars(const ::HIR::TypeData* type, ::std::vector<unsigned int>& out) {
            visit_ty_with(type, [&](const ::HIR::TypeData* inner) {
        ...
        IvarDependencyIndex(Context& context)
            : context(context)
            , associatedTargets(context.possible_ivar_vals.size())
            , possibility_targets(context.possible_ivar_vals.size())
        {
            for (const auto& rule : context.link_assoc) {
                ::std::vector<unsigned int> sources;
                ::std::vector<unsigned int> targets;
                collectDirectIvars(rule.impl_ty, sources);
                if (rule.name != "") {
                    collectDirectIvars(rule.left_ty, sources);
                }
                collectDirectIvars(rule.impl_ty, targets);
                for (const auto& type : rule.params.types) {
        ...
        void disableDependents(unsigned int source) {
            ...
            for (const auto raw_target : associatedTargets[source]) {
                const auto& target = context.ivars.getType(raw_target);
                if (const auto* infer = target->opt_Infer()) {
                    DEBUG("Disable IVar " << infer->index);
                    if (infer->index < context.possible_ivar_vals.size()) {
                        context.possible_ivar_vals[infer->index].forceDisable = true;
                    }
                }
            }
            for (const auto target : possibility_targets[source]) {
                DEBUG("Block IVar " << target);
                context.possible_ivar_vals[target].forceDisable = true;
            }
        }
    };
    ...
    struct IvarBoundRefs {
        ...
        ::std::vector<const ::HIR::ExprNodeCallMethod*> methods;
    };
    ...
    struct IvarBoundIndex {
        ...
        void collectIvars(const ::HIR::TypeData* root, ::std::vector<unsigned int>& out) const {
            ...
            , refs(context.possible_ivar_vals.size())
        {
            ...
            for (const auto& bound : context.link_coerce) {
                dependencies.clear();
                collectIvars(bound->left_ty, dependencies);
                collectIvars((*bound->right_node_ptr)->resType, dependencies);
            ...
            for (const auto& bound : context.link_assoc) {
                dependencies.clear();
                collectIvars(bound.impl_ty, dependencies);
            ...
            for (const auto* node_ptr_dyn : context.to_visit) {
                if (const auto* node_ptr = cast<const ::HIR::ExprNodeCallMethod>(node_ptr_dyn)) {
                    dependencies.clear();
                    collectIvars(context.getType(node_ptr->mValue->resType), dependencies);
                    IvarDependencyIndex::deduplicate(dependencies);
                    addRefs(dependencies, &IvarBoundRefs::methods, node_ptr);
                }
            }
        }
    ...
    bool checkIvarPossFailsBounds(const Span& sp, Context& context, const IvarBoundRefs& boundRefs, const ::HIR::TypeData* ty_l, const ::HIR::TypeData* new_ty) {
        ...
        struct Cb {
            ...
            const HIR::TypeData* new_ty;

            Cb(bool& used_ty, const Span& sp, const Context& context, unsigned int ivar_idx, const HIR::TypeData* new_ty)
                : used_ty(used_ty)
                , sp(sp)
                , context(context)
                , ivar_idx(ivar_idx)
            ...
            bool operator()(const ::HIR::TypeData* ty, ::HIR::TypeRef& out_ty) {
                ...
                out_ty = cloneTyWith(context.crate.types, sp, rty, *this);
                return true;
            }
        ...
        for (const auto* bound : boundRefs.coercions) {
            used_ty = false;
            auto t_l = cloneTyWith(context.crate.types, sp, bound->left_ty, cb);
            auto t_r = cloneTyWith(context.crate.types, sp, (*bound->right_node_ptr)->resType, cb);
            if (!used_ty) {
                //DEBUG("[" << ty_l << "] Skip Corerce R" << bound->rule_idx << " - " << bound->left_ty << " := " << (*bound->right_node_ptr)->m_res_type);
                continue;
            }
            t_l = context.mResolve.expandAssociatedTypes(sp, mv$(t_l));
            t_r = context.mResolve.expandAssociatedTypes(sp, mv$(t_r));
            DEBUG("Check Coerce R" << bound->rule_idx << " - " << bound->left_ty << " := " << (*bound->right_node_ptr)->resType);
            DEBUG("Testing " << t_l << " := " << t_r);

            switch (checkCoerceTys(context, sp, t_l, t_r, nullptr, bound->right_node_ptr)) {
                case CoerceResult::Fail:
                    DEBUG("Fail - Invalid");
                    return true;
                case CoerceResult::Unsize:
                ...
                case CoerceResult::Equality:
                    ...
                    if (t_l->compareWithPlaceholders(sp, t_r, context.ivars.callbackResolveInfer()) == HIR::Compare::Unequal) {
                        DEBUG("- Bound failed");
                        return true;
                    }
                    break;
        ...
        if (ivar_idx < context.ivarsSized.size() && context.ivarsSized[ivar_idx]) {
            if (context.mResolve.type_is_sized(sp, new_ty) == ::HIR::Compare::Unequal) {
        ...
        for (const auto& pty : context.possible_ivar_vals.at(ty_l->as_Infer().index).types_coerce_to) {
            ...
            switch (pty.op) {
                case Context::IVarPossible::CoerceTy::Coercion:
                    res = checkCoerceTys(context, sp, pty.ty, new_ty, nullptr, &stub_node);
                    break;
                case Context::IVarPossible::CoerceTy::Unsizing:
                    res = checkUnsizeTys(context, sp, pty.ty, new_ty, nullptr, &stub_node);
                    break;
            }
            switch (res) {
                case CoerceResult::Unsize:
                ...
                case CoerceResult::Equality:
                    ...
                    if (pty.ty->compareWithPlaceholders(sp, new_ty, context.ivars.callbackResolveInfer()) == HIR::Compare::Unequal) {
                        DEBUG("- Bound failed");
                        return true;
                    }
                    break;
        ...
        for (const auto* bound : boundRefs.associated) {
            used_ty = false;
            auto t = cloneTyWith(context.crate.types, sp, bound->impl_ty, cb);
            ...
            context.mResolve.findTraitImpls(sp, bound->trait, p, t, [&](const auto impl, auto cmp) {
                ...
                if (bound->name != "") {
                    ...
                    if (aty == ::HIR::TypeRef()) {
                        ...
                        return false;
                    } else if (aty->compareWithPlaceholders(sp, bound->left_ty, context.ivars.callbackResolveInfer()) == HIR::Compare::Unequal) {
                        DEBUG("[check_ivar_poss__fails_bounds] ATY " << context.ivars.fmtType(aty) << " != left " << context.ivars.fmtType(bound->left_ty));
                        boundFailed = true;
                        // - Bail instantly
                        return true;
                    } else {
        ...
        // Handle methods
        for (const auto* node_ptr : boundRefs.methods) {
            const auto& node = *node_ptr;
            const auto& ty_tpl = context.getType(node.mValue->resType);

            bool used_ty = false;
            auto t = cloneTyWith(context.crate.types, sp, ty_tpl, [&](const auto& ty, auto& out_ty) {
            ...
            ::std::vector<::std::pair<TraitResolution::AutoderefBorrow, ::HIR::Path>> possible_methods;
            unsigned int derefCount = context.mResolve.autoderefFindMethod(node.span(), node.traits, node.traitParamIvars, node.traitParamTypeIvars, t, node.method, possible_methods);
            DEBUG("> deref_count = " << derefCount << ", possible_methods={" << possible_methods << "}");
    ...
    struct PossibleType {
        ...
        static PossibleType concrete(decltype(cls) cls, const ::HIR::TypeData* ty) {
    ...
    struct TypeRestrictiveOrdering {
        ...
        static const ::HIR::TypeData* match_and_extract_ptr_ty(const ::HIR::TypeData* ptr_tpl, const ::HIR::TypeData* ty) {
        ...
        static Ordering getOrderingInfer(const Span& sp, const ::HIR::TypeData* r) {
        ...
        static Ordering getOrderingTy(const Span& sp, const Context& context, const ::HIR::TypeData* l, const ::HIR::TypeData* r, bool& out_unordered) {
            if (l == r) {
                return OrdEqual;
            }
            if (l->is_Infer()) {
            ...
            if (l->is_Path()) {
                ...
                default:
                    ...
                    TU_ARMA(Path, te_r) {
                        ...
                        } else if (TU_TEST1(te_r.binding, Struct, ->structMarkings.canUnsize)) {
            ...
            if (r->is_Path()) {
                // Path types can be unsize targets, and can also act like infers
                switch (getOrderingTy(sp, context, r, l, out_unordered)) {
                    case OrdLess:
                        return OrdGreater;
                    case OrdEqual:
                        return OrdEqual;
        ...
        static Ordering getOrderingPtr(const Span& sp, const Context& context, const ::HIR::TypeData* l, const ::HIR::TypeData* r, bool& out_unordered, bool deep = true) {
            Ordering cmp;
                ...
                default:
                    ...
                    TU_ARMA(Borrow, te_l) {
                        ...
                        if (cmp == OrdEqual && deep) {
                            cmp = getOrderingTy(sp, context, context.ivars.getType(te_l.inner), context.ivars.getType(te_r.inner), out_unordered);
                        }
                    }
    ...
    struct InfoOrdering {
        ...
        static bool is_infer(const ::HIR::TypeData* ty) {
            if (ty->is_Infer()) {
                return true;
            }
        ...
        static bool compareScore(int& score, const ::HIR::TypeData* ty_l, const ::HIR::TypeData* ty_r) {
        ...
        static eInfoOrdering compare(const ::HIR::TypeData* ty_l, const ::HIR::TypeData* ty_r) {
            if (is_infer(ty_l)) {
                if (is_infer(ty_r)) {
                    return Same;
                }
        ...
        static eInfoOrdering compareTop(const Context& context, const ::HIR::TypeData* ty_l, const ::HIR::TypeData* ty_r, bool should_deref) {
            if (context.ivars.types_equal(ty_l, ty_r)) {
                return Same;
            }
            if (is_infer(ty_l)) {
                return Incompatible;
            ...
            if (should_deref) {
                if (const auto* le = ty_l->opt_Borrow()) {
                    ...
                    return compareTop(context, context.ivars.getType(le->inner), context.ivars.getType(re.inner), false);
                } else if (TU_TEST2(*ty_l, Path, .binding, Struct, ->structMarkings.coerceUnsized != ::HIR::StructMarkings::Coerce::None)) {
                    const auto& le = ty_l->as_Path();
                    const auto& re = ty_r->as_Path();
                    if (le.binding != re.binding) {
                        return Incompatible;
                    }
                    auto param_idx = le.binding.as_Struct()->structMarkings.coerceParam;
                    assert(param_idx != ~0u);
                    return compareTop(context, context.ivars.getType(le.path.mData.as_Generic().mParams.types.at(param_idx)), context.ivars.getType(re.path.mData.as_Generic().mParams.types.at(param_idx)), false);
    ...
    bool checkIvarPoss(Context& context, const IvarBoundIndex& boundIndex, unsigned int i, Context::IVarPossible& ivar_ent, IvarPossFallbackType fallbackTy = IvarPossFallbackType::None) {
        static Span _span;
        const auto& sp = _span;
        const bool honourDisable = (fallbackTy != IvarPossFallbackType::IgnoreWeakDisable);

        const auto& ty_l = context.ivars.getType(i);
        const auto& boundRefs = boundIndex[i];

        if (!TU_TEST1(*ty_l, Infer, .index == i)) {
            if (ivar_ent.hasRules()) {
            ...
            for (const auto& t : ivar_ent.types_coerce_to) {
                for (const auto& t2 : ivar_ent.types_coerce_from) {
                    ...
                    if (t.ty == t2.ty && t.ty != ty_l) {
                        ...
                        context.equateTypes(sp, ty_l, t.ty);
                        return true;
                    }
                }
            }
        ...
        // Skip Conditions
        // ---
        if (ivar_ent.hasBounded && (!ivar_ent.boundsIncludeSelf && ivar_ent.bounded.empty())) {
            DEBUG(i << ": Bounded, but bound set empty");
            return false;
        }
        if (ivar_ent.forceDisable && fallbackTy != IvarPossFallbackType::FinalOption) {
        ...
        // - This list needs to include flags to say if the type can be dereferenced.
        {
            ...
            if (addPlaceholders && ivar_ent.forceNoFrom) {
            ...
            if (addPlaceholders && ivar_ent.forceNoTo) {
            ...
            DEBUG(i << ": bounds = " << (ivar_ent.hasBounded ? "" : "? ") << (ivar_ent.boundsIncludeSelf ? "+, " : "") << ivar_ent.bounded);
            ...
            if (ivar_ent.hasBounded && !ivar_ent.boundsIncludeSelf) {
                // Look for a bound that matches all other restrictions
                const HIR::TypeData* bestTy = nullptr;
                bool foundTwo = false;
                for (const auto& bTy : ivar_ent.bounded) {
                    // Check bound against bounds
                    if (!checkIvarPossFailsBounds(sp, context, boundRefs, ty_l, bTy)) {
                        if (bestTy) {
            ...
            {
                struct H {
                    static const ::HIR::TypeData* getBorrowInner(const ::HIR::TypeData* ty) {
                        if (ty->is_Borrow()) {
                ...
                bool failed = false;
                const HIR::TypeData* foundTy = nullptr;
                for (const auto& boundedTy : ivar_ent.bounded) {
                    // Skip ivars
                    ...
                    for (const auto& t : possible_tys) {
                        ...
                        if (boundedTy->compareWithPlaceholders(sp, t.ty, context.ivars.callbackResolveInfer()) != HIR::Compare::Unequal) {
                            if (!foundTy) {
                                foundTy = boundedTy;
                            } else if (foundTy == boundedTy) {
                                // Same type still, continue
                            } else if (boundedTy->compareWithPlaceholders(sp, foundTy, context.ivars.callbackResolveInfer()) == HIR::Compare::Unequal) {
                                // Incompatible types
                                failed = true;
                            } else {
                ...
                if (foundTy && !failed) {
                    ...
                    auto t = cloneTyWith(context.crate.types, sp, foundTy, [&](const HIR::TypeData* t1, HIR::TypeRef& out) -> bool {
                        if (t1->is_Infer()) {
                            const auto& t = context.getType(t1);
                            if (t->is_Infer()) {
                                out = context.ivars.new_ivar_tr();
                            } else {
                                out = t;
                            }
                            return true;
            ...
            if (fallbackTy != IvarPossFallbackType::None && ::std::count_if(possible_tys.begin(), possible_tys.end(), PossibleType::is_source_s) == 1 && !ivar_ent.forceNoFrom && !ivar_ent.hasBounded) {
                ...
                if (!context.ivars.type_contains_ivars(ent.ty) && !(ent.ty)->is_Diverge()) {
                    if (!checkIvarPossFailsBounds(sp, context, boundRefs, ty_l, ent.ty)) {
                        DEBUG("Single concrete source, " << ent.ty);
                        context.equateTypes(sp, ty_l, ent.ty);
                        return true;
                    }
                }
            }
            if (fallbackTy == IvarPossFallbackType::IgnoreWeakDisable && possible_tys.size() == 1) {
            ...
            if (::std::count_if(possible_tys.begin(), possible_tys.end(), PossibleType::is_dest_s) == 1 && ::std::count_if(possible_tys.begin(), possible_tys.end(), PossibleType::is_source_s) == 1 && !ivar_ent.forceNoFrom && !ivar_ent.forceNoTo && !ivar_ent.hasBounded) {
                const auto& entS = *::std::find_if(possible_tys.begin(), possible_tys.end(), PossibleType::is_source_s);
                const auto& entD = *::std::find_if(possible_tys.begin(), possible_tys.end(), PossibleType::is_dest_s);

                // Only if both options are coerce?
                // TODO: And this ivar isn't Sized bounded?
                if (entS.is_coerce() && entD.is_coerce()) {
                    bool src_noivars = !context.ivars.type_contains_ivars(entS.ty);
                    bool dstNoivars = !context.ivars.type_contains_ivars(entD.ty);
                    bool src_valid = !checkIvarPossFailsBounds(sp, context, boundRefs, ty_l, entS.ty);
                    bool dstValid = !checkIvarPossFailsBounds(sp, context, boundRefs, ty_l, entD.ty);

                    if (src_valid) {
                        if (src_noivars) {
                            DEBUG("Single each way, concrete source, " << entS.ty);
                            context.equateTypes(sp, ty_l, entS.ty);
                            return true;
                        }
                    }
                    if (dstValid) {
                        if (dstNoivars) {
                            ...
                            context.equateTypes(sp, ty_l, entD.ty);
                            return true;
                        }
                    }
            ...
            if (possible_tys.size() == 1 && possible_tys[0].is_source() && !ivar_ent.forceNoFrom) {
                const auto* ty_p = possible_tys[0].ty;
                if (possible_tys[0].is_unsize()) {
                    HIR::TypeRef tmp_ty;

                    do {
                        if (!checkIvarPossFailsBounds(sp, context, boundRefs, ty_l, ty_p)) {
                            DEBUG("Single possibility failed bounds, trying deref - " << ty_p);
                            break;
                        }
                    } while ((ty_p = context.mResolve.autoderef(sp, ty_p, tmp_ty)));
                    if (!ty_p) {
            ...
            if (n_src_ivars == 0 || fallbackTy == IvarPossFallbackType::Assume) {
                const ::HIR::TypeData* ptr_ty = nullptr;
                if (::std::any_of(possible_tys.begin(), possible_tys.end(), [&](const auto& ent) {
                    ...
                    for (const auto& ent : possible_tys) {
                        ...
                        if (ptr_ty == nullptr) {
                            ptr_ty = ent.ty;
                        } else if (TypeRestrictiveOrdering::getOrderingPtr(sp, context, ent.ty, ptr_ty, unused_unordered, /*deep=*/false) == OrdLess) {
                            ptr_ty = ent.ty;
                        } else {
                        }
                    }
                ...
                for (const auto& ent : possible_tys) {
                    ...
                    const ::HIR::TypeData* inner_ty = (ptr_ty ? TypeRestrictiveOrdering::match_and_extract_ptr_ty(ptr_ty, ent.ty) : ent.ty);
                    if (!inner_ty) {
                        continue;
                    }
...
// Look for the least permissive of the available destination types and assign to that
            ...
            if (::std::all_of(possible_tys.begin(), possible_tys.end(), PossibleType::is_coerce_s)) {
                ...
                const ::HIR::TypeData* destType = nullptr;
                for (const auto& ent : possible_tys) {
                    if (ent.is_dest()) {
                        continue;
                    }
                    ...
                    auto cmp = TypeRestrictiveOrdering::getOrderingPtr(sp, context, ent.ty, destType, is_unordered);
                    switch (cmp) {
                        case OrdLess:
                ...
                if ((num_distinct > 1 || fallbackTy == IvarPossFallbackType::Assume) && destType && !is_unordered) {
                    DEBUG("- Least-restrictive source " << destType);
                    context.equateTypes(sp, ty_l, destType);
                    return true;
                }
            }
#endif
...
// Look for the least permissive of the available destination types and assign to that
            ...
            // - Comparison function: Returns one of Incomparible,Less,Same,More - Representing the amount of type information present.
            {
                ...
                for (auto it = possible_tys.begin(); it != possible_tys.end(); ++it) {
                    ...
                    for (auto it2 = it + 1; it2 != possible_tys.end(); ++it2) {
                        ...
                        switch (InfoOrdering::compareTop(context, it->ty, it2->ty, /*should_deref=*/it->is_coerce())) {
            ...
            if (fallbackTy == IvarPossFallbackType::Assume) {
                ...
                if (::std::all_of(possible_tys.begin(), possible_tys.end(), PossibleType::is_coerce_s) && n_ivars == 0) {
                    ...
                    bool is_unordered = false;
                    for (const auto& ent : possible_tys) {
                        if (visit_ty_with(ent.ty, [](const ::HIR::TypeData* t) {
                            return t->is_Infer();
                        })) {
                            anyIvarPresent = true;
                        }
                    ...
                    if (destType && n_ivars == 0 && anyIvarPresent == false && !TU_TEST1(*destType, NodeType, .is_Closure()) && !is_unordered) {
                        DEBUG("Suitable option " << destType << " from " << possible_tys);
                        context.equateTypes(sp, ty_l, destType);
                        return true;
                    }
                }
            ...
            for (auto it = possible_tys.begin(); it != possible_tys.end();) {
                ...
                if (it->ty == ty_l) {
                    remove_option = true;
                } else if (!allowUnsized && context.mResolve.type_is_sized(sp, it->ty) == ::HIR::Compare::Unequal) {
                    remove_option = true;
                } else {
                    // Keep
                }
                ...
                for (const auto& other_opt : possible_tys) {
                    ...
                    if (other_opt.ty == it->ty) {
                        ...
                        if (!(it->ty)->is_Infer() && other_opt.is_coerce() == it->is_coerce() && other_opt.is_source() != it->is_source()) {
                            ...
                            context.equateTypes(sp, ty_l, it->ty);
                            return true;
                        }
            ...
            if (possible_tys.size() >= 2
                // && n_ivars == 0
                && (fallbackTy == IvarPossFallbackType::FinalOption || n_src_ivars == 0)
                ...
                if (const auto* te = (possible_tys[0].ty)->opt_NamedFunction()) {
                    new_ty = context.crate.types.function(te->decay(context.crate.types, sp));
                } else if (const auto* t1_nodep = TU_OPT1(*possible_tys[0].ty, NodeType, .opt_Closure())) {
                    auto ft = HIR::TypeDataFunctionPointer{HIR::GenericParams(), false, false, RcString::new_interned(ABI_RUST), (*t1_nodep)->returnType, {}};
                    for (const auto& t : (*t1_nodep)->mArgs) {
                        ft.argTypes.push_back(t.second);
                    }
                    new_ty = context.crate.types.function(std::move(ft));
                } else {
                    BUG(sp, "");
                }
                DEBUG("HACK: All options are closures/functions, adding a function pointer - " << new_ty);
                context.equateTypes(sp, ty_l, new_ty);
                return true;
            }
            ...
            for (auto it = possible_tys.begin(); it != possible_tys.end();) {
                ...
                if (it->is_source() && !(it->ty)->is_Infer()) {
                    ...
                    if (it->is_coerce()) {
                        ...
                        dty = context.mResolve.autoderef(sp, dty, tmp);
                    ...
                    if (dty) {
                        for (const auto& other_opt : possible_tys) {
                            ...
                            if (other_opt.is_coerce()) {
                                ...
                                oty = context.mResolve.autoderef(sp, oty, tmp2);
                ...
                if (!remove_option && !(it->ty)->is_Infer() && checkIvarPossFailsBounds(sp, context, boundRefs, ty_l, it->ty)) {
                    remove_option = true;
            ...
            if (n_src_ivars == 0 && /*n_dst_ivars == 0 &&*/ possible_tys.empty() && possibly_diverge && fallbackTy == IvarPossFallbackType::IgnoreWeakDisable) {
                auto t = context.crate.types.diverge();
                if (!checkIvarPossFailsBounds(sp, context, boundRefs, ty_l, t)) {
                    DEBUG("Possibly `!` and no other options - setting");
                    context.equateTypes(sp, ty_l, context.crate.types.diverge());
                    return true;
                }
            }
            ...
            if ((!honourDisable || !ivar_ent.hasBounded)) {
                ...
                if (fallbackTy != IvarPossFallbackType::None && n_dst_ivars == 0 && ::std::count_if(possible_tys.begin(), possible_tys.end(), PossibleType::is_dest_s) == 1) {
            ...
            if (ivar_ent.bounded.size() == 0) {
                if (possible_tys.size() > 0 && !honourDisable && n_ivars == 0) {
            ...
            if (!possible_tys.empty() && (!ivar_ent.boundsIncludeSelf || fallbackTy == IvarPossFallbackType::FinalOption)) {
                DEBUG("Checking bounded [" << ivar_ent.bounded << "]");
                ::std::vector<const ::HIR::TypeData*> feasableBounds;
                for (const auto& new_ty : ivar_ent.bounded) {
                    ...
                    for (const auto& opt : possible_tys) {
                        ...
                        switch (new_ty->compareWithPlaceholders(sp, opt.ty, context.ivars.callbackResolveInfer())) {
                        ...
                        if (opt.is_source()) {
                            ...
                            cmp = checkUnsizeTys(context, sp, new_ty, opt.ty, nullptr);
                        } else {
                            // Destination type, this option must deref to it
                            DEBUG("(checking bounded) > " << opt.ty << " =? " << new_ty);
                            cmp = checkUnsizeTys(context, sp, opt.ty, new_ty, nullptr);
                        }
            ...
            hasNoCoercePosiblities = possible_tys.empty() && n_ivars == 0;
        }

        if (hasNoCoercePosiblities && !ivar_ent.bounded.empty()) {
            ...
            ::std::vector<const ::HIR::TypeData*> goodTypes;
            goodTypes.reserve(ivar_ent.bounded.size());
            for (const auto& new_ty : ivar_ent.bounded) {
                DEBUG("- Test " << new_ty << " against current rules");
                if (checkIvarPossFailsBounds(sp, context, boundRefs, ty_l, new_ty)) {
                } else {
                    goodTypes.push_back(new_ty);
            ...
            DEBUG(goodTypes.size() << " valid options (" << n_good_ints << " primitives)");
            ...
            if (goodTypes.empty()) {
                ...
                context.equateTypes(sp, ty_l, goodTypes.front());
                return true;
            } else if (goodTypes.size() > 0 && fallbackTy == IvarPossFallbackType::FinalOption) {
                auto typ_is_borrow = [&](const ::HIR::TypeData* typ) {
                    return typ->is_Borrow();
                };
                // NOTE: We want to select from sets of primitives and generics (which can be interchangable)
                if (::std::all_of(goodTypes.begin(), goodTypes.end(), typ_is_borrow) == ::std::any_of(goodTypes.begin(), goodTypes.end(), typ_is_borrow)) {
...
void TypecheckCodeCS(const typeck::ModuleState& ms, t_args& args, const ::HIR::TypeData* result_type, ::HIR::ExprPtr& expr) {
    ...
    Context context{ms.crate, ms.implGenerics, ms.itemGenerics, ms.modPaths.back(), ms.currentTrait, ms.currentTraitImpl};

    // - Build up ruleset from node tree
    TypecheckCodeCSEnumerateRules(context, ms, args, result_type, expr, root_ptr);
    ...
    while (context.take_changed() /*&& context.has_rules()*/ && count < MAX_ITERATIONS) {
        ...
        if (!context.ivars.peek_changed()) {
            ...
            for (size_t i = 0; i < context.link_coerce.size();) {
                ...
                auto& src_ty = (*ent->right_node_ptr)->resType;
                src_ty = context.mResolve.expandAssociatedTypes(span, mv$(src_ty)); // TODO: This was commented, why?
                ent->left_ty = context.mResolve.expandAssociatedTypes(span, mv$(ent->left_ty));
            ...
            for (unsigned int i = 0; i < context.link_assoc.size();) {
                ...
                for (auto& ty : rule.params.types) {
                    ty = context.mResolve.expandAssociatedTypes(rule.span, mv$(ty));
                }
                if (rule.name != "") {
                    rule.left_ty = context.mResolve.expandAssociatedTypes(rule.span, mv$(rule.left_ty));
                    // HACK: If the left type is `!`, remove the type bound
                    //if( rule.left_ty->is_Diverge() ) {
                    //    rule.name = "";
                    //}
                }
                rule.impl_ty = context.mResolve.expandAssociatedTypes(rule.span, mv$(rule.impl_ty));
                ...
                merge_associated_possibilities(context, capturedPossibilities);
                ...
                if (result == AssociatedCheckResult::Complete) {
                    ...
                    if ((result == AssociatedCheckResult::Stalled || result == AssociatedCheckResult::Ambiguous)
                        ...
                        rule.stalled_possibilities = mv$(capturedPossibilities);
                    } else {
                        rule.stalled_on.clear();
        ...
        if (!context.ivars.peek_changed()) {
            ...
            for (unsigned int i = 0; i < context.possible_ivar_vals.size(); i++) {
                if (checkIvarPoss(context, *ivar_bound_index, i, context.possible_ivar_vals[i])) {
        ...
        if (!context.ivars.peek_changed()) {
            ...
            for (unsigned int i = 0; i < context.possible_ivar_vals.size(); i++) {
                if (checkIvarPoss(context, *ivar_bound_index, i, context.possible_ivar_vals[i], IvarPossFallbackType::Backwards)) {
                    break;
                }
            }
        }
        ...
        if (!context.ivars.peek_changed()) {
            ...
            for (unsigned int i = 0; i < context.possible_ivar_vals.size(); i++) {
                if (checkIvarPoss(context, *ivar_bound_index, i, context.possible_ivar_vals[i], IvarPossFallbackType::Assume)) {
                    break;
                }
            }
        }
        ...
        if (!context.ivars.peek_changed()) {
            ...
            for (unsigned int i = 0; i < context.possible_ivar_vals.size(); i++) {
                if (checkIvarPoss(context, *ivar_bound_index, i, context.possible_ivar_vals[i], IvarPossFallbackType::IgnoreWeakDisable)) {
                    break;
                } else {
                    //assert( !context.m_ivars.peek_changed() );
                }
        ...
        if (!context.ivars.peek_changed()) {
            ...
            for (unsigned int i = 0; i < context.possible_ivar_vals.size(); i++) {
                if (checkIvarPoss(context, *ivar_bound_index, i, context.possible_ivar_vals[i], IvarPossFallbackType::PickFirstBound)) {
                    break;
                }
            }
        }
        if (!context.ivars.peek_changed()) {
            ...
            for (unsigned int i = 0; i < context.possible_ivar_vals.size(); i++) {
                if (checkIvarPoss(context, *ivar_bound_index, i, context.possible_ivar_vals[i], IvarPossFallbackType::FinalOption)) {
                    break;
                }
            }
        }
                ...
                if (!ent.types_default.empty()) {
                    ...
                    if (TU_TEST1(*ty_l, Infer, .index == i)) {
                        if (ent.types_default.size() != 1) {
                            ...
                            context.ivars.set_ivar_to(i, *ent.types_default.begin());
                        }
                    }
                }
            }
        ...
        if (!context.ivars.peek_changed()) {
            ...
            if (!context.link_coerce.empty()) {
                ...
                ent->left_ty = context.mResolve.expandAssociatedTypes(sp, mv$(ent->left_ty));
                DEBUG("- Equate coercion R" << ent->rule_idx << " " << ent->left_ty << " := " << src_ty);

                context.equateTypes(sp, ent->left_ty, src_ty);
            }
        }
#endif
        ...
        count++;
        context.mResolve.compactIvars(context.ivars);
    }
    ...
    if (context.hasRules()) {
            ...
            if (rule.name == "") {
                ERROR(
                    ...
                    "type annotations needed: cannot infer a type satisfying `"
                        << context.ivars.fmtType(rule.impl_ty) << ": "
                        << rule.trait << context.ivars.fmt(rule.params) << "`"
                );
            } else {
                ERROR(
                    rule.span,
                    ...
                    "type annotations needed: cannot infer `"
                        << context.ivars.fmtType(rule.left_ty) << " = <"
                        << context.ivars.fmtType(rule.impl_ty) << " as "
                        << rule.trait << context.ivars.fmt(rule.params) << ">::"
                        << rule.name << "`"
                );
            }
        ...
        context.dump();
        for (const auto& coercionP : context.link_coerce) {
            const auto& coercion = *coercionP;
            const auto& sp = (**coercion.right_node_ptr).span();
            const auto& src_ty = (**coercion.right_node_ptr).resType;
            WARNING(sp, W0000, "Spare Rule - " << context.ivars.fmtType(coercion.left_ty) << " := " << context.ivars.fmtType(src_ty));
        }
        for (const auto& rule : context.link_assoc) {
            const auto& sp = rule.span;
            if (rule.name == "") {
                WARNING(sp, W0000, "Spare Rule - " << context.ivars.fmtType(rule.impl_ty) << " : " << rule.trait << rule.params);
            } else {
                WARNING(sp, W0000, "Spare Rule - " << context.ivars.fmtType(rule.left_ty) << " = < " << context.ivars.fmtType(rule.impl_ty) << " as " << rule.trait << rule.params << " >::" << rule.name);
            }
        }
    ...
    DEBUG("root_ptr = " << root_ptr->type_name() << " " << root_ptr->resType);

    // - Recreate the pointer
    expr.reset(root_ptr.release());
    //  > Steal the binding types
    expr.mBindings.reserve(context.mBindings.size());
    for (auto& binding : context.mBindings) {
        expr.mBindings.push_back(binding.ty);
    }
...

    {
        ...
        StaticTraitResolve static_resolve(ms.crate);
        static_resolve.set_both_generics_raw(ms.implGenerics, ms.itemGenerics);
        ...
        struct VisitMethodConst: public HIR::ExprVisitorDef {
            ...
            void visit(HIR::ExprNodeCallMethod& node) override {
                ...
                TU_MATCH_HDRA( (node.methodPath.mData), {)
                ...
                if(found)
                {
                    ...
                    MonomorphState out_params(ms.crate.types);
                    auto val_ref = static_resolve.getValue(node.span(), node.methodPath, out_params, /*signature_only=*/true, nullptr);
                    const HIR::Function& fcn = *val_ref.as_Function();
                    const HIR::GenericParams& gpDef = fcn.mParams;
                    ConvertHIRConstantEvaluateMethodParams(node.span(), ms.crate, ms.modPaths.back(), ms.implGenerics, ms.itemGenerics, &gpDef, *params_ptr);
                }
            }
        } v(ms, static_resolve);
...
namespace {
    inline ::HIR::SimplePath getRuleParentPath(const ::HIR::SimplePath& sp) {
        return sp.parent();
    }

    inline ::HIR::GenericPath getRuleParentPath(const ::HIR::GenericPath& gp) {
        return ::HIR::GenericPath(gp.mPath.parent(), gp.mParams.clone());
    }
}
...
namespace typecheck {
    ...
    bool visitCallPopulateCacheUfcsInherent(Context& context, const Span& sp, ::HIR::Path& path, ::HIR::ExprCallCache& cache, const ::HIR::Function*& fcn_ptr);
    ...
    public:
        ...
        ::HIR::Compare match_ty(const ::HIR::GenericRef& g, const ::HIR::TypeData* ty, ::HIR::t_cb_resolve_type _resolve_cb) override {
            assert(g.binding < impl_params.types.size());
            auto& slot = impl_params.types[g.binding];
        ...
        ::HIR::Compare match_val(const ::HIR::GenericRef& g, const ::HIR::ConstGeneric& sz) override {
            assert(g.binding < impl_params.values.size());
            ASSERT_BUG(Span(), impl_params.values[g.binding] == ::HIR::ConstGeneric(), "TODO: Multiple values? " << impl_params.values[g.binding] << " and " << sz);
...
#if 1
        for (size_t i = 0; i < param_defs.types.size(); i++) {
            const auto& ty = params.types[i];
            const auto& typ = param_defs.types[i];
            if (const auto* te = ty->opt_Infer()) {
                if (!typ.defaultValue->is_Infer()) {
                    if (auto* ent = context.getIvarPossibilities(sp, te->index)) {
                        auto defTy = ms.monomorph_type(sp, typ.defaultValue);
                        DEBUG("Added default for " << ty << ": " << defTy);
                        ent->types_default.insert(std::move(defTy));
                    }
                }
            }
        }
    ...
    template <typename T>
    void fix_param_count_(const Span& sp, Context& context, const ::HIR::TypeData* self_ty, bool use_defaults, const T& path, const ::HIR::GenericParams& param_defs, ::HIR::PathParams& params) {
        if (params.mLifetimes.size() == param_defs.mLifetimes.size()) {
        } else if (params.mLifetimes.size() > param_defs.mLifetimes.size()) {
        ...
        if (params.types.size() == param_defs.types.size()) {
            ...
            while (params.types.size() < param_defs.types.size()) {
                ...
                if (use_defaults) {
                    ...
                    } else if (monomorphise_type_needed(typ.defaultValue)) {
                        auto cb = MonomorphStatePtr(context.crate.types, self_ty, nullptr, nullptr);
                        params.types.push_back(cb.monomorph_type(sp, typ.defaultValue));
                    } else {
                        params.types.push_back(typ.defaultValue);
                    }
                } else {
                    params.types.push_back(context.ivars.new_ivar_tr());
    ...
    void fixParamCount(const Span& sp, Context& context, const ::HIR::TypeData* self_ty, bool use_defaults, const ::HIR::Path& path, const ::HIR::GenericParams& param_defs, ::HIR::PathParams& params) {
        fix_param_count_(sp, context, self_ty, use_defaults, path, param_defs, params);
    }

    void fixParamCount(const Span& sp, Context& context, const ::HIR::TypeData* self_ty, bool use_defaults, const ::HIR::GenericPath& path, const ::HIR::GenericParams& param_defs, ::HIR::PathParams& params) {
        fix_param_count_(sp, context, self_ty, use_defaults, path, param_defs, params);
    }

    void applyBoundsAsRulesTrait(Context& context, const Span& sp, const ::HIR::TypeData* real_type, const ::HIR::TraitPath& trait_path, const MonomorphHrlsOnly& ms_hrl) {
        // If there's no type bounds, emit a trait bound
        // - Otherwise, the assocated type bounds will serve the same purpose
        if (trait_path.typeBounds.size() == 0) {
            context.addTraitBound(sp, real_type, trait_path.mPath.mPath, ms_hrl.monomorph_path_params(sp, trait_path.mPath.mParams, true));
        }

        // Associated type equalities
        for (const auto& assoc : trait_path.typeBounds) {
            context.equateTypesAssoc(sp, ms_hrl.monomorph_type(sp, assoc.second.type, true), assoc.second.source_trait.mPath, ms_hrl.monomorph_path_params(sp, assoc.second.source_trait.mParams, true), real_type, assoc.first.c_str(), ms_hrl.monomorph_path_params(sp, assoc.second.atyParams, true), false);
        }
        // Associated type trait bounds:
        for (const auto& assoc : trait_path.traitBounds) {
            auto atyTy = context.crate.types.path(::HIR::Path(real_type, assoc.second.source_trait.clone(), assoc.first, assoc.second.atyParams.clone()), {});
            for (const auto& tr : assoc.second.traits) {
                applyBoundsAsRulesTrait(context, sp, atyTy, tr, ms_hrl);
            }
        }
    }

    void applyBoundsAsRules(Context& context, const Span& sp, const ::HIR::GenericParams& params_def, const Monomorphiser& ms, bool is_impl_level) {
        TRACE_FUNCTION;
        for (const auto& bound : params_def.bounds) {
            TU_MATCH_HDRA( (bound), {)
            TU_ARMA(Lifetime, be) {
                }
                ...
                TU_ARMA(TraitBound, be) {
                    ...
                    const auto& outer_hrtb = be.hrtbs ? *be.hrtbs : emptyHrtb;
                    DEBUG("Bound for" << outer_hrtb.fmtArgs() << " " << be.type << ":  " << be.trait);
                    ASSERT_BUG(sp, int(!outer_hrtb.is_empty()) + int(!(be.trait.hrtbs ? *be.trait.hrtbs : emptyHrtb).is_empty()) < 2, "Unexpected nested HRTBs: for" << outer_hrtb.fmtArgs() << " " << be.type << ":  " << be.trait);
                    ...
                    auto pp_hrl = (real_trait.hrtbs && !real_trait.hrtbs->is_empty()) ? real_trait.hrtbs->make_empty_params(true) : outer_hrtb.make_empty_params(true);
                    DEBUG("outer_hrb = " << outer_hrtb.fmtArgs() << ", pp_hrl = " << pp_hrl);
                    auto ms_hrl = MonomorphHrlsOnly(context.crate.types, pp_hrl);
                    applyBoundsAsRulesTrait(context, sp, real_type, real_trait, ms_hrl);
                }
                TU_ARMA(TypeEquality, be) {
                    auto real_type_left = context.mResolve.expandAssociatedTypes(sp, ms.monomorph_type(sp, be.type));
                    auto real_type_right = context.mResolve.expandAssociatedTypes(sp, ms.monomorph_type(sp, be.other_type));
                    context.equateTypes(sp, real_type_left, real_type_right);
                }
            }
        }

        for (size_t i = 0; i < params_def.types.size(); i++) {
            if (params_def.types[i].isSized) {
                ::HIR::TypeRef ty = context.crate.types.generic("", (is_impl_level ? 0 : 256) + i);
                context.require_sized(sp, ms.getType(Span(), ty->as_Generic()));
            }
        }
    }
    ...
    bool visit_call_populate_cache(Context& context, const Span& sp, ::HIR::Path& path, ::HIR::ExprCallCache& cache) {
        ...
        MonomorphHrlsOnly(context.crate.types, HIR::PathParams()).monomorph_path(sp, path);
        ...
        struct Monomorph: public Monomorphiser {
            ...
            const HIR::PathParams hrlParams;

            Monomorph(Context& context, const HIR::TypeData* self_ty, const HIR::PathParams* impl_params, const HIR::PathParams& fcn_params, HIR::PathParams hrlParams)
                : Monomorphiser(context.crate.types)
                , context(context)
                , self_ty(self_ty)
                , impl_params(impl_params)
            ...
            ::HIR::TypeRef getType(const Span& sp, const HIR::GenericRef& e) const override {
                if (e.name == "Self" || e.is_self()) {
                    ...
                    if (impl_params) {
                        auto idx = e.idx();
                        ASSERT_BUG(sp, idx < impl_params->types.size(), "Generic param (impl) out of input range - " << e << " >= " << impl_params->types.size());
                        return context.getType(impl_params->types[idx]);
                    } else {
                        BUG(sp, "Impl-level parameter on free function (" << e << ")");
                    }
                } else if (e.binding < 512) {
                    auto idx = e.idx();
                    ASSERT_BUG(sp, idx < fcn_params.types.size(), "Generic param out of input range - " << e << " >= " << fcn_params.types.size());
                    return context.getType(fcn_params.types[idx]);
                } else {
                    BUG(sp, "Generic binding out of total range (" << e << ")");
                }
            }

            ::HIR::ConstGeneric getValue(const Span& sp, const HIR::GenericRef& e) const override {
                if (e.binding < 256) {
                    ASSERT_BUG(sp, impl_params, "Impl-level value parameter on free function (" << e << ")");
                    auto idx = e.idx();
                    ASSERT_BUG(sp, idx < impl_params->values.size(), "Generic value (impl) out of input range - " << e << " >= " << impl_params->values.size());
                    return context.ivars.getValue(impl_params->values[idx]).clone();
                } else if (e.binding < 512) {
                    auto idx = e.idx();
                    ASSERT_BUG(sp, idx < fcn_params.values.size(), "Generic value out of input range - " << e << " >= " << fcn_params.values.size());
                    return context.ivars.getValue(fcn_params.values[idx]).clone();
                } else {
                    BUG(sp, "Generic value bounding out of total range (" << e << ")");
                }
            }

            ::HIR::LifetimeRef getLifetime(const Span& sp, const ::HIR::GenericRef& e) const override {
                if (e.group() == 0) {
                    ASSERT_BUG(sp, impl_params, "Impl-level lifetime parameter on free function (" << e << ")");
                    auto idx = e.idx();
                    ASSERT_BUG(sp, idx < impl_params->mLifetimes.size(), "Generic lifetime (impl) out of input range - " << e << " >= " << impl_params->mLifetimes.size());
                    // If this resolves to a HRL (group 3) then return an empty lifetime
                    auto rv = impl_params->mLifetimes[idx];
                    if (rv.is_hrl()) {
                        return HIR::LifetimeRef();
                    }
                    return rv;
                    ...
                    auto idx = e.idx();
                    ASSERT_BUG(sp, idx < fcn_params.mLifetimes.size(), "Generic lifetime out of input range - " << e << " >= " << fcn_params.mLifetimes.size());
                    return fcn_params.mLifetimes[idx];
                } else if (e.group() == 3) {
                    auto idx = e.idx();
                    ASSERT_BUG(sp, idx < fcn_params.mLifetimes.size(), "Generic lifetime out of input range - " << e << " >= " << hrlParams.mLifetimes.size());
        ...
        TU_ARMA(Generic, e) {
                const auto& fcn = context.crate.getFunctionByPath(sp, e.mPath);
                fixParamCount(sp, context, ::HIR::TypeRef(), false, path, fcn.mParams, e.mParams);
                fcn_ptr = &fcn;
                cache.fcnParams = &fcn.mParams;

                //const auto& params_def = fcn.m_params;
                const auto& path_params = e.mParams;
                cache.monomorph.reset(new Monomorph(context, nullptr, nullptr, path_params, {}));
            }
            TU_ARMA(UfcsKnown, e) {
                const auto& trait = context.crate.getTraitByPath(sp, e.trait.mPath);
                fixParamCount(sp, context, e.type, true, path, trait.mParams, e.trait.mParams);
                ...
                fixParamCount(sp, context, e.type, false, path, fcn.mParams, e.params);
                cache.fcnParams = &fcn.mParams;
            ...
            TU_ARMA(UfcsInherent, e) {
                ...
                if (!visitCallPopulateCacheUfcsInherent(context, sp, path, cache, fcn_ptr)) {
                    return false;
                }
            }
        }
        ...
        cache.monomorph->set_consteval_state(context.crate, HIR::ItemPath(path));
        ...
        for(const auto& arg : fcn.mArgs) {
            ...
            cache.argTypes.push_back(monomorph.monomorph_type(sp, arg.second, false));
        }
        {
            TRACE_FUNCTION_FR("RET " << path << " - " << fcn.returnType, "Ret " << cache.argTypes.back());
            cache.argTypes.push_back(monomorph.monomorph_type(sp, fcn.returnType, false));
        }
    ...
    bool visitCallPopulateCacheUfcsInherent(Context& context, const Span& sp, ::HIR::Path& path, ::HIR::ExprCallCache& cache, const ::HIR::Function*& fcn_ptr) {
        auto& e = path.mData.as_UfcsInherent();
        ...
        assert(impl_ptr);
        DEBUG("Found impl" << impl_ptr->mParams.fmtArgs() << " " << impl_ptr->mType);
        fixParamCount(sp, context, e.type, false, path, fcn_ptr->mParams, e.params);
        ...
        impl_params.mLifetimes.resize(impl_ptr->mParams.mLifetimes.size());
        if (impl_ptr->mParams.is_generic()) {
            while (impl_params.types.size() < impl_ptr->mParams.types.size()) {
                impl_params.types.push_back(context.crate.types.infer());
            }
            impl_params.values.resize(impl_ptr->mParams.values.size());
            OwnedImplMatcher matcher(impl_params);

            auto cmp = impl_ptr->mType->match_test_generics_fuzz(sp, e.type, context.ivars.callbackResolveInfer(), matcher);
            if (cmp == ::HIR::Compare::Fuzzy) {
                ...
                for (auto& ty : impl_params.types) {
                    if (ty->is_Infer() && ty->as_Infer().index == ~0u) {
                        ...
                        ty = context.ivars.new_ivar_tr();
                    }
                }

                context.ivars.addIvarsParams(impl_params);
                ...
                auto impl_monomorph_cb = MonomorphStatePtr(context.crate.types, e.type, &impl_params, nullptr);
                auto impl_ty_mono = impl_monomorph_cb.monomorph_type(sp, impl_ptr->mType, false);
                DEBUG("- impl_ty_mono = " << impl_ty_mono);

                context.equateTypes(sp, impl_ty_mono, e.type);
        ...
        // TODO: Use a copy of `MonomorphStatePtr` that calls `context.get_type`
        cache.monomorph.reset(new MonomorphStatePtr(context.crate.types, e.type, &impl_params, &fcn_params));

        // Add trait bounds for all impl and function bounds
        applyBoundsAsRules(context, sp, impl_ptr->mParams, *cache.monomorph, /*is_impl_level=*/true);

        // Equate `Self` and `impl_ptr->m_type` (after monomorph)
        {
            ::HIR::TypeRef tmp;
            const auto& impl_ty_m = cache.monomorph->maybe_monomorph_type(sp, tmp, impl_ptr->mType);

            context.equateTypes(sp, e.type, impl_ty_m);
        }
    ...
    public:
        ...
        void inner_visit_type(::HIR::TypeRef& ty) {
            rewrite_ty_with(context.crate.types, ty, [this](HIR::TypeRef&, HIR::TypeData& data) -> bool {
                if (auto* te = data.opt_Path()) {
                    if (te->path.mData.is_Generic()) {
                        auto& params = te->path.mData.as_Generic().mParams;
                        const HIR::GenericParams* param_defs = nullptr;
                        TU_MATCH_HDRA( (te->binding), { )
                        TU_ARMA(Struct, pbe)    param_defs = &pbe->mParams;
                            TU_ARMA(Enum, pbe) param_defs = &pbe->mParams;
                            TU_ARMA(Union, pbe) param_defs = &pbe->mParams;
                        ...
                        if(param_defs)
                        {
                            populate_defaults(Span(), context, MonomorphStatePtr(context.crate.types, nullptr, &params, nullptr), *param_defs, params);
                        }
                    }
                }
                return false;
    ...
    class ExprVisitorEnum: public ::HIR::ExprVisitor {
        ...
        struct RetTarget {
            const ::HIR::TypeData* ret_type;
            const ::HIR::TypeData* resume_type;
            ...
            RetTarget(const ::HIR::TypeData* ret_type, const ::HIR::TypeData* resume_type, const ::HIR::TypeData* yield_type)
                : ret_type(ret_type)
        ...
        ::std::vector<::HIR::ExprNodeLoop*> loop_blocks; // Used for `break` type markings

        // TEMP: List of in-scope traits for buildup
        ::HIR::t_trait_list traits;

    public:
        ExprVisitorEnum(Context& context, ::HIR::t_trait_list baseTraits, const ::HIR::TypeData* ret_type)
            : context(context)
            , ret_type(ret_type)
        ...
        void visit(::HIR::ExprNodeBlock& node) override {
            ...
            if (node.valueNode) {
                ...
                this->context.require_sized(snp->span(), snp->resType);
                snp->visit(*this);
                ...
                if (!diverges) {
                    ...
                    } else if (is_diverge(snp->resType)) {
                        diverges = true;
                    } else {
                        diverges = false;
                    }
        ...
        void visit(::HIR::ExprNodeAsm2& node) override {
            ...
            for (auto& v : node.mParams) {
                ...
                TU_ARMA(Const, e) {
                    ...
                    TU_ARMA(Reg, e) {
                        if (e.val_in) {
                            this->context.addIvars(e.val_in->resType);
                            visit_node_ptr(e.val_in);
                        }
                        if (e.val_out) {
                            this->context.addIvars(e.val_out->resType);
        ...
        void visit(::HIR::ExprNodeReturn& node) override {
            ...
            this->context.equateTypesCoerce(node.span(), ret_ty, node.mValue);
        ...
        void visit(::HIR::ExprNodeYield& node) override {
            ...
            if (this->closureRetTypes.empty() || this->closureRetTypes.back().yield_type == nullptr) {
                ERROR(node.span(), E0000, "`yield` outside a generator closure");
            }
            const auto* ret_ty = this->closureRetTypes.back().yield_type;
            const auto* resume_ty = this->closureRetTypes.back().resume_type;
            this->context.equateTypesCoerce(node.span(), ret_ty, node.mValue);

            this->push_inner_coerce(true);
            node.mValue->visit(*this);
            this->pop_inner_coerce();
            this->context.equateTypes(node.span(), node.resType, resume_ty);
        }
        ...
        void visit(::HIR::ExprNodeLoop& node) override {
            ...
            this->context.equateTypes(node.span(), node.mCode->resType, this->context.crate.types.unit());
        ...
        void visit(::HIR::ExprNodeLoopControl& node) override {
            ...
            // Break types
            if (!node.isContinue) {
                ::HIR::ExprNodeLoop* loop_node_ptr;
                if (node.label != "") {
                ...
                if (node.mValue) {
                    ...
                    node.mValue->visit(*this);
                    this->pop_inner_coerce();
                    this->context.equateTypesCoerce(node.span(), loop_node.resType, node.mValue);
                    this->context.require_sized(node.span(), node.mValue->resType);
                } else {
                    this->context.equateTypes(node.span(), loop_node.resType, this->context.crate.types.unit());
                }
            }
            this->context.equateTypes(node.span(), node.resType, this->context.crate.types.diverge());
        }
        ...
        void visit(::HIR::ExprNodeLet& node) override {
            ...
            if (node.mValue) {
                ...
                if (node.mType->is_Infer()) {
                    this->context.equateTypes(node.span(), node.mType, node.mValue->resType);
                    this->push_inner_coerce(true);
                }
                // otherwise coercions apply
                else {
                    this->context.equateTypesCoerce(node.span(), node.mType, node.mValue);
                    this->push_inner_coerce(true);
                }
        ...
        void visit(::HIR::ExprNodeMatch& node) override {
            ...
            auto val_type = this->context.ivars.new_ivar_tr();

            {
                ...
                this->context.equateTypes(node.span(), val_type, node.mValue->resType);
            ...
            for (auto& arm : node.arms) {
                ...
                for (auto& pat : arm.patterns) {
                    this->context.handlePattern(node.span(), pat, val_type);
                }
                ...
                for (auto& c : arm.guards) {
                    ...
                    if (c.is_if) {
                        this->context.equateTypes(c.val->span(), this->context.crate.types.primitive(::HIR::CoreType::Bool), c.val->resType);
                    } else {
                        this->context.handlePattern(node.span(), c.pat, c.val->resType);
                    }
                }

                this->context.addIvars(arm.mCode->resType);
                this->context.equateTypesCoerce(node.span(), node.resType, arm.mCode);
        ...
        void visit(::HIR::ExprNodeAssign& node) override {
            ...
            if (node.op == ::HIR::ExprNodeAssign::Op::None) {
                this->context.equateTypesCoerce(node.span(), node.slot->resType, node.mValue);
            } else {
                ...
                assert(lang_item);
                const auto& trait_path = this->context.crate.getLangItemPathOpt(lang_item);

                auto ty = this->context.ivars.new_ivar_tr();
                this->context.equateTypesCoerce(node.span(), ty, node.mValue);
                if (!trait_path.components().empty()) {
                    this->context.equateTypesAssoc(node.span(), this->context.crate.types.infer(), trait_path, HIR::PathParams(mv$(ty)), node.slot->resType, "", {}, true, operator_kind);
            ...
            auto _2 = this->push_inner_coerce_scoped(node.op == ::HIR::ExprNodeAssign::Op::None);
            node.mValue->visit(*this);
            this->context.require_sized(node.span(), node.mValue->resType);
        ...
        void visit(::HIR::ExprNodeBinOp& node) override {
            ...
            const auto& left_ty = node.left->resType;
            ::HIR::TypeRef right_ty_inner = this->context.ivars.new_ivar_tr();
            const auto& right_ty = right_ty_inner; //node.m_right->m_res_type;
            this->context.equateTypesCoerce(node.span(), right_ty_inner, node.right);
            ...
            switch (node.op) {
                ...
                case ::HIR::ExprNodeBinOp::Op::CmpGtE: {
                    this->context.equateTypes(node.span(), node.resType, this->context.crate.types.primitive(::HIR::CoreType::Bool));
                    ...
                    assert(item_name);
                    const auto& op_trait = this->context.crate.getLangItemPathOpt(item_name);
                    ...
                    if (!op_trait.components().empty()) {
                        this->context.equateTypesAssoc(node.span(), this->context.crate.types.infer(), op_trait, HIR::PathParams(right_ty), left_ty, "", {}, true, operator_kind);
                    } else {
                        this->context.equateTypes(node.span(), left_ty, right_ty);
                    }
                    break;
                }
                ...
                case ::HIR::ExprNodeBinOp::Op::BoolOr:
                    this->context.equateTypes(node.span(), node.resType, this->context.crate.types.primitive(::HIR::CoreType::Bool));
                    this->context.equateTypes(node.span(), left_ty, this->context.crate.types.primitive(::HIR::CoreType::Bool));
                    this->context.equateTypes(node.span(), right_ty, this->context.crate.types.primitive(::HIR::CoreType::Bool));
                    break;
                default: {
                    const char* item_name = nullptr;
                    auto operator_kind = typeck::PrimitiveOperator::None;
                    ...
                    if (!op_trait.components().empty()) {
                        this->context.equateTypesAssoc(node.span(), node.resType, op_trait, HIR::PathParams(right_ty), left_ty, "Output", {}, true, operator_kind);
                    } else {
                        this->context.equateTypes(node.span(), node.resType, left_ty);
        ...
        void visit(::HIR::ExprNodeUniOp& node) override {
            ...
            const auto& op_trait = this->context.crate.getLangItemPathOpt(item_name);
            if (!op_trait.components().empty()) {
                this->context.equateTypesAssoc(node.span(), node.resType, op_trait, ::HIR::PathParams{}, node.mValue->resType, "Output", {}, true, operator_kind);
            } else {
        ...
        void visit(::HIR::ExprNodeIndex& node) override {
            ...
            TRACE_FUNCTION_F(&node << " ... [ ... ]");
            this->context.addIvars(node.mValue->resType);
            node.cache.index_ty = this->context.ivars.new_ivar_tr();
            this->context.addIvars(node.index->resType);

            node.mValue->visit(*this);
            node.index->visit(*this);
            this->context.equateTypesCoerce(node.index->span(), node.cache.index_ty, node.index);
        ...
        void visit(::HIR::ExprNodeDeref& node) override {
            ...
            const auto& ty = this->context.getType(node.mValue->resType);
            const auto& op_trait = this->context.crate.getLangItemPathOpt("deref");
            const ::HIR::TypeData* inner = nullptr;
            bool is_owned_box = false;
            if (const auto* e = ty->opt_Borrow()) {
                inner = e->inner;
            } else if (const auto* e = ty->opt_Pointer()) {
                inner = e->inner;
            } else if (const auto* e = this->context.mResolve.type_is_owned_box(node.span(), ty)) {
                inner = e;
                is_owned_box = true;
            }
            if (!inner) {
            ...
            const bool hasVisibleImpl = !is_owned_box
                && !op_trait.components().empty()
                && !this->context.is_current_native_deref_receiver(op_trait, ty)
                && this->context.mResolve.findTraitImpls(node.span(), op_trait, {}, ty, [&](ImplRef impl, HIR::Compare) {
                    return !this->context.is_current_operator_impl(impl);
                });
            if (hasVisibleImpl) {
                node.traitUsed = ::HIR::ExprNodeDeref::TraitUsed::Trait;
                this->context.equateTypesAssoc(
                    node.span(), node.resType, op_trait, {}, node.mValue->resType,
        ...
        void addIvarsGenericPath(const Span& sp, ::HIR::GenericPath& gp) {
            for (auto& ty : gp.mParams.types) {
                this->context.addIvars(ty);
            }
        }

        void addIvarsPath(const Span& sp, ::HIR::Path& path) {
            TU_MATCH(::HIR::Path::Data, (path.mData), (e), (Generic, this->addIvarsGenericPath(sp, e);), (UfcsKnown, this->context.addIvars(e.type); this->addIvarsGenericPath(sp, e.trait); for (auto& ty : e.params.types) this->context.addIvars(ty);), (UfcsUnknown, TODO(sp, "Hit a UfcsUnknown (" << path << ") - Is this an error?");), (UfcsInherent, this->context.addIvars(e.type); for (auto& ty : e.params.types) this->context.addIvars(ty);))
        }

        ::HIR::TypeRef getStructenumTy(const Span& sp, bool is_struct, ::HIR::GenericPath& gp) {
            if (is_struct) {
                const auto& e = this->context.crate.getTypeitemByPath(sp, gp.mPath);
                if (e.is_Struct()) {
                    const auto& str = e.as_Struct();
                    fixParamCount(sp, this->context, ::HIR::TypeRef(), false, gp, str.mParams, gp.mParams);

                    return this->context.crate.types.path(gp.clone(), ::HIR::TypePathBinding::make_Struct(&str));
                } else if (e.is_Union()) {
                    const auto& u = e.as_Union();
                    fixParamCount(sp, this->context, ::HIR::TypeRef(), false, gp, u.mParams, gp.mParams);

                    return this->context.crate.types.path(gp.clone(), ::HIR::TypePathBinding::make_Union(&u));
                } else {
                    BUG(sp, "Path " << gp << " doesn't refer to a struct/union");
                }
            } else {
                auto s_path = getRuleParentPath(gp.mPath);

                const auto& enm = this->context.crate.getEnumByPath(sp, s_path);
                fixParamCount(sp, this->context, ::HIR::TypeRef(), false, gp, enm.mParams, gp.mParams);

                return this->context.crate.types.path(::HIR::GenericPath(mv$(s_path), gp.mParams.clone()), ::HIR::TypePathBinding::make_Enum(&enm));
            }
        }
        ...
        void visit(::HIR::ExprNodeTupleVariant& node) override {
            ...
            this->context.ivars.addIvarsParams(node.mPath.mParams);
            ...
            TU_ARMA(Unbound, e) {
                ...
                TU_ARMA(Enum, e) {
                    ...
                    ASSERT_BUG(sp, var_ty->as_Path().binding.is_Struct(), "Pointed variant of TupleVariant (" << node.mPath << ") isn't a Tuple");
                    ASSERT_BUG(sp, var_ty->as_Path().binding.as_Struct() != nullptr, "Pointed variant of TupleVariant (" << node.mPath << ") isn't a Tuple");
            ...
            auto monomorph_cb = MonomorphStatePtr(this->context.crate.types, ty, &node.mPath.mParams, nullptr);
            ...
            for( unsigned int i = 0; i < node.mArgs.size(); i ++ )
            {
                ...
                if (monomorphise_type_needed(desTyR)) {
                    node.argTypes[i] = monomorph_cb.monomorph_type(sp, desTyR);
                    desTy = &node.argTypes[i];
                }

                this->context.equateTypesCoerce(node.span(), *desTy, node.mArgs[i]);
            }

            auto _ = this->push_inner_coerce_scoped(true);
            for( auto& val : node.mArgs ) {
                val->visit(*this);
                this->context.require_sized(node.span(), val->resType);
            }
        }
        ...
        void visit(::HIR::ExprNodeStructLiteral& node) override {
            ...
            if (node.realPath == HIR::GenericPath()) {
                this->context.addIvars(node.mType);
            }
            ...
            if (node.realPath == HIR::GenericPath()) {
                auto t = this->context.mResolve.expandAssociatedTypes(sp, mv$(node.mType));
                node.mType = HIR::TypeRef();
                if (node.isStruct) {
                    ASSERT_BUG(sp, TU_TEST1((*t), Path, .path.mData.is_Generic()), "Struct literal with non-Generic path - " << t);
                    node.realPath = t->as_Path().path.mData.as_Generic().clone();
                } else {
                    ASSERT_BUG(sp, TU_TEST1((*t), Path, .path.mData.is_UfcsInherent()), "Enum struct literal with non-UfcsInherent path - " << t);
                    auto& it = t->as_Path().path.mData.as_UfcsInherent().type;
                    auto& name = t->as_Path().path.mData.as_UfcsInherent().item;
                    ASSERT_BUG(sp, TU_TEST1((*it), Path, .path.mData.is_Generic()), "Struct literal with non-Generic path - " << t);
                    node.realPath = it->as_Path().path.mData.as_Generic().clone();
                    node.realPath.mPath += name;
                }
            }
            auto& ty_path = node.realPath;

            // - Create ivars in path, and set result type
            const auto ty = this->getStructenumTy(node.span(), node.isStruct, ty_path);
            this->context.equateTypes(node.span(), node.resType, ty);
            if (const auto* expectedHint = this->context.coercionHint(node)) {
                ...
                if (actualPath && expectedPath
                    && actualPath->path.mData.is_Generic()
                    && expectedPath->path.mData.is_Generic()
                    && actualPath->path.mData.as_Generic().mPath
                        == expectedPath->path.mData.as_Generic().mPath
                    && ty->compareWithPlaceholders(
                           sp,
                           expected,
                           this->context.ivars.callbackResolveInfer()
                       ) != ::HIR::Compare::Unequal) {
            ...
            TU_ARMA(Unbound, e) {
                ...
                */
                ...
                TU_ARMA(Struct, e) {
                    if (e->mData.is_Unit() || e->mData.is_Tuple()) {
                        ASSERT_BUG(node.span(), node.values.size() == 0, "Values provided for " << e->mData.tag_str() << "-like struct");
            ...
            auto monomorph_cb = MonomorphStatePtr(this->context.crate.types, ty, &ty_path.mParams, nullptr);
            ...
            for( auto& val : node.values)
            {
                ...
                DEBUG(name << " : " << desTyR);
                if (monomorphise_type_needed(desTyR)) {
                    if (desTyCache == ::HIR::TypeRef()) {
                        desTyCache = monomorph_cb.monomorph_type(node.span(), desTyR);
                    } else {
            ...
            applyBoundsAsRules(context, node.span(), *generics, monomorph_cb, /*is_impl_level=*/true);

            for( auto& val : node.values ) {
                val.second->visit(*this);
                this->context.require_sized(node.span(), val.second->resType);
            }
            if( node.baseValue ) {
                auto _ = this->push_inner_coerce_scoped(false);
                node.baseValue->visit(*this);
        ...
        void visit(::HIR::ExprNodeCallPath& node) override {
            this->visit_path(node.span(), node.mPath);
            ...
            const bool cacheOk = visit_call_populate_cache(this->context, node.span(), node.mPath, node.cache);
            if (cacheOk) {
                assert(node.cache.argTypes.size() >= 1);
                unsigned int expArgc = node.cache.argTypes.size() - 1;
        ...
        void visit(::HIR::ExprNodeCallValue& node) override {
            ...
            for (auto& val : node.mArgs) {
                this->context.addIvars(val->resType);
                node.argIvars.push_back(this->context.ivars.new_ivar_tr());
            }
        ...
        void visit(::HIR::ExprNodeCallMethod& node) override {
            ...
            auto visit_trait_inner = [&method_name, &max_num_params, &max_num_value_params, &possible_traits](const HIR::SimplePath& p, const HIR::Trait& tr, bool push) {
                auto it = tr.values.find(method_name);
                if (it == tr.values.end()) {
                    return;
                }
                ...
                if (tr.mParams.types.size() > max_num_params) {
                    max_num_params = tr.mParams.types.size();
                }
                if (tr.mParams.values.size() > max_num_value_params) {
                    max_num_value_params = tr.mParams.values.size();
                }
            ...
            auto visit_trait = [&visit_trait_inner](const HIR::SimplePath& p, const HIR::Trait& trait) {
                ...
                for (const auto& pt : trait.allParentTraits) {
                    visit_trait_inner(pt.mPath.mPath, *pt.traitPtr, false);
                }
            };
            ...
            if (context.mResolve.current_trait_path()) {
                const HIR::SimplePath& tp = context.mResolve.current_trait_path()->mPath;
                const HIR::Trait& tr = context.mResolve.crate.getTraitByPath(node.span(), tp);
                visit_trait(tp, tr);
            }
            //  > Store the possible set of traits for later
            node.traits = mv$(possible_traits);
            node.traitParamTypeIvars = max_num_params;
            node.traitParamIvars.reserve(max_num_params + max_num_value_params);
            for (unsigned int i = 0; i < max_num_params; i++) {
                node.traitParamIvars.push_back(this->context.ivars.new_ivar());
            }
            for (unsigned int i = 0; i < max_num_value_params; i++) {
                node.traitParamIvars.push_back(this->context.ivars.new_ivar_val());
            }
            ...
            if (canCoerceInnerResult()) {
                ...
                if (const auto* e = ty->opt_Tuple()) {
                    ...
                    for (const auto& val : node.vals) {
                        (void)val;
                        tuple_tys.push_back(this->context.ivars.new_ivar_tr());
                    }
                    this->context.equateTypes(node.span(), node.resType, this->context.crate.types.tuple(mv$(tuple_tys)));
                } else {
                    // mismatch
                    ERROR(node.span(), E0000, "Tuple literal used where a non-tuple expected - " << ty);
                }
                const auto& inner_tys = this->context.getType(node.resType)->as_Tuple();
                assert(inner_tys.size() == node.vals.size());

                for (unsigned int i = 0; i < inner_tys.size(); i++) {
                    this->context.equateTypesCoerce(node.span(), inner_tys[i], node.vals[i]);
                }
            } else {
                // No inner coerce, just equate the return type.
                ::std::vector<::HIR::TypeRef> tuple_tys;
                for (const auto& val : node.vals) {
                    tuple_tys.push_back(val->resType);
                }
                this->context.equateTypes(node.span(), node.resType, this->context.crate.types.tuple(mv$(tuple_tys)));
            }
        ...
        void visit(::HIR::ExprNodeArrayList& node) override {
            ...
            auto arrayTy = this->context.crate.types.array(context.ivars.new_ivar_tr(), node.vals.size());
            this->context.equateTypes(node.span(), node.resType, arrayTy);
            // Cleanly equate into array (with coercions)
            const auto& inner_ty = arrayTy->as_Array().inner;
            for (auto& val : node.vals) {
                this->equateTypesInnerCoerce(node.span(), inner_ty, val);
            }
        ...
        void visit(::HIR::ExprNodeArraySized& node) override {
            ...
            auto ty = this->context.crate.types.array(context.ivars.new_ivar_tr(), node.mSize.clone());
            this->context.equateTypes(node.span(), node.resType, ty);
            // Equate with coercions
            const auto& inner_ty = ty->as_Array().inner;
            this->equateTypesInnerCoerce(node.span(), inner_ty, node.val);
        ...
        void visit(::HIR::ExprNodeLiteral& node) override {
            ...
            TU_ARMA(Integer, e) {
                    DEBUG("_Literal (: " << e.mType << " = " << e.mValue << ")");
                    if (e.mType != ::HIR::CoreType::Str) {
                        ty = this->context.crate.types.primitive(e.mType);
                    } else {
                        ty = this->context.crate.types.infer(~0, ::HIR::InferClass::Integer);
                    }
                }
                TU_ARMA(Float, e) {
                    DEBUG("_Literal (: " << node.resType << " = " << e.mValue << ")");
                    if (e.mType != ::HIR::CoreType::Str) {
                        ty = this->context.crate.types.primitive(e.mType);
                    } else {
                        ty = this->context.crate.types.infer(~0, ::HIR::InferClass::Float);
                    }
                }
                TU_ARMA(Boolean, e) {
                    DEBUG("_Literal ( " << (e ? "true" : "false") << ")");
                    ty = this->context.crate.types.primitive(::HIR::CoreType::Bool);
                }
                TU_ARMA(String, e) {
                    // TODO: &'static
                    DEBUG("_Literal (&str)");
                    ty = this->context.crate.types.borrow(::HIR::BorrowType::Shared, this->context.crate.types.primitive(::HIR::CoreType::Str), ::HIR::LifetimeRef::new_static());
                }
                TU_ARMA(ByteString, e) {
                    // TODO: &'static
                    DEBUG("_Literal (&[u8])");
                    ty = this->context.crate.types.borrow(::HIR::BorrowType::Shared, this->context.crate.types.array(this->context.crate.types.primitive(::HIR::CoreType::U8), e.size()), ::HIR::LifetimeRef::new_static());
                }
                TU_ARMA(CString, e) {
                    DEBUG("_Literal (&CStr)");
                    ...
                    ty = this->context.crate.types.borrow(::HIR::BorrowType::Shared, ty, ::HIR::LifetimeRef::new_static());
                }
            }
            this->context.addIvars(ty);
            this->context.equateTypes(node.span(), node.resType, ty);
        ...
        void visit(::HIR::ExprNodePathValue& node) override {
            const auto& sp = node.span();
            this->visit_path(sp, node.mPath);
            TRACE_FUNCTION_F(&node << " " << node.mPath);

            this->addIvarsPath(node.span(), node.mPath);

            TU_MATCH_HDRA( (node.mPath.mData), {)
            TU_ARMA(Generic, e) {
                    switch (node.target) {
                        case ::HIR::ExprNodePathValue::UNKNOWN:
                            BUG(sp, "_PathValue with target=UNKNOWN and a Generic path - " << e.mPath);
                        case ::HIR::ExprNodePathValue::FUNCTION: {
                            const auto& f = this->context.crate.getFunctionByPath(sp, e.mPath);
                            fixParamCount(sp, this->context, ::HIR::TypeRef(), false, e, f.mParams, e.mParams);

                            auto ms = MonomorphStatePtr(this->context.crate.types, nullptr, nullptr, &e.mParams);
                            auto ty = this->context.crate.types.intern(HIR::TypeData::make_NamedFunction({node.mPath.clone(), &f}));

                            // Apply bounds
                            applyBoundsAsRules(this->context, sp, f.mParams, ms, /*is_impl_level=*/false);

                            DEBUG("> " << node.mPath << " = " << ty);
                            this->context.equateTypes(sp, node.resType, ty);
                        } break;
                        case ::HIR::ExprNodePathValue::STRUCT_CONSTR: {
                            const auto& s = this->context.crate.getStructByPath(sp, e.mPath);
                            //const auto& se = s.m_data.as_Tuple();
                            fixParamCount(sp, this->context, ::HIR::TypeRef(), false, e, s.mParams, e.mParams);
                            ...
                            auto ty = this->context.crate.types.intern(HIR::TypeData::make_NamedFunction({node.mPath.clone(), &s}));

                            this->context.equateTypes(sp, node.resType, ty);
                        } break;
                        case ::HIR::ExprNodePathValue::ENUM_VAR_CONSTR: {
                            const auto& var_name = e.mPath.components().back();
                            auto enumPath = getRuleParentPath(e.mPath);
                            const auto& enm = this->context.crate.getEnumByPath(sp, enumPath);
                            fixParamCount(sp, this->context, ::HIR::TypeRef(), false, e, enm.mParams, e.mParams);
                            size_t idx = enm.findVariant(var_name);
                            ASSERT_BUG(sp, idx != SIZE_MAX, "Missing variant - " << e.mPath);
                            ...
                            auto ty = this->context.crate.types.intern(HIR::TypeData::make_NamedFunction({node.mPath.clone(), HIR::TypeDataNamedFunctionTy::make_EnumConstructor({&enm, idx})}));
                            this->context.equateTypes(sp, node.resType, ty);
                        } break;
                        case ::HIR::ExprNodePathValue::STATIC: {
                            const auto& v = this->context.crate.getStaticByPath(sp, e.mPath);
                            DEBUG("static v.m_type = " << v.mType);
                            this->context.equateTypes(sp, node.resType, v.mType);
                        } break;
                        case ::HIR::ExprNodePathValue::CONSTANT: {
                ...
                TU_ARMA(UfcsKnown, e) {
                    const auto& trait = this->context.crate.getTraitByPath(sp, e.trait.mPath);
                    fixParamCount(sp, this->context, e.type, true, e.trait, trait.mParams, e.trait.mParams);

                    // 1. Add trait bound to be checked.
                    this->context.addTraitBound(sp, e.type, e.trait.mPath, e.trait.mParams.clone());
                    ...
                    if (it == trait.values.end()) {
                        ERROR(sp, E0000, "`" << e.item << "` is not a value member of trait " << e.trait.mPath);
                    }
                TU_MATCH_HDRA( (it->second), {)
                TU_ARMA(Constant, ie) {
                            auto ms = MonomorphStatePtr(this->context.crate.types, e.type, &e.trait.mParams, nullptr);
                            ::HIR::TypeRef tmp;
                            const auto& ty = ms.maybe_monomorph_type(sp, tmp, ie.mType);
                            this->context.equateTypes(sp, node.resType, ty);
                        }
                        TU_ARMA(Static, ie) {
                            TODO(sp, "Monomorpise associated static type - " << ie.mType);
                        }
                        TU_ARMA(Function, ie) {
                            ...
                            auto ty = this->context.crate.types.intern(HIR::TypeData::make_NamedFunction({node.mPath.clone(), &ie}));
                            this->context.equateTypes(node.span(), node.resType, ty);
                        }
                }
                }
                TU_ARMA(UfcsInherent, e) {
                    ...
                    const ::HIR::Function* fcn_ptr = nullptr;
                    const ::HIR::Constant* constPtr = nullptr;
                    const ::HIR::TypeImpl* impl_ptr = nullptr;
                    ...
                    if (count == 0) {
                        ERROR(sp, E0000, "Failed to locate associated value " << node.mPath);
                    }
                    if (count > 1) {
                        TODO(sp, "Revisit _PathValue when UfcsInherent has multiple options - " << node.mPath);
                    }

                    assert(fcn_ptr || constPtr);
                    assert(impl_ptr);

                    if (fcn_ptr) {
                        fixParamCount(sp, this->context, e.type, false, node.mPath, fcn_ptr->mParams, e.params);
                    } else {
                        fixParamCount(sp, this->context, e.type, false, node.mPath, constPtr->mParams, e.params);
                    }
                    ...
                    if (impl_ptr->mParams.is_generic()) {
                        while (impl_params.types.size() < impl_ptr->mParams.types.size()) {
                            impl_params.types.push_back(this->context.crate.types.infer());
                        }
                        impl_params.values.resize(impl_ptr->mParams.values.size());
                        OwnedImplMatcher matcher(impl_params);
                        // NOTE: Could be fuzzy.
                        bool r = impl_ptr->mType->match_test_generics(sp, e.type, this->context.ivars.callbackResolveInfer(), matcher);
                        for (auto& ty : impl_params.types) {
                        ...
                        if (!r) {
                            auto t = MonomorphStatePtr(this->context.crate.types, nullptr, &impl_params, nullptr).monomorph_type(sp, impl_ptr->mType);
                    ...
                    if (fcn_ptr) {
                        ...
                        auto ms = MonomorphStatePtr(this->context.crate.types, e.type, &impl_params, &fcn_params);

                        // Bounds (both impl and fn)
                        applyBoundsAsRules(this->context, sp, impl_ptr->mParams, ms, /*is_impl_level=*/true);
                        applyBoundsAsRules(this->context, sp, fcn_ptr->mParams, ms, /*is_impl_level=*/false);

                        auto ty = this->context.crate.types.intern(HIR::TypeData::make_NamedFunction({node.mPath.clone(), fcn_ptr}));
                        this->context.equateTypes(node.span(), node.resType, ty);
                    } else // !fcn_ptr, ergo const_ptr
                    {
                        assert(constPtr);
                        auto monomorph_cb = MonomorphStatePtr(this->context.crate.types, e.type, &impl_params, &e.params);

                        ::HIR::TypeRef tmp;
                        const auto& ty = monomorph_cb.maybe_monomorph_type(sp, tmp, constPtr->mType);

                        applyBoundsAsRules(this->context, sp, impl_ptr->mParams, monomorph_cb, /*is_impl_level=*/true);
                        this->context.equateTypes(node.span(), node.resType, ty);
                    }
                }
            }
        ...
        void visit(::HIR::ExprNodeVariable& node) override {
            TRACE_FUNCTION_F(&node << " " << node.mName << "{" << node.slot << "}");

            this->context.equateTypes(node.span(), node.resType, this->context.getVar(node.span(), node.slot));
        }

        void visit(::HIR::ExprNodeConstParam& node) override {
            TRACE_FUNCTION_F(&node << " " << node.mName << "{" << node.mBinding << "}");

            this->context.equateTypes(node.span(), node.resType, this->context.mResolve.getConstParamType(node.span(), node.mBinding));
        }
    ...
    private:
        ...
        void visit_path(const Span& sp, ::HIR::Path& path) {
            TU_MATCH(::HIR::Path::Data, (path.mData), (e), (Generic, this->visit_generic_path(sp, e);), (UfcsKnown, this->context.addIvars(e.type); this->visit_generic_path(sp, e.trait); for (auto& ty : e.params.types) this->context.addIvars(ty);), (UfcsUnknown, TODO(sp, "Hit a UfcsUnknown (" << path << ") - Is this an error?");), (UfcsInherent, this->context.addIvars(e.type); for (auto& ty : e.params.types) this->context.addIvars(ty);))
        }
        ...
        void equateTypesInnerCoerce(const Span& sp, const ::HIR::TypeData* target, ::HIR::ExprNodeP& node) {
...
void TypecheckCodeCSEnumerateRules(Context& context, const typeck::ModuleState& ms, t_args& args, const ::HIR::TypeData* result_type, ::HIR::ExprPtr& expr, ::HIR::ExprNodeP& root_ptr) {
    TRACE_FUNCTION;
    ...
    struct M: public Monomorphiser {
        Context& context;
        ::HIR::ExprPtr& expr;
        mutable const ::HIR::TypeData* curSelf;
        ...
        ::HIR::TypeRef monomorph_type(const Span& sp, const ::HIR::TypeData* tpl, bool allowInfer = true) const override {
            if (const auto* e = tpl->opt_ErasedType()) {
                if (const auto* ee = e->inner.opt_Fcn()) {
                    ...
                    ASSERT_BUG(sp, expr.erasedTypes[ee->index] == HIR::TypeRef(), "Multiple-visits to erased type #" << ee->index);
                    expr.erasedTypes[ee->index] = context.ivars.new_ivar_tr();
                    auto rv = expr.erasedTypes[ee->index];

                    auto prev_cur_self = this->curSelf;
                    this->curSelf = rv;
                    DEBUG(tpl << " -> " << rv);

                    auto prev_hrls = this->hrls;
                    for (const auto& trait : e->traits) {
                        auto pp_hrl = trait.hrtbs ? trait.hrtbs->make_empty_params(true) : HIR::PathParams();
                        this->hrls = &pp_hrl;
                        if (trait.typeBounds.size() == 0) {
                            context.equateTypesAssoc(sp, context.crate.types.infer(), trait.mPath.mPath, this->monomorph_path_params(sp, trait.mPath.mParams, allowInfer), rv, "", {}, false);
                        } else {
                            for (const auto& aty : trait.typeBounds) {
                                auto atyCloned = this->monomorph_type(sp, aty.second.type);
                                auto params = this->monomorph_path_params(sp, trait.mPath.mParams, allowInfer);
                                context.equateTypesAssoc(sp, std::move(atyCloned), trait.mPath.mPath, std::move(params), rv, aty.first.c_str(), aty.second.atyParams, false);
                            }
                        }
                        this->hrls = nullptr;
                    }
    ...
    if (true) {
        ...
        context.addIvars(root_ptr->resType);
        root_ptr->visit(visitor);
    }

    DEBUG("--- Enumerating");
    context.record_coercion_hint(new_res_ty, root_ptr);
    typecheck::ExprVisitorEnum visitor(context, ms.traits, new_res_ty);
    context.addIvars(root_ptr->resType);
    root_ptr->visit(visitor);

    DEBUG("Return type = " << new_res_ty << ", root_ptr = " << root_ptr->type_name() << " " << root_ptr->resType);
    context.equateTypesCoerce(sp, new_res_ty, root_ptr);
}
...
bool Context::IVarPossible::hasRules() const {
    ...
    if (forceNoTo || !types_coerce_to.empty()) {
        return true;
    }
    if (forceNoFrom || !types_coerce_from.empty()) {
        return true;
    }
...
Context::Context(const ::HIR::Crate& crate, const ::HIR::GenericParams* impl_params, const ::HIR::GenericParams* item_params, const ::HIR::SimplePath& mod_path, const ::HIR::GenericPath* current_trait, const ::HIR::TraitImpl* current_trait_impl)
    ...
    , ivars(crate.types)
    , mResolve(ivars, crate, impl_params, item_params, mod_path, current_trait)
    , next_rule_idx(0)
    , mLangBox(crate.getLangItemPathOpt("owned_box")) {
    mResolve.set_inherent_type_constraint([this](const Span& sp, const ::HIR::TypeData* receiver, const ::HIR::TypeData* impl_type) {
        this->equateTypesInner(sp, receiver, impl_type);
    });
}
const ::HIR::TypeData* Context::coercionHint(const ::HIR::ExprNode& node) const {
...
bool Context::is_current_native_deref_receiver(const ::HIR::SimplePath& derefTrait, const ::HIR::TypeData* operand) const {
    const auto* current_trait = mResolve.current_trait_path();
    const auto& ty = ivars.getType(operand);
    const auto* borrow = ty->opt_Borrow();
    return currentTraitImpl
        && current_trait
        && current_trait->mPath == derefTrait
        && borrow
        && typeck::primitive_operator_has_builtin(typeck::PrimitiveOperator::Deref, borrow->inner)
        && currentTraitImpl->matches_type(borrow->inner, ivars.callbackResolveInfer());
}

::std::ostream& operator<<(::std::ostream& os, const Context::Coercion& v) {
    os << "R" << v.rule_idx << " " << v.left_ty << " := " << v.right_node_ptr << " " << &**v.right_node_ptr << " (" << (*v.right_node_ptr)->resType << ")";
    return os;
}
::std::ostream& operator<<(::std::ostream& os, const Context::Associated& v) {
    ...
    if (v.name == "") {
        ...
        os << v.left_ty << " = " << "< `" << v.impl_ty << "` as `" << v.trait << v.params << "` >::" << v.name << v.atyPp;
    }
    if (v.is_operator) {
        os << " - op";
    }
