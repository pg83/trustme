...
namespace {
    ...
    class CanonicalizeTraitGoal final: public Monomorphiser {
        ...
        RcString canonicalPlaceholderName(const RcString& name) const {
            ...
            auto canonical = RcString::newInterned(
    ...
    struct MatchHrls: public HIR::MatchGenerics, public Monomorphiser {
        ...
        virtual ::HIR::Compare matchTy(const ::HIR::GenericRef& g, const ::HIR::TypeData* ty, HIR::t_cb_resolve_type resolve_cb) {
    ...
    HIR::PathParams getHrls(HIR::TypeInterner& types, const Span& sp, const ::HIR::GenericParams& x, const ::HIR::PathParams& trait_pps, const ::HIR::PathParams& desPps) {
        return getHrls(types, sp, &x, trait_pps, desPps);
    }
    ...
    HIR::PathParams getHrls(HIR::TypeInterner& types, const Span& sp, const ::std::unique_ptr<::HIR::GenericParams>& x, const ::HIR::PathParams& trait_pps, const ::HIR::PathParams& desPps) {
        return getHrls(types, sp, x.get(), trait_pps, desPps);
    }
}
...
void HMTypeInferrence::print_type(::std::ostream& os, const ::HIR::TypeData* tr, LList<const ::HIR::TypeData*> outer_stack) const {
    ...
    TU_ARMA(Infer, e) {
        ...
        TU_ARMA(Closure, nodeP) {
                    ...
                    os << ")->";
                    this->print_type(os, nodeP->returnType, stack);
                }
...
void HMTypeInferrence::addIvars(::HIR::TypeRef& type) {
    if (type->is_Infer() && type->as_Infer().index == ~0u) {
        type = newIvarTr(type->as_Infer().ty_class);
        this->markChange();
        DEBUG("New ivar " << type);
        return;
    }
...
void HMTypeInferrence::ivarValUnify(unsigned int leftSlot, unsigned int right_slot) {
    Span sp;
    ASSERT_BUG(sp, leftSlot < values.size(), "slot " << leftSlot << " >= " << values.size());
    ASSERT_BUG(sp, right_slot < values.size(), "slot " << leftSlot << " >= " << values.size());
    ASSERT_BUG(sp, !values[leftSlot].isAlias(), "slot " << leftSlot);
    ASSERT_BUG(sp, !values[right_slot].isAlias(), "slot " << right_slot);

    if (/*const auto* re =*/values[right_slot].val->opt_Infer()) {
        DEBUG("Set ValIVar " << right_slot << " = @" << leftSlot);
        values[right_slot].alias = leftSlot;
...
void HMTypeInferrence::set_ivar_to(unsigned int slot, ::HIR::TypeRef type) {
    auto sp = Span();
    auto& root_ivar = this->getPointedIvar(slot);
    ...
    if (const auto* lE = type->opt_Infer()) {
        ...
        if (lE->ty_class != ::HIR::InferClass::None) {
            TU_MATCH_DEF(
                ...
                (Primitive, checkTypeClassPrimitive(sp, type, lE->ty_class, e);),
                (Infer,
                 // Check for right having a ty_class
                 if (e.ty_class != ::HIR::InferClass::None && e.ty_class != lE->ty_class) { ERROR(sp, E0000, "Unifying types with mismatching literal classes - " << type << " := " << root_ivar.type); })
            )
        }
        ...
        auto& r_ivar = this->getPointedIvar(lE->index);
        r_ivar.alias = slot;
        r_ivar.type = nullptr;
#else
        DEBUG("Set IVar " << slot << " = @" << lE->index);
        root_ivar.alias = lE->index;
            ...
            switch (e->ty_class) {
                ...
                case ::HIR::InferClass::Float:
                    ...
                    if (const auto* lE = type->opt_Primitive()) {
                        checkTypeClassPrimitive(sp, type, e->ty_class, *lE);
...
void HMTypeInferrence::ivarUnify(unsigned int leftSlot, unsigned int right_slot) {
    auto sp = Span();
    if (leftSlot != right_slot) {
        auto& leftIvar = this->getPointedIvar(leftSlot);

        // TODO: Assert that setting this won't cause a loop.
        auto& root_ivar = this->getPointedIvar(right_slot);

        if (const auto* re = root_ivar.type->opt_Infer()) {
            DEBUG("Class unify " << leftIvar.type << " <- " << root_ivar.type);
            ...
            if (re->ty_class != ::HIR::InferClass::None) {
                if (const auto* le = leftIvar.type->opt_Infer()) {
                    if (le->ty_class != ::HIR::InferClass::None && le->ty_class != re->ty_class) {
                        ERROR(sp, E0000, "Unifying types with mismatching literal classes - " << leftIvar.type << " := " << root_ivar.type);
                    }
                    if (le->ty_class == ::HIR::InferClass::None) {
                        leftIvar.type = types.infer(le->index, re->ty_class);
                    }
                } else if (const auto* le = leftIvar.type->opt_Primitive()) {
                    checkTypeClassPrimitive(sp, leftIvar.type, re->ty_class, *le);
                } else {
        ...
        DEBUG("IVar " << root_ivar.type->as_Infer().index << " = @" << leftSlot);
        root_ivar.alias = leftSlot;
        root_ivar.type = nullptr;
...
bool HMTypeInferrence::pathparams_contain_ivars(const ::HIR::PathParams& pps, bool onlyUnbound) const {
    for (const auto& ty : pps.types) {
        if (this->type_contains_ivars(ty, onlyUnbound)) {
            return true;
        }
    }
    return false;
...
bool HMTypeInferrence::type_contains_ivars(const ::HIR::TypeData* ty, bool onlyUnbound) const {
    if (!ty->hasTypeInfer()) {
        return false;
    }
    TRACE_FUNCTION_F("ty = " << ty);
    auto path_contains_ivars = [this](const HIR::Path& path, bool onlyUnbound) {
        TU_MATCH(::HIR::Path::Data, (path.mData), (pe), (Generic, return this->pathparams_contain_ivars(pe.mParams, onlyUnbound);), (UfcsKnown, if (this->type_contains_ivars(pe.type, onlyUnbound)) return true; if (this->pathparams_contain_ivars(pe.trait.mParams, onlyUnbound)) return true; return this->pathparams_contain_ivars(pe.params, onlyUnbound);), (UfcsInherent, if (this->type_contains_ivars(pe.type, onlyUnbound)) return true; return this->pathparams_contain_ivars(pe.params, onlyUnbound);), (UfcsUnknown, BUG(Span(), "UfcsUnknown");))
        throw "";
    };
    ...
    (Path,
        return path_contains_ivars(e.path, onlyUnbound);
        ),
    (Borrow,
        return type_contains_ivars(e.inner, onlyUnbound);
        ),
    (Pointer,
        return type_contains_ivars(e.inner, onlyUnbound);
        ),
    ...
    (Function,
        for(const auto& arg : e.argTypes)
            if( type_contains_ivars(arg, onlyUnbound) )
                return true;
        return type_contains_ivars(e.mRettype, onlyUnbound);
        ),
    (TraitObject,
        for(const auto& marker : e.markers)
            if( pathparams_contain_ivars(marker.mParams, onlyUnbound) )
                return true;
        return pathparams_contain_ivars(e.mTrait.mPath.mParams, onlyUnbound);
        ),
    (ErasedType,
        TU_MATCH_HDRA( (e.inner), {)
        TU_ARMA(Fcn, ee) {
            return path_contains_ivars(ee.origin, onlyUnbound);
}

TU_ARMA(Known, ee) {
    return type_contains_ivars(ee, onlyUnbound);
}
...
TU_ARMA(Alias, ee) {
    ...
    (Tuple,
        for(const auto& st : e)
            if( type_contains_ivars(st, onlyUnbound) )
                return true;
        return false;
        )
    )
    ...
    throw "";
        ...
        namespace {
            ::HIR::Compare compareValue(const Span& sp, const ::HIR::ConstGeneric& leftRaw, const ::HIR::ConstGeneric& right_raw, const HMTypeInferrence& infer) {
                const auto& left = leftRaw.is_Infer() ? infer.getValue(leftRaw.as_Infer().index) : leftRaw;
                const auto& right = right_raw.is_Infer() ? infer.getValue(right_raw.as_Infer().index) : right_raw;
                if (left == right) {
                    return ::HIR::Compare::Equal;
                }
        ...
        //
        // -------------------------------------------------------------------------------------------------------------------
        bool TraitResolution::iterateBoundsTraits(const Span& sp, const HIR::TypeData* type, const HIR::SimplePath& trait, t_cb_bound cb) const {
        ...
        bool TraitResolution::iterateBoundsTraits(const Span& sp, const HIR::TypeData* type, t_cb_bound cb) const {
        ...
        bool TraitResolution::iterateBoundsTraits(const Span& sp, t_cb_bound cb) const {
            for (const auto& b : traitBounds) {
        ...
        bool TraitResolution::iterateAtyBounds(const Span& sp, const ::HIR::Path::Data::Data_UfcsKnown& pe, ::std::function<bool(const ::HIR::TraitPath&)> cb) const {
            ...
            if (!this->trait_contains_type(sp, pe.trait, this->crate.getTraitByPath(sp, pe.trait.mPath), pe.item.c_str(), trait_path)) {
                BUG(sp, "Cannot find associated type " << pe.item << " anywhere in trait " << pe.trait);
            }
            DEBUG("trait_path=" << trait_path);
            const auto& trait_ref = crate.getTraitByPath(sp, trait_path.mPath);
        ...
        bool TraitResolution::findTraitImplsMagic(const Span& sp, const ::HIR::SimplePath& trait, const ::HIR::PathParams& params, const ::HIR::TypeData* ty, t_cb_trait_impl_r callback) const {
                ...
                // Structs: Can delegate their metadata
                    ...
                    switch (str.structMarkings.dst_type) {
                        ...
                        case HIR::StructMarkings::DstType::TraitObject: {
                            ...
                            tail_ty = this->expandAssociatedTypes(sp, std::move(tail_ty));

                            return this->findTraitImpls(sp, trait, params, tail_ty, [&](ImplRef impl, HIR::Compare cmp) {
            ...
            if (trait == mLangUnsize) {
                ...
                bool rv = false;
                auto cb = [&](auto newDst) {
                    ::HIR::PathParams real_params{mv$(newDst)};
        ...
        bool TraitResolution::findTraitImplsTypes(const Span& sp, const ::HIR::SimplePath& trait, const ::HIR::PathParams& params, const ::HIR::TypeData* type, t_cb_trait_impl_r callback) const {
    TU_MATCH_HDRA( (*type), {)
    default:
        break;
        TU_ARMA(NodeType, e) {
            ...
            for (const auto& trait_path : e.traits) {
                if (trait == trait_path.mPath.mPath) {
                    ...
                    if (cmp != ::HIR::Compare::Unequal) {
                        ...
                        auto hrls = getHrls(crate.types, sp, trait_path.hrtbs, trait_path.mPath.mParams, params);
                ...
                bool rv = false;
                bool isSupertrait = false;
                this->findNamedTraitInTrait(sp, trait, params, *trait_path.traitPtr, trait_path.mPath.mPath, trait_path.mPath.mParams, type, [&](const HIR::TraitPath& iTp) {
                    // The above is just the monomorphised params and associated set. Comparison is still needed.
                    auto cmp = this->comparePp(sp, iTp.mPath.mParams, params);
                    if (cmp != ::HIR::Compare::Unequal) {
                        // Invoke callback with a proper ImplRef
                        ...
                        ASSERT_BUG(sp, !trait_path.hrtbs || !iTp.hrtbs, "TODO: Handle two layers of HRTBs - " << trait_path << " and " << iTp);
                        auto hrls = trait_path.hrtbs ? getHrls(crate.types, sp, trait_path.hrtbs, iTp.mPath.mParams, params) : getHrls(crate.types, sp, iTp.hrtbs, iTp.mPath.mParams, params);
                        auto ir = ImplRef(std::move(hrls), type, iTp.mPath.mParams.clone(), mv$(assocClone));
        ...
        TU_ARMA(Path, e) {
            if (e.binding.is_Opaque()) {
                ...
                auto rv = this->iterateAtyBounds(sp, pe, [&](const HIR::TraitPath& bound) {
                    ...
                    auto pp_hrb = hrlsDef.makeEmptyParams(true);
                    monomorphCb.pp_hrb = &pp_hrb;
                    ...
                    if (monomorphisePathparamsNeeded(bParams)) {
                        params_mono_o = monomorphCb.monomorphPathParams(sp, bParams, false);
                    ...
                    for (const auto& aty : bound.typeBounds) {
                        bAtys.insert(::std::make_pair(aty.first, ::HIR::TraitPath::AtyEqual{monomorphCb.monomorphGenericpath(sp, aty.second.source_trait, false), {}, monomorphCb.monomorphType(sp, aty.second.type)}));
                    }
    ...
    if( search_bounds && findTraitImplsBound(sp, trait, params, type, callback) )
        return true;
...
class NextTraitGoalEvaluator {
        ...
        CanonicalGoal canonicalizeGoal(
            ...
            const auto canonicalType = canonicalizer.monomorphType(
                span(), type, true
            );
                ...
                for (const auto& entry : *associated) {
                    result.associated.insert({
                        ...
                        ::HIR::TraitPath::AtyEqual{
                            ...
                            canonicalizer.monomorphPathParams(
                                span(), entry.second.atyParams, true
                            ),
                            canonicalizer.monomorphType(
        ...
        ::std::optional<size_t> availableDepthForNested() {
            ...
            return parent.encounteredOverflow
        ...
        bool candidateHasPlaceholders(const Candidate& candidate) const {
            if (type_has_candidate_placeholder(candidate.impl.getImplType(crate.types))
                || params_have_candidate_placeholders(
                    candidate.impl.getTraitParams(crate.types)
                )) {
                return true;
            }
        ...
        bool candidateNeedsResponseConstraints(
            ...
            return candidate.markerImpl
                && params_need_response_constraints(candidate.markerImplParams);
        }
        ...
        bool orphanCheckTraitRef(
            ...
            const auto self_result = orphanVisitType(type, perspective);
        ...
        bool trait_ref_is_knowable(
            ...
            const auto& trait_def = crate.getTraitByPath(span(), trait);
            if (trait.crate_name() == crate.crateName
                || trait_def.isFundamental) {
                return true;
            }

            return orphanCheckTraitRef(
        ...
        static size_t hashType(const ::HIR::TypeData* type) {
            ...
            if (const auto* trait_object = type->opt_TraitObject()) {
                return hashMix(0xa0, hashSimplePath(trait_object->mTrait.mPath.mPath));
            }
            if (type->is_Diverge()) {
                return 0xb0;
            }
        ...
        static size_t goalHash(
            ...
            if (associated && !associated->empty()) {
                ...
                for (const auto& entry : *associated) {
                    ...
                    result = hashMix(result, hashSimplePath(entry.second.source_trait.mPath));
        ...
        ImplRef monomorphImplRef(
            ...
            if (const auto* impl = source.mData.opt_TraitImpl()) {
                ...
                result = ImplRef(
                    ...
                    monomorph.monomorphType(span(), bounded->type, true),
                    monomorph.monomorphPathParams(
                        span(), *bounded->trait_args, true
                    ),
                    monomorphAssociated(bounded->assoc)
                );
            } else {
                ...
                result = ImplRef(
                    ...
                    monomorph.monomorphType(span(), owned.type, true),
                    monomorph.monomorphPathParams(
                        span(), owned.trait_args, true
                    ),
                    monomorphAssociated(&owned.assoc)
                );
            }
        ...
        bool isSameImpl(const ImplRef& left, const ImplRef& right) const {
            ...
            return left.getImplType(crate.types) == right.getImplType(crate.types)
                && left.getTraitParams(crate.types) == right.getTraitParams(crate.types)
        ...
        bool param_env_candidate_is_non_global(const Candidate& candidate) const {
            ...
            if (type_has_unknown(candidate.impl.getImplType(crate.types))
        ...
        void push_candidate(
            ...
            for (size_t i = 0; i < candidates.size(); i++) {
                const bool same_source = candidates[i]->markerImpl == markerImpl
                    && candidates[i]->autoBuiltin == autoBuiltin
                    && candidates[i]->source == source;
                const bool same = markerImpl
                    ? same_source
                        && candidates[i]->markerImplParams == markerImplParams
                    : same_source && isSameImpl(candidates[i]->impl, impl);
                if (same) {
                ...
                crate.findAutoTraitImpls(
                    ...
                    [&](const ::HIR::MarkerImpl& impl) {
                        ...
                        if (match != ::HIR::Compare::Unequal) {
                            ...
                            auto response_type = monomorph.monomorphType(
                                span(), impl.mType, false
                            );
                            auto response_params = monomorph.monomorphPathParams(
        ...
        ::HIR::TypeRef makeAssociatedProjection(
            ...
            return makeAssociatedProjection(
                impl.getImplType(crate.types), source_trait, name, associatedParams
            );
        }
        ...
        bool bindCandidatePlaceholders(
            ...
            for (const auto& requirement : associated) {
                ...
                if (!use_candidate_response
                    ...
                    candidateOutput = mResolve.expandAssociatedTypes(
                        span(), ::std::move(candidateOutput)
                    );
                }
                const auto match = (use_candidate_response
                    ? candidateOutput
                    : requirement.second.type)->matchTestGenericsFuzz(
                    span(),
            ...
            if (binder.changed && candidate.markerImpl) {
                auto monomorph = MonomorphStatePtr(
                    crate.types, nullptr, &candidate.markerImplParams, nullptr
                );
                auto& response = candidate.impl.mData.as_Bounded();
                response.type = monomorph.monomorphType(
                    span(), candidate.markerImpl->mType, false
                );
                response.trait_args = monomorph.monomorphPathParams(
                    span(), candidate.markerImpl->traitArgs, false
                );
            }
        ...
        bool bindCandidateResponse(
            ...
            const auto saved = candidateParams->clone();
            auto match = nestedType->matchTestGenericsFuzz(
                span(),
                response.getImplType(crate.types),
                mResolve.ivars.callbackResolveInfer(),
                binder
            );
            match &= nestedParams.matchTestGenericsFuzz(
                span(),
                response.getTraitParams(crate.types),
                mResolve.ivars.callbackResolveInfer(),
                binder
            );
            if (match == ::HIR::Compare::Unequal) {
            ...
            if (binder.changed && candidate.markerImpl) {
                ...
                bounded.type = monomorph.monomorphType(
                    span(), candidate.markerImpl->mType, false
                );
                bounded.trait_args = monomorph.monomorphPathParams(
                    span(), candidate.markerImpl->traitArgs, false
                );
            }
            return binder.changed;
        ...
        Certainty evaluateCandidate(
            ...
            if (associated) {
                bindCandidatePlaceholders(
                    *candidate,
                    candidate->impl.getImplType(crate.types),
                    *associated,
                    true
                );
            }
            ...
            if (autoBuiltin) {
                ...
                const auto structural = evaluateAutoBuiltin(
            ...
            const auto assocResult = matchAssociatedTypes(
            ...
            for (const auto& bound : implParamsDef->bounds) {
                if (const auto* be = bound.opt_TraitBound()) {
                    ...
                    auto monomorphBound = [&](auto& ms) {
                        ...
                        auto hrbGuard = ms.push_hrb(outer_present ? *be->hrtbs : noHrbs);
                        ...
                        const auto hrlParams = outer_present
                        ...
                        nestedParams = hrlMonomorph.monomorphPathParams(
                        ...
                        for (const auto& aty : boundTrait.typeBounds) {
                            ...
                            value.atyParams = hrlMonomorph.monomorphPathParams(
                    ...
                    if (bindCandidatePlaceholders(
                            *candidate, nestedType, nestedAssociated
                        )) {
                        ...
                        if (markerImpl) {
                            ...
                            auto ms = candidate->impl.getCbMonomorphTraitimpl(
                                crate.types, span(), {}
                            );
                            monomorphBound(ms);
                        }
                    ...
                    if (nested == Certainty::Ambiguous
                        ...
                        const bool hasResponse = evaluate(
                            span(),
                            nestedTrait,
                            nestedParams,
                            nestedType,
                    ...
                    if (markerImpl) {
                        ...
                        valid_for = ms.monomorphLifetime(
        ...
        Certainty solve_goal(
            ...
            if (goalHasUnassignedInfer(
                    goalParams, goalType, associated
                )) {
                return Certainty::Ambiguous;
            }
            ...
            for (auto& param : goalParams.types) {
                param = mResolve.expandAssociatedTypes(
                    span(), ::std::move(param)
                );
            }
            if (goalHasUnassignedInfer(
            ...
            if (self_is_unresolved_projection_over_ivar(goalType)) {
                return Certainty::Ambiguous;
            }
            const auto& resolved_type = mResolve.resolve_type(goalType);
            ...
            if (findActiveGoal(
                ...
                return crate.getTraitByPath(span(), trait).isCoinductive
                    ? Certainty::Proven
                    : Certainty::Ambiguous;
            }
            ...
            struct FrameGuard {
                ...
                size_t index;
                ~FrameGuard() {
                    const bool encounteredOverflow =
                        self.frames[index]->encounteredOverflow;
            ...
            try {
                assembleCandidates(frameIndex, trait, goalParams, resolved_type);
            ...
            for (size_t i = 0; i < candidateCount; i++) {
                ...
                suppress_auto_builtin |= candidate->isPositiveMarkerImpl()
                    && result != Certainty::NoSolution;
            ...
            if (!suppress_auto_builtin && !negativeProven) {
            ...
            if (saw_ambiguous
                || mResolve.type_contains_ivars(resolved_type)
                || mResolve.params_contain_ivars(goalParams)
                || (coherenceMode
                    && !trait_ref_is_knowable(trait, goalParams, resolved_type))) {
                return cacheResult(Certainty::Ambiguous);
            }
        ...
        ImplRef materializeRootAssociated(
            ...
            const static ::HIR::PathParams noParams;
            const auto& item_params = assocParams ? *assocParams : noParams;
            if (impl.getType(crate.types, assocName, item_params) != ::HIR::TypeRef()) {
                return impl;
            }
        ...
        bool responses_equal(
            ...
            auto types_equal_after_normalization = [&](const ::HIR::TypeData* lhs,
                ...
                auto normalizedLhs = mResolve.expandAssociatedTypes(
                    span(), lhs
                );
                ...
                const auto* resolved_lhs = mResolve.resolve_type(normalizedLhs);
                const auto* resolved_rhs = mResolve.resolve_type(normalizedRhs);
                return resolved_lhs == resolved_rhs
                    || resolved_lhs->equalsIgnoringRegions(resolved_rhs);
            };
            ...
            if (!types_equal_after_normalization(
                    left.getImplType(crate.types), right.getImplType(crate.types)
                )
            ...
            return types_equal_after_normalization(
                left.getType(crate.types, assocName, params),
                right.getType(crate.types, assocName, params)
            );
        }
    ...
    public:
        ...
        bool evaluateOverlap(
            ...
            auto goalParams = leftMonomorph.monomorphPathParams(
                callSpan, left.traitArgs, true
            );

            ::HIR::PathParams right_params;
            const auto right_match = mResolve.fticCheckParams(
                callSpan,
                trait,
                &goalParams,
                goalType,
            ...
            const auto& trait_def = crate.getTraitByPath(callSpan, trait);
            push_candidate(
                frameIndex,
                ImplRef(::std::move(leftParams), trait_def, trait, left),
                ::HIR::Compare::Equal,
            ...
            const auto right_result = evaluateCandidate(frameIndex, 1, trait, nullptr);
        ...
        bool evaluate(
            ...
            CanonicalizeTraitGoal canonicalizer(crate.types);
            const auto canonical = canonicalizeGoal(
                goalParams, resolved_type, nullptr, canonicalizer
            );
            ...
            if (cacheableResponse) {
                if (const auto* cached = findCachedGoal(
                    ...
                    auto response = monomorphImplRef(
                        cached->response, instantiator
                    );
                    return callback(
                        instantiateForCaller(::std::move(response)),
            ...
            auto emitResponse = [&](ImplRef response, ::HIR::Compare certainty) {
                ...
                auto canonicalResponse = monomorphImplRef(
            ...
            if (findActiveGoal(
                ...
                return callback(
                    ImplRef(resolved_type, &goalParams, &noAssociated),
                    coinductive
                        ? ::HIR::Compare::Equal
                        : ::HIR::Compare::Fuzzy
                );
            ...
            try {
                assembleCandidates(
                    frameIndex, trait, goalParams, resolved_type
                );
            } catch (const TraitResolution::RecursionDetected&) {
                return false;
            }
            ...
            if (candidateAssocType) {
                if (const auto* erased = candidateAssocType->opt_ErasedType()) {
                    if (const auto* alias = erased->inner.opt_Alias();
                        alias && alias->inner->isPublicTo(mResolve.visPath)) {
            ...
            for (size_t i = 0; i < candidateCount; i++) {
                auto certainty = evaluateCandidate(
                    frameIndex,
                    i,
                    trait,
                ...
                if (!candidate->isNegative()) {
                    const auto assocCertainty = matchRootAssociated(
                        trait,
                        candidate->impl,
                        assocName,
                        candidateAssocType,
            ...
            const bool hasNonGlobalParamEnv = ::std::any_of(
            ...
            for (const auto* candidate : frame.viable) {
                hasPreferredNonImpl |= isEnvironmentOrBuiltin(candidate->impl)
                    && candidate->certainty == Certainty::Proven;
            }
            ...
            for (size_t i = 0; i < frame.viable.size(); i++) {
                ...
                for (size_t j = i + 1; j < frame.viable.size(); j++) {
                    ...
                    if (coherenceMode
                        || !mResolve.implsOverlap(span(), left, right)) {
                        continue;
                    }
...
TraitResolution::TraitResolution(
    ...
    , currentTraitPtr(current_trait ? &crate.getTraitByPath(Span(), current_trait->mPath) : nullptr)
{
...
bool TraitResolution::implsOverlap(
    const Span& sp,
    const ImplRef& left,
    const ImplRef& right
) const {
    ...
    if (!gTraitSolverConfig.coherence
        || !leftImpl || !right_impl
        || !leftImpl->impl || !right_impl->impl) {
        return left.overlaps_with(crate, right);
    }
    if (!leftImpl->trait_path || !right_impl->trait_path
        || *leftImpl->trait_path != *right_impl->trait_path) {
        return false;
    }
    if (leftImpl->impl == right_impl->impl) {
        return true;
    }
    ...
    return coherenceResolve->nextSolver->evaluateOverlap(
        sp,
        *leftImpl->trait_path,
        *leftImpl->impl,
        *right_impl->impl
...
bool TraitResolution::findTraitImpls(
    ...
    if (gTraitSolverConfig.globally && magicTraitImpls) {
        return findTraitImplsNext(
            sp, trait, params, type, ::std::move(callback)
        );
    }
    return findTraitImplsLegacy(
        sp, trait, params, type, ::std::move(callback), magicTraitImpls
    );
}
            ...
            struct H {
                ...
                static void expandAssociatedTypesTp(const Span& sp, const TraitResolution& res, ::HIR::TraitPath& input, LList<const ::HIR::TypeData*> stack) {
                    ...
                    for (auto& arg : input.typeBounds) {
                        expandAssociatedTypesParams(sp, res, arg.second.source_trait.mParams, stack);
    ...
    TU_ARMA(Infer, e) {
        ...
        TU_ARMA(Generic, pe) {
                    ConvertHIRConstantEvaluateMethodParams(sp, crate, visPath, implGenerics, itemGenerics, e.binding.getGenerics(), pe.mParams);
                ...
                TU_ARMA(UfcsInherent, pe) {
                    ...
                    H::expandAssociatedTypesParams(sp, *this, pe.impl_params, stack);
        ...
        bool TraitResolution::expandAssociatedTypesInplaceUfcsInherent(const Span& sp, ::HIR::TypeRef& input, LList<const ::HIR::TypeData*> stack) const {
            ...
            if (inherentTypeConstraint) {
                auto selected_type = MonomorphStatePtr(crate.types, nullptr, &impl_params, nullptr).monomorphType(sp, selected_impl->mType);
        ...
        TU_ARMA(ErasedType, te) {
            ...
            for (const auto& trait : te.traits) {
                ...
                if (trait_path.mPath == trait_gp.mPath) {
                    ...
                    if (cmp != ::HIR::Compare::Unequal) {
                        auto hrls = getHrls(crate.types, sp, trait.hrtbs, trait_gp.mParams, trait_path.mParams);
                        {
                ...
                // - Check if the desired trait is a supertrait of this.
                // NOTE: `params` (aka des_params) is not used (TODO)
                bool isSupertrait = this->findNamedTraitInTrait(sp, trait_path.mPath, trait_path.mParams, *trait.traitPtr, trait_gp.mPath, trait_gp.mParams, pe.type, [&](const HIR::TraitPath& iTp) {
                    ...
                    if (cmp != ::HIR::Compare::Unequal) {
                        ...
                        if (it != trait.typeBounds.end()) {
                            auto hrls = getHrls(crate.types, sp, (trait.hrtbs && !trait.hrtbs->is_empty()) ? trait.hrtbs.get() : iTp.hrtbs.get(), iTp.mPath.mParams, trait_path.mParams);
                            DEBUG("hrls = " << hrls);
                            input = MonomorphHrlsOnly(crate.types, hrls).monomorphType(sp, it->second.type);
                            return true;
                        }
    ...
    if(!rv)
    {
        rv = this->iterateBoundsTraits(sp, pe.type, trait_path.mPath, [&](HIR::Compare cmp, const ::HIR::TypeData* boundType, const ::HIR::GenericPath& boundTrait, const CachedBound& boundInfo) -> bool {
    ...
    if(const auto* te_inner = pe.type->opt_Path())
    {
        if (const auto* pe_inner_p = te_inner->path.mData.opt_UfcsKnown()) {
            ...
            //
            ::HIR::GenericPath trait_path;
            if (!this->trait_contains_type(sp, pe_inner.trait, this->crate.getTraitByPath(sp, pe_inner.trait.mPath), pe_inner.item.c_str(), trait_path)) {
                BUG(sp, "Cannot find associated type " << pe_inner.item << " anywhere in trait " << pe_inner.trait);
            }
            const auto& trait_ptr = this->crate.getTraitByPath(sp, trait_path.mPath);
    ...
    if( this->findTraitImplsMagic(sp, trait_path.mPath, trait_path.mParams, pe.type, [&](auto impl, auto qual)->bool {
    ...
    if( this->findTraitImplsTypes(sp, trait_path.mPath, trait_path.mParams, pe.type, [&](auto impl, auto qual)->bool {
        DEBUG("[expand_associated_types__UfcsKnown] Found " << impl << " qual=" << qual);
        // If it's a fuzzy match, keep going (but count if a concrete hasn't been found)
        if (qual == ::HIR::Compare::Fuzzy) {
        } else {
    ...
    rv = this->findTraitImplsCrate(sp, trait_path.mPath, trait_path.mParams, pe.type, cbFindImpl);
    if( !rv ) {
        isBound = true;
        rv = findTraitImplsBound(sp, trait_path.mPath, trait_path.mParams, pe.type, cbFindImpl);
    }
    ...
    if( !this->ivars.type_contains_ivars(input, false) ) {
        ...
        //
        // -------------------------------------------------------------------------------------------------------------------
        bool TraitResolution::findNamedTraitInTrait(const Span& sp, const ::HIR::SimplePath& des, const ::HIR::PathParams& desParams, const ::HIR::Trait& trait_ptr, const ::HIR::SimplePath& trait_path, const ::HIR::PathParams& pp, const ::HIR::TypeData* target_type, t_cb_find_trait callback) const {
            TRACE_FUNCTION_F(des << desParams << " in " << trait_path << pp);
            ...
            for (const auto& pt : trait_ptr.allParentTraits) {
                auto pt_mono = monomorphCb.monomorphTraitpath(sp, pt, false);
        ...
        bool TraitResolution::findTraitImplsBound(const Span& sp, const ::HIR::SimplePath& trait, const ::HIR::PathParams& params, const ::HIR::TypeData* type, t_cb_trait_impl_r callback) const {
            TRACE_FUNCTION_F("trait = " << trait << params << ", type = " << type);
            ...
            // > Would maybe want a list of all explicit and implied bounds instead.
            {
                bool rv = this->iterateBoundsTraits(sp, type, trait, [&](HIR::Compare cmp, const HIR::TypeData* boundTy, const ::HIR::GenericPath& boundTrait, const CachedBound& boundInfo) -> bool {
                    ...
                    if (::std::any_of(
                        ...
                        normalisedParams = stored_params.clone();
                ...
                bool rv = this->iterateBoundsTraits(sp, assocInfo->type, assocInfo->trait.mPath, [&](HIR::Compare cmp, const HIR::TypeData* boundTy, const ::HIR::GenericPath& boundTrait, const CachedBound& boundInfo) -> bool {
                    ...
                    for (const auto& bound : at.traitBounds) {
                        if (bound.mPath.mPath == trait) {
                            ...
                            auto tp_mono = monomorphCb.monomorphTraitpath(sp, bound, false);
                            if (tp_mono.hrtbs) {
                                auto p = tp_mono.hrtbs->makeEmptyParams(true);
                                tp_mono = MonomorphHrlsOnly(crate.types, p).monomorphTraitpath(sp, tp_mono, true, true);
                            }
                            // - Expand associated types
                            this->expandAssociatedTypesParams(sp, tp_mono.mPath.mParams);
        ...
        bool TraitResolution::findTraitImplsCrate(const Span& sp, const ::HIR::SimplePath& trait, const ::HIR::PathParams* params_ptr, const ::HIR::TypeData* type, t_cb_trait_impl_r callback) const {
            ...
            const bool hasParams = params_ptr != nullptr;
            if (hasParams) {
            ...
            for (const auto& activeGoal : legacyTraitGoalStack) {
                if (!activeGoal.matches(
                        trait, canonicalParams, hasParams, canonicalType
                    )) {
                    continue;
                }
                ...
                DEBUG("Legacy trait goal recurred: " << trait
                    ...
                    << " for " << type << ", result=" << cmp);
                return callback(ImplRef(type, params_ptr, &nullAssoc), cmp);
            }
            ...
            bool hasFreshInputs = !hasParams || type_is_fresh(type);
            if (hasParams && !hasFreshInputs) {
                hasFreshInputs = ivars.pathparams_contain_ivars(
                    *params_ptr, false
                );
                for (const auto& param : params_ptr->types) {
                    hasFreshInputs = hasFreshInputs || type_is_fresh(param);
                }
            ...
            if (hasFreshInputs) {
                ...
                for (const auto& activeGoal : legacyTraitGoalStack) {
                    ...
                    return callback(
                        ImplRef(type, params_ptr, &nullAssoc),
                        ::HIR::Compare::Fuzzy
                    );
                }
            }
            ...
            if (crate.getTraitByPath(sp, trait).isMarker) {
                ...
                if (type->is_Infer()) {
                    return callback(ImplRef(type, params_ptr, &nullAssoc), ::HIR::Compare::Fuzzy);
                }
                ...
                if (markings) {
                    ...
                    if (it != markings->autoImpls.end()) {
                        if (!it->second.conditions.empty()) {
                            ...
                            return callback(ImplRef(type, params_ptr, &nullAssoc), ::HIR::Compare::Equal);
                        } else {
                            return false;
                        }
                    }
                ...
                this->crate.findAutoTraitImpls(trait, type, this->ivars.callbackResolveInfer(), [&](const auto& impl) -> bool {
                    ...
                    // Compare with `params`
                    HIR::PathParams impl_params;
                    auto match = this->fticCheckParams(sp, trait, params_ptr, type, impl.mParams, impl.traitArgs, impl.mType, impl_params);
                    if (match == ::HIR::Compare::Unequal) {
                    ...
                    auto ty_mono = monomorph.monomorphType(sp, impl.mType, false);
            ...
            return this->crate.findTraitImpls(trait, type, this->ivars.callbackResolveInfer(), [&](const HIR::TraitImpl& impl) {
                ...
                return callback(ImplRef(mv$(impl_params), crate.getTraitByPath(sp, trait), trait, impl), match);
            });
        }
        ...
        ::HIR::Compare TraitResolution::checkAutoTraitImplDestructure(const Span& sp, const ::HIR::SimplePath& trait, const ::HIR::PathParams* params_ptr, const ::HIR::TypeData* type) const {
            ...
            auto type_impls_trait = [&](const auto& innerTy) -> ::HIR::Compare {
                ...
                this->findTraitImpls(sp, trait, *params_ptr, innerTy, [&](auto, auto cmp) {
                    lRes = cmp;
                    return (cmp == ::HIR::Compare::Equal);
                });
    ...
    default:
        ...
        TU_ARMA(Generic, e) {
            auto lRes = ::HIR::Compare::Unequal;
            this->findTraitImpls(sp, trait, *params_ptr, type, [&](auto, auto cmp) {
                lRes = cmp;
                return (cmp == ::HIR::Compare::Equal);
            });
            return lRes;
    ...
    throw "";
        ...
        ::HIR::Compare TraitResolution::fticCheckParams(
            ...
            public:
                ...
                ::HIR::Compare matchTy(const ::HIR::GenericRef& g, const ::HIR::TypeData* ty, ::HIR::t_cb_resolve_type resolve_cb) override {
                    ...
                    if (outImplParams.types[g.binding] == HIR::TypeRef()) {
                        ...
                        auto rv = outImplParams.types[g.binding]->compareWithPlaceholders(sp, ty, resolve_cb);
                        ...
                        if (rv == ::HIR::Compare::Fuzzy && outImplParams.types[g.binding]->is_Infer()) {
                            ...
                            const auto& existingResolved = resolve_cb.getType(sp, outImplParams.types[g.binding]);
                            ...
                            if (existingInfer && existingInfer->index != ~0u) {
                                const bool recursive = visit_ty_with(ty, [&](const ::HIR::TypeData* inner) {
                                    if (const auto* infer = inner->opt_Infer()) {
                                        ...
                                        const auto& resolved = resolve_cb.getType(sp, inner);
                                        const auto* resolved_infer = resolved->opt_Infer();
                                        return resolved_infer && resolved_infer->index == existingInfer->index;
                                    }
                                    return false;
                                });
                                if (recursive) {
                ...
                ::HIR::Compare matchLft(
                    ...
                    ASSERT_BUG(
                        sp,
                        g.binding < outImplParams.mLifetimes.size(),
            ...
            GetParams get_params{sp, outImplParams};
            ...
            match &= implTy->matchTestGenericsFuzz(sp, type, this->ivars.callbackResolveInfer(), get_params);
            if (params_ptr) {
                const auto& params = *params_ptr;
                match &= implTraitArgs.matchTestGenericsFuzz(sp, params, this->ivars.callbackResolveInfer(), get_params);
                if (match == ::HIR::Compare::Unequal) {
            ...
            Matcher matcher{crate.types, sp, outImplParams, placeholder_name, placeholders};
            ...
            TU_ARMA(Lifetime, be) {
                ...
                TU_ARMA(TraitBound, be) {
                    ...
                    auto real_type = matcher.monomorphType(sp, be.type, false);
                    auto real_trait = matcher.monomorphTraitpath(sp, be.trait, false);
                    // TODO: If `real_trait` has HRLs, replace them?
                    if (real_trait.hrtbs) {
                        auto p = real_trait.hrtbs->makeEmptyParams(true);
                        real_trait.hrtbs.reset();
                        real_trait = MonomorphHrlsOnly(crate.types, p).monomorphTraitpath(sp, real_trait, true);
                    }
                    real_type = this->expandAssociatedTypes(sp, mv$(real_type));
                    ...
                    if (TU_TEST1(*real_type, Infer, .isLit() == false)) {
                    ...
                    auto rv = this->findTraitImpls(sp, real_trait_path.mPath, real_trait_path.mParams, real_type, [&](auto impl, auto implCmp) {
                        ...
                        if (cmp == ::HIR::Compare::Fuzzy) {
                            ...
                            DEBUG("[ftic_check_params] " << real_type << " ?= " << iTy);
                            cmp &= real_type->matchTestGenericsFuzz(sp, iTy, cbInfer, matcher);
                            DEBUG("[ftic_check_params] " << real_trait_path.mParams << " ?= " << iTp);
                            cmp &= real_trait_path.mParams.matchTestGenericsFuzz(sp, iTp, cbInfer, matcher);
                        ...
                        for (const auto& assocBound : real_trait.typeBounds) {
                            ...
                            if (tmp == ::HIR::TypeRef()) {
                                ...
                                tmp = this->expandAssociatedTypes(sp, crate.types.path(::HIR::Path(::HIR::Path::Data::Data_UfcsKnown{real_type, real_trait_path.clone(), assocBound.first, {}}), {}));
                                ty = tmp;
                            } else {
            ...
            for (size_t i = 0; i < implParamsDef.types.size(); i++) {
                if (implParamsDef.types.at(i).isSized) {
                    if (outImplParams.types[i] != HIR::TypeRef()) {
                        auto cmp = type_is_sized(sp, outImplParams.types[i]);
                        if (cmp == ::HIR::Compare::Unequal) {
        ...
        namespace {
            bool trait_contains_method_inner(const ::HIR::Trait& trait_ptr, const RcString& name, const ::HIR::Function*& outFcnPtr) {
        ...
        const ::HIR::Function* TraitResolution::trait_contains_method(const Span& sp, const ::HIR::GenericPath& trait_path, const ::HIR::Trait& trait_ptr, const ::HIR::TypeData* self, const RcString& name, ::HIR::GenericPath& outPath) const {
            TRACE_FUNCTION_FR("trait_path=" << trait_path << ",name=" << name, outPath);
            ...
            auto monomorphCb = MonomorphStatePtr(crate.types, self, &trait_path.mParams, nullptr);
            for (const auto& st : trait_ptr.allParentTraits) {
                if (trait_contains_method_inner(*st.traitPtr, name, rv)) {
                    ...
                    auto _h = monomorphCb.push_hrb(st.hrtbs ? *st.hrtbs : emptyHrtbs);
        ...
        bool TraitResolution::trait_contains_type(const Span& sp, const ::HIR::GenericPath& trait_path, const ::HIR::Trait& trait_ptr, const char* name, ::HIR::GenericPath& outPath) const {
            TRACE_FUNCTION_FR(trait_path << " has " << name, outPath);
            ...
            auto monomorphCb = MonomorphStatePtr(crate.types, nullptr, &trait_path.mParams, nullptr);
            for (const auto& st : trait_ptr.allParentTraits) {
    ...
    throw "";
        ...
        ::HIR::Compare TraitResolution::canUnsize(const Span& sp, const ::HIR::TypeData* dstTy, const ::HIR::TypeData* src_ty, ::std::function<void(::HIR::TypeRef newDst)>* newTypeCallback, ::std::function<void(const ::HIR::TypeData* dst, const ::HIR::TypeData* src)>* inferCallback) const {
            TRACE_FUNCTION_F(dstTy << " <- " << src_ty);

            // 1. Test for type equality
            {
                auto cmp = dstTy->compareWithPlaceholders(sp, src_ty, ivars.callbackResolveInfer());
                if (cmp == ::HIR::Compare::Equal) {
                    return ::HIR::Compare::Unequal;
                }
            }
            ...
            if (dstTy->is_Infer() || src_ty->is_Infer()) {
                ...
                if (inferCallback) {
                    (*inferCallback)(dstTy, src_ty);
                }
                return ::HIR::Compare::Fuzzy;
            }

            {
                bool foundBound = this->iterateBoundsTraits(sp, src_ty, mLangUnsize, [&](HIR::Compare cmp, const ::HIR::TypeData* beType, const ::HIR::GenericPath& beTrait, const CachedBound& info) -> bool {
                    ...
                    if (cmp != ::HIR::Compare::Equal) {
                        TODO(sp, "Found bound " << dstTy << "=" << beDst << " <- " << src_ty << "=" << beType);
                    }
                    return true;
                });
                if (foundBound) {
            ...
            if (src_ty->is_Path() && src_ty->as_Path().path.mData.is_UfcsKnown()) {
                ...
                auto foundBound = this->iterateAtyBounds(sp, pe, [&](const ::HIR::TraitPath& bound) {
                    ...
                    ::HIR::TypeRef tmp_ty;
                    const auto& beDst = monomorphCb.maybeMonomorphType(sp, tmp_ty, beDstTpl);
                    ...
                    if (cmp != ::HIR::Compare::Equal) {
                        DEBUG("[can_unsize] > Found bound (fuzzy) " << dstTy << "=" << beDst << " <- " << src_ty);
            ...
            if (dstTy->is_Path() && src_ty->is_Path()) {
                ...
                if (dstIsUnsizable || src_is_unsizable) {
                    ...
                    if (dstGp == src_gp) {
                        ...
                        const auto& dstInner = ivars.getType(dstGp.mParams.types.at(str.structMarkings.unsized_param));
                        const auto& src_inner = ivars.getType(src_gp.mParams.types.at(str.structMarkings.unsized_param));
                        ...
                        auto cb = [&](auto d) {
                            ...
                            dstGpNew.mParams.types.at(str.structMarkings.unsized_param) = mv$(d);
                        ...
                        if (newTypeCallback) {
                            ...
                            return this->canUnsize(sp, dstInner, src_inner, &cbP, inferCallback);
                        } else {
                            return this->canUnsize(sp, dstInner, src_inner, nullptr, inferCallback);
                        }
                    } else {
            ...
            if (const auto* de = dstTy->opt_TraitObject()) {
                ...
                if (de->mTrait.mPath.mPath == ::HIR::SimplePath()) {
                    ...
                    good = findTraitImpls(sp, de->mTrait.mPath.mPath, de->mTrait.mPath.mParams, src_ty, [&](const auto impl, auto cmp) {
                        if (cmp == ::HIR::Compare::Unequal) {
                            return false;
                        }
                        ...
                        auto remap_source_trait = [&](const ::HIR::GenericPath& source_trait) {
                            ...
                            auto goalMonomorph = MonomorphStatePtr(
                            ...
                            for (const auto& parent : de->mTrait.traitPtr->allParentTraits) {
                                ...
                                auto goalParent = goalMonomorph.monomorphGenericpath(
                                    sp, parent.mPath, false
                                );
                                const auto parent_cmp = comparePp(
                                    sp, goalParent.mParams, source_trait.mParams
                                );
                                if (parent_cmp == ::HIR::Compare::Unequal
                                    || (foundEqual && parent_cmp != ::HIR::Compare::Equal)) {
                                    continue;
                                }

                                auto response_parent = response_monomorph.monomorphGenericpath(
                                    sp, parent.mPath, false
                                );
                        ...
                        for (const auto& aty : de->mTrait.typeBounds) {
                            ...
                            auto desired = this->expandAssociatedTypes(
                                sp, aty.second.type
                            );
                ...
                for (const auto& marker : de->markers) {
                    ...
                    good &= findTraitImpls(sp, marker.mPath, marker.mParams, src_ty, cb);
                }

                if (good && total_cmp == ::HIR::Compare::Fuzzy && newTypeCallback) {
                    (*newTypeCallback)(crate.types.intern(::HIR::TypeData::make_TraitObject(mv$(tmp_e))));
                }
                return total_cmp;
            }
        ...
        TraitResolution::AutoderefResult TraitResolution::autoderefStep(
            ...
            const auto& ty = this->ivars.getType(ty_in);
            if (ty->is_Infer()) {
        ...
        unsigned int TraitResolution::autoderefFindMethod(
            ...
            try {
                ...
                const auto& top_ty_r = this->ivars.getType(top_ty);
                const auto* currentTy = top_ty_r;
                ...
                do {
                    ...
                    if (ty->is_Borrow() && should_pause(this->ivars.getType(ty->as_Borrow().inner))) {
                        return ~0u;
                    }
                    // TODO: Pause on Box<_>?
                    DEBUG(derefCount << ": " << ty);
                    ...
                    // Non-referenced
                    if (this->findMethod(sp, traits, ivars, type_ivar_count, ty, method_name, curAccess, AutoderefBorrow::None, possibilities)) {
                        DEBUG("FOUND *{" << derefCount << "}, fcn_path = " << possibilities.back().second);
                    }

                    // Auto-ref
                    auto borrowTy = crate.types.borrow(::HIR::BorrowType::Shared, ty);
                    if (this->findMethod(sp, traits, ivars, type_ivar_count, borrowTy, method_name, MethodAccess::Move, AutoderefBorrow::Shared, possibilities)) {
                        DEBUG("FOUND & *{" << derefCount << "}, fcn_path = " << possibilities.back().second);
                    }
                    borrowTy = crate.types.borrow(::HIR::BorrowType::Unique, ty);
                    if (curAccess >= MethodAccess::Unique && this->findMethod(sp, traits, ivars, type_ivar_count, borrowTy, method_name, MethodAccess::Move, AutoderefBorrow::Unique, possibilities)) {
                        DEBUG("FOUND &mut *{" << derefCount << "}, fcn_path = " << possibilities.back().second);
                    }
                    borrowTy = crate.types.borrow(::HIR::BorrowType::Owned, ty);
                    if (curAccess >= MethodAccess::Move && this->findMethod(sp, traits, ivars, type_ivar_count, borrowTy, method_name, MethodAccess::Move, AutoderefBorrow::Owned, possibilities)) {
            ...
            switch (fcn.receiver) {
                ...
                case ::HIR::Function::Receiver::Custom: {
                    ...
                    // - match_test_generics, if it succeeds return the matched Self
                    {
                        ...
                        if (receiver_type->matchTestGenerics(sp, ty, this->ivars.callbackResolveInfer(), getself)) {
                            ASSERT_BUG(sp, getself.detectedSelfTy, "Unable to determine receiver type when matching " << receiver_type << " and " << ty);
        ...
        bool TraitResolution::findMethod(const Span& sp, const HIR::t_trait_list& traits, const ::std::vector<unsigned>& ivars, unsigned int type_ivar_count, const ::HIR::TypeData* ty, const RcString& method_name, MethodAccess access, AutoderefBorrow borrowType, /* Out -> */ ::std::vector<::std::pair<AutoderefBorrow, ::HIR::Path>>& possibilities) const {
            bool rv = false;
            ...
            auto getIvaredParams = [&](const ::HIR::GenericParams& tpl) -> ::HIR::PathParams {
                ...
                ASSERT_BUG(sp, nParams <= type_ivar_count, "Not enough type ivars allocated for method: " << nParams << " needed but " << type_ivar_count << " allocated by caller\ntpl = " << tpl.fmtArgs());
                ...
                for (unsigned int i = 0; i < nParams; i++) {
                    ...
                    ASSERT_BUG(sp, this->ivars.getType(trait_params.types.back())->as_Infer().index == ivars[i], "A method selection ivar was bound");
                }
                const unsigned int nValues = tpl.values.size();
                ASSERT_BUG(sp, nValues <= ivars.size() - type_ivar_count, "Not enough value ivars allocated for method: " << nValues << " needed but " << ivars.size() - type_ivar_count << " allocated by caller\ntpl = " << tpl.fmtArgs());
            ...
            crate.inherentMethodCache.find(sp, method_name, ty, this->ivars.callbackResolveInfer(), [&](const HIR::TypeData* self_ty, const HIR::TypeImpl& impl) {
                if (!impl.methods.at(method_name).publicity.isVisible(this->visPath)) {
                    // Ignore method: Not visibile
                    return;
                }
                ::HIR::PathParams impl_params;
                auto cmp = fticCheckParams(sp, ::HIR::SimplePath(), nullptr, self_ty, impl.mParams, {}, impl.mType, impl_params);
                if (cmp != HIR::Compare::Unequal) {
                    DEBUG("Found `impl" << impl.mParams.fmtArgs() << " " << impl.mType << "` fn " << method_name /* << " - " << top_ty*/);
            ...
            for (const auto& tb : traitBounds) {
                ...
                assert(eTraitInfo.trait_ptr);
                // 1. Find the named method in the trait.
                ::HIR::GenericPath finalTraitPath;
                const ::HIR::Function* fcn_ptr;
                if (!(fcn_ptr = this->trait_contains_method(sp, eTraitGp, *eTraitInfo.trait_ptr, eType, method_name, finalTraitPath))) {
                ...
                if (auto self_ty = checkMethodReceiver(sp, *fcn_ptr, ty, access)) {
                    ...
                    if (TU_TEST1(**self_ty, Infer, .isLit() == false)) {
                        return false;
                    }
                    // TODO: Do a fuzzy match here?
                    auto cmp = (*self_ty)->compareWithPlaceholders(sp, eType, cbInfer);
                    if (cmp == ::HIR::Compare::Equal) {
                        ...
                        possibilities.push_back(::std::make_pair(borrowType, ::HIR::Path(::HIR::Path::Data::make_UfcsKnown({*self_ty, mv$(finalTraitPath), method_name, {}}))));
                        DEBUG("++ " << possibilities.back());
                        rv = true;
                        foundBound = true;
                    } else if (cmp == ::HIR::Compare::Fuzzy) {
                        DEBUG("Fuzzy match checking bounded method - " << *self_ty << " != " << eType);
                        ...
                        DEBUG("> Type mismatch - " << *self_ty << " != " << eType);
                    }
                } else {
            ...
            if (currentTraitPath) {
                ::HIR::GenericPath finalTraitPath;
                const ::HIR::Function* fcn_ptr;
                if ((fcn_ptr = this->trait_contains_method(sp, *currentTraitPath, *currentTraitPtr, ty, method_name, finalTraitPath))) {
                    ...
                    if (auto self_ty = checkMethodReceiver(sp, *fcn_ptr, ty, access)) {
                        ...
                        auto trait_params = getIvaredParams(trait.mParams);
                        ...
                        try {
                            ...
                            findTraitImplsCrate(sp, finalTraitPath.mPath, nullptr, *self_ty, [&](auto impl, auto cmp) {
                            ...
                            if (crateImplFound) {
                                DEBUG("Found trait impl " << currentTraitPath->mPath << trait_params << " for " << *self_ty << " (" << this->ivars.fmtType(*self_ty) << ")");
                                possibilities.push_back(::std::make_pair(borrowType, ::HIR::Path(*self_ty, ::HIR::GenericPath(finalTraitPath.mPath, mv$(trait_params)), method_name, {})));
                                DEBUG("++ " << possibilities.back());
                                return true;
                            } else {
                            }
            ...
            if (const auto* ityp = getInnerType(ty, [](const auto& t) {
                ...
                auto addTraitObjectMethod = [&](const ::HIR::Function& fcn, ::HIR::GenericPath finalTraitPath) {
                    ...
                    if (auto self_ty_p = checkMethodReceiver(sp, fcn, ty, access)) {
                        ...
                        possibilities.push_back(::std::make_pair(borrowType, ::HIR::Path(*self_ty_p, mv$(finalTraitPath), method_name, {})));
                        DEBUG("++ " << possibilities.back());
                        rv = true;
                ...
                if (trait_contains_method_inner(trait, method_name, fcn_ptr)) {
                    ...
                    auto monomorphCb = MonomorphStatePtr(crate.types, self_ty, &e.mTrait.mPath.mParams, nullptr);
                    for (const auto& st : trait.allParentTraits) {
                        ...
                        addTraitObjectMethod(*fcn_ptr, std::move(finalTraitPath));
                    }
                }
            ...
            if (const auto* ityp = getInnerType(ty, [](const auto& t) {
                ...
                for (const auto& trait_path : e.traits) {
                    const auto& trait = this->crate.getTraitByPath(sp, trait_path.mPath.mPath);

                    ::HIR::GenericPath finalTraitPath;
                    if (const auto* fcn_ptr = this->trait_contains_method(sp, trait_path.mPath, trait, crate.types.self(), method_name, finalTraitPath)) {
            ...
            // UfcsKnown paths: Can have trait bounds added by the definer
                ...
                for (const auto& bound : assocTy.traitBounds) {
                    ...
                    if (const auto* fcn_ptr = this->trait_contains_method(sp, bound.mPath, *bound.traitPtr, ty_self, method_name, finalTraitPath)) {
                        DEBUG("- Found trait " << finalTraitPath << " (UFCS Known, aty bounds)");

                        if (auto self_ty_p = checkMethodReceiver(sp, *fcn_ptr, ty, access)) {
                            if (*self_ty_p == ityp) {
                                auto pp_hrb = bound.hrtbs ? bound.hrtbs->makeEmptyParams(true) : HIR::PathParams();
                                monomorphCb.pp_hrb = &pp_hrb;
                ...
                for (const auto& bound : trait.mParams.bounds) {
                    ...
                    ::HIR::GenericPath finalTraitPath;
                    if (const auto* fcn_ptr = this->trait_contains_method(sp, be.trait.mPath, *be.trait.traitPtr, crate.types.self(), method_name, finalTraitPath)) {
            ...
            for (const auto& trait_ref : ::reverse(traits)) {
                ...
                ::HIR::GenericPath finalTraitPath;
                const ::HIR::Function* fcn_ptr;
                if (!(fcn_ptr = this->trait_contains_method(sp, *trait_ref.first, *trait_ref.second, crate.types.self(), method_name, finalTraitPath))) {
                    continue;
                }
                ...
                if (auto self_ty_p = checkMethodReceiver(sp, *fcn_ptr, ty, access)) {
                    ...
                    // Use the set of ivars we were given to populate the trait parameters
                    ::HIR::PathParams trait_params = getIvaredParams(trait_ref.second->mParams);
                    ...
                    crateImplFound = findTraitImplsMagic(sp, *trait_ref.first, trait_params, self_ty, [&](auto impl, auto cmp) {
                        return true;
                    });
                    ...
                    try {
                        ...
                        findTraitImplsCrate(sp, *trait_ref.first, nullptr, self_ty, [&](auto impl, auto cmp) {
                            DEBUG("[find_method] " << impl << ", cmp = " << cmp);
                            magicFound = true;
                            crateImplFound = true;
                            return true;
                    ...
                    if (crateImplFound) {
                        DEBUG("Found trait impl " << *trait_ref.first << trait_params << " for " << self_ty << " (" << this->ivars.fmtType(self_ty) << ")");
        ...
        unsigned int TraitResolution::autoderefFindField(const Span& sp, const ::HIR::TypeData* top_ty, const RcString& fieldName, /* Out -> */ ::HIR::TypeRef& fieldType) const {
            unsigned int derefCount = 0;
            ::HIR::TypeRef tmp_type; // Temporary type used for handling Deref
            const auto* currentTy = top_ty;
            if (const auto* e = this->ivars.getType(top_ty)->opt_Borrow()) {
                currentTy = e->inner;
                derefCount += 1;
            }
            ...
            do {
                ...
                derefCount += 1;
                currentTy = this->autoderef(sp, ty, tmp_type);
            } while (currentTy);

            if (/*const auto* e =*/this->ivars.getType(top_ty)->opt_Borrow()) {
                const auto& ty = this->ivars.getType(top_ty);
            ...
            this->ivars.dump();
            TODO(sp, "Error when no field could be found, but type is known - (: " << top_ty << ")." << fieldName);
        }
