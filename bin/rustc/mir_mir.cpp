...
void MIRLValue::RefCommon::fmt(ZeroCopyOutput& os) const {
    os << lv_->root;
...
const HIRType* MIRCloner::monomorph(const HIRType* ty) const {
    ...
    if (auto* r = resolve()) {
        rv = r->expandAssociatedTypes(sp, rv);
    }
    return rv;
}
...
MIRStatement MIRCloner::cloneStmt(const MIRStatement& src) const {
    switch (src.tag()) {
        ...
        case MIRStatement::TAG_SaveDropFlag: {
            TODO(Span(), StringView("clone_bb SaveDropFlag"));
            break;
        }
        case MIRStatement::TAG_LoadDropFlag: {
            TODO(Span(), StringView("clone_bb LoadDropFlag"));
            break;
        }
        case MIRStatement::TAG_ScopeEnd: {
...
MIRConstant MIRCloner::cloneConstant(const MIRConstant& src) const {
    switch (src.tag()) {
        ...
        case MIRConstant::TAG_Generic: {
            ...
            switch (val.tag()) {
                default:
                    TODO(sp, StringView("Monomorphise MIR generic constant ") << ce << StringView(" = ") << val);
                ...
                case HIRConstGeneric::TAG_Evaluated: {
                    ...
                    switch (ty->as_Primitive()) {
                        ...
                        case HIRCoreType::Str:
                            BUG(sp, StringView("`str` const generic"));
                    }
                    break;
                }
            }
...
MIRFunction::MIREnumCachePtr::~MIREnumCachePtr() {
    TransDeleteMIREnumCache(this->p);
    this->p = nullptr;
}
