...
namespace {
    ...
    public:
        ...
        void visit_node_ptr(::HIR::ExprNodeP& node_ptr) override {
            ...
            TRACE_FUNCTION_FR(&*node_ptr << " " << node_tyname << ": " << this->getUsage(), node_ptr->usage);

            node_ptr->usage = this->getUsage();
        ...
        void visit(::HIR::ExprNodeLet& node) override {
            ...
            if (node.mValue) {
                auto _ = this->push_usage(this->getUsageForPattern(node.span(), node.pattern, node.mType));
                this->visit_node_ptr(node.mValue);
            }
        }
        ...
        void visit(::HIR::ExprNodeMatch& node) override {
            {
                ...
                for (const auto& arm : node.arms) {
                    for (const auto& pat : arm.patterns) {
                        DEBUG("_Match: " << pat);
                        vu = ::std::max(vu, this->getUsageForPattern(node.span(), pat, val_ty));
                    }
                }
            ...
            for (auto& arm : node.arms) {
                ...
                for (auto& c : arm.guards) {
                    auto _ = this->push_usage(this->getUsageForPattern(c.val->span(), c.pat, c.val->resType));
        ...
        void visit(::HIR::ExprNodeIndex& node) override {
            ...
            if (this->getUsage() == ::HIR::ValueUsage::Move && mResolve.type_is_copy(node.span(), node.resType)) {
        ...
        void visit(::HIR::ExprNodeField& node) override {
            ...
            if (this->getUsage() == ::HIR::ValueUsage::Move && is_copy) {
                auto _ = push_usage(::HIR::ValueUsage::Borrow);
                this->visit_node_ptr(node.mValue);
            } else {
                this->visit_node_ptr(node.mValue);
        ...
        void visit(::HIR::ExprNodeStructLiteral& node) override {
            ...
            if (node.baseValue) {
                ...
                const ::HIR::t_struct_fields* fieldsPtr;
                if (tpb.is_Enum()) {
                    const auto& enm = *tpb.as_Enum();
                    auto idx = enm.findVariant(ty_path.mPath.components().back());
                ...
                auto _ = push_usage(isMoved ? ::HIR::ValueUsage::Move : ::HIR::ValueUsage::Borrow);
                this->visit_node_ptr(node.baseValue);
            }

            auto _ = push_usage(::HIR::ValueUsage::Move);
            for (auto& fldVal : node.values) {
                this->visit_node_ptr(fldVal.second);
            }
        }
        ...
        void visit(::HIR::ExprNodeClosure& node) override {
                ...
                for (auto& cap : scope.capturedVars) {
                    if (cap.usage == ::HIR::ValueUsage::Borrow) {
                        ...
                        for (const auto& n : cap.fields) {
                            tmp_ty = mResolve.getFieldType(node.span(), *capTyP, n);
                            mResolve.expandAssociatedTypes(node.span(), tmp_ty);
                            capTyP = &tmp_ty;
                        }
                    ...
                    switch (cap.usage) {
                        ...
                        case HIR::ValueUsage::Move: {
                            ...
                            for (const auto& fld : cap.fields) {
                                tmp_ty = mResolve.getFieldType(node.span(), *ty, fld);
                                ty = &tmp_ty;
                            }
            ...
            if (!closureStack.empty()) {
                ...
                for (const auto& v : scope.capturedVars) {
                    markUsedVariable(node.span(), v.root_slot, v.fields, v.usage);
                }
            }
    ...
    private:
        ...
        void addVarDefGenerator(const Span& sp, CoroutineScope& scope, unsigned int slot) {
            ...
            e.definedStack = scope.yield_stack;
        }
        ...
        ::HIR::ValueUsage getUsageForPattern(const Span& sp, const ::HIR::Pattern& pat, const ::HIR::TypeData* outer_ty) const {
            if (pat.mBindings.size() > 0) {
                auto vu = ::HIR::ValueUsage::Borrow;
                for (const auto& pb : pat.mBindings) {
                    vu = std::max(vu, getUsageForPatternBinding(sp, pb, outer_ty));
                }
                return vu;
            }
            ...
            TU_ARMA(Any, pe) {
                ...
                TU_ARMA(Tuple, pe) {
                    ...
                    for (unsigned int i = 0; i < subtys.size(); i++) {
                        rv = ::std::max(rv, getUsageForPattern(sp, pe.sub_patterns[i], subtys[i]));
                    }
                    return rv;
                }
                ...
                TU_ARMA(PathNamed, pe) {
                    ...
                    auto rv = ::HIR::ValueUsage::Borrow;
                    for (const auto& fldPat : pe.sub_patterns) {
                        ...
                        auto sty = mResolve.monomorph_expand(sp, fldIt->ty, monomorph_state);
        ...
        ::HIR::ValueUsage getRealUsage(const Span& sp, unsigned slot, const std::vector<RcString>& fields, ::HIR::ValueUsage usage) {
            ...
            if (usage == ::HIR::ValueUsage::Move) {
                ...
                for (const auto& name : fields) {
                    tmp_ty = mResolve.getFieldType(sp, *ty, name);
                    ty = &tmp_ty;
                }
        ...
        void markUsedVariableGenerator(const Span& sp, CoroutineScope& scope, unsigned int slot, std::vector<RcString> fields, ::HIR::ValueUsage usage) {
            auto& e = scope.used_variables[slot];
            e.lastUsedStack = scope.yield_stack;
...
namespace {
    ...
    struct OutState {
        out_impls_closure_t implsClosure;
        ...
        Counts save_counts() const {
            return Counts{implsClosure.size(), trait_impls.size(), implsType.size()};
        }
    ...
    void OutState::push_new_impls(const Span& sp, ::HIR::Crate& crate) {
        ...
        auto push_trait_impl = [&](const ::HIR::SimplePath& p, std::unique_ptr<::HIR::TraitImpl> ptr) {
            checkState(*ptr);
            auto& trait_impl_list_r = crate.allTraitImpls[p].getListForTypeMut(ptr->mType);
            trait_impl_list_r.push_back(ptr.get());
            auto& trait_impl_list = crate.traitImpls[p].getListForTypeMut(ptr->mType);
        ...
        for (auto& impl : this->implsClosure) {
            switch (impl.first) {
                case ::HIR::ExprNodeClosure::Class::Once:
                    ...
                    push_trait_impl(crate.getLangItemPath(sp, "fn_once"), box$(impl.second));
                    break;
                case ::HIR::ExprNodeClosure::Class::Mut:
                    DEBUG("impl" << impl.second.mParams.fmtArgs() << " FnMut" << impl.second.traitArgs << " for " << impl.second.mType);
                    push_trait_impl(crate.getLangItemPath(sp, "fn_mut"), box$(impl.second));
                    break;
                case ::HIR::ExprNodeClosure::Class::Shared:
                    DEBUG("impl" << impl.second.mParams.fmtArgs() << " Fn" << impl.second.traitArgs << " for " << impl.second.mType);
                    push_trait_impl(crate.getLangItemPath(sp, "fn"), box$(impl.second));
                    break;
                case ::HIR::ExprNodeClosure::Class::NoCapture:
                    BUG(sp, "");
                    break;
        ...
        for (auto& impl : this->trait_impls) {
            ...
            push_trait_impl(crate.getLangItemPath(sp, impl.first), box$(impl.second));
        }
    ...
    public:
        ...
        void visit(::HIR::ExprNodeVariable& node) override {
            ...
            // 2. Is it a capture?
            {
                ...
                if (bindingIt != captures.end()) {
                    ...
                    mReplacement = NEWNODE(node.resType, Field, node.span(), getSelf(node.span()), RcString::new_interned(FMT(bindingIt - captures.begin())));
        ...
        void visit(::HIR::ExprNodeField& node) override {
            ...
            if (auto* innerVar = cast<::HIR::ExprNodeVariable>(inner)) {
                ...
                auto bindingIt = ::std::find_if(captures.begin(), captures.end(), [&](const HIR::ExprNodeClosure::AvuCache::Capture& x) {
                    return x.root_slot == innerVar->slot && x.fields == fields;
                });
                if (bindingIt != captures.end()) {
                    mReplacement = NEWNODE(node.resType, Field, node.span(), getSelf(node.span()), RcString::new_interned(FMT(bindingIt - captures.begin())));
                    if (bindingIt->usage != ::HIR::ValueUsage::Move) {
    ...
    public:
        ...
        void visit(::HIR::ExprNodeCast& node) override {
            ...
            if (TU_TEST1((*node.mValue->resType), NodeType, .is_Closure())) {
                ...
                auto fnPath = ::HIR::Path(mv$(closure_type), rcstring_call_free);
                fnPath.mData.as_UfcsInherent().impl_params = src_node.objPath.mParams.clone();
                ...
                auto res_ty = crate.types.intern(::HIR::TypeData::make_NamedFunction({fnPath.clone(), fcn_ptr}));

                DEBUG("PathValue " << fnPath);
                node.mValue = NEWNODE(mv$(res_ty), PathValue, sp, mv$(fnPath), ::HIR::ExprNodePathValue::FUNCTION);
            }
            ::HIR::ExprVisitorDef::visit(node);
        }
    ...
    struct H {
        static void fixFnParams(::HIR::ExprPtr& code, const ::HIR::TypeData* self_ty, const ::HIR::TypeData* argsTy) {
        ...
        static ::HIR::TypeImpl makeFnfree(HIR::TypeInterner& types, ::HIR::GenericParams params, ::HIR::TypeRef closure_type, ::std::vector<::std::pair<::HIR::Pattern, ::HIR::TypeRef>> args, ::HIR::TypeRef ret_ty, ::HIR::ExprPtr code) {
            ...
            return ::HIR::TypeImpl{std::move(params), std::move(closure_type), makeMap1(rcstring_call_free, ::HIR::TypeImpl::VisImplEnt<::HIR::Function>{::HIR::Publicity::new_global(), false, ::HIR::Function(::HIR::Function::Receiver::Free, ::HIR::GenericParams{}, mv$(args), ret_ty, mv$(code))}), {}, {}, ::HIR::SimplePath()};
        }

        static ::HIR::TraitImpl makeFnonce(::HIR::GenericParams params, ::HIR::PathParams trait_params, ::HIR::TypeRef closure_type, ::std::pair<::HIR::Pattern, ::HIR::TypeRef> argsArgent, ::HIR::TypeRef ret_ty, ::HIR::ExprPtr code) {
            auto ty_of_self = closure_type;
            fixFnParams(code, ty_of_self, argsArgent.second);
            return ::HIR::TraitImpl{mv$(params), mv$(trait_params), mv$(closure_type), makeMap1(rcstring_call_once, ::HIR::TraitImpl::ImplEnt<::HIR::Function>{false, ::HIR::Function{::HIR::Function::Receiver::Value, ::HIR::GenericParams{}, makeVec2(::std::make_pair(::HIR::Pattern{HIR::PatternBinding{false, ::HIR::PatternBinding::Type::Move, rcstring_self, 0}, {}}, mv$(ty_of_self)), mv$(argsArgent)), ret_ty, mv$(code)}}), {}, {}, makeMap1(RcString::new_interned("Output"), ::HIR::TraitImpl::ImplEnt<::HIR::TypeRef>{false, mv$(ret_ty)}), ::HIR::SimplePath()};
        }

        static ::HIR::TraitImpl makeFnmut(HIR::TypeInterner& types, ::HIR::GenericParams params, ::HIR::PathParams trait_params, ::HIR::TypeRef closure_type, ::std::pair<::HIR::Pattern, ::HIR::TypeRef> argsArgent, ::HIR::TypeRef ret_ty, ::HIR::ExprPtr code) {
            ...
            fixFnParams(code, ty_of_self, argsArgent.second);
            return ::HIR::TraitImpl{mv$(params), mv$(trait_params), mv$(closure_type), makeMap1(rcstring_call_mut, ::HIR::TraitImpl::ImplEnt<::HIR::Function>{false, ::HIR::Function{::HIR::Function::Receiver::BorrowUnique, mv$(fcn_params), makeVec2(::std::make_pair(::HIR::Pattern{{false, ::HIR::PatternBinding::Type::Move, rcstring_self, 0}, {}}, mv$(ty_of_self)), mv$(argsArgent)), ret_ty, mv$(code)}}), {}, {}, {}, ::HIR::SimplePath()};
        }

        static ::HIR::TraitImpl makeFn(HIR::TypeInterner& types, ::HIR::GenericParams params, ::HIR::PathParams trait_params, ::HIR::TypeRef closure_type, ::std::pair<::HIR::Pattern, ::HIR::TypeRef> argsArgent, ::HIR::TypeRef ret_ty, ::HIR::ExprPtr code) {
            auto ty_of_self = types.borrow(::HIR::BorrowType::Shared, closure_type);
            fixFnParams(code, ty_of_self, argsArgent.second);
            return ::HIR::TraitImpl{mv$(params), mv$(trait_params), mv$(closure_type), makeMap1(rcstring_call, ::HIR::TraitImpl::ImplEnt<::HIR::Function>{false, ::HIR::Function{::HIR::Function::Receiver::BorrowShared, ::HIR::GenericParams{}, makeVec2(::std::make_pair(::HIR::Pattern{{false, ::HIR::PatternBinding::Type::Move, rcstring_self, 0}, {}}, mv$(ty_of_self)), mv$(argsArgent)), ret_ty, mv$(code)}}), {}, {}, {}, ::HIR::SimplePath()};
        }
    };
    ...
    public:
        ...
        private:
            ...
            TypeNeed type_bound_needed(const Span& sp, const HIR::TypeData* ty) const {
                ...
                visit_ty_with(ty, getNeededCb(rv));
                return rv;
            }
            ...
            TypeNeed type_bound_needed(const Span& sp, const HIR::TraitPath& tp) const {
                auto rv = TypeNeed::NoGenerics;
                visit_trait_path_tys_with(tp, getNeededCb(rv));
                return rv;
            }
            ...
            bool boundNeeded(const Span& sp, const ::HIR::GenericBound& b) const {
                ...
                TU_ARMA(Lifetime, e) {
                        ...
                        if (e.valid_for.isParam() && !contains(constructorPathParams.mLifetimes, e.valid_for)) {
                            return false;
                        }
                        return true;
                    }
        ...
        void visit(::HIR::ExprNodeClosure& node) override {
            ...
            auto monomorph_cb = createParams(sp, mResolve, params, constructorPathParams);
            ...
            for (const auto bindingIdx : node.avuCache.local_vars) {
                localTypes.push_back(monomorph_cb.monomorph_type(sp, variableTypes.at(bindingIdx)));
            }
            ...
            for (const auto& binding : node.avuCache.capturedVars) {
                ...
                for (const auto& n : binding.fields) {
                    tmp_ty = mResolve.getFieldType(sp, *capTyP, n);
                    mResolve.expandAssociatedTypes(sp, tmp_ty);
                    capTyP = &tmp_ty;
                    if (n == "") {
            ...
            if (lifetimeNeeded) {
                ...
                DEBUG("Added by-borrow lifetime: #" << ref_capture_lft_idx << " - " << params.fmtArgs());
            ...
            // - Fix type to replace closure types with known paths
            {
                ...
                for (size_t i = 0; i < captureTypes.size(); i++) {
                    ...
                    fixup.mResolve.expandAssociatedTypes(sp, ty_mono);
...

            {
                ...
                localResolve.set_impl_generics_raw(MetadataType::None, params); // Closure types are sized

                for (const auto& v : captureTypes) {
                    if (!localResolve.type_is_copy(sp, v.ent)) {
            ...
            ::HIR::TypeRef closure_type = mResolve.crate.types.path(::HIR::GenericPath(node.objPath.mPath.clone(), mv$(implPathParams)), ::HIR::TypePathBinding::make_Struct(&closureStructRef));
            ...
            if (node.isCopy && !langCopy.components().empty()) {
                auto& v = const_cast<::HIR::Crate&>(mResolve.crate).traitImpls[langCopy].getListForTypeMut(closure_type);
                v.push_back(box$(
                    ::HIR::TraitImpl{
                        params.clone(),
                        {},
                ...
                const_cast<::HIR::Crate&>(mResolve.crate).allTraitImpls[langCopy].getListForTypeMut(closure_type).push_back(v.back().get());
            }
            ...
            switch (node.cls) {
                ...
                case ::HIR::ExprNodeClosure::Class::NoCapture: {
                    ...
                    struct H2 {
                        static ::std::pair<::HIR::ExprNodeClosure::Class, HIR::TraitImpl> makeDispatch(HIR::TypeInterner& types, stl::ObjPool* pool, const Span& sp, ::HIR::ExprNodeClosure::Class c, ::HIR::GenericParams params, ::HIR::PathParams trait_params, const ::HIR::TypeData* closure_type, const ::HIR::TypeData* argsTy, const ::HIR::TypeData* ret_type) {
                            ...
                            for (size_t i = 0; i < argsTupInner.size(); i++) {
                                ...
                                dispatchArgs.push_back(NEWNODE(ty, Field, sp, NEWNODE(argsTy, Variable, sp, rcstring_arg, 1), RcString::new_interned(FMT(i))));
                                dispatchNodeArgsCache.push_back(ty);
                            }
                            dispatchNodeArgsCache.push_back(ret_type);
                            ...
                            HIR::ExprNodeP dispatchNode = closureMkExprnodep(dispatchCall, ret_type);
                            ...
                            switch (c) {
                                case ::HIR::ExprNodeClosure::Class::Once:
                                    fcn = H::makeFnonce(mv$(params), mv$(trait_params), closure_type, mv$(argsArg), ret_type, mv$(dispatchNode));
                                    break;
                                case ::HIR::ExprNodeClosure::Class::Mut:
                                    fcn = H::makeFnmut(types, mv$(params), mv$(trait_params), closure_type, mv$(argsArg), ret_type, mv$(dispatchNode));
                                    break;
                                case ::HIR::ExprNodeClosure::Class::Shared:
                                    fcn = H::makeFn(types, mv$(params), mv$(trait_params), closure_type, mv$(argsArg), ret_type, mv$(dispatchNode));
                                    break;
                                default:
                                    throw "";
                            }
                    ...
                    if (!mResolve.crate.getLangItemPathOpt("fn_once").components().empty()) {
                        out.implsClosure.push_back(H2::makeDispatch(mResolve.crate.types, pool, sp, ::HIR::ExprNodeClosure::Class::Once, params.clone(), trait_params.clone(), closure_type, argsTy, ret_type));
                    }
                    if (!mResolve.crate.getLangItemPathOpt("fn_mut").components().empty()) {
                        out.implsClosure.push_back(H2::makeDispatch(mResolve.crate.types, pool, sp, ::HIR::ExprNodeClosure::Class::Mut, params.clone(), trait_params.clone(), closure_type, argsTy, ret_type));
                    }
                    if (!mResolve.crate.getLangItemPathOpt("fn").components().empty()) {
                        out.implsClosure.push_back(H2::makeDispatch(mResolve.crate.types, pool, sp, ::HIR::ExprNodeClosure::Class::Shared, params.clone(), trait_params.clone(), closure_type, argsTy, ret_type));
                    }
                    ...
                    out.implsType.push_back(box$(H::makeFnfree(mResolve.crate.types, mv$(params), mv$(closure_type), mv$(argsSplit), mv$(ret_type), std::move(bodyCode))));
                ...
                case ::HIR::ExprNodeClosure::Class::Shared: {
                    ...
                    // - FnOnce
                    {
                        auto* dispatchCall = pool->make<HIR::ExprNodeCallPath>(sp, ::HIR::Path(closure_type, ::HIR::GenericPath(langFn, trait_params.clone()), rcstring_call, HIR::PathParams(HIR::LifetimeRef())), makeVec2(NEWNODE(method_self_ty, Borrow, sp, ::HIR::BorrowType::Shared, NEWNODE(closure_type, Variable, sp, rcstring_self, 0)), NEWNODE(argsTy, Variable, sp, rcstring_arg, 1)));
                        auto dispatchNode = closureMkExprnodep(dispatchCall, ret_type);
                        dispatchCall->cache.argTypes = makeVec3(method_self_ty, argsTy, ret_type);
                        auto argsArg = ::std::make_pair(::HIR::Pattern{{false, ::HIR::PatternBinding::Type::Move, rcstring_args, 1}, {}}, argsTy);
                        out.implsClosure.push_back(::std::make_pair(::HIR::ExprNodeClosure::Class::Once, H::makeFnonce(params.clone(), trait_params.clone(), closure_type, mv$(argsArg), ret_type, mv$(dispatchNode))));
                    }
                    // - FnMut
                    {
                        auto self_ty = mResolve.crate.types.borrow(::HIR::BorrowType::Unique, closure_type);
                        auto* dispatchCall = pool->make<HIR::ExprNodeCallPath>(sp, ::HIR::Path(closure_type, ::HIR::GenericPath(langFn, trait_params.clone()), rcstring_call, HIR::PathParams(HIR::LifetimeRef())), makeVec2(NEWNODE(method_self_ty, Borrow, sp, ::HIR::BorrowType::Shared, NEWNODE(closure_type, Deref, sp, NEWNODE(mv$(self_ty), Variable, sp, rcstring_self, 0))), NEWNODE(argsTy, Variable, sp, rcstring_arg, 1)));
                        auto dispatchNode = closureMkExprnodep(dispatchCall, ret_type);
                        dispatchCall->cache.argTypes = makeVec3(method_self_ty, argsTy, ret_type);
                        auto argsArg = ::std::make_pair(::HIR::Pattern{{false, ::HIR::PatternBinding::Type::Move, rcstring_args, 1}, {}}, argsTy);
                        out.implsClosure.push_back(::std::make_pair(::HIR::ExprNodeClosure::Class::Mut, H::makeFnmut(mResolve.crate.types, params.clone(), trait_params.clone(), closure_type, mv$(argsArg), ret_type, mv$(dispatchNode))));
                    }

                    // - Fn
                    out.implsClosure.push_back(::std::make_pair(::HIR::ExprNodeClosure::Class::Shared, H::makeFn(mResolve.crate.types, mv$(params), mv$(trait_params), mv$(closure_type), ::std::make_pair(mv$(argsPat), mv$(argsTy)), mv$(ret_type), mv$(bodyCode))));
                } break;
                case ::HIR::ExprNodeClosure::Class::Mut: {
                    DEBUG("class=Mut");
                    ...
                    // - FnOnce
                    {
                        auto* dispatchCall = pool->make<HIR::ExprNodeCallPath>(sp, ::HIR::Path(closure_type, ::HIR::GenericPath(langFnMut, trait_params.clone()), rcstring_call_mut, HIR::PathParams(HIR::LifetimeRef())), makeVec2(NEWNODE(method_self_ty, Borrow, sp, ::HIR::BorrowType::Unique, NEWNODE(closure_type, Variable, sp, rcstring_self, 0)), NEWNODE(argsTy, Variable, sp, rcstring_arg, 1)));
                        auto dispatchNode = closureMkExprnodep(dispatchCall, ret_type);
                        dispatchCall->cache.argTypes = makeVec3(method_self_ty, argsTy, ret_type);
                        auto argsArg = ::std::make_pair(::HIR::Pattern{{false, ::HIR::PatternBinding::Type::Move, rcstring_args, 1}, {}}, argsTy);
                        out.implsClosure.push_back(::std::make_pair(::HIR::ExprNodeClosure::Class::Once, H::makeFnonce(params.clone(), trait_params.clone(), closure_type, mv$(argsArg), ret_type, mv$(dispatchNode))));
                    ...
                    // - FnMut (code)
                    out.implsClosure.push_back(::std::make_pair(::HIR::ExprNodeClosure::Class::Mut, H::makeFnmut(mResolve.crate.types, mv$(params), mv$(trait_params), mv$(closure_type), ::std::make_pair(mv$(argsPat), mv$(argsTy)), mv$(ret_type), mv$(bodyCode))));
                } break;
                case ::HIR::ExprNodeClosure::Class::Once:
                    DEBUG("class=Once");
                    // - FnOnce (code)
                    out.implsClosure.push_back(::std::make_pair(::HIR::ExprNodeClosure::Class::Once, H::makeFnonce(mv$(params), mv$(trait_params), mv$(closure_type), ::std::make_pair(mv$(argsPat), mv$(argsTy)), mv$(ret_type), mv$(bodyCode))));
                    break;
            }
        ...
        public:
            ...
            void visit_pattern(const Span& sp, ::HIR::Pattern& pat) override {
                ...
                if (auto* pe = pat.mData.opt_SplitSlice()) {
                    visit_pattern_binding(sp, pe->extraBind);
                }
            }
        ...
        CrVars coroutineVars(const Span& sp, const ::HIR::ExprNodeGenerator::AvuCache& avu_cache, unsigned n_args, const Monomorph& monomorph_cb) const {
            CrVars rv;
            // 3. Classify varibles
        ...
        void visit(::HIR::ExprNodeGenerator& node) override {
            ...
            auto crVars = coroutineVars(node.span(), node.avuCache, 2, monomorph_cb);

            {
                TRACE_FUNCTION_F("-- Rewrite variables");
                ExprVisitorGeneratorRewrite visitor_rewrite(monomorph_cb, crVars.variable_rewrites);
            ...
            auto state_type = mResolve.crate.types.path(::HIR::GenericPath(state_struct_path, params.makeNopParams(mResolve.crate.types, 0)), &state_struct_ptr->as_Struct());
            DEBUG("state_type = " << state_type);
            set_state_type(sp, crVars, state_type);

            auto genStr = ::HIR::Struct{params.clone(), ::HIR::Struct::Repr::Rust, ::HIR::Struct::Data::make_Tuple(mv$(crVars.struct_ents))};
            genStr.markings.hasDropImpl = true;
            ::HIR::SimplePath genStructPath;
            const ::HIR::TypeItem* genStructPtr;
            ::std::tie(genStructPath, genStructPtr) = out.new_type(GENERATOR_PATH_PREFIX, newTypeSuffix, mv$(genStr));
            ...
            node.objPath = ::HIR::GenericPath(genStructPath, monomorph_cb.freeze());
            ...
            // `::path::to::struct`
            auto self_arg_ty = mResolve.crate.types.path(::HIR::GenericPath(genStructPath, params.makeNopParams(mResolve.crate.types, 0)), &genStructRef);
            // `&mut Self`
            ...
            // `Pin<&mut Self>`
            self_arg_ty = mResolve.crate.types.path(::HIR::GenericPath(langPin, ::HIR::PathParams(self_arg_ty)), &mResolve.crate.getStructByPath(sp, langPin));
            resume_args[0] = self_arg_ty;
            crVars.set_arguments(sp, std::move(resume_args));
            ...
            ::HIR::Function* fcnDropPtr;
            {
                ...
                dropImpl.methods.insert(std::make_pair(RcString::new_interned("drop"), ::HIR::TraitImpl::ImplEnt<HIR::Function>{false, std::move(fcnDrop)}));
                fcnDropPtr = &dropImpl.methods.at("drop").data;
                out.trait_impls.push_back(std::make_pair("drop", std::move(dropImpl)));
            }

            // -- Create function
            ::HIR::Function fcnResume;
            // - `self: Pin<&mut {Self}>`
            fcnResume.mArgs.push_back(std::make_pair(HIR::Pattern(), self_arg_ty));
            fcnResume.mArgs.push_back(std::make_pair(HIR::Pattern(), monomorph_cb.monomorph_type(sp, node.resumeTy)));
            ...
            fcnResume.returnType = mResolve.crate.types.path(::HIR::GenericPath(langGeneratorState, std::move(ret_params)), &mResolve.crate.getEnumByPath(sp, langGeneratorState));
            // - ` { ... }`
            ...
            v->captureUsages = std::move(crVars.capture_usages);
            ...
            fcnResume.mCode.mBindings = std::move(crVars.new_locals);
            ...
            impl.methods.insert(std::make_pair(RcString::new_interned("resume"), ::HIR::TraitImpl::ImplEnt<HIR::Function>{false, std::move(fcnResume)}));
        ...
        void visit(::HIR::ExprNodeAsyncBlock& node) override {
            ...
            auto genStr = ::HIR::Struct{params.clone(), ::HIR::Struct::Repr::Rust, ::HIR::Struct::Data::make_Tuple(std::move(crVars.struct_ents))};
            genStr.markings.hasDropImpl = true;
            ::HIR::SimplePath genStructPath;
            const ::HIR::TypeItem* genStructPtr;
            ::std::tie(genStructPath, genStructPtr) = out.new_type(PATH_PREFIX_FUTURE, newTypeSuffix, mv$(genStr));
            const auto& genStructRef = genStructPtr->as_Struct();
            ...
            crVars.set_arguments(sp, {self_arg_ty, contextArgTy});
            ...
            fcnResume.returnType = mResolve.crate.types.path(::HIR::GenericPath(langPoll, std::move(ret_params)), &mResolve.crate.getEnumByPath(sp, langPoll));
            // - ` { ... }`
            // Emit as a top-level generator
            // - It has a populated body, non-zero `m_obj_ptr`, and unset `m_obj_path`
            auto* v = pool->make<::HIR::ExprNodeGeneratorWrapper>(
            ...
            impl.methods.insert(std::make_pair(RcString::new_interned("poll"), ::HIR::TraitImpl::ImplEnt<HIR::Function>{false, std::move(fcnResume)}));
    ...
    public:
        ...
        void visit_crate(::HIR::Crate& crate) override {
            ...
            auto emptyCounts = out.save_counts();

            ::HIR::Visitor::visit_crate(crate);

            // Defensive measure
            out.update_source_module(emptyCounts, root_mod_path);
        ...
        void visit_module(::HIR::ItemPath p, ::HIR::Module& mod) override {
            ...
            out.new_type = [&](const char* prefix, const char* suffix, auto s) -> auto {
                ...
                return ::std::make_pair((p + name).getSimplePath(), ret_ptr);
            };
...
namespace {
    ...
    void expandErasedType(const Span& sp, const StaticTraitResolve& mResolve, HIR::TypeRef& ty) {
        ...
        TU_ARMA(Fcn, ee) {
                ...
                auto val = mResolve.getValue(sp, ee.origin, monomorph_cb);
                ...
                mResolve.expandAssociatedTypes(sp, new_ty);
            }
            TU_ARMA(Alias, ee) {
...
namespace {
    ...
    public:
        ...
        bool checkLifetimes(const Span& sp, const HIR::LifetimeRef& lhs, const HIR::LifetimeRef& rhs, std::vector<HIR::LifetimeRef>& fails) const {
            ...
            if (const auto* lL = opt_local(sp, lhs)) {
            ...
            if (lhs.isParam()) {
                if (rhs.isParam()) {
                    ...
                    bool rv = iterateLftBounds(rhs, [&](const HIR::LifetimeRef& valid_for) -> bool {
                        if (valid_for == lhs)
            ...
            } else if (const auto* lhsL = opt_local(sp, lhs)) {
    ...
    private:
        ...
        void equateTraitpath(const Span& sp, const HIR::TraitPath& lhs, const HIR::TraitPath& rhs) {
            ...
            //    TODO(sp, lhs << " == " << rhs);
            //}
            auto pp_l = lhs.hrtbs ? lhs.hrtbs->makeEmptyParams(true) : HIR::PathParams();
            auto pp_r = rhs.hrtbs ? rhs.hrtbs->makeEmptyParams(true) : HIR::PathParams();
            ...
            equatePps(sp, ms_l.monomorph_path_params(sp, lhs.mPath.mParams, false), ms_r.monomorph_path_params(sp, rhs.mPath.mParams, false));
            ...
            for (const auto& l : lhs.typeBounds) {
                ...
                equateTypes(sp, ms_l.monomorph_type(sp, l.second.type, false), ms_r.monomorph_type(sp, r.type, false));
            }
            for (const auto& l : lhs.traitBounds) {
                const auto& r = rhs.traitBounds.at(l.first);
                for (size_t i = 0; i < l.second.traits.size(); i++) {
                    equateTraitpath(sp, ms_l.monomorph_traitpath(sp, l.second.traits[i], false), ms_r.monomorph_traitpath(sp, r.traits[i], false));
                }
            }
        }
        ...
        void equatePaths(const Span& sp, const HIR::Path& lhs, const HIR::Path& rhs) {
            ...
            TU_ARMA(Generic, le, re) {
                ...
                TU_ARMA(UfcsInherent, le, re) {
                    equateTypes(sp, le.type, re.type);
                    equatePps(sp, le.params, re.params);
                    equatePps(sp, le.impl_params, re.impl_params);
                }
        ...
        void equateTypes(const Span& sp, const HIR::TypeData* lhs, const HIR::TypeData* rhs) {
            ...
            TU_ARMA(Infer, l, r) BUG(sp, "");
                ...
                TU_ARMA(Known, lIe, r_ie) {
                            this->equateTypes(sp, lIe, r_ie);
                        }
                        TU_ARMA(Fcn, lIe, r_ie) {
                            this->equatePaths(sp, lIe.origin, r_ie.origin);
                        }
                        TU_ARMA(Alias, lIe, r_ie) {
                            this->equatePps(sp, lIe.params, r_ie.params);
                        }
                }
                }
                TU_ARMA(Array, l, r) {
        ...
        void equatePatternBinding(const Span& sp, const HIR::PatternBinding& b, const HIR::TypeData* src_ty, const HIR::ExprNode& root_node) {
            const HIR::TypeData* curTy = src_ty;
            ...
            auto getLft = [&]() -> HIR::LifetimeRef {
                if (patternLifetimeStack.empty()) {
                    return getBorrowLifetime(root_node, [&](const HIR::ExprNode& value) {
        ...
        void equatePattern(const Span& sp, const HIR::Pattern& pat, const HIR::TypeData* src_ty, const HIR::ExprNode& root_node) {
            // Match pattern into the type (including bindings)
            if (src_ty->is_Diverge()) {
                return;
            }
            for (const auto& b : pat.mBindings) {
                equatePatternBinding(sp, b, src_ty, root_node);
            }
            const HIR::TypeData* curTy = src_ty;
            ...
            TU_ARMA(Any, _) {
                ...
                TU_ARMA(Slice, pe) {
                    ...
                    for (const auto& subpat : pe.sub_patterns) {
                        equatePattern(sp, subpat, ity, root_node);
                    }
                }
                TU_ARMA(SplitSlice, pe) {
                    const auto& ity = curTy->is_Array() ? curTy->as_Array().inner : curTy->as_Slice().inner;
                    ...
                    if (pe.extraBind.isValid()) {
                        equatePatternBinding(sp, pe.extraBind, curTy, root_node);
                    }
                ...
                TU_ARMA(Tuple, pe) {
                    ...
                    for (size_t i = 0; i < te.size(); i++) {
                        equatePattern(sp, pe.sub_patterns[i], te[i], root_node);
                    }
                }
                TU_ARMA(SplitTuple, pe) {
                    const auto& te = curTy->as_Tuple();
                    ...
                    for (size_t i = 0; i < pe.leading.size(); i++) {
                        equatePattern(sp, pe.leading[i], te[i], root_node);
                    }
                    for (size_t i = 0; i < pe.trailing.size(); i++) {
                        equatePattern(sp, pe.trailing[i], te[te.size() - pe.trailing.size() + i], root_node);
                    }
                }
                TU_ARMA(Box, pe) {
                    ...
                    equatePattern(sp, *pe.sub, innerTy, root_node);
                }
                TU_ARMA(Ref, pe) {
                    const auto& te = curTy->as_Borrow();
                    patternLifetimeStack.push_back(te.lifetime);
                    equatePattern(sp, *pe.sub, te.inner, root_node);
                    patternLifetimeStack.pop_back();
                }

                TU_ARMA(Or, pe) {
                    for (auto& subpat : pe) {
                        equatePattern(sp, subpat, curTy, root_node);
                    }
                }
                ...
                for(size_t i = 0; i < flds->size(); i++)
                {
                        if (i < pe.leading.size()) {
                            auto expTy = mResolve.monomorph_expand(sp, (*flds)[i].ent, ms);
                            equatePattern(sp, pe.leading[i], expTy, root_node);
                        } else if (i > trailing_start) {
                            auto expTy = mResolve.monomorph_expand(sp, (*flds)[i].ent, ms);
                            equatePattern(sp, pe.trailing[i - trailing_start], expTy, root_node);
                        } else {
                        }
                }
                }
        ...
        struct MonomorphAddLifetimes: public Monomorphiser {
            ...
            ::HIR::LifetimeRef monomorph_lifetime(const Span& sp, const ::HIR::LifetimeRef& tpl) const override {
                if (tpl.binding == HIR::LifetimeRef::UNKNOWN || tpl.binding == HIR::LifetimeRef::INFER) {
                    ...
                    if (parent.remove_locals && (parent.state.isLocal(tpl) || parent.state.isIvar(tpl))) {
                        return parent.state.allocateIvar(sp);
                    }
        ...
        void visit_path(::HIR::Visitor::PathContext pc, HIR::Path& p) override {
            ...
            //    }
            //}
            p = getMonomorphAdd().monomorph_path(sp, p, false);
        }

        void visit_path_params(HIR::PathParams& pps) override {
            TRACE_FUNCTION_FR(pps, pps);
            pps = getMonomorphAdd().monomorph_path_params(Span(), pps, false);
        }

        void visit_type(HIR::TypeRef& ty) override {
            TRACE_FUNCTION_FR(ty, ty);
            ty = getMonomorphAdd().monomorph_type(Span(), ty, false);
        }
...
#undef NV
    ...
    public:
        ...
        void visit(::HIR::ExprNodeBorrow& node) override {
            ...
            auto lft = node.isValidStaticBorrowConstant ? ::HIR::LifetimeRef::new_static() : getBorrowLifetime(val_node, [&](const HIR::ExprNode& value) -> HIR::LifetimeRef {
                DEBUG("LOCAL: " << typeid(val_node).name() << " " << typeid(value).name());
                return state.allocateLocal(node, value);
            });
            equateTypes(node.span(), node.resType, mResolve.crate.types.borrow(node.mType, val_node.resType, lft));
        }
        ...
        void visit(::HIR::ExprNodeCast& node) override {
            ...
            if (dst == src) {
                ...
                if (src->is_Function() || src->is_NamedFunction()) {
                    ...
                    auto pp_s = se->hrls.makeEmptyParams(true);
                    ...
                    for (size_t i = 0; i < de->argTypes.size(); i++) {
                        this->equateTypes(node.span(), ms_d.monomorph_type(sp, de->argTypes[i]), ms_s.monomorph_type(sp, se->argTypes[i]));
                    }
                    this->equateTypes(node.span(), ms_d.monomorph_type(sp, de->mRettype), ms_s.monomorph_type(sp, se->mRettype));
                    ...
                    for (size_t i = 0; i < de->argTypes.size(); i++) {
                        this->equateTypes(node.span(), ms_d.monomorph_type(sp, de->argTypes[i]), cnode.mArgs[i].second);
                    }
                    this->equateTypes(node.span(), ms_d.monomorph_type(sp, de->mRettype), cnode.returnType);
                } else {
        ...
        void equateTypeLifetimes(const Span& sp, const HIR::LifetimeRef& dstLft, const HIR::TypeData* ty) {
            struct V: public HIR::Visitor {
                ...
                void visit_type(HIR::TypeRef& t) override {
                    ...
                    auto push_hrls = [&](const HIR::GenericParams& hrlsDef) {
                        hrlPushed = true;
        ...
        void unsize_types(const Span& sp, const ::HIR::TypeData* dst, const ::HIR::TypeData* src) {
            ...
            // If the inner is a path, it must be a struct with an `Unsize` impl somewhere
                ...
                return unsize_types(sp, dstP.mParams.types.at(param), src_p.mParams.types.at(param));
            }
        ...
        void coerceUnsizeTypes(const Span& sp, const ::HIR::TypeData* dst, const ::HIR::TypeData* src) {
            if (dst == src) {
                ...
                this->equateLifetimes(sp, dstTe->lifetime, src_te.lifetime);

                unsize_types(sp, dstTe->inner, src_te.inner);
            ...
            // If the type is a path, it must be a struct with an `CoerceUnsize` impl somewhere
                ...
                switch (str.structMarkings.coerceUnsized) {
                    ...
                    case HIR::StructMarkings::Coerce::Passthrough:
                        return coerceUnsizeTypes(sp, dstP.mParams.types.at(param), src_p.mParams.types.at(param));
        ...
        void visit(::HIR::ExprNodeIndex& node) override {
            HIR::ExprVisitorDef::visit(node);
            auto ty = mResolve.crate.types.path(HIR::Path(node.mValue->resType, HIR::GenericPath(mResolve.crate.getLangItemPath(node.span(), "index"), {node.index->resType}), RcString::new_interned("Output")), {});
        ...
        void visit(::HIR::ExprNodeDeref& node) override {
            ...
            TU_MATCH_HDRA( (*node.mValue->resType), {)
            default: {
                    auto ty = mResolve.crate.types.path(HIR::Path(node.mValue->resType, HIR::GenericPath(mResolve.crate.getLangItemPath(node.span(), "deref"), {}), RcString::new_interned("Target")), {});
                    mResolve.expandAssociatedTypes(node.span(), ty);
                    this->equateTypes(node.span(), node.resType, ty);
                }
                TU_ARMA(Pointer, te) {
        ...
        void visit(::HIR::ExprNodeField& node) override {
            ...
            if (const auto* te = str_ty->opt_Tuple()) {
                ...
                auto fldTy = mResolve.monomorph_expand(sp, fldTyPtr, ms);
        ...
        void visit(::HIR::ExprNodeCallPath& node) override {
            ...
            if (const auto* pe = node.mPath.mData.opt_UfcsInherent()) {
                this->equatePps(node.span(), pe->impl_params, *ms.pp_impl);
            }
            ...
            this->equateTypes(node.span(), node.cache.argTypes.back(), ret_mono);
        ...
        void visit(::HIR::ExprNodeCallValue& node) override {
            ...
            if (const auto* tep = val_ty->opt_Function()) {
                ...
                this->visit_path_params(hrlParams);
                ...
                for (size_t i = 0; i < tep->argTypes.size(); i++) {
                    this->equateTypes(node.mArgs[i]->span(), ms.monomorph_type(node.span(), tep->argTypes[i], false), node.mArgs[i]->resType);
                }
                this->equateTypes(node.span(), node.resType, ms.monomorph_type(node.span(), tep->mRettype, false));
                ...
                if ((*node_pp)->objPath.mPath != HIR::SimplePath()) {
                    ...
                    for (size_t i = 0; i < node.mArgs.size(); i++) {
                        this->equateTypes(node.mArgs[i]->span(), (*node_pp)->mArgs[i].second, node.mArgs[i]->resType);
                    }
                    this->equateTypes(node.span(), node.resType, (*node_pp)->returnType);
                }
                ...
                bool found = mResolve.findImpl(node.span(), trait, &params, val_ty, [&](ImplRef implRef, bool fuzzy) -> bool {
                    ...
                    auto res_ty = implRef.getType(mResolve.crate.types, "Output", {});
                    mResolve.expandAssociatedTypes(node.span(), res_ty);
                    equateTypes(node.span(), node.resType, res_ty);
        ...
        void visit(::HIR::ExprNodeCallMethod& node) override {
            ...
            DEBUG("RET " << ret_mono);
            this->equateTypes(node.span(), node.resType, ret_mono);
        ...
        void visit(::HIR::ExprNodeLiteral& node) override {
            if (node.mData.is_String() || node.mData.is_ByteString()) {
                ...
                this->equateLifetimes(node.span(), be.lifetime, ::HIR::LifetimeRef::new_static());
            }
        }
        ...
        void visit(::HIR::ExprNodeTupleVariant& node) override {
            ...
            for( unsigned int i = 0; i < node.mArgs.size(); i ++ )
            {
                ...
                auto desTy = mResolve.monomorph_expand(node.mArgs[i]->span(), desTyR, ms);
        ...
        void visit(::HIR::ExprNodeStructLiteral& node) override {
            ...
            if( node.baseValue ) {
                ...
                for (const auto& fld : fields) {
                    ...
                    if (it == node.values.end()) {
                        auto dstTy = mResolve.monomorph_expand(node.span(), fld.ty, ms);
                        auto src_ty = mResolve.monomorph_expand(node.span(), fld.ty, ms_base);
                        DEBUG("BASE ." << fld.name << ": " << dstTy << " = " << src_ty);
                        this->equateTypes(node.baseValue->span(), dstTy, src_ty);
                    }
                }
            }
            ...
            for(auto& val : node.values)
            {
                ...
                DEBUG(name << " : " << desTyR);
                auto desTy = mResolve.monomorph_expand(val.second->span(), desTyR, ms);
                DEBUG("." << name << " : " << desTy);
                ...
                for (const auto& cap : node.avuCache.capturedVars) {
                    if (cap.usage != ::HIR::ValueUsage::Move) {
                        ...
                        if (derefSource->is_Infer()) {
                            ...
                            equateTypeLifetimes(node.span(), node.captureLifetime, wrapped_ty);
                        } else {
    ...
    void HIRExpandLifetimeInferExprInner(LocalTraitResolve& resolve, const ::HIR::Function::argsT& args, const HIR::TypeData* ret_ty, HIR::ExprPtr& ep, bool remove_locals, bool isConstContext) {
        ...
        for (size_t curIter = 0, remaining_iters = 1000; remaining_iters--; curIter++) {
            ...
            auto set = [&](LifetimeInferState::IvarLifetime& iv, const HIR::LifetimeRef& lft, const char* logReason) {
                assert(!iv.isKnown());
                auto real_lft = state.getFinalLft(iv.sp, lft);
                DEBUG(state.getLftForIv(iv) << " := " << real_lft << "[" << lft << "] (" << logReason << ")");
                ASSERT_BUG(iv.sp, real_lft != state.getLftForIv(iv), real_lft);
                iv.known = real_lft;
                change = true;
            };
...

        {
            ...
            for (auto& iv : state.ivars) {
                ...
                ASSERT_BUG(iv.sp, !state.opt_ivar(iv.sp, iv.known), "Lifetime resolved to an ivar - " << state.getLftForIv(iv) << " = " << iv.known);
                for (auto& l : iv.destinations) {
                    l = state.getFinalLft(iv.sp, l);
                }
            }
            ...
            cloneTyWith(resolve.crate.types, ep->mSpan, ret_ty, [&](const HIR::TypeData* tpl, HIR::TypeRef& rv) -> bool {
                if (tpl->is_ErasedType() && tpl->as_ErasedType().inner.is_Fcn()) {
                    ...
                    struct V: public HIR::Visitor {
                        ...
                        void equateLifetime(const HIR::LifetimeRef& src) {
                            const auto& src_real = state.getFinalLft(sp, src);
                            if (src_real.isHrl()) {
                                ...
                                for (const auto& dstLft : dstLfts) {
                                    ...
                                    if (state.checkLifetimes(sp, dstLft, src_real, fails)) {
                                        return;
                                    }
                                    DEBUG(dstLft << " = " << src_real << " (" << src << ") failed: " << fails);
                                }
                                ERROR(sp, E0000, "None of erased lifetime lifetime bounds [" << dstLfts << "]: " << src_real << " (" << src << ") passed");
                            }
                        }
                        ...
                        void visit_capture(const ::HIR::LifetimeRef& capture_lifetime, HIR::ValueUsage usage, unsigned root_slot, const std::vector<RcString>& fields) {
                            ...
                            for (const auto& n : fields) {
                                ...
                                tmp_ty = state.mResolve.getFieldType(sp, *capTyP, n);
                                state.mResolve.expandAssociatedTypes(sp, tmp_ty);
                                capTyP = &tmp_ty;
                            }
        ...
        // Commit lifetimes in TAITs
        {
            for (auto& ent : state.taitValues) {
                ...
                ASSERT_BUG(sp, tait_inner.type->equalsIgnoringRegions(transformed_type), "Lifetime expanded TAIT type did not match existing!:\n" << "- Was: " << tait_inner.type << "- Now: " << transformed_type);
        ...
        if (true) { //m_lifetime_state ) {
            ...
            if (a != typeEqualities.end()) {
                ...
                if (!a->second.hrbs.is_empty()) {
                    ...
                    a->first->matchTestGenerics(sp, input, resolve_placeholders, m);
...
namespace {
    ...
    public:
        ...
        ::HIR::ExprNodeP doReborrow(::HIR::ExprNodeP node_ptr) {
            assert(node_ptr);
...
namespace static_borrow_constants {
    ...
    public:
        ...
        void visit(::HIR::ExprNodeBorrow& node) override {
            ...
            if (allConstant) {
                ...
                for (;;) {
                    if (auto* innerNode = cast<::HIR::ExprNodeIndex>(value_ptr_ptr->get())) {
                        value_ptr_ptr = &innerNode->mValue;
                        continue;
                    }
                    if (auto* innerNode = cast<::HIR::ExprNodeUnsize>(value_ptr_ptr->get())) {
                        value_ptr_ptr = &innerNode->mValue;
                        continue;
                    }
                    break;
                ...
                if (false && !isZst && monomorphise_type_needed(value_ptr->resType)) {
        ...
        void visit(::HIR::ExprNodeCallMethod& node) override {
            ...
            if (allConstant) {
                ...
                auto v = mResolve.getValue(node.span(), node.methodPath, ms_unused, true);
        ...
        void visit(::HIR::ExprNodeCallPath& node) override {
            ...
            if (allConstant) {
                MonomorphState ms_unused(mResolve.crate.types);
                auto v = mResolve.getValue(node.span(), node.mPath, ms_unused, true);
    ...
    public:
        ...
        Monomorph createParams(const Span& sp, ::HIR::GenericParams& params, ::HIR::PathParams& constructorPathParams) const {
            DEBUG("Impl: " << mResolve.impl_generics().fmtArgs());
            DEBUG("Item: " << mResolve.item_generics().fmtArgs());
            ...
            for (const auto& lftDef : mResolve.impl_generics().mLifetimes) {
                unsigned i = &lftDef - &mResolve.impl_generics().mLifetimes.front();
            ...
            for (const auto& lftDef : mResolve.item_generics().mLifetimes) {
                unsigned i = &lftDef - &mResolve.item_generics().mLifetimes.front();
            ...
            DEBUG("impl_path_params = " << params.makeNopParams(mResolve.crate.types, 0) << " ofs_*_t=" << ofs_item_t << "," << ofs_impl_t << "," << params.types.size() << " ofs_*_v=" << ofs_item_v << "," << ofs_impl_v << "," << params.values.size() << " ofs_*_l=" << ofs_item_l << "," << ofs_impl_l << "," << params.mLifetimes.size());
        ...
        HIR::ExprPtr extractNode(HIR::ExprNodeP& node, StaticTraitResolve& resolve, HIR::GenericParams& params_def, HIR::PathParams& constrParams) {
            auto monomorph = this->createParams(node->span(), params_def, constrParams);
        ...
        void visit(::HIR::ExprNodeBorrow& node) override {
            ...
            if (node.isValidStaticBorrowConstant) {
                ...
                // If the inner value is already a static or it's a constant, then treat as a valid static
                auto& value_ptr = *value_ptr_ptr;
                if (auto* innerNode = cast<::HIR::ExprNodeDeref>(value_ptr_ptr->get())) {
                    (void)innerNode;
                ...
                auto val_expr = extractNode(value_ptr, resolve, params_def, constrParams);
                ...
                resolve.expandAssociatedTypes(sp, static_ty);
        ...
        void visit(::HIR::ExprNodeConstBlock& node) {
            ...
            if (cast<HIR::ExprNodePathValue>(node.inner.get())) {
                ...
                HIR::GenericParams params_def;
                HIR::PathParams constrParams;
                auto val_expr = extractNode(node.inner, resolve, params_def, constrParams);
    ...
    public:
        ...
        StaticBorrowExprVisitorMutate::t_new_static_cb getNewTyCb() {
        ...
        void visit_crate(::HIR::Crate& crate) override {
            ...
            for (auto& mod_list : newStatics) {
                ...
                for (auto& new_static_pair : mod_list.second) {
                    ...
                    TRACE_FUNCTION_F("New static " << new_static_pair.path << new_static.mParams.fmtArgs());

                    if (!new_static.mParams.isGeneric()) {
                        new_static.mValue.state->stage = ::HIR::ExprState::Stage::Sbc;
                        new_static.valueRes = ::HIR::Evaluator(sp, this->crate, nvs).evaluateConstant(new_static_pair.path, new_static.mValue, new_static.mType);
                        new_static.valueGenerated = true;
                    }
...
void HIRExpandStaticBorrowConstantsExpr(const ::HIR::Crate& crate, const ::HIR::ItemPath& ip, ::HIR::ExprPtr& exp) {
    ...
    const HIR::TypeData* self_type = ip.getTopIp().ty;
    ...
    static_borrow_constants::StaticBorrowExprVisitorMutate ev(resolve, self_type, [&](Span sp, HIR::TypeRef ty, HIR::ExprPtr val_expr, HIR::GenericParams generics, bool is_const) -> HIR::SimplePath {
        ...
        if (!new_static.mParams.isGeneric()) {
            new_static.mValue.state->stage = ::HIR::ExprState::Stage::Sbc;
            new_static.valueRes = ::HIR::Evaluator(sp, crate, nvs).evaluateConstant(path, new_static.mValue, new_static.mType);
            new_static.valueGenerated = true;
        }
...
namespace {
    ...
    public:
        ...
        void visit(::HIR::ExprNodeCallValue& node) override {
            ...
            switch (node.traitUsed) {
                case ::HIR::ExprNodeCallValue::TraitUsed::Fn:
                    ...
                    method_path = ::HIR::Path(ty_val, ::HIR::GenericPath(crate.getLangItemPath(sp, "fn"), mv$(trait_args)), RcString::new_interned("call"), HIR::PathParams(HIR::LifetimeRef()));
                    break;
                case ::HIR::ExprNodeCallValue::TraitUsed::FnMut:
                    ...
                    method_path = ::HIR::Path(ty_val, ::HIR::GenericPath(crate.getLangItemPath(sp, "fn_mut"), mv$(trait_args)), RcString::new_interned("call_mut"), HIR::PathParams(HIR::LifetimeRef()));
                    break;
                case ::HIR::ExprNodeCallValue::TraitUsed::FnOnce:
                    self_arg_type = ty_val;
                    method_path = ::HIR::Path(ty_val, ::HIR::GenericPath(crate.getLangItemPath(sp, "fn_once"), mv$(trait_args)), RcString::new_interned("call_once"));
                    break;
        ...
        bool isBuiltinOperator(const Span& sp, typeck::PrimitiveOperator op, const char* langitem, const ::HIR::TypeData* ty_l, const ::HIR::TypeData* ty_r) const {
            if (!typeck::primitive_operator_has_builtin(op, ty_l, ty_r)) {
                return false;
            }

            ::HIR::PathParams trait_params(ty_r);
            const auto& trait_path = crate.getLangItemPathOpt(langitem);
            if (trait_path.components().empty()) {
                return true;
            }
            return !mResolve.findImpl(sp, trait_path, trait_params, ty_l, [&](ImplRef impl, bool) {
        ...
        bool isBuiltinOperator(const Span& sp, typeck::PrimitiveOperator op, const char* langitem, const ::HIR::TypeData* ty) const {
            ...
            return !mResolve.findImpl(sp, trait_path, ::HIR::PathParams(), ty, [&](ImplRef impl, bool) {
                const auto* trait_impl = impl.mData.opt_TraitImpl();
                return !(currentTraitImpl && trait_impl && trait_impl->impl == currentTraitImpl);
            });
        }
...
#define _(opname) case ::HIR::ExprNodeAssign::Op::opname
            switch (node.op) {
                ...
                if (isBuiltinOperator(sp, operator_kind, langitem, ty_slot, ty_val)) {
                    return;
                }
                break;
...
#undef _
            ...
            ::HIR::GenericPath trait{crate.getLangItemPath(node.span(), langitem), ::HIR::PathParams(ty_val)};
        ...
        void visit(::HIR::ExprNodeBinOp& node) override {
            ...
            if (isBuiltinOperator(sp, operator_kind, langitem, ty_l, ty_r)) {
                return;
            }
            ...
            if (isComparison) {
                ...
                ::HIR::GenericPath trait{crate.getLangItemPath(node.span(), langitem), mv$(trait_params)};
        ...
        void visit(::HIR::ExprNodeUniOp& node) override {
            ...
            if (isBuiltinOperator(sp, operator_kind, langitem, ty_val)) {
                return;
            }
        ...
        void visit(::HIR::ExprNodeUnsize& node) override {
            ...
            if (node.isArrayToSliceAdjustment) {
                const Span& sp = node.span();
                ASSERT_BUG(sp, borrowNode && src_borrow && dstBorrow, "Malformed array-to-slice adjustment");
                ASSERT_BUG(sp, src_borrow->inner->is_Array() && dstBorrow->inner->is_Slice(), "Invalid array-to-slice adjustment types");
...
namespace {
        ...
        void visit_type(::HIR::TypeRef& ty) override {
            if (methodName) {
                ...
                if (const auto* e = ty->opt_ErasedType()) {
                    ...
                    ty = type_interner().path(HIR::Path(selfTy ? selfTy : type_interner().self(), HIR::GenericPath(traitPath->clone(), traitArgs->clone()), ty_name, methodParams->makeNopParams(type_interner(), 1)), {});
                }
            }
        }

        void handleMethod(const ::HIR::SimplePath& trait_path, const ::HIR::PathParams& trait_args, const HIR::TypeData* self_ty, const RcString& name, ::HIR::Function& fcn) {
    ...
    public:
        void visit_trait(::HIR::ItemPath p, ::HIR::Trait& tr) override {
            ...
            auto path = p.getSimplePath();
            auto params = tr.mParams.makeNopParams(type_interner(), 0);
            targetTrait = &tr;
        ...
        void visit_trait_impl(const ::HIR::SimplePath& trait_path, ::HIR::TraitImpl& impl) override {
            ...
            for (auto& v : impl.methods) {
                handleMethod(trait_path, impl.traitArgs, impl.mType, v.first, v.second.data);
            }
            targetImpl = nullptr;
        }
    };
    ...
    public:
        ...
        void visit_trait(::HIR::ItemPath p, ::HIR::Trait& tr) override {
            ...
            ::HIR::GenericPath trait_path(p.getSimplePath(), tr.mParams.makeNopParams(crate.types, 0));
                ...
                bool addEntsFromTrait(const ::HIR::Trait& tr, const ::HIR::GenericPath& trait_path, std::vector<bool>* supertrait_flags) {
                    ...
                    struct M: public Monomorphiser {
                        ...
                        HIR::TypeRef monomorph_type(const Span& sp, const HIR::TypeData* t, bool allowInfer = true) const override {
                            if (t->is_Path() && t->as_Path().path.mData.is_UfcsKnown()) {
                                ...
                                bool hasItem = (it != trait_ptr->typeIndexes.end());
                    ...
                    for (auto& vi : tr.values) {
                        ...
                        TU_ARMA(Function, ve) {
                                ...
                                if (visit_ty_with(fcnType, [&](const auto& t) {
                                    return (t == types.self());
                                })) {
                                ...
                                fields.push_back(HIR::StructField{vi.first, ::HIR::Publicity::new_global(), mv$(fcnType), {}});
                            }
                            TU_ARMA(Static, ve) {
                                if (vi.first != "vtable#") {
            ...
            if (!vtc.addEntsFromTrait(tr, trait_path, &supertrait_flags) || hasConflictingAtyName) {
            ...
            }
            {
                ...
                for (const auto& ty : tr.typeIndexes) {
                    ...
                    auto ty_def = tr.getAtyDef(ty.first);
                    ASSERT_BUG(sp, ty_def.first, "Unable to find ATY defintiion");
                    auto atyParams = ty_def.first->generics.makeNopParams(crate.types, 1);
            ...
            auto itemPath = newType(true, RcString::new_interned(FMT(p.getName() << "#vtable")), ::HIR::Struct(std::move(args), ::HIR::Struct::Repr::C, ::HIR::Struct::Data(mv$(fields))));
    ...
    public:
        ...
        void visit_struct(HIR::ItemPath ip, HIR::Struct& str) {
            ...
            if (p && std::strcmp(p, "#vtable") == 0) {
                auto trait_path = ip.parent->getSimplePath();
                ...
                for (size_t i = 0; i < trait.allParentTraits.size(); i++) {
                    ...
                    if (parent_trait.vtablePath == HIR::SimplePath()) {
                        ...
                        te.binding = &crate.getStructByPath(sp, vtable_gpath.mPath);
                        ...
                        for (const auto& atyIdx : parent_trait.typeIndexes) {
                            ...
                            // If this type is not in the trait path, then check if it has a defined generic
                                ...
                                auto gpMono = MonomorphStatePtr(crate.types, nullptr, &pt.mPath.mParams, nullptr).monomorph_genericpath(sp, *gp);
                                ...
                                ASSERT_BUG(sp, p, "Failed to find " << gpMono << " in parent trait list for " << trait_path);
