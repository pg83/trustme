...
auto AnnotateExprVisitorMark::getUsageForPatternBinding(const Span& sp, const HIRPatternBinding& pb, const HIRTypeData* ty) const -> HIRValueUsage {
    switch (pb.type) {
...
auto AnnotateExprVisitorMark::getUsageForPattern(const Span& sp, const HIRPattern& pat, const HIRTypeData* outerTy) const -> HIRValueUsage {
...
auto AnnotateExprVisitorMark::getRealUsage(const Span& sp, unsigned slot, const std::vector<RcString>& fields, HIRValueUsage usage) -> HIRValueUsage {
...
auto AnnotateExprVisitorMark::markUsedVariableClosure(const Span& sp, ClosureScope& closureRec, unsigned slot, std::vector<RcString> fields, HIRValueUsage usage) -> void {
...
auto AnnotateExprVisitorMark::markUsedVariableGenerator(const Span& sp, CoroutineScope& scope, unsigned int slot, std::vector<RcString> fields, HIRValueUsage usage) -> void {
    auto& e = scope.usedVariables[slot];
...
ClosureExprVisitorMutate::ClosureExprVisitorMutate(ObjPool* pool, const HIRTypeData* closureType, const std::vector<unsigned int>& localVars, const std::vector<HIRExprNodeClosure::AvuCache::Capture>& captures, const Monomorphiser& mcb)
...
[[nodiscard]] auto ClosureExprVisitorMutate::visitType(HIRTypeRef ty) -> HIRTypeRef {
    DEBUG(ty);
...
auto AnonymousTypeMonomorph::monomorphType(const Span& sp, const HIRTypeData* ty, bool allowInfer) const -> HIRTypeRef {
    if (const auto* e = ty->opt_NodeType()) {
        ...
        if (!object || *path == HIRGenericPath()) {
            ASSERT_BUG(sp, allowUnextracted, kind << " type was used before extraction");
            return ty;
        }
        DEBUG(kind << ": " << *path);
        auto concretePath = pathMonomorphiser.monomorphGenericpath(sp, *path, false);
        DEBUG(ty << " -> " << concretePath);
...
[[nodiscard]] auto ClosureExprVisitorFixup::visitType(HIRTypeRef ty) -> HIRTypeRef {
    AnonymousTypeMonomorph m{monomorphiser, allowUnextracted};
    return m.monomorphType(Span(), ty, true);
}

auto H::fixFnParams(HIRExprPtr& code, const HIRTypeData* selfTy, const HIRTypeData* argsTy) -> void {
...
auto H::makeFnonce(HIRGenericParams params, HIRPathParams traitParams, HIRTypeRef closureType, std::pair<HIRPattern, HIRTypeRef> argsArgent, HIRTypeRef retTy, HIRExprPtr code) -> HIRTraitImpl {
...
auto H::makeFnmut(HIRTypeInterner& types, HIRGenericParams params, HIRPathParams traitParams, HIRTypeRef closureType, std::pair<HIRPattern, HIRTypeRef> argsArgent, HIRTypeRef retTy, HIRExprPtr code) -> HIRTraitImpl {
...
auto H::makeFn(HIRTypeInterner& types, HIRGenericParams params, HIRPathParams traitParams, HIRTypeRef closureType, std::pair<HIRPattern, HIRTypeRef> argsArgent, HIRTypeRef retTy, HIRExprPtr code) -> HIRTraitImpl {
...
ClosureExprVisitorExtract::ClosureExprVisitorExtract(const StaticTraitResolve& resolve, const HIRTypeData* selfType, const std::vector<HIRTypeRef>& varTypes, const HIRExprPtr& exprPtr, OutState& out, const char* newTypeSuffix, bool isAsyncDropIntrinsic)
...
auto ClosureExprVisitorExtract::extractReferencedNodeTypes(const Span& sp, HIRTypeRef type) -> void {
...
auto ClosureExprVisitorExtract::visit(HIRExprNodeClosure& node) -> void {
    ...
    localTypes.push_back(resolve_.hirCrate().types.infer());
    for (const auto bindingIdx : node.avuCache.localVars) {
...
auto ClosureExprVisitorExtract::setStateType(const Span& sp, CrVars& vars, HIRTypeRef stateType) const -> void {
...
auto ClosureExprVisitorExtract::DeferredExprFixup::visitRoot(HIRExprPtr& root) -> void {
    root->visit(*this);
    updateType(root->resType);
    for (auto& type : root.bindings) {
        updateType(type);
    }
    for (auto& type : root.erasedTypes) {
        updateType(type);
    }
}

auto ClosureExprVisitorExtract::DeferredExprFixup::visitType(HIRTypeRef type) -> HIRTypeRef {
...
auto ClosureExprVisitorExtract::Monomorph::getType(const Span& sp, const HIRGenericRef& ge) const -> HIRTypeRef {
    size_t rv = SIZE_MAX;
...
auto ClosureExprVisitorExtract::Monomorph::monomorphType(const Span& sp, const HIRTypeData* tpl, bool allowInfer) const -> HIRTypeRef {
...
auto ClosureExprVisitorExtract::Monomorph::updateTypeNeed(TypeNeed& rv, const HIRTypeData* t) const -> bool {
    if (t->is_Generic()) {
...
auto ClosureExprVisitorExtract::Monomorph::typeBoundNeeded(const Span& sp, const HIRTypeData* ty) const -> TypeNeed {
...
auto ClosureExprVisitorExtract::CrVars::setArguments(const Span& sp, std::vector<HIRTypeRef> args) -> void {
...
[[nodiscard]] auto ClosureOuterVisitor::visitType(HIRTypeRef ty) -> HIRTypeRef {
    if (ty->is_Array()) {
...
[[nodiscard]] auto ErasedExprVisitorExtract::visitType(HIRTypeRef ty) -> HIRTypeRef {
    Span sp;
...
[[nodiscard]] auto ErasedOuterVisitorFixup::visitType(HIRTypeRef ty) -> HIRTypeRef {
    Span sp;
    ::visitType(sp, resolve_, ty);
    return ty;
}
...
StaticBorrowExprVisitorMark::StaticBorrowExprVisitorMark(const StaticTraitResolve& resolve, const HIRTypeData* selfType, const HIRExprPtr& exprPtr, bool promoteAllConstFnCalls)
    : HIRExprVisitorDef(resolve.hirCrate().types)
    , resolve_(resolve)
    , selfType(selfType)
    , exprPtr(exprPtr)
...
[[nodiscard]] auto StaticBorrowOuterVisitorMark::visitType(HIRTypeRef ty) -> HIRTypeRef {
    return visitTypeDefaultViaHooks(ty);
}
...
template <typename F>
auto NewStaticCb<F>::create(Span sp, HIRTypeRef type, HIRExprPtr value, HIRGenericParams generics, bool isConst) -> HIRSimplePath {
    return f(sp, mv$(type), mv$(value), mv$(generics), isConst);
}

StaticBorrowExprVisitorMutate::StaticBorrowExprVisitorMutate(const StaticTraitResolve& resolve, const HIRTypeData* selfType, NewStaticCallback& newStaticCb, const HIRExprPtr& exprPtr)
    : HIRExprVisitorDef(resolve.hirCrate().types)
    , resolve_(resolve)
    , selfType(selfType)
...
[[nodiscard]] auto StaticBorrowOuterVisitor::visitType(HIRTypeRef ty) -> HIRTypeRef {
    return visitTypeDefaultViaHooks(ty);
}
...
auto UfcsExprVisitorMutate::isBuiltinOperator(const Span& sp, TypeckPrimitiveOperator op, const char* langitem, const HIRTypeData* tyL, const HIRTypeData* tyR) const -> bool {
    ...
    return !resolve_.findImpl(sp, traitPath, traitParams, tyL, [&](SolverResponse response) {
        return !response.impl || !(currentTraitImpl && response.impl->traitImpl == currentTraitImpl);
    });
}

auto UfcsExprVisitorMutate::isBuiltinOperator(const Span& sp, TypeckPrimitiveOperator op, const char* langitem, const HIRTypeData* ty) const -> bool {
...
[[nodiscard]] auto UfcsOuterVisitor::visitType(HIRTypeRef ty) -> HIRTypeRef {
    return visitTypeDefaultViaHooks(ty);
}
...
auto VisitorImplTrait::handleMethod(const HIRSimplePath& traitPath, const HIRPathParams& traitArgs, const HIRTypeData* selfTy, const RcString& name, HIRFunction& fcn) -> void {
