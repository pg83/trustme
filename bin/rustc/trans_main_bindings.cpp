...
static void enumerateDestructorType(EnumState& state, HIRTypeRef type) {
...
static HIRTypeRef implicitDropType(const HIRPath& path, const HIRSimplePath& dropTrait) {
...
static void TransEnumerateCommonPostRun(EnumState& state) {
    while (!state.fcnQueue.empty()) {
        ...
        TRACE_FUNCTION_F("Function " << find_if(state.rv.functions.begin(), state.rv.functions.end(), [&](const auto& x) {
...
bool TransEnumerateGeneratedLiteral(const WireBoard& wb, TransList& list, const EncodedLiteral& literal) {
    ...
    for (const auto& relocation : literal.relocations) {
        if (relocation.p && !transListContainsPath(list, *relocation.p)) {
            ASSERT_BUG(Span(), !monomorphisePathNeeded(*relocation.p), "Generated literal contains a generic translation path: " << *relocation.p);
...
auto Builder::inTemporary(HIRTypeRef ty, MIRRValue val) -> MIRLValue {
