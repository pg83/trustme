...
public:
    ...
    public:
        ...
        iterator operator+(size_t i) const {
            return iterator(m_inner + i);
        }

        bool operator==(const iterator& x) const {
            return m_inner == x.m_inner;
        }
    ...
    public:
        ...
        const_iterator operator+(size_t i) const {
            return const_iterator(m_inner + i);
        }
    ...
    iterator lower_bound(const K2& k) {
        return iterator(std::lower_bound(m_data.begin(), m_data.end(), k, [&](const ::std::unique_ptr<item_t>& kv, const K2& k) {
            return m_cmp(kv->first, k);
        }));
    }

    template <typename K2>
    iterator upper_bound(const K2& k) {
        return iterator(std::upper_bound(m_data.begin(), m_data.end(), k, [&](const K2& k, const ::std::unique_ptr<item_t>& kv) {
            return m_cmp(k, kv->first);
        }));
    }
    ...
    const_iterator lower_bound(const K2& k) const {
        return const_iterator(std::lower_bound(m_data.begin(), m_data.end(), k, [&](const ::std::unique_ptr<item_t>& kv, const K2& k) {
            return m_cmp(kv->first, k);
        }));
    }
    ...
    const_iterator upper_bound(const K2& k) const {
        return const_iterator(std::upper_bound(m_data.begin(), m_data.end(), k, [&](const K2& k, const ::std::unique_ptr<item_t>& kv) {
            return m_cmp(k, kv->first);
        }));
    }
