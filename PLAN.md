...
Полный исходник 1.90 у нас уже является артефактом `std_src`: [build.py](/home/pg/monorepo/trustme/build.py:227), [fetch.py](/home/pg/monorepo/trustme/tests/std/fetch.py:18). Но тесты не должны зависеть от этого архива: нужные файлы копируются из временного checkout непосредственно в `tests/rust_1_90/upstream/` и хранятся в git пофайлово.
...
Первая реализация сохраняет один исходный тест на одну build-ноду. К шардированию стоит переходить только после того, как корпус собран и измерен.
