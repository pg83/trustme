...
| still failing on the current tree | 465 |
| fixed, or no longer reproducing, since the gate | 166 |
...
Sixty-seven programs build but execute incorrectly:

| runtime result | tests | note |
|---|---:|---|
| Rust panic, exit 101 | 61 | group by the failed semantic assertion, never by exit code |
