...
## P1 — compiler crashes on valid code (~184)

One cluster = one bug. Fixing the top 2 clusters greens ~6 tests.

| location | tests | example |
|---|---|---|
| `hir_typeck_expr_cs.cpp:8507` "Spare rules left" | 1 | `pattern/byte-string-inference.rs` |
...
1. Top-2 crash clusters (~6 tests)
