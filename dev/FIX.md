...
| still failing on the current tree | 407 |
| fixed, or no longer reproducing, since the gate | 224 |
...
| wrong runtime behaviour, panic, abort, or output | 51 |
...
Fifty-one programs build but execute incorrectly:

| runtime result | tests | note |
|---|---:|---|
| Rust panic, exit 101 | 45 | group by the failed semantic assertion, never by exit code |
