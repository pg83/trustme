...
| still failing on the current tree | 406 |
| fixed, or no longer reproducing, since the gate | 225 |
...
| wrong runtime behaviour, panic, abort, or output | 50 |
...
Fifty programs build but execute incorrectly:

| runtime result | tests | note |
|---|---:|---|
| Rust panic, exit 101 | 44 | group by the failed semantic assertion, never by exit code |
