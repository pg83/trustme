...
1. Repeated parser feature families: 10 associated-type lifetime-bound forms,
   9 explicit tail calls using `become`, 9 frontmatter forms, and 7
   ergonomic-clone `use` forms. The three remaining `TOK_DOUBLE_DOT` failures
   are range/expression syntax.
...
The 549 unfinished ordinary compiler rejections and one pathless-`--extern`
driver rejection split as follows:

| area | tests | largest stable groups |
|---|---:|---|
| parser | 208 | 99 at `parse_parseerror.cpp:63`, 87 at line 56, 19 at line 68, 3 in `parse_common.cpp` |
