...
Следующий функциональный пункт — GAT/lifetime relation для `<P as Pattern>::Searcher<'_>: Searcher<'a>` в `str/iter.rs:1077` полного `core -Znext-solver`, затем удаление глобальной fuzzy relation/impl matching, строго unit-first. Macro hygiene остаётся отдельной архитектурной задачей.
