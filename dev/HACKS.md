...
3. Trait solver использует глобальное «не знаю — значит fuzzy».
   ...
   Coinductivity больше не выводится только из auto-trait: отдельный HIR-флаг повторяет rustc 1.90 `TraitDef::is_coinductive`, устанавливается для auto traits и `#[rustc_coinductive]` и сохраняется в cross-crate metadata. Минимальный explicit-cycle unit, а также upstream `provisional-cache-impacts-behavior.rs` и `opportunistic-region-resolve.rs` зелёные. `leak-check-coinductive-cycle.rs` и `inductive-cycle-discarded-coinductive-constraints.rs` всё ещё не выводят ответные type constraints; это незакрытая часть provisional-response модели, а не повод снова считать любой fuzzy cycle доказанным.
...
Следующая последовательность: ложные markers в parser/lexer, infrastructure, driver, общих HIR definitions, metadata, безопасных MIR invariants и backend conventions переименованы. `Pointee::Metadata`, always-unsized layout, array→slice HIR, generic array metadata, grouped match, or-pattern guard/source order и pattern/remainder drop scheduling закрыты. Явная финальная ambiguity associated goal, probe/confirm для method autoderef, feature-gated subtrait shadowing и базовая explicit coinductivity закрыты; следующий функциональный пункт — перенос constraints из provisional responses и оставшаяся глобальная fuzzy relation/impl matching с максимальным fan-out, строго unit-first. Macro hygiene остаётся отдельным архитектурным пунктом, но больше не оправдывает тихое удаление proc-macro.
