...
- Дыра в self-типе UFCS-вызова не заполняется через аргумент.
  `<*const _>::from(&value)` в upstream разрешается рефлексивным
  `impl<T> From<T> for T` (никакого `From<&T> for *const T` в core нет):
  параметр трейта становится `*const _`, и `&i32` коэрцируется в
  `*const i32`, что и задаёт указуемый тип. У нас цель
  `*const _13: From<_5>` собирает двух кандидатов, из которых
  `From<NonZero<T>> for T` даёт 0, а рефлексивный — 2 (доказан), то есть
  выбор однозначен. Но связывание `_5 = *const ?E` выражено через
  собственный экзистенциал impl'а и до `_13` не доходит: после
  `applySolverResponse` `_5` остаётся открытым, коэрция `_5 := &i32`
  откладывается навсегда, и остаётся правило `req ty *const _ impl
  From<&i32>`. Красные примеры: `tst/rust_doctest/upstream/core/mem/
  mod__L1155_21.rs`, `coretests/tests/iter/adapters/intersperse.rs:35`.
  Работает, если указуемый тип задан иначе: аннотацией у let,
  `<*const i32>::from`, либо аргументом уже-указателем.
