...
Следующий функциональный пункт — несовпадение `Iterator::Item` (`Dst/*I:1*/ != placeholder`) для `Copied<&mut I>` в `iter/adapters/copied.rs:195` полного `core -Znext-solver`, затем удаление глобальной fuzzy relation/impl matching, строго unit-first. Macro hygiene остаётся отдельной архитектурной задачей.
