...
Следующий функциональный пункт — projection equality `Ret::Residual = <_ as Try>::Residual` в `array/mod.rs:581` полного `core -Znext-solver`, затем удаление глобальной fuzzy relation/impl matching, строго unit-first. Macro hygiene остаётся отдельной архитектурной задачей.
