...
- 120 строк с `hack/HACK/hacky/hackery/hackiness` в 29 файлах.
- 90 точных употреблений слова `HACK`.
- 114 комментариев и 6 диагностических `DEBUG`-строк.
- По подсистемам: frontend — 42, HIR/typeck — 53, MIR — 6, backend — 17, инфраструктура — 2.
...
4. Drop/move/match содержат реальные семантические обходы.
   ...
   - Grouped match сначала полностью flatten’ится: [mir_from_hir.cpp](/home/pg/monorepo/trustme/bin/rustc/mir_from_hir.cpp:6272).

   Guard-failure для раскрытых or-pattern теперь переходит к следующему кандидату того же arm, а декартово произведение альтернатив сохраняет левосторонний depth-first порядок. Пустой wildcard discard теперь материализует временное значение и сохраняет его drop: закрыты `issues/issue-6892.rs`, `destructuring-assignment/drop-order.rs` и `drop/issue-90752.rs`. Порядок drop для pattern bindings теперь строится из первой/последней match-кандидатуры как в rustc; аргументы, block locals, tail temporaries и let-else initializer/failure scopes разделены. Это дополнительно закрыло `drop/or-pattern-drop-order.rs`, `pattern/move-ref-patterns/move-ref-patterns-dynamic-semantics.rs`, `drop/issue-23338-ensure-param-drop-order.rs` и `let-else/let-else-drop-order.rs`. Non-Copy place на границе expression statement теперь потребляется в statement temporary, что закрыло `drop/issue-48962.rs`. Shallow drop частично перемещённого `Box<T, A>` теперь пропускает synthetic pointee path, но после `Box::drop` уничтожает реальные поля, включая allocator; закрыт `drop/box-conditional-drop-allocator.rs`. Desugared `for` теперь ограничивает head temporaries собственной terminating scope, а static promotion рекурсивно отклоняет весь кандидат, если любой вычисляемый subexpression требует drop; закрыты `drop/for-expr-temporary-drop-scope.rs`, `consts/promoted_const_call4.rs` и `dropck/issue-34053.rs`. GC удаляет drop local только при отсутствии любой записи в достижимых blocks; conditional assignment сохраняет local и drop flag, что закреплено оптимизированным unit. Лишняя ручная отметка `Drop` в bitmap живости удалена: state full-validator определяется assignment/move/drop-flag переходами, а `-O` unit с двумя conditional locals закрепляет как validity, так и temporary unification. CTFE evaluator теперь исполняет drop flags и структурно проверяет только активный enum payload; неживые branches, `None` и неустановленный conditional local разрешены, а живой non-const destructor отклоняется. Значения и заимствованные allocations, достижимые из результа constant, не считаются уничтожаемыми внутри evaluation; это закреплено units для active/inactive flag, `Some`/`None` и returned value/reference. SplitSlice больше не разрушает leading/trailing rulesets при построении их cross-product, что закрыло `or-patterns/slice-patterns.rs`; bindings-after-`@` соседний trigger также зелёный. Всего шестнадцать измеренных runtime failure закрыты. Полное flattening grouped match остаётся архитектурным риском, но все четыре измеренных runtime arm-selection target теперь зелёны; остальные drop/move обходы остаются отдельной работой.

   Box-only move path и его раскрытие до внутреннего pointer не являются trustme-обходом: rustc 1.90 так же создаёт `deref_subpath` только для `Box<T>` и затем заменяет Box deref физическими полями `Unique/NonNull`. Units закрепляют move-out, shallow drop и единичный drop allocator.
