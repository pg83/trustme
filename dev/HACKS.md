...
Следующий функциональный пункт — `Flatten<Option::IntoIter<char::EscapeDebug>>: Iterator` в `str/mod.rs:2955` полного `core -Znext-solver`, затем удаление глобальной fuzzy relation/impl matching, строго unit-first. Macro hygiene остаётся отдельной архитектурной задачей.
