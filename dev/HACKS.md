...
Следующий функциональный пункт — `FnOnce<(usize,)>::Output = Args` для closure в `portable-simd/crates/core_simd/src/masks.rs:440` полного `core -Znext-solver`, затем удаление глобальной fuzzy relation/impl matching, строго unit-first. Macro hygiene остаётся отдельной архитектурной задачей.
