...
def load(path):
    ...
    def context(header):
        # A sub-unit .tu whose generated cpp compiles in another header's
        # context (several include points in one hand-written header).
        # build.py reads this call too, to depend the cpp on that header.
        if context_header[0] is not None:
            raise TuError("context() may only be set once")
        context_header[0] = header

    def local():
        # For unions local to one .cpp: payload types live in that file
        # (often in its anonymous namespace), so no separate TU can compile
        # the bodies. The generated pair stays the usual thin-header/bodies
        # split, but the cpp emits no context include: the client cpp
        # includes the _tu.h at the union's spot and the _tu.cpp at the end
        # of the file, where everything is complete. build.py reads this
        # call too, and does not compile the _tu.cpp separately.
        local_mode[0] = True

    code = compile(path.read_text(encoding="utf-8"), str(path), "exec")
    try:
        exec(code, {"generate": generate, "v": Variant, "context": context,
                    "local": local})
    except TuError as error:
        raise SystemExit(f"{path}: {error}")
    if not unions:
        raise SystemExit(f"{path}: no generate() calls")
    if local_mode[0] and context_header[0] is not None:
        raise SystemExit(f"{path}: local() makes context() meaningless")
    return unions, context_header[0], local_mode[0]
...
def emit_cpp_union(out, union):
    ...
    for _, field_name, *rest in union.extra_fields:
        out.line(f"{field_name} = ::std::move(x.{field_name});")
    if union.allow_incomplete:
        out.line("ptr_ = x.ptr_;")
        out.line("x.ptr_ = nullptr;")
    else:
...
def main():
    ...
    unions, context_header, local_mode = load(input_path)
    if context_header is None:
        context_header = f"{stem}.h"
