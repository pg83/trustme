...
namespace {
    ...
    constexpr size_t META_SEGMENTS = 290;
    ...
    constexpr size_t MASK_BYTES = PAGE / 16 / 8;
    constexpr uintptr_t MASK_TABLE = PAGE_TABLE + PAGE_TABLE_BYTES;
    constexpr size_t MASK_TABLE_BYTES = HEAP_SEGMENTS * PAGES_PER_SEGMENT * MASK_BYTES;
    static_assert(MASK_TABLE + MASK_TABLE_BYTES <= HEAP_BASE);
    ...
    uint64_t* masksOf(const Page* page) {
        return reinterpret_cast<uint64_t*>(MASK_TABLE + size_t(page - pageAt(0)) * MASK_BYTES);
    }
    ...
    void mapSegment() {
        ...
        const size_t tableEnd = MASK_TABLE + size_t(mapped + 1) * PAGES_PER_SEGMENT * MASK_BYTES - 1;
    ...
    void* takeSlot(ClassState& state, uint64_t* word, uint64_t bits) {
        const unsigned bit = unsigned(__builtin_ctzll(bits));
        *word = bits & (bits - 1);
        state.chunk = word;
        state.base = pageAddress(state.current) + uintptr_t(word - masksOf(state.current)) * 64 * state.size;
        state.current->live++;
        return reinterpret_cast<void*>(state.base + bit * state.size);
    }

    void* refillClass(size_t index) {
        auto& state = classes[index];
        if (auto* page = state.current) {
            uint64_t* masks = masksOf(page);
            for (uint32_t k = 0; k < state.chunks; k++) {
                if (masks[k]) {
                    return takeSlot(state, masks + k, masks[k]);
                }
            }
        }
        if (auto* page = state.partial) {
            ...
            refuse("partial page without free slots");
        }
        const size_t size = classSize(index);
        const uint32_t capacity = uint32_t(PAGE / size);
        auto* page = takePages(1);
        page->cls = uint16_t(index);
        state.current = page;
        state.size = size;
        state.inverse = uint32_t(((uint64_t(1) << 32) + size - 1) / size);
        state.chunks = (capacity + 63) / 64;
        uint64_t* masks = masksOf(page);
        for (uint32_t k = 0; k < state.chunks; k++) {
            masks[k] = ~uint64_t(0);
        }
        if (capacity % 64) {
            masks[state.chunks - 1] = (uint64_t(1) << (capacity % 64)) - 1;
        }
        return takeSlot(state, masks, masks[0]);
    }

    void* allocateSmall(size_t index) {
        auto& state = classes[index];
        if (const uint64_t bits = state.chunk ? *state.chunk : 0) {
            const unsigned bit = unsigned(__builtin_ctzll(bits));
            *state.chunk = bits & (bits - 1);
            state.current->live++;
            return reinterpret_cast<void*>(state.base + bit * state.size);
        }
    ...
    void release(void* p) {
        ...
        if (cls < CLASSES) {
            auto& state = classes[cls];
            const uint32_t slot = uint32_t((uint64_t(reinterpret_cast<uintptr_t>(p) - pageAddress(page)) * state.inverse) >> 32);
            masksOf(page)[slot >> 6] |= uint64_t(1) << (slot & 63);
            page->live--;
            if (page == state.current || (page->linked && page->live != 0)) {
                return;
            }
        }
        releaseSlow(p, page);
