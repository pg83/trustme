...
static u64 candAlloc(u64 n, u64 T) {
    if (n > T) return mmapAlloc(n);
    int c = classIndex(n); u64 cs = classSize(c);
    if (cs > P / 4) { u32 k = (u32)((cs + P - 1) / P); u32 pg = takePages(k); pages[pg].cls = 0xFFFF; return pageAddr(pg); }
    u32 pg = 0xFFFFFFFFu;
    if (pagePolicy == 0) { while (clsSN[c]) { u32 q = clsStack[c][clsSN[c] - 1]; if (pages[q].cls == (u32)c && pages[q].nfree) { pg = q; break; } pages[q].inStack = 0; clsSN[c]--; } }
    else { u32 best = 0xFFFFFFFFu, bestScore = 0; u32 j = 0; for (u32 i = 0; i < clsSN[c]; i++) { u32 q = clsStack[c][i]; if (pages[q].cls == (u32)c && pages[q].nfree) { clsStack[c][j++] = q; u32 score = pagePolicy == 1 ? 0xFFFFFFFFu - pages[q].nfree : pages[q].nfree; if (best == 0xFFFFFFFFu || score > bestScore) { best = q; bestScore = score; } } else pages[q].inStack = 0; } clsSN[c] = j; pg = best; }
    u64 a;
    if (pg != 0xFFFFFFFFu) { a = pageAddr(pg) + pages[pg].freeSlots[--pages[pg].nfree]; pages[pg].live++; if (!pages[pg].nfree && pagePolicy == 0) { pages[pg].inStack = 0; clsSN[c]--; } }
    else {
        if (!clsHave[c] || pages[clsCur[c]].off + cs > P) { u32 q = takePages(1); pages[q].cls = (u32)c; clsCur[c] = q; clsHave[c] = 1; }
        pg = clsCur[c]; a = pageAddr(pg) + pages[pg].off; pages[pg].off += (u32)cs; pages[pg].live++;
    }
    return a;
}
static void candFree(u64 a, u64 n, u64 T) {
    if (n > T) return;
    u32 pg = (u32)((a - (0x2000ull << 32)) / P); int c = classIndex(n); u64 cs = classSize(c);
    if (cs > P / 4) { releasePages(pg); return; }
    pages[pg].live--;
    if (pages[pg].live == 0 && pg != clsCur[c]) { pages[pg].cls = 0xFFFE; pages[pg].nfree = 0; pages[pg].inStack = 0; releasePages(pg); return; }
    if (pages[pg].nfree == pages[pg].fcap) { pages[pg].fcap = pages[pg].fcap ? pages[pg].fcap * 2 : 64; pages[pg].freeSlots = realloc(pages[pg].freeSlots, pages[pg].fcap * 4); }
    pages[pg].freeSlots[pages[pg].nfree++] = (u32)(a - pageAddr(pg));
    clsPush(c, pg);
}
int main(int argc, char** argv) {
    if (argc < 3) { fprintf(stderr, "loc trace {real | vert T | bump T | cand P T segPolicy(0 lifo,1 lowest) pagePolicy(0 lifo,1 fullest,2 emptiest)}\n"); return 2; }
    ...
    const char* mode = argv[2]; u64 T = argc > 3 ? strtoull(argv[3], 0, 0) : 0;
    int isReal = !strcmp(mode, "real"), isVert = !strcmp(mode, "vert"), isBump = !strcmp(mode, "bump"), isCand = !strcmp(mode, "cand");
    if (isCand) { P = strtoull(argv[3], 0, 0); T = strtoull(argv[4], 0, 0); segPolicy = atoi(argv[5]); pagePolicy = atoi(argv[6]); pagesPerSeg = (u32)(SEG / P); pages = calloc(MAXPAGE, sizeof(Page)); segStack = calloc(MAXSEG * 4, 4); }
    u64 nAlloc = 0; u64 Tm = 262144;
    for (u64 i = 0; i < nrec; i++) {
        u64 op = rec[i].ptr >> 60, ptr = rec[i].ptr & ((1ull << 60) - 1);
        if (op == 1) {
            u64 n = rec[i].size ? rec[i].size : 1, a; nAlloc++;
            if (isReal) a = ptr; else if (isVert) a = vertAlloc(n, T); else if (isBump) a = bumpAlloc(n, T); else a = candAlloc(n, T);
            hput(ptr, a, n); if (n <= Tm) onAlloc(a, isReal ? n : (isBump ? ((n + 15) & ~15ull) : n));
        } else {
            u64 a, n; if (!hpop(ptr, &a, &n)) continue;
            if (n <= Tm) onFree(a, isReal ? n : (isBump ? ((n + 15) & ~15ull) : n));
            if (isVert) vertFree(a, n, T); else if (isCand) candFree(a, n, T);
        }
    }
    printf("%-28s peak4K=%6llu MB peak2M=%6llu MB  win256: lines=%5.1f pages4K=%5.1f regions2M=%4.1f", argc > 2 ? argv[2] : "", (unsigned long long)(p4k.peak * 4096 / 1048576), (unsigned long long)(p2m.peak * 2), sum64 / windows, sum4k / windows, sum2m / windows);
    if (isCand) printf("  P=%llu seg=%d page=%d segs=%llu peakSegs=%llu segRel=%llu pageRel=%llu", (unsigned long long)P, segPolicy, pagePolicy, (unsigned long long)nSegs, (unsigned long long)peakNonEmptySegs, (unsigned long long)segReleases, (unsigned long long)pageReleases);
    printf("\n");
    return 0;
}
