...
- [ ] Продолжить `#![no_core]` policy для отсутствующих lang items. Уже не добавляются implicit `Sized` bounds; `find_trait_impls_magic` не ищет `coerce_unsized`, `fn_ptr_trait`, `discriminant_kind`, `pointee_trait` или `tuple_trait`, пока разрешается другой trait; встроенный `[T; N] -> [T]` проходит без `Unsize` и incompatible shapes остаются compile-fail; raw-pointer cast не передаёт пустой путь `Unsize` в trait resolver; no-core binary с обычным `main` запускается напрямую без выдуманного `start`, а no_std без `start` по-прежнему получает диагностику; no-core без panic handler не генерирует неиспользуемый panic shim, но no_std handler по-прежнему требует. Следующий отдельный blocker: `gccrs/torture/operator_overload_1.rs` исполняется, но теряет side effect пользовательского `Add::add`, когда результат `1 + 2` отброшен. Сначала отдельный runtime unit; не подкладывать фиктивный lang item.
