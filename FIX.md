...
- [x] ~~Исправить `traits/inheritance/repeated-supertrait.rs`: method lookup теряет `same_as` у trait object с повторяющимся generic supertrait.~~ Исправлено сквозным путём: UFCS resolver ищет `<dyn Trait>::item` в principal trait и его supertraits; method lookup отдаёт все одноимённые supertrait instances существующему inference/dedup пути вместо выбора первого; vtable generator мономорфизирует method signatures параметрами конкретного supertrait и использует уже вычисленный `st_mono`. Красный `test_repeated_generic_supertrait.rs` проверяет `Compare<i32> + Compare<u32>` через dynamic method calls и UFCS; он и полный upstream `repeated-supertrait.rs` проходят compile+runtime на свежей `libstd`. Временных owning-контейнеров и новых heap allocations не добавлено.
