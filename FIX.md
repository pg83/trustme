...
- [ ] Продолжить `#![no_core]` policy для отсутствующих lang items. Уже не добавляются implicit `Sized` bounds и `find_trait_impls_magic` не ищет `coerce_unsized`, `fn_ptr_trait`, `discriminant_kind`, `pointee_trait` или `tuple_trait`, пока разрешается другой trait. Следующие отдельные blockers: встроенный `[u8; N] -> [u8]` сейчас требует отсутствующий `unsize` (`gccrs_compile/issue-1128.rs`), а raw-pointer cast в `gccrs/torture/operator_overload_1.rs` доходит до пустого lang-item path assertion. На каждый случай сначала отдельный no-core unit; сохранять structural validation и не подкладывать фиктивный trait.
