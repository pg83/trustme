#!/usr/bin/env python3
"""Extract and reference-check code fences from the Async Book.

Usage: import.py /path/to/async-book /path/to/rustc
"""
...
def main() -> int:
    ...
    if not source_root.is_dir():
        parser.error(f"missing Async Book sources under {args.checkout}")
    ...
    for markdown in sorted(source_root.rglob("*.md")):
        ...
        for line, info, code_lines in common.fences(
            ...
            settings = common.case_mode(info, "2018")
            if settings is None:
                continue
    ...
    print(f"accepted {len(accepted)} of {len(candidates)} Async Book fences")
    return 0
