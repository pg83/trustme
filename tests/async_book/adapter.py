#!/usr/bin/env python3
"""Compile and execute the standalone Async Book code fences."""
...
def fail_output(result: subprocess.CompletedProcess, case: str, stage: str) -> int:
    ...
    print(f"FAIL Async Book {case}: {stage} exit {result.returncode}", file=sys.stderr)
...
def main() -> int:
    ...
    with lib.workdir() as work:
        ...
        for index, (case, origin, edition, mode) in enumerate(selected):
            print(f"[Async Book {mode}] {origin}", file=sys.stderr, flush=True)
            ...
            if mode not in ("pass", "panic"):
                raise SystemExit(f"unknown Async Book mode {mode!r} for {case}")
            try:
                ...
                print(f"FAIL Async Book {case}: timed out after 30 seconds", file=sys.stderr)
                return 1
