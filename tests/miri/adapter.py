#!/usr/bin/env python3
"""Compile and run one shard of self-contained Miri pass tests."""
...
def fail_output(result: subprocess.CompletedProcess, case: str, stage: str) -> int:
    ...
    print(f"FAIL Miri {case}: {stage} exit {result.returncode}", file=sys.stderr)
...
def main() -> int:
    ...
    with lib.workdir() as work:
        ...
        for index, case in enumerate(selected):
            print(f"[Miri pass] {case}", file=sys.stderr, flush=True)
            ...
            try:
                ...
                print(f"FAIL Miri {case}: timed out after 30 seconds", file=sys.stderr)
                return 1
